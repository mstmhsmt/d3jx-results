  ...
  DiffView.prototype = {
    ...
    init: function(pane, orig, diffDts, options) {
      ...
      */
      ...
      this.showDts = options.showDts !== false;
...
*/
  ...
  function registerUpdate(dv) {
    ...
    function update(mode) {
      ...
      if (dv.showDts) {
        updateMarksDts(2, dv.edit, dv.diffDts, editDts);
        updateMarksDts(1, dv.orig, dv.diffDts, origDts);
      }
  ...
  function updateMarksDts(idx, cm, diffDts, state) {
    ...
    //console.log("["+idx+"][updateMarksDts] vp.from="+vp.from+", vp.to="+vp.to);
    cm.operation(function() {
      if (state.from == state.to || vp.from - state.to > 20 || state.from - vp.to > 20) {
        ...
        if (vp.from < state.from) {
          markChangesDts(idx, cm, diffDts, state.marked, vp.from, state.from);
          state.from = vp.from;
        }
        if (vp.to > state.to) {
          markChangesDts(idx, cm, diffDts, state.marked, state.to, vp.to);
          state.to = vp.to;
        }
      }
    });
  ...
  function markChangesDts(idx, cm, diffDts, marks, from, to) {
    //console.log("["+idx+"][markChangesDts] "+marks.length+" marks, from="+from+", to="+to);
    var margin = to - from;
    if (from > margin) from -= margin;
    to += margin;
    var doc = cm.getDoc();
    var top = new Pos(from, 0), bot = cm.clipPos(new Pos(to - 1, 0));

    var editDts, tag, lab, segs, cls, IGN, MISS;
    var seg, st, ed, st_pos, ed_pos, stl, edl;
    var i, j, k;
    var m, ms, skip;

    // for edits
    for (i = 0; i < diffDts.length; ++i) {
      editDts = diffDts[i];
      ...
      for (j = 0; j < segs.length; j++) {
        ...
        if (st < ed) {
          ...
          stl = st_pos.line;
          edl = ed_pos.line;
          if (from <= edl && edl <= to || from <= stl && stl <= to) {
  ...
  function createClip(baseBound, defs, id, cm) {
    var clip = document.createElementNS(svgNS, "clipPath");
    attrs(defs.appendChild(clip), "id", id/*, "clipPathUnits", "objectBoundingBox"*/);
    var elem = cm.getWrapperElement();
    var bound = elem.getBoundingClientRect();

    //console.log("[createClip] bound: left="+bound.left+", top="+bound.top+", height="+bound.height+", width="+bound.width);
    ...
    var rect = {x:x-baseBound.left,y:bound.top-baseBound.top+dialogHeight,
                height:bound.height-dialogHeight-hscrollbarHeight,width:width};

    attrs(clip.appendChild(document.createElementNS(svgNS, "rect")), 
          "x", rect.x, "y", rect.y, "height", rect.height, "width", rect.width);

    return [x, rect, dialogHeight > 0, lineGutterWidth];
  }
    ...
    if (cmL.annotateScrollbar && cmR.annotateScrollbar) {
      ...
      var cmp = function(a, b) {
        if (a.from.line < b.from.line)
          return -1;
        else if (a.from.line > b.from.line)
          return 1;
        else
          return 0;
      }
...
*/
  ...
  function handleEditClick(cm, ev) {
    ...
    if (ms.length > 0) {
      ...
      case "MOVREL":
      case "MOVE":
        if (ed.connectorVisible) {
          ed.connectorVisible = false
        } else {
          ed.connectorVisible = true;
        }
        makeConnections(cm.dv);
        break;
      }
    }
  }
  ...
  function drawConnectorsDts(dv, diffDts, isEss) {
    ...
    //console.log("entering drawConnectorsDts");
    ...
    var editXRectD = createClip(bound, dfs, editClipId, editCM);
    ...
    if (origDshown || editDshown) {
      ...
      var leftCM = dv.type == "left" ? origCM : editCM;
      var gclip = document.createElementNS(svgNS, "clipPath");
      attrs(dfs.appendChild(gclip), "id", "glasspaneClip");
      var gbound = dv.mv.glasspane.getBoundingClientRect();

      //console.log("gbound: left="+gbound.left+", top="+gbound.top+", height="+gbound.height+", width="+gbound.width);
      ...
      attrs(gclip.appendChild(document.createElementNS(svgNS, "rect")),
    ...
    var cms   = new Array(origCM, editCM);
    var docs  = new Array(origDoc, editDoc);
    var clips = new Array(origClipId, editClipId);
    ...
    for (var idx = 0; idx < diffDts.length; idx++) {
      ...
      var skips = new Array(true, true);
      var paths = new Array("", "");
      var vps   = new Array(dv.orig.getViewport(), dv.edit.getViewport());
      ...
      for (var i = 0; i < 2; i++) { // for segments
        ...
        if (lab in editDts) {
          ...
          for (var j = 0; j < segs.length; j++) {
            ...
            //console.log("spos={line:"+spos.line+",ch:"+spos.ch+"},epos={line:"+epos.line+",ch:"+epos.ch+"}");
      ...
      case "CORR": // for essentials
        ...
        if (tag != "ALIGN" && !editDts.connectorVisible)
          continue;
        ...
        if (dv.type == "left") {
          if (spos1.line == epos1.line)
            fromP = origCM.charCoords(epos1, "page");
          else
            fromP = origCM.charCoords(new Pos(spos1.line, origDoc.getLine(spos1.line).length - 1), "page");
          
          toP = editCM.charCoords(spos2, "page");
          
          fromX = fromP.right - dx;
          toX = Math.max(toP.left, editX) - dx;
          
        } else {
          fromP = origCM.charCoords(spos1, "page");
          
          if (spos2.line == epos2.line)
            toP = editCM.charCoords(epos2, "page");
          else
            toP = editCM.charCoords(new Pos(spos2.line, editDoc.getLine(spos2.line).length - 1), "page");
          
          fromX = Math.max(fromP.left, origX) - dx;
          toX = toP.right - dx;
        }
        fromY = fromP.top - dy;
        toY = toP.top - dy;
        
        var mx = Math.abs(toX + fromX) / 2;
    ...
    if (curveMov != "") {
      ...
      attrs(svg.appendChild(dv.mv.connectorMov), 
            "d", curveMov, 
    ...
    if (curveRel != "") {
      ...
      attrs(svg.appendChild(dv.mv.connectorRel), 
            "d", curveRel, 
            "clip-path", "url(#glasspaneClip)", 
    ...
    if (curveMovRel != "") {
      ...
      attrs(svg.appendChild(dv.mv.connectorMovRel), 
            "d", curveMovRel, 
            "clip-path", "url(#glasspaneClip)", 
    ...
    if (curveAlign != "") {
      //console.log('curveAlign="'+curveAlign+'"');
      attrs(svg.appendChild(dv.mv.connectorAlign), 
            "d", curveAlign, 
            "clip-path", "url(#glasspaneClip)", 
    ...
    if (curveCorr != "") {
      ...
      attrs(svg.appendChild(dv.mv.connectorCorr), 
            "d", curveCorr, 
            "clip-path", "url(#glasspaneClip)", 
  ...
  var MergeView = CodeMirror.MergeView = function(node, options) {
    ...
    var diffLeft = options.diffLeft, diffRight = options.diffRight == null ? options.diff : options.diffRight;
  ...
  MergeView.prototype = {
    ...
    annotate: function() {
      if (this.left) {
        //console.log("[annotate] showDts="+this.left.showDts);
        var dv = this.left;
        if (dv.showDts) {

          if (!dv.dtsAnnot) {
            dv.dtsAnnot = new DtsScrollbarAnnotation(dv.orig, dv.edit, dv.diffDts);
          } else {
            try {
              dv.dtsAnnot.clear();
            } catch (exn) {
          ...
          if (!dv.dtsAnnotEss) {
            dv.dtsAnnotEss = new DtsScrollbarAnnotation(dv.orig, dv.edit, this.diffDtsEss);
          } else {
            try {
