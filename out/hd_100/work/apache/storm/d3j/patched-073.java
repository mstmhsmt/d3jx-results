...
class PartitionManager
{
   ...
   Long _committedTo;
   LinkedList<MessageAndRealOffset> _waitingToEmit =
     new LinkedList<MessageAndRealOffset>();
   ...
   public
   PartitionManager(DynamicPartitionConnections connections, 
     ...
     _failedMsgRetryManager =
     new ExponentialBackoffMsgRetryManager(_spoutConfig.retryInitialDelayMs,
       _spoutConfig.retryDelayMultiplier, _spoutConfig.retryDelayMaxMs);
     ...
     try {
       ...
       if (json != null) {
         jsonTopologyId =
         (String)((Map<Object,Object>)json.get("topology")).get("id");
     ...
     Long currentOffset =
       KafkaUtils.getOffset(_consumer, topic, id.partition, spoutConfig);
   ...
   EmitState next(SpoutOutputCollector collector) {
     ...
     while (true) {
       ...
       if (_spoutConfig.scheme instanceof MessageMetadataSchemeAsMultiScheme) {
         tups =
         KafkaUtils.generateTuples((MessageMetadataSchemeAsMultiScheme)_spoutConfig.scheme,
           toEmit.msg, _partition, toEmit.offset);
       } else {
         tups =
         ...
         if (_spoutConfig.topicAsStreamId) {
           for (List<Object> tup : tups) {
             collector.emit(_spoutConfig.topic, tup,
               new KafkaMessageId(_partition, toEmit.offset));
           }
         } else {
           for (List<Object> tup : tups) {
             collector.emit(tup,
   ...
   void fill() {
     ...
     try {
       msgs =
       KafkaUtils.fetchMessages(_spoutConfig, _consumer, _partition, offset);
     } catch (TopicOffsetOutOfRangeException e) {
       _emittedToOffset =
       KafkaUtils.getOffset(_consumer, _partition.topic,
   ...
   void commit() {
     ...
     if (_committedTo != lastCompletedOffset) {
       ...
       Map<Object,Object> data =
         (Map<Object,Object>)ImmutableMap.builder().put("topology",
                               ImmutableMap.of("id", _topologyInstanceId,
                                 "name",
                                 _stormConf.get(Config.TOPOLOGY_NAME))).put("offset",
                               lastCompletedOffset).put("partition",
                               _partition.partition).put("broker",
                               ImmutableMap.of("host", _partition.host.host,
                                 "port", _partition.host.port)).put("topic",
                               _partition.topic).build();
