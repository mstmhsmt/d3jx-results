...
public class BlockJUnit4ClassRunner extends ParentRunner<FrameworkMethod> {
    ...
    private void validateFields(List<Throwable> errors) {
        for (FrameworkField each : getTestClass().getAnnotatedFields(Rule.class)) validateRuleField(each.getField(), errors);
    }
    ...
    private Statement withMethodRules(FrameworkMethod method, Object target, Statement result) {
        ...
        for (org.junit.rules.MethodRule each : getMethodRules(target)) if (!testRules.contains(each))
...
=======
for (TestRule each : getTestRules(target)) result = each.apply(result, describeChild(method));return result;
