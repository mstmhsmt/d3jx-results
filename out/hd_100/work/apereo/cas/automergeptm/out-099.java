 ...
 * @since 4.0.0
 */
@Slf4j @ToString @Setter public class ChainingPrincipalResolver implements PrincipalResolver {
  /**
  ...
  /**
     ...
     */
  @Override public Principal resolve(final Credential credential, final Optional<Principal> principal, final Optional<AuthenticationHandler> handler) {
    ...
    final int count = principals.stream().map((p) -> p.getId().trim().toLowerCase()).collect(Collectors.toSet()).size();
    if (count > 1) {
  ...
  @Override public IPersonAttributeDao getAttributeRepository() {
