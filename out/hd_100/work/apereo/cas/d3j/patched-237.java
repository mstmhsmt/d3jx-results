...
{
   public static final String ACTION_THROTTLED_LOGIN_ATTEMPT =
     "THROTTLED_LOGIN_ATTEMPT";
   ...
   @Override
   public
   boolean preHandle(final HttpServletRequest request, 
     ...
     if (exceedsThreshold(request)) {
       ...
       request.setAttribute(WebUtils.CAS_ACCESS_DENIED_REASON,
         "screen.blocked.message");
       response.sendError(HttpStatus.SC_LOCKED,
         "Access Denied for user [" + StringEscapeUtils.escapeHtml4(request.getParameter(this.usernameParameter)) + "] from IP Address [" + 
   ...
   void postHandle(final HttpServletRequest request, 
     ...
     if (recordEvent) {
       LOGGER.debug("Recording submission failure for [{}]",
   ...
   protected
   void recordThrottle(final HttpServletRequest request) {
     LOGGER.warn("Throttling submission from [{}]. More than [{}] failed login attempts within [{}] seconds. " + "Authentication attempt exceeds the failure threshold [{}]",
       request.getRemoteAddr(), this.failureThreshold,
   ...
   protected
   boolean calculateFailureThresholdRateAndCompare(final List<Date> failures) {
     if (failures.size() < 2) {
       return false;
     }
     ...
     if (rate > getThresholdRate()) {
       LOGGER.warn("Authentication throttling rate [{}] exceeds the defined threshold [{}]",
         rate, getThresholdRate());
       return true;
     }
     return false;
   }
   protected
   String getUsernameParameterFromRequest(final HttpServletRequest request) {
     return request.getParameter(StringUtils.defaultString(usernameParameter,
                                   "username"));
   }
   protected
   Date getFailureInRangeCutOffDate() {
     final var cutoff =
       ZonedDateTime.now(ZoneOffset.UTC).minusSeconds(getFailureRangeInSeconds());
     return DateTimeUtils.timestampOf(cutoff);
   }
   protected
   void recordAuditAction(final HttpServletRequest request, 
                          final String actionName) {
     final var userToUse = getUsernameParameterFromRequest(request);
     final var clientInfo = ClientInfoHolder.getClientInfo();
     final var resource =
       StringUtils.defaultString(request.getParameter(CasProtocolConstants.PARAMETER_SERVICE),
         "N/A");
     final var context =
       new AuditActionContext(userToUse, resource, actionName,
         this.applicationCode,
         DateTimeUtils.dateOf(ZonedDateTime.now(ZoneOffset.UTC)),
         clientInfo.getClientIpAddress(), clientInfo.getServerIpAddress());
