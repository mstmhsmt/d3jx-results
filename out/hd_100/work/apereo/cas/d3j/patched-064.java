  ...
  extends AbstractCRLRevocationChecker
{
   ...
   public
   CRLDistributionPointRevocationChecker(final Cache crlCache) {
     if (crlCache == null) {
   ...
   @Override
   protected
   X509CRL getCRL(final X509Certificate cert) {
     final URL[] urls = getDistributionPoints(cert);
     logger.debug("Distribution points for {}: {}.",
       CertUtils.toString(cert), Arrays.asList(urls));
     try {
       for (final URL url : urls) {
         final Element item = this.crlCache.get(url);
         if (item != null) {
           logger.debug("Found CRL in cache for {}",
             CertUtils.toString(cert));
   ...
   private
   URL[] getDistributionPoints(final X509Certificate cert) {
     final DistributionPointList points;
     try {
       points = new ExtensionReader(cert).readCRLDistributionPoints();
     } catch (final Exception e) {
       logger.error("Error reading CRLDistributionPoints extension field on " + 
         CertUtils.toString(cert), e);
       return new URL[0];
     } 
     ...
     for (final DistributionPoint point : points.getItems()) {
       ...
       if (location instanceof String) {
         ...
         logger.warn("{} not supported. String or GeneralNameList expected.",
           location);
       }
     }
   ...
   private
   void addURL(final List<URL> list, final String uriString) {
     try {
