             ...
             PersistenceRoot, DescriptorByNameOwner, OnMaster
{
   ...
   public int number;
   @Restricted(NoExternalUse.class) protected volatile transient RunT
   previousBuild;
   @Restricted(NoExternalUse.class) protected volatile transient RunT
   nextBuild;
   ...
   private static final SimpleDateFormat CANONICAL_ID_FORMATTER =
     new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss");
   protected static final ThreadLocal<SimpleDateFormat> ID_FORMATTER =
     new IDFormatterProvider();
   private
   static
   final
   class IDFormatterProvider extends ThreadLocal<SimpleDateFormat>
   {
      @Override
      protected
   ...
   ;
   protected
   Run(JobT job)  throws IOException{
     this(job,
     new GregorianCalendar());
     this.number = project.assignBuildNumber();
   }
   protected
   Run(JobT job, Calendar timestamp) {
     this(job,
     timestamp.getTimeInMillis());
   }
   protected
   Run(JobT job, long timestamp) {
     this.project = job;
   ...
   protected
   Run(JobT project, File buildDir)  throws IOException{
     this(project,
     parseTimestampFromBuildDir(buildDir));
   ...
   static
   class InvalidDirectoryNameException extends IOException
   {
   ...
   static
   long parseTimestampFromBuildDir(File buildDir)
     ...
     try {
       ...
       throw new IOException2("Interrupted while resolving symlink directory " + buildDir,
               e);
     } 
   }
   ...
   public
   <T extends Cause> T getCause(Class<T> type) {
   ...
   String getTruncatedDescription() {
     ...
     final String ending = "...";
     final int sz = description.length(), maxTruncLength =
       maxDescrLength - ending.length();
     boolean inTag = false;
     int displayChars = 0;
   ...
   String getDurationString() {
     if (isBuilding())
       return Messages.Run_InProgressDuration(Util.getTimeSpanString(
   ...
   BallColor getIconColor() {
     ...
     if (pb == null)
       baseColor = BallColor.NOTBUILT; else
   ...
   public
   void setDisplayName(String value) throws IOException {
   ...
   public
   List<RunT> getPreviousBuildsOverThreshold(int numberOfBuilds, 
                                             Result threshold) {
   ...
   synchronized
   @Nonnull
   ArtifactManager pickArtifactManager() throws IOException {
     ExtensionList<ArtifactManager> managers =
       Jenkins.getInstance().getExtensionList(ArtifactManager.class);
     if (artifactManager != null) {
   ...
   public
   List<Artifact> getArtifactsUpTo(int n) {
   ...
   public static final int LIST_CUTOFF =
     Integer.parseInt(System.getProperty("hudson.model.Run.ArtifactList.listCutoff",
                        "16"));
   public static final int TREE_CUTOFF =
     Integer.parseInt(System.getProperty("hudson.model.Run.ArtifactList.treeCutoff",
                        "40"));
   public
   final
   class ArtifactList extends ArrayList<Artifact>
   {
      private static final long serialVersionUID = 1L;
      private LinkedHashMap<Artifact,String> tree =
        new LinkedHashMap<Artifact,String>();
      public
      Map<Artifact,String> getTree() {
        return tree;
      }
      public
      ...
      private
      String combineLast(String[] token, int n) {
   ...
   class Artifact
   {
      ...
      public
      Artifact(String name, String relativePath, String href, String len, 
               String treeNodeId) {
        this.name = name;
   ...
   InputStream getLogInputStream() throws IOException {
     ...
     return new ByteArrayInputStream(charset != null ? message.getBytes(charset) : 
              message.getBytes());
   }
   public
   Reader getLogReader() throws IOException {
     if (charset == null)
       return new InputStreamReader(getLogInputStream()); else
       return new InputStreamReader(getLogInputStream(), charset);
   }
   public
   void writeLogTo(long offset, XMLOutput out) throws IOException {
     try {
   ...
   public
   void writeWholeLogTo(OutputStream out)
   ...
   AnnotatedLargeText getLogText() {
     return new AnnotatedLargeText(getLogFile(), getCharset(),
              !isLogUpdated(), this);
   }
   @Override
   protected
   SearchIndexBuilder makeSearchIndex() {
     SearchIndexBuilder builder =
       super.makeSearchIndex().add("console").add("changes");
     for (Action a : getActions()) {
   ...
   public
   synchronized
   void deleteArtifacts() throws IOException {
   ...
   void delete() throws IOException {
     ...
     File link =
       new File(project.getBuildDir(), String.valueOf(getNumber()));
     link.delete();
   ...
   @SuppressWarnings("unchecked")
   private
   void removeRunFromParent() {
     getParent().removeRun((RunT)this);
   }
   static
   void reportCheckpoint(CheckPoint id) {
     RunnerStack.INSTANCE.peek().checkpoints.report(id);
   }
   static
   void waitForCheckpoint(CheckPoint id) throws InterruptedException {
     while (true) {
       Run b =
         RunnerStack.INSTANCE.peek().getBuild().getPreviousBuildInProgress();
       if (b == null)
         return;
   ...
   protected
   abstract
   class Runner extends RunExecution{}
   public
   abstract
   class RunExecution
   {
      ...
      class CheckpointSet
      {
         ...
         protected
         synchronized
         void report(CheckPoint identifier) {
           checkpoints.add(identifier);
           notifyAll();
         }
         protected
         synchronized
         boolean waitForCheckPoint(CheckPoint identifier)
           throws InterruptedException {
           final Thread t = Thread.currentThread();
           final String oldName = t.getName();
           t.setName(oldName + " : waiting for " + identifier + " on " + 
             getFullDisplayName());
           try {
      ...
      public
      abstract
      void cleanUp(@Nonnull BuildListener listener) throws Exception;
      public
      RunT getBuild() {
        return _this();
      }
   ...
   static
   final
   class RunnerAbortedException extends RuntimeException
   {
      private static final long serialVersionUID = 1L;
   }
   protected
       ...
       try {
         try {
           ...
           if (project instanceof BuildableItemWithBuildWrappers && build instanceof AbstractBuild) {
             BuildableItemWithBuildWrappers biwbw =
           ...
           Util.createSymlink(getParent().getBuildDir(), getId(),
             String.valueOf(getNumber()), listener);
           ...
           Executor.currentExecutor().recordCauseOfInterruption(Run.this,
             listener);
   ...
   private
   void handleFatalBuildProblem(BuildListener listener, Throwable e) {
     if (listener != null) {
       ...
       LOGGER.log(SEVERE,
         getDisplayName() + " failed to build and we don't even have a listener",
         e);
     }
   }
   protected
   ...
   List<String> getLog(int maxLines) throws IOException {
     ...
     BufferedReader reader =
       new BufferedReader(new InputStreamReader(new FileInputStream(getLogFile()),
                            getCharset()));
     try {
     ...
     if (lineCount > maxLines)
       logLines.set(0,
         "[...truncated " + (lineCount - (maxLines - 1)) + " lines...]");
     return ConsoleNote.removeNotes(logLines);
   }
   public
   void doBuildStatus(StaplerRequest req, StaplerResponse rsp)
     throws IOException {
   ...
   class Summary
   {
      ...
      public
      Summary(boolean worse, String message) {
        this.isWorse = worse;
        this.message = message;
      }
   }
   public
   Summary getBuildStatusSummary() {
     if (isBuilding()) {
       ...
       case STILL_FAILING:
         ...
         if (since == null)
           return new Summary(false,
                    Messages.Run_Summary_BrokenForALongTime());
         RunT failedBuild = since.getNextBuild();
         return new Summary(false,
                  Messages.Run_Summary_BrokenSince(failedBuild.getDisplayName()));
       case NOW_UNSTABLE:
   ...
   private
   Summary determineDetailedUnstableSummary(Boolean worseOverride) {
     if (((Run)this) instanceof AbstractBuild) {
       AbstractTestResultAction trN =
         ((AbstractBuild)(Run)this).getTestResultAction();
       Run prev = getPreviousBuild();
       AbstractTestResultAction trP =
         prev == null ? null : ((AbstractBuild)prev).getTestResultAction();
       if (trP == null) {
         ...
         if (trN != null && trN.getFailCount() != 0) {
           ...
           if (trP.getFailCount() < trN.getFailCount())
             return new Summary(worseOverride != null ? worseOverride : true,
                      Messages.Run_Summary_MoreTestsFailing(trN.getFailCount() - 
                        trP.getFailCount(), trN.getFailCount()));
           if (trP.getFailCount() > trN.getFailCount())
             return new Summary(worseOverride != null ? worseOverride : false,
                      Messages.Run_Summary_LessTestsFailing(trP.getFailCount() - 
                        trN.getFailCount(), trN.getFailCount()));
           return new Summary(worseOverride != null ? worseOverride : false,
   ...
   public
   void doBuildNumber(StaplerResponse rsp) throws IOException {
   ...
   public
   void doBuildTimestamp(StaplerRequest req, StaplerResponse rsp, 
     ...
     rsp.setStatus(HttpServletResponse.SC_OK);
     DateFormat df =
       format == null ? DateFormat.getDateTimeInstance(DateFormat.SHORT,
                          DateFormat.SHORT, Locale.ENGLISH) : new SimpleDateFormat(format,
                                                                req.getLocale());
     rsp.getWriter().print(df.format(getTime()));
   }
   public
   void doConsoleText(StaplerRequest req, StaplerResponse rsp)
     throws IOException {
     rsp.setContentType("text/plain;charset=UTF-8");
     FlushProofOutputStream out =
       new FlushProofOutputStream(rsp.getCompressedOutputStream(req));
     try {
   ...
   public
   void doProgressiveLog(StaplerRequest req, StaplerResponse rsp)
     throws IOException {
     getLogText().doProgressText(req, rsp);
   }
   public
   boolean canToggleLogKeep() {
   ...
   public
   void doToggleLogKeep(StaplerRequest req, StaplerResponse rsp)
   ...
   public
   final
   void keepLog() throws IOException {
     keepLog(true);
   }
   public
   void keepLog(boolean newValue) throws IOException {
     checkPermission(UPDATE);
     keepLog = newValue;
     save();
   }
   @RequirePOST
   public
   void doDoDelete(StaplerRequest req, StaplerResponse rsp)
     throws IOException, ServletException {
   ...
   public
   synchronized
   void doSubmitDescription(StaplerRequest req, StaplerResponse rsp)
     throws IOException, ServletException {
   ...
   EnvVars getCharacteristicEnvVars() {
     ...
     env.put("BUILD_TAG",
       "jenkins-" + getParent().getFullName().replace('/', '-') + "-" + number);
     return env;
   }
   public
   ...
   Run<?,?> fromExternalizableId(String id) {
     ...
     Job<?,?> job =
       Jenkins.getInstance().getItemByFullName(jobName, Job.class);
     if (job == null) {
   ...
   @RequirePOST
   public
   HttpResponse doConfigSubmit(StaplerRequest req)
   ...
   protected
   void submit(JSONObject json) throws IOException {
   ...
   public static final FeedAdapter<Run> FEED_ADAPTER_LATEST =
     new DefaultFeedAdapter()
     {
        @Override
        public
        String getEntryID(Run e) {
          return "tag:hudson.dev.java.net,2008:" + e.getParent().getAbsoluteUrl();
        }
     };
   public
   final
   class KeepLogBuildBadge implements BuildBadgeAction
   {
      public
   ...
   public static final PermissionGroup PERMISSIONS =
     new PermissionGroup(Run.class, Messages._Run_Permissions_Title());
   public static final Permission DELETE =
     new Permission(PERMISSIONS, "Delete",
       Messages._Run_DeletePermission_Description(), Permission.DELETE,
       PermissionScope.RUN);
   public static final Permission UPDATE =
     new Permission(PERMISSIONS, "Update",
       Messages._Run_UpdatePermission_Description(), Permission.UPDATE,
       PermissionScope.RUN);
   public static final Permission ARTIFACTS =
     new Permission(PERMISSIONS, "Artifacts",
       Messages._Run_ArtifactsPermission_Description(), null,
       Functions.isArtifactsPermissionEnabled(), new PermissionScope[]{
       PermissionScope.RUN });
   private
   static
   ...
   {
      public
      String getEntryTitle(Run entry) {
        return entry + " (" + entry.getBuildStatusSummary().message + ")";
      }
      public
      String getEntryUrl(Run entry) {
        return entry.getUrl();
      }
      public
      String getEntryID(Run entry) {
        return "tag:" + "hudson.dev.java.net," + entry.getTimestamp().get(Calendar.YEAR) + ":" + 
        entry.getParent().getName() + ':' + entry.getId();
      }
      public
      String getEntryDescription(Run entry) {
        return entry.getDescription();
      }
      public
      Calendar getEntryTimestamp(Run entry) {
        return entry.getTimestamp();
      }
      public
      String getEntryAuthor(Run entry) {
        return JenkinsLocationConfiguration.get().getAdminAddress();
      }
   }
   @Override
   public
   Object getDynamic(String token, StaplerRequest req, StaplerResponse rsp) {
   ...
   {
      public
      void doDynamic(StaplerResponse rsp) throws IOException {
