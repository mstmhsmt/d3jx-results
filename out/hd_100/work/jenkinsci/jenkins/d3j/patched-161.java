...
public
class ApiTokenProperty extends UserProperty
{
   private volatile Secret apiToken;
   private static final Logger LOGGER =
     Logger.getLogger(ApiTokenProperty.class.getName());
   @SuppressFBWarnings(value="MS_SHOULD_BE_FINAL",
                       justification="Accessible via System Groovy Scripts")
   ...
   String getApiToken() {
     ...
     if (LOGGER.isLoggable(Level.FINER)) {
       LOGGER.log(Level.FINER, "Deprecated usage of getApiToken (trace)",
         new Exception());
     }
   ...
   boolean matchesPassword(String token) {
     ...
     ApiTokenStore.HashedToken matchingToken =
       tokenStore.findMatchingToken(token);
   ...
   private
   boolean hasPermissionToSeeToken() {
   ...
   class TokenInfoAndStats
   {
      ...
      public
      TokenInfoAndStats(@Nonnull ApiTokenStore.HashedToken token, 
                        @Nonnull ApiTokenStats.SingleTokenStats stats) {
   ...
   @Override
   public
   UserProperty reconfigure(StaplerRequest req, @CheckForNull JSONObject form)
     throws FormException {
     if (form == null) {
       return this;
     }
     Object tokenStoreData = form.get("tokenStore");
     Map<String,JSONObject> tokenStoreTypedData =
       convertToTokenMap(tokenStoreData);
     this.tokenStore.reconfigure(tokenStoreTypedData);
     return this;
   }
   private
   Map<String,JSONObject> convertToTokenMap(Object tokenStoreData) {
     ...
     throw HttpResponses.error(400,
             "Unexpected class received for the token store information");
   }
   private
   void addJSONTokenIntoMap(Map<String,JSONObject> tokenMap, 
   ...
   @Deprecated
   public
   void changeApiToken() throws IOException {
     user.checkPermission(Jenkins.ADMINISTER);
     LOGGER.log(Level.FINE, "Deprecated usage of changeApiToken");
     ApiTokenStore.HashedToken existingLegacyToken =
       tokenStore.getLegacyToken();
     _changeApiToken();
   ...
   class DescriptorImpl extends UserPropertyDescriptor
   {
      ...
      @Restricted(NoExternalUse.class)
      public
      String getNoLegacyToken() {
        return Messages.ApiTokenProperty_NoLegacyToken();
      }
      public
      ApiTokenProperty newInstance(User user) {
      ...
      private
      ApiTokenProperty forceNewInstance(User user, boolean withLegacyToken) {
        if (withLegacyToken) {
      ...
      @Restricted(NoExternalUse.class)
      public
      boolean mustDisplayLegacyApiToken(User propertyOwner) {
        ApiTokenProperty property =
          propertyOwner.getProperty(ApiTokenProperty.class);
      ...
      @Restricted(NoExternalUse.class)
      public
      boolean hasCurrentUserRightToGenerateNewToken(User propertyOwner) {
        ...
        return User.idStrategy().equals(propertyOwner.getId(),
                 currentUser.getId());
      }
      @Deprecated
      @RequirePOST
      public
      HttpResponse doChangeToken(@AncestorInPath User u, StaplerResponse rsp)
        throws IOException {
        u.checkPermission(Jenkins.ADMINISTER);
        LOGGER.log(Level.FINE,
          "Deprecated action /changeToken used, consider using /generateNewToken instead");
        ...
        rsp.setHeader("script",
          "document.getElementById('apiToken').value='" + p.getApiToken() + "'");
      ...
      @RequirePOST
      public
      HttpResponse doGenerateNewToken(@AncestorInPath User u, 
        ...
        if (StringUtils.isBlank(newTokenName)) {
          tokenName =
          String.format("Token created on %s",
        ...
        ApiTokenStore.TokenUuidAndPlainValue tokenUuidAndPlainValue =
          p.tokenStore.generateNewToken(tokenName);
        u.save();
        return HttpResponses.okJSON(new HashMap<String,String>()
                                    {
                                       ...
                                       {
                                          put("tokenUuid",
                                            tokenUuidAndPlainValue.tokenUuid);
                                          put("tokenName", tokenName);
                                          put("tokenValue",
                                            tokenUuidAndPlainValue.plainValue);
                                       }
                                    });
      }
      @RequirePOST
      public
      HttpResponse doRename(@AncestorInPath User u, 
        ...
        if (StringUtils.isBlank(tokenUuid)) {
          return HttpResponses.errorWithoutStack(400,
                   "The tokenUuid cannot be empty");
        }
        ApiTokenProperty p = u.getProperty(ApiTokenProperty.class);
        if (p == null) {
          return HttpResponses.errorWithoutStack(400,
                   "The user does not have any ApiToken yet, try generating one before.");
        }
      ...
      @RequirePOST
      public
      HttpResponse doRevoke(@AncestorInPath User u, 
                            @QueryParameter String tokenUuid)
        throws IOException {
        u.checkPermission(Jenkins.ADMINISTER);
        if (StringUtils.isBlank(tokenUuid)) {
        ...
        ApiTokenStore.HashedToken revoked =
          p.tokenStore.revokeToken(tokenUuid);
        if (revoked != null) {
          if (revoked.isLegacy()) {
            p.apiToken = null;
          }
   ...
   @Restricted(NoExternalUse.class) @Deprecated public static final
   HMACConfidentialKey API_KEY_SEED =
     new HMACConfidentialKey(ApiTokenProperty.class, "seed", 16);
}