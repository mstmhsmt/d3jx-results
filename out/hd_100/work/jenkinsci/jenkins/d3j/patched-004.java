...
class QueueTest
{
   ...
   void persistence() throws Exception {
     ...
     q.save();
     System.out.println(FileUtils.readFileToString(new File(r.jenkins.getRootDir(),
                                                     "queue.xml")));
   ...
   @Test
   public
   void recover_from_legacy_list() throws Exception {
     Queue q = r.jenkins.getQueue();
     assertEquals(1, q.getItems().length);
   ...
   private
   void resetQueueState() throws IOException {
   ...
   void queue_id_to_run_mapping() throws Exception {
     ...
     FreeStyleBuild build =
       r.assertBuildStatusSuccess(testProject.scheduleBuild2(0));
   ...
   static
   final
   class FileItemPersistenceTestServlet extends HttpServlet
   {
   ...
   @Test
   public
   void fileItemPersistence() throws Exception {
     ...
     try {
       ...
       @SuppressWarnings("deprecation") HtmlPage p =
         (HtmlPage)wc.getPage("http://localhost:" + connector.getLocalPort() + '/');
   ...
   void foldableCauseAction() throws Exception {
     ...
     assertTrue("Build page should combine duplicates and show counts: " + buildPage,
       buildPage.contains("Started by user SYSTEM (2 times) " + "Started by an SCM change (3 times) " + "Started by timer (2 times) " + "Started by remote host 1.2.3.4 with note: test (2 times) " + "Started by remote host 4.3.2.1 with note: test " + "Started by remote host 1.2.3.4 with note: foo"));
   }
   @Issue("JENKINS-8790")
   @Test
   public
   void flyweightTasks() throws Exception {
     ...
     m.addProperty(new ParametersDefinitionProperty(new StringParameterDefinition("FOO",
                                                      "value")));
     m.getBuildersList().add(new Shell("sleep 3"));
     m.setAxes(new AxisList(new TextAxis("DoesntMatter", "aaa", "bbb")));
     List<Future<MatrixBuild>> futures =
       new ArrayList<Future<MatrixBuild>>();
     for (int i = 0; i < 3; i++) {
       futures.add(m.scheduleBuild2(0, new UserIdCause(),
                     new ParametersAction(new StringParameterValue("FOO",
                                            "value" + i))));
     }
   ...
   @Test
   public
   void flyweightTasksWithoutMasterExecutors() throws Exception {
   ...
   void inQueueTaskLookupByAPI() throws Exception {
     ...
     String queueTaskId =
       queueItems.getXmlDocument().getElementsByTagName("id").item(0).getTextContent();
     assertNotNull(queueTaskId);
     XmlPage queueItem =
       webclient.goToXml("queue/item/" + queueTaskId + "/api/xml");
   ...
   void upstreamDownstreamCycle() throws Exception {
     ...
     trigger.getPublishersList().add(new BuildTrigger(String.format("%s, %s, %s, %s",
                                                        chain1.getName(),
                                                        chain2a.getName(),
                                                        chain2b.getName(),
                                                        chain3.getName()),
     ...
     chain1.getPublishersList().add(new BuildTrigger(String.format("%s, %s",
                                                       chain2a.getName(),
                                                       chain2b.getName()),
                                      true));
     ...
     assertThat("The cycle should have been defanged and chain1 executed",
       queue.getItem(chain1), nullValue());
     assertThat("The cycle should have been defanged and chain2a executed",
       queue.getItem(chain2a), nullValue());
     assertThat("The cycle should have been defanged and chain2b executed",
       queue.getItem(chain2b), nullValue());
     assertThat("The cycle should have been defanged and chain3 executed",
       queue.getItem(chain3), nullValue());
   }
   private
   static
   class TestFlyweightTask extends TestTask implements Queue.FlyweightTask
   {
      Executor exec;
   ...
   void taskEquality() throws Exception {
     ...
     ScheduleResult result =
       r.jenkins.getQueue().schedule2(new TestTask(cnt), 0);
   ...
   private
   static
   class TestTask extends AbstractQueueTask
   {
   ...
   void accessControl() throws Exception {
     ...
     QueueItemAuthenticatorConfiguration.get().getAuthenticators().add(
       new MockQueueItemAuthenticator(Collections.singletonMap(p.getFullName(),
                                        alice)));
     p.getBuildersList().add(new TestBuilder()
                             {
                                @Override
   ...
   private static Authentication alice =
     new UsernamePasswordAuthenticationToken("alice", "alice",
   ...
   @Test
   public
   void pendingsConsistenceAfterErrorDuringMaintain()
     ...
     TopLevelItemDescriptor descriptor =
       new TopLevelItemDescriptor(FreeStyleProject.class)
       {
          @Override
          public
          FreeStyleProject newInstance(ItemGroup parent, String name) {
     ...
     FreeStyleProject projectError =
       (FreeStyleProject)r.jenkins.createProject(descriptor, "throw-error");
     ...
     while (project2.getLastBuild() == null) {
       ...
       if (e.isIdle()) {
         assertTrue("Node went to idle before project had" + project2.getDisplayName() + " been started",
           v.isDone());
       }
       Thread.sleep(1000);
     }
     ...
     for (Queue.BuildableItem item : items) {
       assertFalse("Project " + project2.getDisplayName() + " stuck in pendings",
         item.task.getName().equals(project2.getName()));
     }
   }
   @Test
   public
   ...
   void queueApiOutputShouldBeFilteredByUserPermission() throws Exception {
     ...
     ProjectMatrixAuthorizationStrategy str =
     ...
     Map<Permission,Set<String>> permissions =
     ...
     AuthorizationMatrixProperty prop1 =
       new AuthorizationMatrixProperty(permissions);
     ...
     for (DomNode element : p.getFirstChild().getFirstChild().getChildNodes()) {
       if (element.getNodeName().equals("task")) {
         assertEquals(((DomElement)element).getElementsByTagName("name").size(),
           1);
         assertEquals(((DomElement)element).getElementsByTagName("name").item(0).getFirstChild().toString(),
           "project");
         assertEquals(((DomElement)element).getElementsByTagName("url").size(),
           1);
       }
     }
   ...
   public
   static
   class MyQueueTaskDispatcher extends QueueTaskDispatcher
   {
      @Override
      public
      CauseOfBlockage canTake(Node node, Queue.BuildableItem item) {
   ...
   void testBlockBuildWhenUpstreamBuildingLock() throws Exception {
     ...
     r.getInstance().save();
     final FreeStyleProject projectA =
       r.createFreeStyleProject(prefix + "A");
     projectA.getBuildersList().add(new SleepBuilder(5000));
     final FreeStyleProject projectB =
       r.createFreeStyleProject(prefix + "B");
     projectB.getBuildersList().add(new SleepBuilder(10000));
     projectB.setBlockBuildWhenUpstreamBuilding(true);
     final FreeStyleProject projectC =
       r.createFreeStyleProject(prefix + "C");
     projectC.getBuildersList().add(new SleepBuilder(10000));
     projectC.setBlockBuildWhenUpstreamBuilding(true);
     projectA.getPublishersList().add(new BuildTrigger(Arrays.asList(projectB),
                                        Result.SUCCESS));
     projectB.getPublishersList().add(new BuildTrigger(Arrays.asList(projectC),
                                        Result.SUCCESS));
     ...
     final QueueTaskFuture<FreeStyleBuild> taskB =
       projectB.scheduleBuild2(0, new TimerTriggerCause());
     final QueueTaskFuture<FreeStyleBuild> taskC =
     ...
     long buildBEndTime =
       buildB.getStartTimeInMillis() + buildB.getDuration();
     assertTrue("Project B build should be finished before the build of project C starts. " + "B finished at " + buildBEndTime + ", C started at " + 
       buildC.getStartTimeInMillis(),
