...
public abstract class Run<JobT extends Job<JobT, RunT>, RunT extends Run<JobT, RunT>> extends Actionable implements ExtensionPoint, Comparable<RunT>, AccessControlled, PersistenceRoot, DescriptorByNameOwner, OnMaster {
    ...
    volatile private String displayName;

    protected transient volatile State state;
    ...
    transient volatile private RunExecution runner;

    final private static SimpleDateFormat CANONICAL_ID_FORMATTER = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss");

    protected final static ThreadLocal<SimpleDateFormat> ID_FORMATTER = new IDFormatterProvider();

    final private static class IDFormatterProvider extends ThreadLocal<SimpleDateFormat> {
    ...
    final public Charset getCharset() {
        if (charset == null)
    ...
    @Exported
    final public boolean isKeepLog() {
    ...
    final public long getStartTimeInMillis() {
        if (startTime == 0)
            return timestamp;
        return startTime;
    }
    ...
    final public RunT getPreviousCompletedBuild() {
        RunT r = getPreviousBuild();
        while (r != null && r.isBuilding()) r = r.getPreviousBuild();
        return r;
    }

    final public RunT getPreviousBuildInProgress() {
    ...
    final public String getAbsoluteUrl() {
        return project.getAbsoluteUrl() + getNumber() + '/';
    }

    final public String getSearchUrl() {
        return getNumber() + "/";
    }
    ...
    final public static int LIST_CUTOFF = Integer.parseInt(System.getProperty("hudson.model.Run.ArtifactList.listCutoff", "16"));

    final public static int TREE_CUTOFF = Integer.parseInt(System.getProperty("hudson.model.Run.ArtifactList.treeCutoff", "40"));

    final public class ArtifactList extends ArrayList<Artifact> {
    ...
    static void waitForCheckpoint(CheckPoint id) throws InterruptedException {
        while (true) {
    ...
    public abstract class RunExecution {
        ...
        final private class CheckpointSet {

            final private Set<CheckPoint> checkpoints = new HashSet<CheckPoint>();
            ...
            protected synchronized boolean waitForCheckPoint(CheckPoint identifier) throws InterruptedException {
                ...
                try {
                    while (!allDone && !checkpoints.contains(identifier)) wait();
        ...
        final private CheckpointSet checkpoints = new CheckpointSet();

        final private Map<Object, Object> attributes = new HashMap<Object, Object>();
    ...
    final public static class RunnerAbortedException extends RuntimeException {
    ...
    final public void keepLog() throws IOException {
        keepLog(true);
    }
    ...
    final public EnvVars getCharacteristicEnvVars() {
    ...
    final public static XStream XSTREAM = new XStream2();

    final public static XStream2 XSTREAM2 = (XStream2) XSTREAM;
    ...
    final private static Logger LOGGER = Logger.getLogger(Run.class.getName());

    final public static Comparator<Run> ORDER_BY_DATE = new Comparator<Run>() {
    ...
    final public static FeedAdapter<Run> FEED_ADAPTER = new DefaultFeedAdapter();

    final public static FeedAdapter<Run> FEED_ADAPTER_LATEST = new DefaultFeedAdapter() {
    ...
    final public class KeepLogBuildBadge implements BuildBadgeAction {
    ...
    final public static PermissionGroup PERMISSIONS = new PermissionGroup(Run.class, Messages._Run_Permissions_Title());

    final public static Permission DELETE = new Permission(PERMISSIONS, "Delete", Messages._Run_DeletePermission_Description(), Permission.DELETE, PermissionScope.RUN);

    final public static Permission UPDATE = new Permission(PERMISSIONS, "Update", Messages._Run_UpdatePermission_Description(), Permission.UPDATE, PermissionScope.RUN);

    final public static Permission ARTIFACTS = new Permission(PERMISSIONS, "Artifacts", Messages._Run_ArtifactsPermission_Description(), null, Functions.isArtifactsPermissionEnabled(), new PermissionScope[] { PermissionScope.RUN });
    ...
    @Nonnull
    final public synchronized ArtifactManager pickArtifactManager() throws IOException {
    ...
    @Nonnull
    final public ArtifactManager getArtifactManager() {
        if (artifactManager != null) {
