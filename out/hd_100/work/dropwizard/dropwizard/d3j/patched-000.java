...
public
class Environment extends AbstractLifeCycle
{
   ...
   public
   Environment(String name, Configuration configuration, 
               ObjectMapperFactory objectMapperFactory) {
     this.name = name;
     this.configuration = configuration;
     this.objectMapperFactory = objectMapperFactory;
     this.config =
     new DropwizardResourceConfig(false)
     {
        @Override
        public
        void validate() {
   ...
   public
   void scanPackagesForResourcesAndProviders(Class<?>... classes) {
   ...
   public
   void manage(LifeCycle managed) {
     lifeCycle.addBean(checkNotNull(managed));
   }
   public
   ServletBuilder addServlet(Servlet servlet, String urlPattern) {
     final ServletHolder holder =
       new NonblockingServletHolder(checkNotNull(servlet));
     final ServletBuilder servletConfig =
       new ServletBuilder(holder, servlets);
     servletConfig.addUrlPattern(checkNotNull(urlPattern));
     return servletConfig;
   }
   public
   ServletBuilder addServlet(Class<? extends Servlet> klass, 
                             String urlPattern) {
   ...
   public
   FilterBuilder addFilter(Filter filter, String urlPattern) {
   ...
   public
   FilterBuilder addFilter(Class<? extends Filter> klass, String urlPattern) {
   ...
   public
   void addServletListeners(EventListener... listeners) {
   ...
   public
   void addProtectedTarget(String target) {
     protectedTargets.add(checkNotNull(target));
   }
   public
   void setBaseResource(Resource baseResource) throws IOException {
   ...
   public
   void enableJerseyFeature(String name) {
     config.getFeatures().put(checkNotNull(name), Boolean.TRUE);
   }
   public
   void disableJerseyFeature(String name) {
     config.getFeatures().put(checkNotNull(name), Boolean.FALSE);
   }
   public
   void setJerseyProperty(String name, @Nullable Object value) {
     config.getProperties().put(checkNotNull(name), value);
   }
   public
   ExecutorService managedExecutorService(String nameFormat, 
                                          int corePoolSize, 
                                          int maximumPoolSize, 
   ...
   public
   ScheduledExecutorService managedScheduledExecutorService(String nameFormat,
                                                            int corePoolSize) {
     final ThreadFactory threadFactory =
       new ThreadFactoryBuilder().setNameFormat(nameFormat).build();
   ...
   private
   void logHealthChecks() {
     final ImmutableSet.Builder<String> builder = ImmutableSet.builder();
     for (HealthCheck healthCheck : healthChecks.build()) {
       final String canonicalName =
         healthCheck.getClass().getCanonicalName();
   ...
   void logEndpoints() {
     ...
     for (Class<?> klass : builder.build()) {
       ...
       final ImmutableList.Builder<String> endpoints =
       ...
       for (AnnotatedMethod method : annotatedMethods(klass)) {
         ...
         final StringBuilder pathBuilder =
           new StringBuilder().append(rootPath).append(path);
         ...
         for (HttpMethod verb : method.getMetaMethodAnnotations(HttpMethod.class)) {
           endpoints.add(String.format("    %-7s %s (%s)", verb.value(),
                           pathBuilder.toString(), klass.getCanonicalName()));
         }
       }
   ...
   private
   MethodList annotatedMethods(Class<?> resource) {
   ...
   public
   ResourceConfig getJerseyResourceConfig() {
     return config;
   }
   public
   ...
   public
   void setJerseyServletContainer(ServletContainer jerseyServletContainer) {
