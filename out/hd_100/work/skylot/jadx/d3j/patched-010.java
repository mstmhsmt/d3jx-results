...
@JadxVisitor(name = "ModVisitor", desc = "Modify method instructions",
      ...
      for (int i = 0; i < size; i++) {
        ...
        case CONST_CLASS: {
          ...
          if (insn.getType() == InsnType.CONST_STR) {
            ...
            f = parentClass.getConstFieldByLiteralArg(
                (LiteralArg)insn.getArg(0));
          }
          if (f != null) {
            InsnNode inode =
                new IndexInsnNode(InsnType.SGET, f.getFieldInfo(), 0);
        ...
        case NEW_ARRAY:
          ...
          if (next < size) {
            ...
            if (ni.getType() == InsnType.FILL_ARRAY) {
              InsnNode filledArr = makeFilledArrayInsn(mth, (NewArrayNode)insn,
                                                       (FillArrayNode)ni);
              if (filledArr != null) {
        ...
        case ARITH:
          ...
          if (arithNode.getArgsCount() == 2) {
            ...
            if (litArg.isLiteral()) {
              FieldNode f =
                  parentClass.getConstFieldByLiteralArg((LiteralArg)litArg);
              if (f != null) {
                InsnNode fGet =
                    new IndexInsnNode(InsnType.SGET, f.getFieldInfo(), 0);
    ...
    if (arg.isRegister()) {
      ...
      if (sVar != null && sVar.getUseCount() == 1 && !sVar.isUsedInPhi()) {
        ...
        if (assignInsn != null && assignInsn.getType() == InsnType.CHECK_CAST) {
          ArgType assignCastType =
              (ArgType)((IndexInsnNode)assignInsn).getIndex();
...
<<<<<<< /root/work/skylot/jadx/d3j/patched-010-0.java
  private static void processInvoke(MethodNode mth, BlockNode block,
                                    int insnNumber,
    ...
    if (co.isSuper() && (co.getArgsCount() == 0 || parentClass.isEnum())) {
      ...
      MethodNode defCo =
          parentClass.searchMethodByShortId(callMth.getShortId());
      if (defCo == null || defCo.isNoCode()) {
        remove = true;
      }
    }
    if (parentClass.isAnonymous() && mth.isDefaultConstructor() &&
        co.isSuper()) {
      remove = true;
    }
    if (remove) {
    ...
    if (co.isNewInstance()) {
      ...
      if (newInstInsn != null) {
        ...
        if (!resultArg.equals(instArg)) {
          for (RegisterArg useArg :
               new ArrayList<>(instArg.getSVar().getUseList())) {
...
||||||| /root/work/skylot/jadx/d3j/base-010.java
  private static void processInvoke(MethodNode mth, BlockNode block,
                                    int insnNumber,
                                    InstructionRemover remover) {
    ClassNode parentClass = mth.getParentClass();
...
=======
>>>>>>> /root/work/skylot/jadx/d3j/patched-010-1.java
  ...
  private static Map<InsnArg, FieldNode>
  getArgsToFieldsMapping(MethodNode callMthNode, ConstructorInsn co) {
    ...
    if (callMth.getArgsCount() != 0 &&
        callMth.getArgumentsTypes().get(0).equals(
            parentClass.getClassInfo().getType())) {
      startArg = 1;
    }
...
<<<<<<< /root/work/skylot/jadx/d3j/patched-010-0.java
  private static ConstructorInsn processConstructor(MethodNode mth,
                                                    ConstructorInsn co) {
    MethodNode callMth = mth.dex().resolveMethod(co.getCallMth());
    if (callMth == null || !callMth.getAccessFlags().isSynthetic() ||
        !allArgsNull(co)) {
      return null;
    }
    ClassNode classNode =
        mth.dex().resolveClass(callMth.getParentClass().getClassInfo());
    if (classNode == null) {
      return null;
    }
    boolean passThis = co.getArgsCount() >= 1 && co.getArg(0).isThis();
    String ctrId = "<init>(" +
                   (passThis ? TypeGen.signature(co.getArg(0).getType()) : "") +
                   ")V";
    MethodNode defCtr = classNode.searchMethodByShortId(ctrId);
    if (defCtr == null) {
      return null;
    }
    ConstructorInsn newInsn = new ConstructorInsn(
        defCtr.getMethodInfo(), co.getCallType(), co.getInstanceArg());
...
=======
>>>>>>> /root/work/skylot/jadx/d3j/patched-010-1.java
  private static InsnNode makeFilledArrayInsn(MethodNode mth,
    ...
    if (!elType.equals(insnElementType) &&
        !insnArrayType.equals(ArgType.OBJECT)) {
      ErrorsCounter.methodWarn(mth,
                               "Incorrect type for fill-array insn " +
                                   InsnUtils.formatOffset(insn.getOffset()) +
                                   ", element type: " + elType +
                                   ", insn element type: " + insnElementType);
    }
    if (!elType.isTypeKnown()) {
      LOG.warn("Unknown array element type: {} in mth: {}", elType, mth);
      elType = insnElementType.isTypeKnown() ? insnElementType
                                             : elType.selectFirst();
      if (elType == null) {
...
<<<<<<< /root/work/skylot/jadx/d3j/patched-010-0.java
  private static InsnNode removeAssignChain(InsnNode insn,
                                            InstructionRemover remover,
                                            InsnType insnType) {
    if (insn == null) {
      return null;
...
=======
>>>>>>> /root/work/skylot/jadx/d3j/patched-010-1.java
  private static void processMoveException(BlockNode block, InsnNode insn,
                                           InstructionRemover remover) {
