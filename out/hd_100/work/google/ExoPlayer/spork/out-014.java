...
public final class DefaultHlsExtractorFactory implements HlsExtractorFactory {
   ...
   * Creates a factory for HLS segment extractors.
   *
   * @param payloadReaderFactoryFlags
   * 		Flags to add when constructing any {@link DefaultTsPayloadReaderFactory} instances. Other flags may be added on top of {@code payloadReaderFactoryFlags} when creating {@link DefaultTsPayloadReaderFactory}.
   */
  ...
  public Result createExtractor(Extractor previousExtractor, Uri uri, Format format, List<Format> muxedCaptionFormats, DrmInitData drmInitData, TimestampAdjuster timestampAdjuster, Map<String, List<String>> responseHeaders, ExtractorInput extractorInput) throws InterruptedException, IOException {
    ...
    if (!(extractorByFileExtension instanceof Mp3Extractor)) {
      Mp3Extractor mp3Extractor = /* flags= */
    ...
    if (!(extractorByFileExtension instanceof FragmentedMp4Extractor)) {
      FragmentedMp4Extractor fragmentedMp4Extractor = /* flags= */
      /* sideloadedTrack= */
      new FragmentedMp4Extractor(0, timestampAdjuster, null, drmInitData, muxedCaptionFormats != null ? muxedCaptionFormats : Collections.emptyList());
