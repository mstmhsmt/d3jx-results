...
public abstract class MediaCodecRenderer extends BaseRenderer {
  ...
  private static final byte[] ADAPTATION_WORKAROUND_BUFFER =
      Util.getBytesFromHexString("0000016742C00BDA259000000168CE0F13200000016" +
                                 "588840DCE7118A0002FBF1C31C3275D78");
  ...
  public MediaCodecRenderer(
      int trackType, MediaCodecSelector mediaCodecSelector,
    ...
    buffer = new DecoderInputBuffer(
        DecoderInputBuffer.BUFFER_REPLACEMENT_MODE_DISABLED);
  ...
  public final int supportsFormat(Format format) throws ExoPlaybackException {
    try {
      ...
      if ((formatSupport & FORMAT_SUPPORT_MASK) > FORMAT_UNSUPPORTED_DRM &&
        ...
        formatSupport =
            (formatSupport & ~FORMAT_SUPPORT_MASK) | FORMAT_UNSUPPORTED_DRM;
      }
  ...
  protected MediaCodecInfo getDecoderInfo(MediaCodecSelector mediaCodecSelector,
      ...
      throws DecoderQueryException {
    return mediaCodecSelector.getDecoderInfo(format.sampleMimeType,
      ...
      try {
        ...
        if (codecInfo == null && drmSessionRequiresSecureDecoder) {
          ...
          if (codecInfo != null) {
            Log.w(
                TAG,
                "Drm session requires secure decoder for " + mimeType + ", but "
                    + "no secure decoder available. Trying to proceed with " +
                    codecInfo.name + ".");
          }
        }
      } catch (DecoderQueryException e) {
        throwDecoderInitError(new DecoderInitializationException(
            format, e, drmSessionRequiresSecureDecoder,
            DecoderInitializationException.DECODER_QUERY_ERROR));
      }
      if (codecInfo == null) {
        throwDecoderInitError(new DecoderInitializationException(
            format, null, drmSessionRequiresSecureDecoder,
...
<<<<<<< /root/work/google/ExoPlayer/d3j/patched-058-0.java
    String codecName = codecInfo.name;
||||||| /root/work/google/ExoPlayer/d3j/base-058.java
    String codecName = decoderInfo.name;
    codecIsAdaptive = decoderInfo.adaptive &&
                      !codecNeedsDisableAdaptationWorkaround(codecName);
=======
    String codecName = codecInfo.name;
    codecIsAdaptive = decoderInfo.adaptive;
>>>>>>> /root/work/google/ExoPlayer/d3j/patched-058-1.java
    codecNeedsDiscardToSpsWorkaround =
        codecNeedsDiscardToSpsWorkaround(codecName, format);
    codecNeedsFlushWorkaround = codecNeedsFlushWorkaround(codecName);
    codecNeedsAdaptationWorkaround = codecNeedsAdaptationWorkaround(codecName);
    codecNeedsEosPropagationWorkaround =
        codecNeedsEosPropagationWorkaround(codecName);
    codecNeedsEosFlushWorkaround = codecNeedsEosFlushWorkaround(codecName);
    codecNeedsEosOutputExceptionWorkaround =
        codecNeedsEosOutputExceptionWorkaround(codecName);
    codecNeedsMonoChannelCountWorkaround =
        codecNeedsMonoChannelCountWorkaround(codecName, format);
    try {
      ...
      throwDecoderInitError(new DecoderInitializationException(
          format, e, drmSessionRequiresSecureDecoder, codecName));
    }
  ...
  protected boolean shouldInitCodec(MediaCodecInfo codecInfo) { return true; }
  protected final MediaCodec getCodec() { return codec; }
  protected final MediaCodecInfo getCodecInfo() { return codecInfo; }
  @Override
  ...
  private boolean feedInputBuffer() throws ExoPlaybackException {
    ...
    if (codecNeedsAdaptationWorkaroundBuffer) {
      ...
      codec.queueInputBuffer(inputIndex, 0, ADAPTATION_WORKAROUND_BUFFER.length,
                             0, 0);
      inputIndex = C.INDEX_UNSET;
    ...
    try {
      ...
      if (bufferEncrypted) {
        ...
        codec.queueInputBuffer(inputIndex, 0, buffer.data.limit(),
                               presentationTimeUs, 0);
      }
      inputIndex = C.INDEX_UNSET;
      codecReceivedBuffers = true;
  ...
  private static MediaCodec.CryptoInfo
  getFrameworkCryptoInfo(DecoderInputBuffer buffer,
                         int adaptiveReconfigurationBytes) {
    MediaCodec.CryptoInfo cryptoInfo =
        buffer.cryptoInfo.getFrameworkCryptoInfoV16();
  ...
  private boolean shouldWaitForKeys(boolean bufferEncrypted)
      throws ExoPlaybackException {
    if (drmSession == null ||
    ...
    if (drmSessionState == DrmSession.STATE_ERROR) {
      throw ExoPlaybackException.createForRenderer(drmSession.getError(),
                                                   getIndex());
    }
  ...
  protected void onInputFormatChanged(Format newFormat)
    ...
    format = newFormat;
    boolean drmInitDataChanged = !Util.areEqual(
        format.drmInitData, oldFormat == null ? null : oldFormat.drmInitData);
    if (drmInitDataChanged) {
      if (format.drmInitData != null) {
        ...
        pendingDrmSession = drmSessionManager.acquireSession(
            Looper.myLooper(), format.drmInitData);
  ...
  protected void onOutputFormatChanged(MediaCodec codec,
  ...
  public boolean isReady() {
    return format != null && !waitingForKeys &&
        (isSourceReady() || outputIndex >= 0 ||
         (codecHotswapDeadlineMs != C.TIME_UNSET &&
          SystemClock.elapsedRealtime() < codecHotswapDeadlineMs));
  }
  protected long getDequeueOutputBufferTimeoutUs() { return 0; }
  @SuppressWarnings("deprecation")
      ...
      if (codecNeedsEosOutputExceptionWorkaround && codecReceivedEos) {
        try {
          outputIndex = codec.dequeueOutputBuffer(
              outputBufferInfo, getDequeueOutputBufferTimeoutUs());
      ...
      if (outputIndex >= 0) {
        ...
        if ((outputBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) !=
          ...
          shouldSkipOutputBuffer =
              shouldSkipOutputBuffer(outputBufferInfo.presentationTimeUs);
        }
    ...
    if (codecNeedsEosOutputExceptionWorkaround && codecReceivedEos) {
      try {
        processedOutputBuffer = processOutputBuffer(
            positionUs, elapsedRealtimeUs, codec, outputBuffers[outputIndex],
            outputIndex, outputBufferInfo.flags,
  ...
  processOutputBuffer(long positionUs, long elapsedRealtimeUs, MediaCodec codec,
  ...
  isDrmSchemeSupported(DrmSessionManager drmSessionManager,
  ...
  private static boolean codecNeedsFlushWorkaround(String name) {
    return Util.SDK_INT < 18 ||
        (Util.SDK_INT == 18 && ("OMX.SEC.avc.dec".equals(name) ||
                                "OMX.SEC.avc.dec.secure".equals(name))) ||
