...
final class ChangeableUrls implements Iterable<URL> {
  ...
  public int size() { return this.urls.size(); }
  public URL[] toArray() {
    return this.urls.toArray(new URL[this.urls.size()]);
  }
  public List<URL> toList() { return Collections.unmodifiableList(this.urls); }
  @Override
  ...
  getUrlsFromClassPathOfJarManifestIfPossible(URL url) {
  ...
  private static List<URL> getUrlsFromClassPathAttribute(URL base,
                                                         Manifest manifest) {
    if (manifest == null) {
      return Collections.<URL>emptyList();
    }
...
<<<<<<< /root/work/spring-projects/spring-boot/d3j/patched-157-0.java
...
||||||| /root/work/spring-projects/spring-boot/d3j/base-157.java
    if (StringUtils.hasText(classPathAttribute)) {
      for (String entry :
           StringUtils.delimitedListToStringArray(classPathAttribute, " ")) {
        try {
          urls.add(new URL(base, entry));
        } catch (MalformedURLException ex) {
          throw new IllegalStateException(
...
=======
    ...
    List<URL> urls = new ArrayList<URL>();
    entries.length for (String entry : StringUtils.delimitedListToStringArray(
                            classPath, " ")) {
      try {
        urls.add(new URL(base, entry));
      } catch (MalformedURLException ex) {
        throw new IllegalStateException(
            "Class-Path attribute contains malformed URL", ex);
>>>>>>> /root/work/spring-projects/spring-boot/d3j/patched-157-1.java
      }
    }
    return urls;
  }
