 ...
 * @since 2.0.0
 */
@Configuration(proxyBeanMethods = false) @ConditionalOnEnabledEndpoint(endpoint = LogFileWebEndpoint.class) @EnableConfigurationProperties(value = LogFileWebEndpointProperties.class) public class LogFileWebEndpointAutoConfiguration {
  @Bean @ConditionalOnMissingBean @Conditional(value = LogFileCondition.class) public LogFileWebEndpoint logFileWebEndpoint(Environment environment, LogFileWebEndpointProperties properties) {
    return new LogFileWebEndpoint(environment, properties.getExternalFile());
  }

  private static class LogFileCondition extends SpringBootCondition {
    @SuppressWarnings(value = { "deprecation" }) @Override public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
