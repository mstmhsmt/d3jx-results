...
public class SEach extends AStatement {
  ...
  public SEach(Location location, String type, String name,
               AExpression expression, SBlock block) {
    super(location);
    this.type = type;
    this.name = name;
  ...
  void analyze(Locals locals) {
    ...
    try {
      ...
      throw createError(
          new IllegalArgumentException("Not a type [" + this.type + "]."));
    }
    ...
    } else if (expression.actual.sort == Sort.DEF ||
               Iterable.class.isAssignableFrom(expression.actual.clazz)) {
      analyzeIterable(locals, type);
    } else {
      throw createError(new IllegalArgumentException(
          "Illegal for each type [" + expression.actual.name + "]."));
    }
    if (block == null) {
      throw createError(
          new IllegalArgumentException("Extraneous for each loop."));
    }
  ...
  void analyzeArray(Locals variables, Type type) {
    array = variables.addVariable(location, expression.actual,
                                  "#array" + location.getOffset(), true, false);
    index = variables.addVariable(location, Definition.INT_TYPE,
                                  "#index" + location.getOffset(), true, false);
    indexed = Definition.getType(expression.actual.struct,
  ...
  void analyzeIterable(Locals variables, Type type) {
    iterator =
        variables.addVariable(location, Definition.getType("Iterator"),
                              "#itr" + location.getOffset(), true, false);
    if (expression.actual.sort == Sort.DEF) {
      method = null;
    } else {
      method =
          expression.actual.struct.methods.get(new MethodKey("iterator", 0));
      if (method == null) {
        throw createError(new IllegalArgumentException(
            "Unable to create iterator for the type [" +
            expression.actual.name + "]."));
      }
    }
    cast = AnalyzerCaster.getLegalCast(location, Definition.DEF_TYPE, type,
                                       true, true);
  }
  @Override
  ...
  void writeArray(MethodWriter writer) {
    ...
    writer.visitVarInsn(variable.type.type.getOpcode(Opcodes.ISTORE),
                        variable.slot);
    block.write(writer);
    writer.goTo(begin);
    writer.mark(end);
  ...
  void writeIterable(MethodWriter writer) {
    ...
    if (method == null) {
      ...
      String desc = org.objectweb.asm.Type.getMethodDescriptor(
          itr.type, Definition.DEF_TYPE.type);
      writer.invokeDynamic("iterator", desc, DEF_BOOTSTRAP_HANDLE,
<<<<<<< /root/work/elastic/elasticsearch/d3j/patched-146-0.java
                           (Object)DefBootstrap.ITERATOR);
||||||| /root/work/elastic/elasticsearch/d3j/base-146.java
                           (Object)DefBootstrap.ITERATOR, 0);
=======
                           (Object)DefBootstrap.ITERATOR, (Object)0);
>>>>>>> /root/work/elastic/elasticsearch/d3j/patched-146-1.java
    } else if (java.lang.reflect.Modifier.isInterface(
                   method.owner.clazz.getModifiers())) {
      writer.invokeInterface(method.owner.type, method.method);
    } else {
      writer.invokeVirtual(method.owner.type, method.method);
    }
    writer.visitVarInsn(iterator.type.type.getOpcode(Opcodes.ISTORE),
                        iterator.slot);
    Label begin = new Label();
    Label end = new Label();
    writer.mark(begin);
    writer.visitVarInsn(iterator.type.type.getOpcode(Opcodes.ILOAD),
                        iterator.slot);
