...
public
class FieldSortBuilderTests extends AbstractSortTestCase<FieldSortBuilder>
{
   @Override
   protected
   FieldSortBuilder createTestItem() {
     return randomFieldSortBuilder();
   }
   public
   static
   FieldSortBuilder randomFieldSortBuilder() {
     String fieldName =
       rarely() ? FieldSortBuilder.DOC_FIELD_NAME : randomAsciiOfLengthBetween(1,
                                                      10);
   ...
   @Override
   protected
   FieldSortBuilder mutate(FieldSortBuilder original) throws IOException {
     ...
     switch (parameter) {
       case 0:
         ...
         break;
       case 1:
         mutated.setNestedFilter(RandomSortDataGenerator.nestedFilter(
                                   mutated.getNestedFilter()));
         break;
       case 2:
   ...
   @Override
   protected
   void sortFieldAssertions(FieldSortBuilder builder, SortField sortField)
     throws IOException {
     ...
     assertEquals(builder.order() == SortOrder.ASC ? false : true,
   ...
   @Override
   protected
   FieldSortBuilder fromXContent(QueryParseContext context, String fieldName)
     throws IOException {
     return FieldSortBuilder.fromXContent(context, fieldName);
   }
   public
   void testReverseOptionFails() throws IOException {
     QueryParseContext context =
       new QueryParseContext(indicesQueriesRegistry);
     context.parseFieldMatcher(new ParseFieldMatcher(Settings.EMPTY));
     String json = "{ \"post_date\" : {\"reverse\" : true} },\n";
     XContentParser parser =
       XContentFactory.xContent(json).createParser(json);
     parser.nextToken();
     parser.nextToken();
     parser.nextToken();
