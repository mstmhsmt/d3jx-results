...
class DefaultSearchContext extends SearchContext
{
   ...
   public
   DefaultSearchContext(long id, ShardSearchRequest request, 
                        ...
                        TimeValue timeout, FetchPhase fetchPhase) {
     ...
     this.searcher =
     new ContextIndexSearcher(engineSearcher, indexService.cache().query(),
       indexShard.getQueryCachingPolicy());
   ...
   void preProcess() {
     if (scrollContext == null) {
       ...
       int maxResultWindow =
         indexService.getIndexSettings().getMaxResultWindow();
       if (resultWindow > maxResultWindow) {
         throw new QueryPhaseExecutionException(this,
                 "Result window is too large, from + size must be less than or equal to: [" + maxResultWindow + "] but was [" + resultWindow + "]. See the scroll api for a more efficient way to request large data sets. " + "This limit can be set by changing the [" + 
                 IndexSettings.MAX_RESULT_WINDOW_SETTING.getKey() + "] index level parameter.");
       }
     }
     aliasFilter =
     indexService.aliasFilter(indexShard.getQueryShardContext(),
       request.filteringAliases());
     if (query() == null) {
       parsedQuery(ParsedQuery.parsedMatchAllQuery());
     }
     if (queryBoost() != AbstractQueryBuilder.DEFAULT_BOOST) {
       parsedQuery(new ParsedQuery(new FunctionScoreQuery(query(),
                                     new WeightFactorFunction(queryBoost)),
                     parsedQuery()));
     }
     ...
     try {
       ...
       throw new QueryPhaseExecutionException(this,
               "Failed to rewrite main query", e);
     } 
   }
   private
   ParsedQuery buildFilteredQuery() {
     ...
     if (Queries.isConstantMatchAllQuery(query())) {
       ...
       result =
       new BooleanQuery.Builder().add(query, Occur.MUST).add(searchFilter,
         Occur.FILTER).build();
     }
   ...
   @Nullable
   public
   Query searchFilter(String[] types) {
     return createSearchFilter(types, aliasFilter,
              mapperService().hasNested());
   }
   static
   Query createSearchFilter(String[] types, Query aliasFilter, 
   ...
   @Override
   public
   long getOriginNanoTime() {
     return originNanoTime;
   }
   @Override
   protected
   long nowInMillisImpl() {
   ...
   @Override
   public
   SearchContext scrollContext(ScrollContext scrollContext) {
   ...
   SubPhaseContext getFetchSubPhaseContext(FetchSubPhase.ContextFactory<SubPhaseContext> contextFactory) {
     String subPhaseName = contextFactory.getName();
     if (subPhaseContexts.get(subPhaseName) == null) {
       subPhaseContexts.put(subPhaseName,
   ...
   @Override
   public
   void suggest(SuggestionSearchContext suggest) {
     this.suggest = suggest;
   }
   @Override
   public
   List<RescoreSearchContext> rescore() {
     if (rescore == null) {
   ...
   @Override
   public
   void addRescore(RescoreSearchContext rescore) {
   ...
   @Override
   public
   void terminateAfter(int terminateAfter) {
   ...
   @Override
   public
   SearchContext searchAfter(FieldDoc searchAfter) {
   ...
   private
   void filteredQuery(ParsedQuery filteredQuery) {
   ...
   @Override
   public
   SearchContext docIdsToLoad(int[] docIdsToLoad, int docsIdsToLoadFrom, 
   ...
   SearchLookup lookup() {
     if (searchLookup == null) {
       searchLookup =
       new SearchLookup(mapperService(), fieldData(), request.types());
     }
     return searchLookup;
   }
   @Override
   ...
   @Override
   public
   MappedFieldType smartNameFieldType(String name) {
   ...
   @Override
   public
   Map<Class<?>,Collector> queryCollectors() {
   ...
   public
   void setProfilers(Profilers profilers) {
