...
public class QueryRescorerIT extends ESIntegTestCase {
    // NORELEASE
    @Test
    @org.elasticsearch.messy.tests.AwaitsFix(bugUrl = "Need to fix default window size for rescorers so that they are applied")
    public void testEnforceWindowSize() {
        ...
        for (int j = 0; j < iters; j++) {
            ...
            assertFirstHit(searchResponse, hasScore(100.0F));
    ...
    public void testEquivalence() throws Exception {
        ...
        for (int i = 0; i < iters; i++) {
            ...
            SearchResponse rescored = // no weight - so we basically use the same score as the actual query
                    // ensure we hit the same shards for tie-breaking
            client().prepareSearch().setSearchType(SearchType.QUERY_THEN_FETCH).setPreference("test").setQuery(QueryBuilders.matchQuery("field1", query).operator(Operator.OR)).setFrom(0).setSize(resultSize).setRescorer(RescoreBuilder.queryRescorer(QueryBuilders.constantScoreQuery(QueryBuilders.matchPhraseQuery("field1", intToEnglish).slop(3))).setQueryWeight(1.0F).setRescoreQueryWeight(0.0F)).setRescoreWindow(rescoreWindow).execute().actionGet();
            SearchResponse plain = // ensure we hit the same shards for tie-breaking
            client().prepareSearch().setSearchType(SearchType.QUERY_THEN_FETCH).setPreference("test").setQuery(QueryBuilders.matchQuery("field1", query).operator(Operator.OR)).setFrom(0).setSize(resultSize).execute().actionGet();
            // check equivalence
            assertEquivalent(query, plain, rescored);
            rescored = // ensure we hit the same shards for tie-breaking
    ...
    public void testScoring() throws Exception {
        ...
        for (String scoreMode : scoreModes) {
            for (int i = 0; i < (numDocs - 4); i++) {
                ...
                SearchResponse rescored = // ensure we hit the same shards for tie-breaking
