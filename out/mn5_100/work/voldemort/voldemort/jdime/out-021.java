...
public class RoutedStore implements Store<ByteArray, byte[]> {
    ...
    Function<List<GetResult<Versioned<byte[]>>>, Void> readRepairFunction = new Function<List<GetResult<Versioned<byte[]>>>, Void>() {
      public Void apply(List<GetResult<Versioned<byte[]>>> nodeResults) {
        ...
        for (GetResult<Versioned<byte[]>> getResult : nodeResults) {
          fillRepairReadsValues(nodeValues, getResult.key, getResult.node, getResult.retrieved);
        }
        repairReads(nodeValues);
        return null;
      }
  ...
  private <R extends java.lang.Object> List<R> get(final ByteArray key, StoreOp<R> fetcher, Function<List<GetResult<R>>, Void> preReturnProcedure) throws VoldemortException {
    StoreUtils.assertValidKey(key);
    final List<Node> nodes = availableNodes(routingStrategy.routeRequest(key.get()));
  ...
  private void fillRepairReadsValues(final List<NodeValue<ByteArray, byte[]>> nodeValues, final ByteArray key, Node node, List<Versioned<byte[]>> fetched) {
    if (repairReads) {
      if (fetched.size() == 0) {
  ...
  private void checkRequiredReads(final List<Node> nodes) throws InsufficientOperationalNodesException {
  ...
  private <R extends java.lang.Object> String formatNodeValues(List<GetResult<R>> results) {
  ...
  private Versioned<byte[]> incremented(Versioned<byte[]> versioned, int nodeId) {
    return new Versioned<byte[]>(versioned.getValue(), ((VectorClock) versioned.getVersion()).incremented(nodeId, time.getMilliseconds()));
  }
  ...
  private final class GetCallable<R extends java.lang.Object> implements Callable<GetResult<R>> {
    ...
    public GetResult<R> call() throws Exception {
      ...
      try {
        if (logger.isTraceEnabled()) {
          logger.trace("Attempting get operation on node " + node.getId() + " for key \'" + ByteUtils.toHexString(key.get()) + "\'.");
        }
  ...
  private final static class GetResult<R extends java.lang.Object> {
    final Node node;
        ...
        if (repairReads) {
          ...
          for (ByteArray nodeKey : nodeKeys) {
            if (!retrieved.containsKey(nodeKey)) {
              fillRepairReadsValues(nodeValues, nodeKey, node, Collections.<Versioned<byte[]>>emptyList());
            }
          }
        }
        failureDetector.recordSuccess(node);
  ...
  private interface StoreOp<R extends java.lang.Object> {
