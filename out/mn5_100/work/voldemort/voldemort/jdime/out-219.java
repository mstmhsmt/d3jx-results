...
public class VoldemortServer extends AbstractService {
  ...
  public VoldemortServer(VoldemortConfig config) {
    ...
    this.metadata = MetadataStore.readFromDirectory(new File(this.voldemortConfig.getMetadataDirectory()), voldemortConfig.getNodeId());
  ...
  @Override protected void startInner() throws VoldemortException {
  ...
  /**
     ...
     */
  @Override protected void stopInner() throws VoldemortException {
  ...
  @JmxOperation(description = "force restore data from replication") public void restoreDataFromReplication(int numberOfParallelTransfers) {
    AdminClient adminClient = RebalanceUtils.createTempAdminClient(voldemortConfig, metadata.getCluster(), numberOfParallelTransfers * 2, numberOfParallelTransfers * 2);
    try {
