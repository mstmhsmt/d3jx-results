 ...
 * 
 */
@RunWith(value = Parameterized.class) public class AdminServiceFailureTest extends TestCase {
  ...
  @Override @Before public void setUp() throws IOException {
    ...
    failingStorageEngine = new RandomlyFailingDelegatingStore<ByteArray, byte[]>(new InMemoryStorageEngine<ByteArray, byte[]>(storeDefs.get(0).getName()));
    adminServer = getAdminServer(cluster.getNodeById(0), cluster, storeDefs, failingStorageEngine);
    adminClient = ServerTestUtils.getAdminClient(cluster);
    adminServer.start();
  }

  private AbstractSocketService getAdminServer(Node node, Cluster cluster, List<StoreDefinition> storeDefs, StorageEngine<ByteArray, byte[]> storageEngine) throws IOException {
    ...
    return 
<<<<<<< commits-mn5_100/voldemort/voldemort/54c90faffbc00bea025ff3edd8759e9a1f2ccce3/AdminServiceFailureTest-790a4b5.java
    ServerTestUtils.getSocketService(useNio, requestHandlerFactory, node.getAdminPort(), 2, 2, 10000)
=======
    new SocketService(new SocketRequestHandlerFactory(null, storeRepository, ServerTestUtils.createMetadataStore(cluster, storeDefs), ServerTestUtils.createServerConfig(0, TestUtils.createTempDir().getAbsolutePath(), null, null, new Properties()), null, null), node.getAdminPort(), 2, 2, 10000, "test-admin-service", false)
>>>>>>> commits-mn5_100/voldemort/voldemort/1f1d05f4e817dcdbd110f56b028ea341f933512b/AdminServiceFailureTest-2b9d75b.java
    ;
  }

  @Override @After public void tearDown() throws IOException {
    try {
      adminServer.stop();
      adminClient.stop();
    } catch (Exception e) {
  ...
  @Test public void testWithStartFailure() {
  ...
  private <K extends java.lang.Object> void consumeIterator(Iterator<K> iterator) {
