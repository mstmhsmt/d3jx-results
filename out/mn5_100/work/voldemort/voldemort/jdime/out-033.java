 ...
 * 
 */
@JmxManaged public class SocketServer extends Thread {
  ...
  public SocketServer(int port, int defaultThreads, int maxThreads, int socketBufferSize, RequestHandlerFactory handlerFactory) {
    this.port = port;
  ...
  private final RejectedExecutionHandler rejectedExecutionHandler = new RejectedExecutionHandler() {
    public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
      ...
      if (interrupted()) {
        logger.info("Denying connection from " + session.getSocket().getRemoteSocketAddress() + ", server is shutting down.");
      } else {
        logger.error("Too many open connections, " + executor.getActiveCount() + " of " + executor.getLargestPoolSize() + " threads in use, denying connection from " + session.getSocket().getRemoteSocketAddress());
      }
      try {
  ...
  private void configureSocket(Socket socket) throws SocketException {
    ...
    if (socket.getReceiveBufferSize() != this.socketBufferSize) {
      logger.debug("Requested socket receive buffer size was " + this.socketBufferSize + " bytes but actual size is " + socket.getReceiveBufferSize() + " bytes.");
    }
    if (socket.getSendBufferSize() != this.socketBufferSize) {
      logger.debug("Requested socket send buffer size was " + this.socketBufferSize + " bytes but actual size is " + socket.getSendBufferSize() + " bytes.");
    }
  }
  ...
  @JmxOperation(description = "Kill all the active sessions.") public void killActiveSessions() {
  ...
  @JmxGetter(name = "port", description = "The port on which the server accepts connections.") public int getPort() {
    return this.port;
  }

  @JmxGetter(name = "maxThreads", description = "The maximum number of threads that can be started on the server.") public int getMaxThreads() {
    return this.maxThreads;
  }

  @JmxGetter(name = "currentThreads", description = "The current number of utilized threads on the server.") public int getCurrentThreads() {
    return this.threadPool.getActiveCount();
  }

  @JmxGetter(name = "remainingThreadCapacity", description = "The number of additional threads that can be allocated before reaching the maximum.") public int getRemainingThreads() {
