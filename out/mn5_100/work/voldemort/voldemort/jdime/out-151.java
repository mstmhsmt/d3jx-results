...
public class AdminServiceRequestHandler implements RequestHandler {
    ...
    if (voldemortConfig != null) {
      ...
      if (className == null || className.trim().length() == 0) {
        ...
        try {
          ...
          this.fileFetcher = (FileFetcher) ReflectUtils.callConstructor(cls, new Class<?>[] { Props.class }, new Object[] { voldemortConfig.getAllProps() });
        } catch (Exception e) {
  ...
  public StreamRequestHandler handleRequest(final DataInputStream inputStream, final DataOutputStream outputStream) throws IOException {
    ...
    switch (request.getType()) {
      ...
      ProtoUtils.writeMessage(outputStream, handleSwapStoresAndCleanState(request.getSwapStoresAndCleanState()));
      break;
      default:
  ...
  public StreamRequestHandler handleFetchPartitionFiles(VAdminProto.FetchPartitionFilesRequest request) {
    return new FetchPartitionFileStreamRequestHandler(request, metadataStore, voldemortConfig, storeRepository);
  }
  ...
  public StreamRequestHandler handleFetchPartitionEntries(VAdminProto.FetchPartitionEntriesRequest request) {
    ...
    if (fetchValues) {
      ...
      if (fetchMasterValues) {
        return new FetchMasterEntriesStreamRequestHandler(request, metadataStore, errorCodeMapper, voldemortConfig, storeRepository, networkClassLoader);
      } else {
        return new FetchEntriesStreamRequestHandler(request, metadataStore, errorCodeMapper, voldemortConfig, storeRepository, networkClassLoader);
      }
    } else {
      return new FetchKeysStreamRequestHandler(request, metadataStore, errorCodeMapper, voldemortConfig, storeRepository, networkClassLoader);
    }
  }

  public StreamRequestHandler handleUpdatePartitionEntries(VAdminProto.UpdatePartitionEntriesRequest request) {
    return new UpdatePartitionEntriesStreamRequestHandler(request, errorCodeMapper, voldemortConfig, storeRepository, networkClassLoader);
  }
  ...
  public VAdminProto.AsyncOperationStatusResponse handleRebalanceNode(VAdminProto.InitiateRebalanceNodeRequest request) {
    ...
    try {
      ...
      RebalancePartitionsInfo rebalanceStealInfo = new RebalancePartitionsInfo(request.getStealerId(), request.getDonorId(), request.getPartitionsList(), request.getDeletePartitionsList(), request.getStealMasterPartitionsList(), request.getUnbalancedStoreList(), encodeROStoreVersionDirMap(request.getStealerRoStoreToDirList()), encodeROStoreVersionDirMap(request.getDonorRoStoreToDirList()), request.getAttempt());
    ...
    try {
      ...
      if (request.hasPushVersion()) {
        ...
        if (pushVersion <= store.getCurrentVersionId()) {
          throw new VoldemortException("Version of push specified (" + pushVersion + ") should be greater than current version " + store.getCurrentVersionId());
        }
      } else {
...
<<<<<<< commits-mn5_100/voldemort/voldemort/566b4bc712f6f3945a1a74bf1293f070c615868b/AdminServiceRequestHandler-e4e2c2d.java
...
>>>>>>> commits-mn5_100/voldemort/voldemort/01fad80c857610cedc5b146c6f6d3c405509ac79/AdminServiceRequestHandler-2d43de5.java
          }  finally {
            adminClient.stop();
          }
        }
