...
public
class VoldemortServer extends AbstractService
{
   private static final Logger logger =
     Logger.getLogger(VoldemortServer.class.getName());
   ...
   VoldemortServer(VoldemortConfig config) {
     ...
     this.metadata =
     MetadataStore.readFromDirectory(new File(this.voldemortConfig.getMetadataDirectory()),
       voldemortConfig.getNodeId());
     this.identityNode =
     metadata.getCluster().getNodeById(voldemortConfig.getNodeId());
   ...
   VoldemortServer(VoldemortConfig config, Cluster cluster) {
     ...
     this.storeRepository = new StoreRepository();
     ConfigurationStorageEngine metadataInnerEngine =
       new ConfigurationStorageEngine("metadata-config-store",
     ...
     this.metadata =
     new MetadataStore(metadataInnerEngine, voldemortConfig.getNodeId());
     this.services = createServices();
   }
   public
   AsyncOperationRunner getAsyncRunner() {
     return asyncRunner;
   }
   private
   List<VoldemortService> createServices() {
     List<VoldemortService> services = new ArrayList<VoldemortService>();
     SchedulerService scheduler =
       new SchedulerService(voldemortConfig.getSchedulerThreads(),
         SystemTime.INSTANCE);
     StorageService storageService =
       new StorageService(storeRepository, metadata, scheduler,
         voldemortConfig);
     asyncRunner =
     new AsyncOperationRunner(scheduler, ASYNC_REQUEST_CACHE_SIZE);
     services.add(storageService);
     services.add(scheduler);
     services.add(asyncRunner);
     if (voldemortConfig.isHttpServerEnabled())
       services.add(new HttpService(this, storageService, storeRepository,
     ...
     if (voldemortConfig.isSocketServerEnabled()) {
       RequestHandlerFactory socketRequestHandlerFactory =
         new SocketRequestHandlerFactory(storageService,
           this.storeRepository, this.metadata, this.voldemortConfig,
       ...
       if (voldemortConfig.getUseNioConnector()) {
         ...
         services.add(new NioSocketService(socketRequestHandlerFactory,
                        ...
                        "nio-socket-server", voldemortConfig.isJmxEnabled()));
       } else {
         ...
         services.add(new SocketService(socketRequestHandlerFactory,
                        ...
                        voldemortConfig.getMaxThreads(),
                        voldemortConfig.getSocketBufferSize(),
                        "socket-server", voldemortConfig.isJmxEnabled()));
       }
     }
     if (voldemortConfig.isAdminServerEnabled()) {
       ...
       if (voldemortConfig.isEnableRebalanceService()) {
         RebalancerService rebalancerService =
           new RebalancerService(metadata, voldemortConfig, asyncRunner,
             scheduler);
         services.add(rebalancerService);
         rebalancer = rebalancerService.getRebalancer();
       }
       SocketRequestHandlerFactory adminRequestHandlerFactory =
         new SocketRequestHandlerFactory(storageService,
           this.storeRepository, this.metadata, this.voldemortConfig,
           this.asyncRunner, rebalancer);
       if (voldemortConfig.getUseNioConnector()) {
         ...
         services.add(new NioSocketService(adminRequestHandlerFactory,
                        ...
                        voldemortConfig.getNioConnectorSelectors(),
                        "admin-server", voldemortConfig.isJmxEnabled()));
       } else {
     ...
     if (voldemortConfig.isGossipEnabled()) {
       services.add(new GossipService(this.metadata, scheduler,
                      voldemortConfig));
     }
     if (voldemortConfig.isJmxEnabled())
       services.add(new JmxService(this, this.metadata.getCluster(),
                      storeRepository, services));
     return ImmutableList.copyOf(services);
   }
   @Override
   protected
   void startInner() throws VoldemortException {
   ...
   @Override
   protected
   void stopInner() throws VoldemortException {
     List<VoldemortException> exceptions =
       new ArrayList<VoldemortException>();
   ...
   void main(String[] args) throws Exception {
     ...
     try {
       if (args.length == 0)
         config = VoldemortConfig.loadFromEnvironmentVariable(); else if (args.length == 1)
         config = VoldemortConfig.loadFromVoldemortHome(args[0]); else
   ...
   public
   List<VoldemortService> getServices() {
     return services;
   }
   public
   ...
   void restoreDataFromReplication(int numberOfParallelTransfers) {
     AdminClient adminClient =
       RebalanceUtils.createTempAdminClient(voldemortConfig,
         metadata.getCluster(), numberOfParallelTransfers * 2,
         numberOfParallelTransfers * 2);
     try {
       adminClient.restoreDataFromReplications(metadata.getNodeId(),
