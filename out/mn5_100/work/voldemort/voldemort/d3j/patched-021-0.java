...
public class RoutedStore implements Store<ByteArray, byte[]> {
  private static final Logger logger =
      Logger.getLogger(RoutedStore.class.getName());
  private final static StoreOp<Versioned<byte[]>> VERSIONED_OP =
      new StoreOp<Versioned<byte[]>>() {
        public List<Versioned<byte[]>> execute(Store<ByteArray, byte[]> store,
                                               ByteArray key) {
          return store.get(key);
        }
      };
  private final static StoreOp<Version> VERSION_OP = new StoreOp<Version>() {
    public List<Version> execute(Store<ByteArray, byte[]> store,
                                 ByteArray key) {
  ...
  public RoutedStore(String name,
                     ...
                     Cluster cluster, StoreDefinition storeDef,
                     int numberOfThreads, boolean repairReads, long timeoutMs,
                     FailureDetector failureDetector) {
    this(name, innerStores, cluster, storeDef, repairReads,
         Executors.newFixedThreadPool(numberOfThreads), timeoutMs,
         failureDetector, SystemTime.INSTANCE);
  }
  public RoutedStore(String name,
                     Map<Integer, Store<ByteArray, byte[]>> innerStores,
                     Cluster cluster, StoreDefinition storeDef,
                     boolean repairReads, ExecutorService threadPool,
                     long timeoutMs, FailureDetector failureDetector,
                     Time time) {
    ...
    if (storeDef.getPreferredReads() < storeDef.getRequiredReads())
      throw new IllegalArgumentException(
          "storeDef.getPreferredReads() must be greater or equal to " +
          "storeDef.getRequiredReads().");
    if (storeDef.getPreferredWrites() < storeDef.getRequiredWrites())
      throw new IllegalArgumentException(
          "storeDef.getPreferredWrites() must be greater or equal to " +
          "storeDef.getRequiredWrites().");
    if (storeDef.getPreferredReads() > innerStores.size())
      throw new IllegalArgumentException(
          "storeDef.getPreferredReads() is larger than the total number of " +
          "nodes!");
    if (storeDef.getPreferredWrites() > innerStores.size())
      throw new IllegalArgumentException(
          "storeDef.getPreferredWrites() is larger than the total number of " +
          "nodes!");
    this.name = name;
    this.innerStores =
        new ConcurrentHashMap<Integer, Store<ByteArray, byte[]>>(innerStores);
  ...
  public boolean delete(final ByteArray key, final Version version)
    ...
    for (final Node node : nodes) {
      this.executor.execute(new Runnable() {
        public void run() {
          try {
            boolean deleted =
                innerStores.get(node.getId()).delete(key, version);
    ...
    if (this.storeDef.getPreferredWrites() <= 0) {
      ...
      for (int i = 0; i < numNodes; i++) {
        try {
          ...
          if (!acquired)
            logger.warn("Delete operation timed out waiting for operation " +
                        i + " to complete after waiting " + timeoutMs + " ms.");
    ...
    if (successes.get() < storeDef.getRequiredWrites())
      throw new InsufficientOperationalNodesException(
          this.storeDef.getRequiredWrites() + " deletes required, but " +
              successes.get() + " succeeded.",
          failures);
    else
      return deletedSomething.get();
  }
  public Map<ByteArray, List<Versioned<byte[]>>>
  getAll(Iterable<ByteArray> keys) throws VoldemortException {
    StoreUtils.assertValidKeys(keys);
    Map<ByteArray, List<Versioned<byte[]>>> result =
        StoreUtils.newEmptyHashMap(keys);
    ...
    for (ByteArray key : keys) {
      ...
      List<Node> preferredNodes =
          Lists.newArrayListWithCapacity(preferredReads);
    ...
    try {
      futures =
          executor.invokeAll(callables, timeoutMs * 3, TimeUnit.MILLISECONDS);
    } catch (InterruptedException e) {
      throw new InsufficientOperationalNodesException(
          ...
          for (Node node : extraNodes) {
            try {
              List<Versioned<byte[]>> values =
                  innerStores.get(node.getId()).get(key);
    ...
    for (Map.Entry<ByteArray, MutableInt> mapEntry :
         keyToSuccessCount.entrySet()) {
  ...
  public List<Versioned<byte[]>> get(ByteArray key) {
    Function<List<GetResult<Versioned<byte[]>>>, Void> readRepairFunction =
        new Function<List<GetResult<Versioned<byte[]>>>, Void>() {
          public Void apply(List<GetResult<Versioned<byte[]>>> nodeResults) {
            List<NodeValue<ByteArray, byte[]>> nodeValues =
                Lists.newArrayListWithExpectedSize(nodeResults.size());
            for (GetResult<Versioned<byte[]>> getResult : nodeResults)
              fillRepairReadsValues(nodeValues, getResult.key, getResult.node,
                                    getResult.retrieved);
            repairReads(nodeValues);
            return null;
          }
  ...
  private <R> List<R> get(final ByteArray key, StoreOp<R> fetcher,
    ...
    int nodeIndex = 0;
    List<Callable<GetResult<R>>> callables =
        Lists.newArrayListWithCapacity(attempts);
    ...
    while (successes < this.storeDef.getPreferredReads() &&
           nodeIndex < nodes.size()) {
      Node node = nodes.get(nodeIndex);
      try {
        retrieved.add(new GetResult<R>(
            node, key, fetcher.execute(innerStores.get(node.getId()), key),
            null));
        ++successes;
        failureDetector.recordSuccess(node);
      } catch (UnreachableStoreException e) {
  ...
  private void
  fillRepairReadsValues(final List<NodeValue<ByteArray, byte[]>> nodeValues,
                        final ByteArray key, Node node,
    ...
    if (repairReads) {
      if (fetched.size() == 0)
        ...
        for (Versioned<byte[]> f : fetched)
          nodeValues.add(
              new NodeValue<ByteArray, byte[]>(node.getId(), key, f));
      }
    }
  }
  private NodeValue<ByteArray, byte[]> nullValue(Node node, ByteArray key) {
    return new NodeValue<ByteArray, byte[]>(node.getId(), key,
                                            new Versioned<byte[]>(null));
  }
  private void
  repairReads(final List<NodeValue<ByteArray, byte[]>> nodeValues) {
    if (!repairReads || nodeValues.size() <= 1 ||
        storeDef.getPreferredReads() <= 1)
      return;
    this.executor.execute(new Runnable() {
      public void run() {
        for (NodeValue<ByteArray, byte[]> v :
             readRepairer.getRepairs(nodeValues)) {
          try {
            ...
            if (logger.isDebugEnabled())
              logger.debug("Read repair cancelled due to application level " +
                           "exception on node " +
  ...
  private void checkRequiredReads(final List<Node> nodes)
    ...
    if (nodes.size() < this.storeDef.getRequiredReads())
      throw new InsufficientOperationalNodesException(
          "Only " + nodes.size() + " nodes in preference list, but " +
  ...
  public void put(final ByteArray key, final Versioned<byte[]> versioned)
    ...
    boolean noTimeout = blockOnPut(startNs, semaphore, 0, blockCount, successes,
                                   storeDef.getPreferredWrites());
    if (successes.get() < storeDef.getRequiredWrites()) {
      if (noTimeout) {
        int startingIndex = blockCount - 1;
        blockCount = Math.max(storeDef.getPreferredWrites() - 1, attempts);
        blockOnPut(startNs, semaphore, startingIndex, blockCount, successes,
                   storeDef.getRequiredWrites());
      }
      if (successes.get() < storeDef.getRequiredWrites())
        throw new InsufficientOperationalNodesException(
            successes.get() + " writes succeeded, but " +
  ...
  private boolean blockOnPut(long startNs, Semaphore semaphore,
                             int startingIndex, int blockCount,
                             AtomicInteger successes, int successesRequired) {
    for (int i = startingIndex; i < blockCount; i++) {
      try {
        ...
        if (!acquiredPermit) {
          logger.warn("Timed out waiting for put # " + (i + 1) + " of " +
                      blockCount + " to succeed.");
          return false;
        }
  ...
  private Versioned<byte[]> incremented(Versioned<byte[]> versioned,
                                        int nodeId) {
    return new Versioned<byte[]>(
        versioned.getValue(), ((VectorClock)versioned.getVersion())
                                  .incremented(nodeId, time.getMilliseconds()));
  }
  ...
  private final static class GetResult<R> {
    ...
    public GetResult(Node node, ByteArray key, List<R> retrieved,
                     Throwable exception) {
      this.node = node;
      this.key = key;
  ...
  private final class GetAllCallable implements Callable<GetAllResult> {
    ...
    public GetAllResult call() {
      ...
      try {
        ...
        if (repairReads) {
          for (Map.Entry<ByteArray, List<Versioned<byte[]>>> entry :
               retrieved.entrySet())
            fillRepairReadsValues(nodeValues, entry.getKey(), node,
                                  entry.getValue());
          for (ByteArray nodeKey : nodeKeys) {
            if (!retrieved.containsKey(nodeKey))
              fillRepairReadsValues(nodeValues, nodeKey, node,
