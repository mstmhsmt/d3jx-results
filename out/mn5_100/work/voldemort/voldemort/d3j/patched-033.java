...
class SocketServer extends Thread
{
   ...
   public
   SocketServer(int port, int defaultThreads, int maxThreads, 
                int socketBufferSize, RequestHandlerFactory handlerFactory) {
     this.port = port;
     ...
     this.threadPool =
     new ThreadPoolExecutor(defaultThreads, maxThreads, 0,
   ...
   private final ThreadFactory threadFactory =
     new ThreadFactory()
     {
        ...
        Thread newThread(Runnable r) {
          String name =
            "voldemort-server-" + threadIdSequence.getAndIncrement();
        ...
        void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
          ...
          if (interrupted()) {
            ...
            logger.error("Too many open connections, " + executor.getActiveCount() + " of " + 
              executor.getLargestPoolSize() + " threads in use, denying connection from " + 
     ...
     try {
       ...
       while (!isInterrupted() && !serverSocket.isClosed()) {
         ...
         this.threadPool.execute(new SocketServerSession(activeSessions,
                                   socket, handlerFactory, sessionId));
       }
   ...
   void configureSocket(Socket socket) throws SocketException {
     ...
     if (socket.getReceiveBufferSize() != this.socketBufferSize)
       logger.debug("Requested socket receive buffer size was " + this.socketBufferSize + " bytes but actual size is " + 
         socket.getReceiveBufferSize() + " bytes.");
     if (socket.getSendBufferSize() != this.socketBufferSize)
       logger.debug("Requested socket send buffer size was " + this.socketBufferSize + " bytes but actual size is " + 
   ...
   @JmxGetter(name="port",
              description="The port on which the server accepts connections.")
   public
   int getPort() {
     return this.port;
   }
   @JmxGetter(name="maxThreads",
              description="The maximum number of threads that can be started on the server.")
   public
   int getMaxThreads() {
     return this.maxThreads;
   }
   @JmxGetter(name="currentThreads",
              description="The current number of utilized threads on the server.")
   public
   int getCurrentThreads() {
     return this.threadPool.getActiveCount();
   }
   @JmxGetter(name="remainingThreadCapacity",
              description="The number of additional threads that can be allocated before reaching the maximum.")
   public
