...
public
class ReadOnlyStorageEngine implements StorageEngine<ByteArray,byte[]>
{
   private static Logger logger =
     Logger.getLogger(ReadOnlyStorageEngine.class);
   public static final int MEMORY_OVERHEAD_PER_KEY =
     ReadOnlyUtils.KEY_HASH_SIZE + 4 + 12 + 8;
   ...
   public
   ReadOnlyStorageEngine(String name, SearchStrategy searchStrategy, 
                         RoutingStrategy routingStrategy, int nodeId, 
                         File storeDir, int numBackups) {
   ...
   void open(File versionDir) {
     ...
     try {
       ...
       logger.info("Creating symbolic link for '" + getName() + "' using directory " + 
         versionDir.getAbsolutePath());
       Utils.symlink(versionDir.getAbsolutePath(),
         storeDir.getAbsolutePath() + File.separator + "latest");
   ...
   File getCurrentVersion() {
     ...
     if (versionDirs == null || versionDirs.length == 0) {
       ...
       return ReadOnlyUtils.findKthVersionedDir(versionDirs,
                versionDirs.length - 1, versionDirs.length - 1)[0];
     }
   }
   public
   ...
   public
   void swapFiles(String newStoreDirectory) {
     ...
     if (newVersionId == -1 || previousVersionId == -1)
       throw new VoldemortException("Unable to parse folder names (" + 
               newVersionDir.getName() + "," + previousVersionDir.getName() + ") since format(version-n) is incorrect");
     ...
     try {
       ...
       try {
         ...
         fileModificationLock.writeLock().unlock();
         if (success)
           logger.info("Swap operation completed successfully on store " + 
             getName() + ", releasing lock."); else
   ...
   void deleteBackups() {
     File[] storeDirList =
       ReadOnlyUtils.getVersionDirs(storeDir, 0L, currentVersionId);
     if (storeDirList.length > (numBackups + 1)) {
       File[] extraBackups =
         ReadOnlyUtils.findKthVersionedDir(storeDirList, 0,
           storeDirList.length - (numBackups + 1) - 1);
   ...
   private
   void deleteAsync(final File file) {
   ...
   public
   void rollback(File rollbackToDir) {
     ...
     try {
       ...
       File[] backUpDirs =
         ReadOnlyUtils.getVersionDirs(storeDir, versionId, Long.MAX_VALUE);
       if (backUpDirs.length <= 1) {
         logger.warn("No rollback performed since there are no back-up directories");
         return;
       }
       backUpDirs =
       ReadOnlyUtils.findKthVersionedDir(backUpDirs, 0,
         backUpDirs.length - 1);
       if (isOpen)
         close();
       open(rollbackToDir);
       ...
       for (int index = 1; index < backUpDirs.length; index++) {
         Utils.move(backUpDirs[index],
           new File(storeDir,
             backUpDirs[index].getName() + "." + df.format(new Date()) + ".bak"));
       }
     } finally{
       fileModificationLock.writeLock().unlock();
   ...
   public
   List<Versioned<byte[]>> get(ByteArray key) throws VoldemortException {
     ...
     try {
       ...
       int location =
         searchStrategy.indexOf(fileSet.indexFileFor(chunk),
           ByteUtils.md5(key.get()), fileSet.getIndexFileSize(chunk));
       if (location >= 0) {
   ...
   public
   Map<ByteArray,List<Versioned<byte[]>>> getAll(Iterable<ByteArray> keys)
       ...
       for (ByteArray key : keys) {
         ...
         int valueLocation =
           searchStrategy.indexOf(fileSet.indexFileFor(chunk),
             ByteUtils.md5(key.get()), fileSet.getIndexFileSize(chunk));
         if (valueLocation >= 0)
           keysAndValueLocations.add(new KeyValueLocation(chunk, key,
                                       valueLocation));
       }
       Collections.sort(keysAndValueLocations);
       for (KeyValueLocation keyVal : keysAndValueLocations) {
         byte[] value =
           readValue(keyVal.getChunk(), keyVal.getValueLocation());
         results.put(keyVal.getKey(),
           Collections.singletonList(Versioned.value(value)));
       }
       return results;
     } finally{
       fileModificationLock.readLock().unlock();
   ...
   private
   byte[] readValue(int chunk, int valueLocation) {
   ...
   public
   boolean delete(ByteArray key, Version version) throws VoldemortException {
     throw new UnsupportedOperationException("Delete is not supported on this store, it is read-only.");
   }
   public
   void put(ByteArray key, Versioned<byte[]> value) throws VoldemortException {
   ...
   public
   Object getCapability(StoreCapabilityType capability) {
     throw new NoSuchCapabilityException(capability, getName());
   }
   private
   final
   static
   class KeyValueLocation implements Comparable<KeyValueLocation>
   {
      private final int chunk;
      private final ByteArray key;
      private final int valueLocation;
      private
      KeyValueLocation(int chunk, ByteArray key, int valueLocation) {
        super();
        this.chunk = chunk;
        this.key = key;
      ...
      public
      int compareTo(KeyValueLocation kvl) {
        if (chunk == kvl.getChunk()) {
          if (valueLocation == kvl.getValueLocation())
            return ByteUtils.compare(getKey().get(), kvl.getKey().get()); else
   ...
   public
   List<Version> getVersions(ByteArray key) {
