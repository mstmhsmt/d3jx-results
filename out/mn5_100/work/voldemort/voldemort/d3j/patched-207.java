...
class ServerTestUtils
{
   ...
   StoreRepository getStores(String storeName, String clusterXml, 
     ...
     MetadataStore metadata =
       new MetadataStore(new InMemoryStorageEngine<String,String>("metadata"),
         0);
   ...
   AbstractSocketService getSocketService(boolean useNio, String clusterXml, 
     ...
     RequestHandlerFactory factory =
       new SocketRequestHandlerFactory(getStores(storeName, clusterXml,
                                         storesXml));
   ...
   SocketStore getSocketStore(String storeName, int port, 
     ...
     return new SocketStore(storeName,
              new SocketDestination("localhost", port, type), socketPool,
              false);
   }
   public
   static
   Context getJettyServer(String clusterXml, String storesXml, 
     ...
     RequestHandler handler =
       new SocketRequestHandlerFactory(repository).getRequestHandler(requestFormat);
   ...
   Cluster getLocalCluster(int numberOfNodes, int[] ports) {
     ...
     for (int i = 0; i < numberOfNodes; i++)
       nodes.add(new Node(i, "localhost", ports[2 * i], ports[2 * i + 1],
                   ImmutableList.of(i)));
   ...
   List<StoreDefinition> getStoreDefs(int numStores) {
     ...
     for (int i = 0; i < numStores; i++)
       defs.add(new StoreDefinition("test" + i,
                  InMemoryStorageConfiguration.TYPE_NAME, serDef, serDef,
                  RoutingTier.SERVER,
                  RoutingStrategyType.CONSISTENT_STRATEGY, 2, 1, 1, 1, 1, 1,
                  1));
     return defs;
   }
   public
   ...
   StoreDefinition getStoreDef(String storeName, int replicationFactor, 
     ...
     return new StoreDefinition(storeName,
              InMemoryStorageConfiguration.TYPE_NAME, serDef, serDef,
              RoutingTier.SERVER, strategyType, replicationFactor, preads,
              rreads, pwrites, rwrites, 1, 1);
   }
   public
   static
   VoldemortConfig createServerConfig(int nodeId, String baseDir, 
                                      String clusterFile, String storeFile)
     throws IOException {
   ...
   public
   static
   NativeAdminClientRequestFormat getAdminClient(Node identityNode, 
                                                 MetadataStore metadataStore) {
     return new NativeAdminClientRequestFormat(metadataStore,
              new SocketPool(2, 10000, 100000, 32 * 1024));
   }
   public
   static
   AdminClientRequestFormat getAdminClient(Node identityNode, 
                                           MetadataStore metadataStore, 
                                           boolean useProtocolBuffers) {
     if (useProtocolBuffers)
       return new ProtoBuffAdminClientRequestFormat(metadataStore,
                new SocketPool(2, 10000, 100000, 32 * 1024)); else
