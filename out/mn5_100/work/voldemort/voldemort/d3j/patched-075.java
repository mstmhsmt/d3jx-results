  ...
  extends AbstractKeyBasedAction<ByteArray,Void,PutPipelineData>
{
   ...
   PerformParallelPutRequests(PutPipelineData pipelineData, 
                              Event completeEvent, ByteArray key, 
                              byte[] transforms, 
                              FailureDetector failureDetector, int preferred, 
                              int required, long timeoutMs, 
   ...
   void execute(final Pipeline pipeline) {
     ...
     if (logger.isDebugEnabled())
       logger.debug("Serial put requests determined master node as " + 
         master.getId() + ", submitting remaining requests in parallel");
     ...
     for (int i = firstParallelNodeIndex;
          i < (firstParallelNodeIndex + attempts); i++) {
       ...
       NonblockingStoreCallback callback =
         ...
         {
            public
            void requestComplete(Object result, long requestTime) {
              if (logger.isTraceEnabled())
                logger.trace(pipeline.getOperation().getSimpleName() + " response received (" + requestTime + " ms.) from node " + 
                  node.getId());
              Response<ByteArray,Object> response =
                new Response<ByteArray,Object>(node, key, result,
                  requestTime);
              responses.put(node.getId(), response);
              if (isHintedHandoffEnabled() && pipeline.isFinished()) {
                if (response.getValue() instanceof UnreachableStoreException) {
                  Slop slop =
                    new Slop(pipelineData.getStoreName(), Slop.Operation.PUT,
                      key, versionedCopy.getValue(), node.getId(),
                      new Date());
                  pipelineData.addFailedNode(node);
                  hintedHandoff.sendHint(node, versionedCopy.getVersion(),
                    slop);
                }
              }
              ...
              if (logger.isTraceEnabled())
                logger.trace(attemptsLatch.getCount() + " attempts remaining. Will block " + " for " + 
                  blocksLatch.getCount() + " more ");
              if (pipeline.isFinished() && response.getValue() instanceof Exception)
                handleResponseError(response, pipeline, failureDetector);
            }
         };
       if (logger.isTraceEnabled())
         logger.trace("Submitting " + pipeline.getOperation().getSimpleName() + " request on node " + 
           node.getId());
     ...
     if (pipelineData.getSuccesses() < required) {
       ...
       if (remainingNs > 0) {
         ...
         for (Entry<Integer,Response<ByteArray,Object>> responseEntry : 
         responses.entrySet()) {
           Response<ByteArray,Object> response = responseEntry.getValue();
           if (response.getValue() instanceof Exception) {
             if (handleResponseError(response, pipeline, failureDetector))
       ...
       if (pipelineData.getSuccesses() < required) {
         pipelineData.setFatalError(new InsufficientOperationalNodesException(required + " " + 
                                      pipeline.getOperation().getSimpleName() + "s required, but only " + 
                                      pipelineData.getSuccesses() + " succeeded",
     ...
     if (quorumSatisfied) {
       if (pipelineData.getZonesRequired() != null) {
         ...
         if (zonesSatisfied >= (pipelineData.getZonesRequired() + 1)) {
           ...
           long timeMs =
             (System.nanoTime() - pipelineData.getStartTimeNs()) / Time.NS_PER_MS;
           ...
           if (pipelineData.getZoneResponses().size() >= (pipelineData.getZonesRequired() + 1)) {
             ...
             pipelineData.setFatalError(new InsufficientZoneResponsesException((
                                          pipelineData.getZonesRequired() + 1) + " " + 
                                          pipeline.getOperation().getSimpleName() + "s required zone, but only " + zonesSatisfied + " succeeded"));
             pipeline.abort();
           }
         }
       } else {
