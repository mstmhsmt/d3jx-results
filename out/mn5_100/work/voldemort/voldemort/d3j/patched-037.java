...
public
class VoldemortNativeRequestHandler extends AbstractRequestHandler
  implements RequestHandler
{
   ...
   public
   void handleRequest(DataInputStream inputStream, 
     ...
     if (store == null) {
       writeException(outputStream,
         new VoldemortException("No store named '" + storeName + "'."));
     } else {
       switch (opCode) {
   ...
   public
   boolean isCompleteRequest(final ByteBuffer buffer) {
   ...
   private
   ByteArray readKey(DataInputStream inputStream) throws IOException {
   ...
   private
   void writeResults(DataOutputStream outputStream, 
   ...
   private
   void handleGet(DataInputStream inputStream, DataOutputStream outputStream, 
   ...
   private
   void handleGetAll(DataInputStream inputStream, 
                     DataOutputStream outputStream, 
                     Store<ByteArray,byte[]> store)
     throws IOException {
     int numKeys = inputStream.readInt();
   ...
   private
   void handlePut(DataInputStream inputStream, DataOutputStream outputStream, 
                  Store<ByteArray,byte[]> store)
     throws IOException {
     ByteArray key = readKey(inputStream);
   ...
   private
   void handleDelete(DataInputStream inputStream, 
                     DataOutputStream outputStream, 
                     Store<ByteArray,byte[]> store)
     throws IOException {
     ByteArray key = readKey(inputStream);
   ...
   private
   void writeException(DataOutputStream stream, VoldemortException e)
     throws IOException {
