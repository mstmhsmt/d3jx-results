...
class Entropy
{
   ...
   public
   Entropy(int nodeId, long numKeys) {
     this.nodeId = nodeId;
   ...
   void main(String args[]) throws IOException {
     ...
     parser.accepts("output-dir",
       "[REQUIRED] The output directory where we'll store / retrieve the keys. ").withRequiredArg().describedAs("output-dir").ofType(String.class);
     parser.accepts("op-type",
       "Operation type - false ( save keys ) [ default ], true ( run entropy calculator ) ").withRequiredArg().ofType(Boolean.class);
     parser.accepts("num-keys",
       "Number of keys per store [ Default: " + Entropy.DEFAULT_NUM_KEYS + " ]").withRequiredArg().describedAs("keys").ofType(Long.class);
     ...
     Set<String> missing =
       CmdUtils.missing(options, "cluster-xml", "stores-xml", "output-dir");
     if (missing.size() > 0) {
     ...
     long numKeys =
       CmdUtils.valueOf(options, "num-keys", Entropy.DEFAULT_NUM_KEYS);
     ...
     List<StoreDefinition> storeDefs =
       new StoreDefinitionsMapper().readStoreList(new File(storesXml));
     Entropy detector = new Entropy(nodeId, numKeys);
     detector.generateEntropy(cluster, storeDefs, outputDir, opType,
       negativeTest);
   }
   public
   void generateEntropy(Cluster cluster, List<StoreDefinition> storeDefs, 
                        File storeDir, boolean opType)
     throws IOException {
     generateEntropy(cluster, storeDefs, storeDir, opType, false);
   }
   public
   void generateEntropy(Cluster cluster, List<StoreDefinition> storeDefs, 
                        File storeDir, boolean opType, boolean negativeTest)
     throws IOException {
       ...
       for (StoreDefinition storeDef : storeDefs) {
         ...
         if (!opType) {
           ...
           try {
             ...
             if (nodeId == -1) {
               int numKeysPerNode =
                 (int)Math.floor(numKeys / cluster.getNumberOfNodes());
               for (Node node : cluster.getNodes()) {
                 keys =
                 adminClient.fetchKeys(node.getId(), storeDef.getName(),
                   cluster.getNodeById(node.getId()).getPartitionIds(), null,
                   false);
                 for (long keyId = 0;
                      keyId < numKeysPerNode && keys.hasNext(); keyId++) {
                   ByteArray key = keys.next();
                   if (RebalanceUtils.getNodeIds(strategy.routeRequest(
                                                   key.get()).subList(0, 1)).contains(
                         node.getId())) {
               ...
               List<Integer> partitions =
                 cluster.getNodeById(nodeId).getPartitionIds();
               Map<Integer,Integer> partitionMap =
                 new HashMap<Integer,Integer>();
               int numKeysPerPartition =
                 (int)Math.floor(numKeys / partitions.size());
               int numKeysStored = 0;
               for (int partitionId : partitions) {
                 partitionMap.put(partitionId, 0);
               }
               keys =
               adminClient.fetchKeys(nodeId, storeDef.getName(), partitions,
                 null, false);
               while (keys.hasNext() && numKeysStored < numKeys) {
                 ByteArray key = keys.next();
                 if (RebalanceUtils.getNodeIds(strategy.routeRequest(
                                                 key.get()).subList(0, 1)).contains(nodeId)) {
                   int targetPartition =
                     strategy.getPartitionList(key.get()).get(0);
           ...
           HashMap<Integer,Store<ByteArray,byte[],byte[]>>
           socketStoresPerNode = Maps.newHashMap();
           for (Node node : cluster.getNodes()) {
             socketStoresPerNode.put(node.getId(),
               socketStoreFactory.create(storeDef.getName(), node.getHost(),
             ...
             while (reader.available() != 0) {
               ...
               if (!negativeTest) {
                 ...
                 for (Node node : responsibleNodes) {
                   List<Versioned<byte[]>> value =
                     socketStoresPerNode.get(node.getId()).get(new ByteArray(key),
                       null);
                 ...
                 if (!responsibleNodes.contains(cluster.getNodeById(nodeId))) {
                   List<Versioned<byte[]>> value =
                     socketStoresPerNode.get(nodeId).get(new ByteArray(key),
                       null);
                   if (value == null || value.size() == 0) {
                     deletedKeys++;
                   }
