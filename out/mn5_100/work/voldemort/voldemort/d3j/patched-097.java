...
public
abstract
class AbstractFailureDetector implements FailureDetector
{
   protected final FailureDetectorConfig failureDetectorConfig;
   protected final ConcurrentHashMap<FailureDetectorListener,Object>
   listeners;
   ...
   AbstractFailureDetector(FailureDetectorConfig failureDetectorConfig) {
     ...
     for (Node node : failureDetectorConfig.getNodes()) {
       nodeStatusMap.put(node,
         createNodeStatus(node,
           failureDetectorConfig.getTime().getMilliseconds()));
     }
   }
   private
   NodeStatus createNodeStatus(Node node, long currTime) {
   ...
   @JmxGetter(name="availableNodes",
              description="The available nodes")
   public
   ...
   @JmxGetter(name="unavailableNodes",
              description="The unavailable nodes")
   public
   String getUnavailableNodes() {
     List<String> list = new ArrayList<String>();
     for (Node node : getConfig().getNodes())
       if (!isAvailable(node))
       list.add(String.valueOf(node.getId()));
   ...
   @JmxGetter(name="availableNodeCount",
   ...
   @JmxGetter(name="nodeCount",
              description="The number of total nodes")
   public
   int getNodeCount() {
   ...
   protected
   void setUnavailable(Node node, UnreachableStoreException e) {
     NodeStatus nodeStatus = getNodeStatus(node);
     if (logger.isEnabledFor(Level.WARN)) {
       if (e != null)
         logger.warn(node + " set as unavailable", e); else
   ...
   protected
   void checkNodeArg(Node node) {
     if (node == null)
       throw new IllegalArgumentException("node must be non-null");
   }
   protected
   void checkArgs(Node node, long requestTime) {
     checkNodeArg(node);
     if (requestTime < 0)
       throw new IllegalArgumentException("requestTime - " + requestTime + " - less than 0");
   }
   private
   boolean setAvailable(NodeStatus nodeStatus, boolean isAvailable) {
     synchronized (nodeStatus) {
