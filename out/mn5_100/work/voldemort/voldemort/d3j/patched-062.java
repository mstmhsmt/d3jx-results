...
public
class ConsistentRoutingStrategyTest extends TestCase
{
   private final byte[] key = new byte[0];
   private
   List<Node> getTestNodes() {
     return ImmutableList.of(node(0, 2, 7, 14), node(1, 1, 10, 13),
              node(2, 3, 5, 17), node(3, 0, 11, 16), node(4, 6, 9, 15),
              node(5, 4, 8, 12));
   }
   public
   ConsistentRoutingStrategy getRouter(HashFunction hash, 
                                       int replicationFactor) {
     return new ConsistentRoutingStrategy(hash, getTestNodes(),
              replicationFactor);
   }
   public
   ConsistentRoutingStrategy getRouter(int hashValue, int replicationFactor) {
   ...
   public
   void testLoadBalancing(int numNodes, int tagsPerNode, int numRequests, 
                          int replicationFactor) {
     ...
     for (int i = 0; i < numNodes; i++)
       nodes.add(new Node(i, "host", 8080, 6666,
                   tags.subList(tagsPerNode * i, tagsPerNode * (i + 1))));
     Random random = new Random(2158745224L);
     Collections.shuffle(nodes, random);
     ConsistentRoutingStrategy router =
       new ConsistentRoutingStrategy(new FnvHashFunction(), nodes,
         replicationFactor);
     for (Node n : nodes)
   ...
   private
   void assertWellBalanced(int numNodes, Multiset<Integer> ids) {
     ...
     for (Integer id : ids.elementSet()) {
       System.out.println(id + "\t" + ids.count(id) + "\t" + num.format(
                                                               chiSq(
                                                                 ids.count(id),
                                                                 expected)) + "\t" + 
   ...
   private
   double chiSq(double observed, double expected) {
     return (observed - expected) * (observed - expected) / expected;
   }
   private
   void assertNodeOrder(List<Node> found, int... expected) {
     assertEquals("Router produced unexpected number of nodes.",
       expected.length, found.size());
     for (int i = 0; i < found.size(); i++)
       assertEquals(expected[i], found.get(i).getId());
   }
   private
   Node node(int id, int... tags) {
