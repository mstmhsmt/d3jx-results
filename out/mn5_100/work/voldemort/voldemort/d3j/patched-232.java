...
public class SocketStoreClientFactory extends AbstractStoreClientFactory {
  ...
  public SocketStoreClientFactory(ClientConfig config) {
    super(config);
    this.requestRoutingType = RequestRoutingType.getRequestRoutingType(
        RoutingTier.SERVER.equals(config.getRoutingTier()), false);
    this.storeFactory = new ClientRequestExecutorPool(
        config.getSelectors(), config.getMaxConnectionsPerNode(),
        config.getConnectionTimeout(TimeUnit.MILLISECONDS),
        config.getSocketTimeout(TimeUnit.MILLISECONDS),
<<<<<<< /root/work/voldemort/voldemort/d3j/patched-232-0.java
        config.getSocketBufferSize(), config.getSocketKeepAlive(),
        config.isJmxEnabled());
||||||| /root/work/voldemort/voldemort/d3j/base-232.java
        config.getSocketBufferSize(), config.getSocketKeepAlive());
    if (config.isJmxEnabled())
      JmxUtils.registerMbean(
          storeFactory, JmxUtils.createObjectName(storeFactory.getClass()));
=======
        config.getSocketBufferSize(), config.getSocketKeepAlive(),
        config.isJmxEnabled());
    if (config.isJmxEnabled())
      JmxUtils.registerMbean(
          ...
          JmxUtils.createObjectName(
              JmxUtils.getPackageName(storeFactory.getClass()),
              JmxUtils.getClassName(storeFactory.getClass()) + jmxId()));
>>>>>>> /root/work/voldemort/voldemort/d3j/patched-232-1.java
  }
  @Override
  public <K, V> StoreClient<K, V>
  ...
  protected List<Versioned<String>> getRemoteMetadata(String key, URI url) {
    try {
      ...
      SocketDestination destination = new SocketDestination(
          url.getHost(), url.getPort(), getRequestFormatType());
  ...
  getStore(String storeName, String host, int port, RequestFormatType type) {
  ...
  protected FailureDetector initFailureDetector(final ClientConfig config,
    ...
    ClientStoreVerifier storeVerifier = new ClientStoreVerifier() {
      ...
      protected Store<ByteArray, byte[], byte[]> getStoreInternal(Node node) {
        return SocketStoreClientFactory.this.getStore(
            MetadataStore.METADATA_STORE_NAME, node.getHost(),
            node.getSocketPort(), config.getRequestFormatType());
      }
    };
    FailureDetectorConfig failureDetectorConfig =
        new FailureDetectorConfig(config).setNodes(nodes).setStoreVerifier(
            storeVerifier);
  ...
  protected void validateUrl(URI url) {
    if (!URL_SCHEME.equals(url.getScheme()))
      throw new IllegalArgumentException(
          "Illegal scheme in bootstrap URL for SocketStoreClientFactory:"
  ...
  public void close() {
    ...
    if (failureDetector != null)
      this.failureDetector.removeFailureDetectorListener(
