...
public class BdbStorageEngine implements StorageEngine<ByteArray, byte[]> {
  ...
  public BdbStorageEngine(String name, Environment environment,
                          Database database) {
    this(name, environment, database, false);
  }
  public BdbStorageEngine(String name, Environment environment,
                          Database database, boolean cursorPreload) {
    this.name = Utils.notNull(name);
    this.bdbDatabase = Utils.notNull(database);
    this.environment = Utils.notNull(environment);
    this.versionedSerializer =
        new VersionedSerializer<byte[]>(new IdentitySerializer());
  ...
  private <T> List<T> get(ByteArray key, LockMode lockMode,
    ...
    try {
      ...
      for (ByteArray key : keys) {
        List<Versioned<byte[]>> values =
            get(cursor, key, LockMode.READ_UNCOMMITTED, versionedSerializer);
        if (!values.isEmpty())
  ...
  private static <T> List<T> get(Cursor cursor, ByteArray key,
                                 LockMode lockMode, Serializer<T> serializer)
    ...
    for (OperationStatus status =
             cursor.getSearchKey(keyEntry, valueEntry, lockMode);
  ...
  public void put(ByteArray key, Versioned<byte[]> value)
    ...
    try {
      ...
      for (OperationStatus status =
               cursor.getSearchKey(keyEntry, valueEntry, LockMode.RMW);
           status == OperationStatus.SUCCESS;
        ...
        if (occured == Occured.BEFORE)
          throw new ObsoleteVersionException(
              "Key '" + new String(hexCodec.encode(key.get())) + "' " +
              value.getVersion().toString() + " is obsolete,"
              + " current version is " + clock + ".");
      ...
      if (status != OperationStatus.SUCCESS)
        throw new PersistenceFailureException(
            "Put operation failed with status: " + status);
      succeeded = true;
    } catch (DatabaseException e) {
      logger.error(e);
      throw new PersistenceFailureException(e);
  ...
  public boolean delete(ByteArray key, Version version)
    ...
    try {
      ...
      while (status == OperationStatus.SUCCESS) {
        if (new VectorClock(valueEntry.getData()).compare(version) ==
            Occured.BEFORE) {
          cursor.delete();
  ...
  @JmxOperation(
      description = "A variety of stats about the BDB for this store.")
  public String
<<<<<<< /root/work/voldemort/voldemort/d3j/patched-141-0.java
  getBdbStats() {
    String stats = getStats(false).toString();
    logger.info("Bdb store" + getName() + " stats:\n" + stats + "\n");
    return stats;
||||||| /root/work/voldemort/voldemort/d3j/base-141.java
  getStatsAsString() {
...
=======
  getBdbStats() {
    ...
    return stats;
>>>>>>> /root/work/voldemort/voldemort/d3j/patched-141-1.java
  }
  private static class BdbStoreIterator
    ...
    private Pair<ByteArray, Versioned<byte[]>> getPair(DatabaseEntry key,
                                                       DatabaseEntry value) {
      if (key == null || key.getData() == null) {
        return null;
      } else {
        VectorClock clock = new VectorClock(value.getData());
        byte[] bytes = ByteUtils.copy(value.getData(), clock.sizeInBytes(),
    ...
    public Pair<ByteArray, Versioned<byte[]>> next() {
      if (!isOpen)
        throw new PersistenceFailureException(
            "Call to next() on a closed iterator.");
      DatabaseEntry keyEntry = new DatabaseEntry();
      DatabaseEntry valueEntry = new DatabaseEntry();
      try {
