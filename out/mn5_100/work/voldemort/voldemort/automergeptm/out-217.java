...
public class ServerTestUtils {
  ...
  public static AbstractSocketService getSocketService(boolean useNio, String clusterXml, String storesXml, String storeName, int port) {
    RequestHandlerFactory factory = getSocketRequestHandlerFactory(clusterXml, storesXml, getStores(storeName, clusterXml, storesXml));
    return getSocketService(useNio, factory, port, 5, 10, 10000);
  }

  public static RequestHandlerFactory getSocketRequestHandlerFactory(String clusterXml, String storesXml, StoreRepository storeRepository) {
    return new SocketRequestHandlerFactory(null, storeRepository, createMetadataStore(new ClusterMapper().readCluster(new StringReader(clusterXml)), new StoreDefinitionsMapper().readStoreList(new StringReader(storesXml))), null, null, null);
  }
  ...
  public static SocketStore getSocketStore(String storeName, String host, int port, RequestFormatType type) {
    return getSocketStore(storeName, host, port, type, false);
  }

  public static SocketStore getSocketStore(String storeName, String host, int port, RequestFormatType type, boolean isRouted) {
    SocketPool socketPool = new SocketPool(2, 10000, 100000, 32 * 1024);
    return new SocketStore(storeName, new SocketDestination(host, port, type), socketPool, isRouted);
  }
  ...
  public static MetadataStore createMetadataStore(Cluster cluster, List<StoreDefinition> storeDefs) {
    ...
    innerStore.put(MetadataStore.CLUSTER_KEY, new Versioned<String>(new ClusterMapper().writeCluster(cluster)));
    innerStore.put(MetadataStore.STORES_KEY, new Versioned<String>(new StoreDefinitionsMapper().writeStoreList(storeDefs)));
  ...
  public static void waitForServerStart(Node node) {
    ...
    while (retries-- > 0) {
      store = ServerTestUtils.getSocketStore(MetadataStore.METADATA_STORE_NAME, node.getSocketPort());
      try {
