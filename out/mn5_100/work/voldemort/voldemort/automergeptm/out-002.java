...
@RunWith(value = Parameterized.class) public class AdminServiceFailureTest extends TestCase {
  ...
  private void doOperation(StreamOperations e, int nodeId, String storeName, List<Integer> partitionList) {
    switch (e) {
      ...
      case FETCH_ENTRIES:
      putAlltoStore();
      consumeIterator(getAdminClient().fetchEntries(nodeId, storeName, partitionList, null, false));
      return;
      case FETCH_KEYS:
      putAlltoStore();
      consumeIterator(getAdminClient().fetchKeys(nodeId, storeName, partitionList, null, false));
      return;
      case UPDATE_ENTRIES:
