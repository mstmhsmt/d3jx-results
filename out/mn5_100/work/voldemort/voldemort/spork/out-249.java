...
public class Entropy {
     ...
     *
     * @param nodeId
     * 		Node id. If -1, goes over all of them
     */
    ...
    public static void main(String[] args) throws IOException {
        ...
        parser.accepts("node", "Node id").withRequiredArg().describedAs("node-id").ofType(java.lang.Integer.class);
        parser.accepts("cluster-xml", "[REQUIRED] Path to cluster-xml").withRequiredArg().describedAs("xml").ofType(java.lang.String.class);
        parser.accepts("stores-xml", "[REQUIRED] Path to stores-xml").withRequiredArg().describedAs("xml").ofType(java.lang.String.class);
        parser.accepts("output-dir", "[REQUIRED] The output directory where we'll store / retrieve the keys. ").withRequiredArg().describedAs("output-dir").ofType(java.lang.String.class);
        parser.accepts("op-type", "Operation type - false ( save keys ) [ default ], true ( run entropy calculator ) ").withRequiredArg().ofType(java.lang.Boolean.class);
        parser.accepts("num-keys", ("Number of keys per store [ Default: " + Entropy.DEFAULT_NUM_KEYS) + " ]").withRequiredArg().describedAs("keys").ofType(java.lang.Long.class);
                        ...
                        if (nodeId == (-1)) {
                            ...
                            for (Node node : cluster.getNodes()) {
                                ...
                                for (long keyId = 0; (keyId < numKeysPerNode) && keys.hasNext(); keyId++) {
                                    ...
                                    if (RebalanceUtils.getNodeIds(strategy.routeRequest(key.get()).subList(0, 1)).contains(node.getId())) {
                            ...
                            while (keys.hasNext() && (numKeysStored < numKeys)) {
                                ByteArray key = keys.next();
                                if (RebalanceUtils.getNodeIds(strategy.routeRequest(key.get()).subList(0, 1)).contains(nodeId)) {
                    ...
                    try {
                        ...
                        while (reader.available() != 0) {
                            ...
                            if (!negativeTest) {
                                ...
                                totalKeys++;
                            } else if (!responsibleNodes.contains(cluster.getNodeById(nodeId))) {
