...
<<<<<<< commits-mn5_100/voldemort/voldemort/207b94a3f783fd60d2d5d5d5ecb85f89a6250344/FetchPartitionFileStreamRequestHandler-7f42dd0.java
ReadOnlyStorageEngine storageEngine = AdminServiceRequestHandler.getReadOnlyStorageEngine(metadataStore, storeRepository, request.getStore());HashMap<Integer, List<Integer>> localReplicaToPartitionList = ProtoUtils.decodePartitionTuple(request.getReplicaToPartitionList());this.replicaToPartitionList = Lists.newArrayList();for (int replicaType = 0; replicaType < storeDef.getReplicationFactor(); replicaType++) {
...
=======
this.replicaToPartitionList = ProtoUtils.decodePartitionTuple(request.getReplicaToPartitionList());this.storageEngine = AdminServiceRequestHandler.getReadOnlyStorageEngine(metadataStore, storeRepository, request.getStore());
>>>>>>> commits-mn5_100/voldemort/voldemort/b255dca077a76f41d374d7552b549eec0db34d04/FetchPartitionFileStreamRequestHandler-f169956.java
    ...
    final public void close(DataOutputStream outputStream) throws IOException {
        ProtoUtils.writeEndOfStream(outputStream);
    }

    final public void handleError(DataOutputStream outputStream, VoldemortException e) throws IOException {
        logger.error("handleFetchPartitionFilesEntries failed for request(" + request.toString() + ")", e);
    }
    ...
    private void validateFiles() {
        if (!indexFile.exists() && !dataFile.exists() && 0 == currentChunkId) {
            throw new VoldemortException("Could not find any data for " + currentPair.getSecond().toString() + "_" + currentPair.getFirst().toString() + "_" + currentChunkId);
        }
        if (indexFile.exists() ^ dataFile.exists()) {
            throw new VoldemortException("One of the following does not exist: " + indexFile.toString() + " and " + dataFile.toString() + ".");
        }
    }
    ...
    enum FetchStatus {
        ...
        next_partition, send_data_file, send_index_file
    }
