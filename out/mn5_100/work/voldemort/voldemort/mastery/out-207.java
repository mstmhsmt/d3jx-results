...
public class ServerTestUtils {
    ...
    public static StoreRepository getStores(String storeName, String clusterXml, String storesXml) {
        ...
        MetadataStore metadata = new MetadataStore(new InMemoryStorageEngine<String, String>("metadata"), 0);
        metadata.put(new ByteArray(MetadataStore.CLUSTER_KEY.getBytes()), new Versioned<byte[]>(clusterXml.getBytes()));
        metadata.put(new ByteArray(MetadataStore.STORES_KEY.getBytes()), new Versioned<byte[]>(storesXml.getBytes()));
    ...
    public static AbstractSocketService getSocketService(boolean useNio, RequestHandlerFactory requestHandlerFactory, int port, int coreConnections, int maxConnections, int bufferSize) {
        AbstractSocketService socketService = null;
        if (useNio) {
            socketService = new NioSocketService(requestHandlerFactory, port, bufferSize, coreConnections, "client-request-service", false);
        } else {
    ...
    public static SocketStore getSocketStore(String storeName, int port, RequestFormatType type) {
        ...
        return new SocketStore(storeName, new SocketDestination("localhost", port, type), socketPool, false);
    }

    public static HttpStore getHttpStore(String storeName, RequestFormatType format, int port) {
        return new HttpStore(storeName, "localhost", port, new HttpClient(), new RequestFormatFactory().getRequestFormat(format), false);
    }
    ...
    public static Cluster getLocalCluster(int numberOfNodes, int[] ports) {
        ...
        for (int i = 0; i < numberOfNodes; i++) nodes.add(new Node(i, "localhost", ports[2 * i], ports[2 * i + 1], ImmutableList.of(i)));
    ...
    public static List<StoreDefinition> getStoreDefs(int numStores) {
        ...
        for (int i = 0; i < numStores; i++) defs.add(new StoreDefinition("test" + i, InMemoryStorageConfiguration.TYPE_NAME, serDef, serDef, RoutingTier.SERVER, RoutingStrategyType.CONSISTENT_STRATEGY, 2, 1, 1, 1, 1, 1, 1));
        return defs;
    }

    public static StoreDefinition getStoreDef(String storeName, int replicationFactor, int preads, int rreads, int pwrites, int rwrites, String strategyType) {
        SerializerDefinition serDef = new SerializerDefinition("string");
        return new StoreDefinition(storeName, InMemoryStorageConfiguration.TYPE_NAME, serDef, serDef, RoutingTier.SERVER, strategyType, replicationFactor, preads, rreads, pwrites, rwrites, 1, 1);
    }

    public static VoldemortConfig createServerConfig(int nodeId, String baseDir, String clusterFile, String storeFile) throws IOException {
    ...
    public static NativeAdminClientRequestFormat getAdminClient(Node identityNode, MetadataStore metadataStore) {
        return new NativeAdminClientRequestFormat(metadataStore, new SocketPool(2, 10000, 100000, 32 * 1024));
    }

    public static AdminClientRequestFormat getAdminClient(Node identityNode, MetadataStore metadataStore, boolean useProtocolBuffers) {
        if (useProtocolBuffers)
            return new ProtoBuffAdminClientRequestFormat(metadataStore, new SocketPool(2, 10000, 100000, 32 * 1024));
        else
            return getAdminClient(identityNode, metadataStore);
    }

    public static AbstractSocketService getSocketService(boolean useNio, String clusterXml, String storesXml, String storeName, int port) {
        RequestHandlerFactory factory = new SocketRequestHandlerFactory(getStores(storeName, clusterXml, storesXml));
