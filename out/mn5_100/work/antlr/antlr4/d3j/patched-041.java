...
public class LeftRecursiveRuleAnalyzer extends LeftRecursiveRuleWalker {
  ...
  public LinkedHashMap<Integer, LeftRecursiveRuleAltInfo> binaryAlts =
      new LinkedHashMap<Integer, LeftRecursiveRuleAltInfo>();
  ...
  public List<LeftRecursiveRuleAltInfo> prefixAlts =
      new ArrayList<LeftRecursiveRuleAltInfo>();
  ...
  public LeftRecursiveRuleAnalyzer(GrammarAST ruleAST, Tool tool,
                                   String ruleName, String language) {
    super(new CommonTreeNodeStream(
        new GrammarASTAdaptor(ruleAST.token.getInputStream()), ruleAST));
    this.tool = tool;
  ...
  public void loadPrecRuleTemplates() {
    ...
    if (!recRuleTemplates.isDefined("recRule")) {
      tool.errMgr.toolError(ErrorType.MISSING_CODE_GEN_TEMPLATES,
                            "LeftRecursiveRules");
    }
  ...
  public void setAltAssoc(AltAST t, int alt) {
    ...
    if (altAssociativity.get(alt) != null &&
        altAssociativity.get(alt) != assoc) {
  ...
  public void binaryAlt(AltAST originalAltTree, int alt) {
    ...
    String altLabel =
        altTree.altLabel != null ? altTree.altLabel.getText() : null;
    GrammarAST lrlabel = stripLeftRecursion(altTree);
    String label = lrlabel != null ? lrlabel.getText() : null;
    if (lrlabel != null) {
      leftRecursiveRuleRefLabels.add(
          new Pair<GrammarAST, String>(lrlabel, altLabel));
    }
    ...
    LeftRecursiveRuleAltInfo a = new LeftRecursiveRuleAltInfo(
        alt, altText, label, altLabel, originalAltTree);
    a.nextPrec = nextPrec;
  ...
  public void prefixAlt(AltAST originalAltTree, int alt) {
    ...
    String altLabel =
        altTree.altLabel != null ? altTree.altLabel.getText() : null;
    LeftRecursiveRuleAltInfo a = new LeftRecursiveRuleAltInfo(
        alt, altText, null, altLabel, originalAltTree);
    a.nextPrec = nextPrec;
    prefixAlts.add(a);
  }
  @Override
  ...
  public String getArtificialOpPrecRule() {
    ...
    LinkedHashMap<Integer, LeftRecursiveRuleAltInfo> opPrecRuleAlts =
        new LinkedHashMap<Integer, LeftRecursiveRuleAltInfo>();
  ...
  public AltAST addPrecedenceArgToRules(AltAST t, int prec) {
    ...
    for (GrammarAST rref : outerAltRuleRefs) {
      ...
      boolean rightmost =
          rref == outerAltRuleRefs.get(outerAltRuleRefs.size() - 1);
      if (recursive && rightmost) {
        rref.setText(ruleName + "<" +
                     LeftRecursiveRuleTransformer.PRECEDENCE_OPTION_NAME + "=" +
                     prec + ">");
      }
    }
...
<<<<<<< /root/work/antlr/antlr4/d3j/patched-041-0.java
  public AltAST addPrecedenceArgToLastRule(AltAST t, int prec) {
    ...
    if (last != null && last.getText().equals(ruleName)) {
      last.setText(ruleName + "<" +
                   LeftRecursiveRuleTransformer.PRECEDENCE_OPTION_NAME + "=" +
                   prec + ">");
    }
    return t;
  }
||||||| /root/work/antlr/antlr4/d3j/base-041.java
  public AltAST addPrecedenceArgToLastRule(AltAST t, int prec) {
    if (t == null)
      return null;
    GrammarAST last = null;
...
=======
>>>>>>> /root/work/antlr/antlr4/d3j/patched-041-1.java
  ...
  public static boolean hasImmediateRecursiveRuleRefs(GrammarAST t,
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (rref != null && rref.getType() == RULE_REF &&
  ...
  public GrammarAST stripLeftRecursion(GrammarAST altAST) {
    ...
    Tree rref = first.getChild(1);
    if ((first.getType() == RULE_REF && first.getText().equals(ruleName)) ||
        (rref != null && rref.getType() == RULE_REF &&
         rref.getText().equals(ruleName))) {
      if (first.getType() == ASSIGN)
        lrlabel = (GrammarAST)first.getChild(0);
      altAST.deleteChild(leftRecurRuleIndex);
      GrammarAST newFirstChild =
          (GrammarAST)altAST.getChild(leftRecurRuleIndex);
  ...
  public int precedence(int alt) { return numAlts - alt + 1; }
  ...
  public String toString() {
    return "PrecRuleOperatorCollector{"
        + "binaryAlts=" + binaryAlts + ", ternaryAlts=" + ternaryAlts +
        ", suffixAlts=" + suffixAlts + ", prefixAlts=" + prefixAlts +
        ", otherAlts=" + otherAlts + '}';
  }
}