...
class ATNSerializer
{
   ...
   public ATN atn;
   public
   ATNSerializer(Grammar g, ATN atn) {
     this.g = g;
     this.atn = atn;
   }
   public
   IntegerList serialize() {
     ...
     int nedges = 0;
     Map<IntervalSet,Integer> setIndices =
       new HashMap<IntervalSet,Integer>();
         ...
         switch (edgeType) {
           ...
           case Transition.PRECEDENCE:
             PrecedencePredicateTransition ppt =
               (PrecedencePredicateTransition)t;
             arg1 = ppt.precedence;
             break;
   ...
   String decode(char[] data) {
     ...
     if (version != ATNSimulator.SERIALIZED_VERSION) {
       ...
       throw new UnsupportedOperationException(new InvalidClassException(
                                                 ATN.class.getName(), reason));
     }
     ...
     if (!uuid.equals(ATNSimulator.SERIALIZED_UUID)) {
       String reason =
         String.format(Locale.getDefault(),
           "Could not deserialize ATN with UUID %s (expected %s).", uuid,
           ATNSimulator.SERIALIZED_UUID);
       throw new UnsupportedOperationException(new InvalidClassException(
                                                 ATN.class.getName(), reason));
     }
     p++;
     ...
     for (int i = 0; i < nsets; i++) {
       ...
       for (int j = 0; j < nintervals; j++) {
         ...
         buf.append(getTokenName(ATNSimulator.toInt(data[p]))).append("..").append(
           getTokenName(ATNSimulator.toInt(data[p + 1])));
         p += 2;
       }
       buf.append("\n");
     }
   ...
   public
   static
   String getSerializedAsString(Grammar g, ATN atn) {
     return new String(getSerializedAsChars(g, atn));
   }
   public
   static
   IntegerList getSerialized(Grammar g, ATN atn) {
     return new ATNSerializer(g, atn).serialize();
   }
   public
   static
   char[] getSerializedAsChars(Grammar g, ATN atn) {
     return Utils.toCharArray(getSerialized(g, atn));
   }
   public
   static
   String getDecoded(Grammar g, ATN atn) {
   ...
   private
   void serializeLong(IntegerList data, long value) {
