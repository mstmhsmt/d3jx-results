...
class ParserATNSimulator extends ATNSimulator
{
   ...
   public
   ParserATNSimulator(@NotNull ATN atn, @NotNull DFA[] decisionToDFA, 
   ...
   int adaptivePredict(@NotNull TokenStream input, int decision, 
     ...
     if (debug || debug_list_atn_decisions) {
       System.out.println("adaptivePredict decision " + decision + " exec LA(1)==" + 
         getLookaheadName(input) + " line " + input.LT(1).getLine() + ":" + 
     ...
     try {
       if (dfa.s0 == null) {
         ...
         if (debug || debug_list_atn_decisions) {
           System.out.println("predictATN decision " + dfa.decision + " exec LA(1)==" + 
   ...
   int execATN(@NotNull DFA dfa, @NotNull DFAState s0, 
     ...
     while (true) {
       ...
       if (D == ERROR) {
         ...
         throw noViableAlt(input, outerContext, previousD.configs,
                 startIndex);
       }
       if (D.requiresFullContext && mode != PredictionMode.SLL) {
         ...
         reportAttemptingFullContext(dfa, conflictingAlts, D.configs,
   ...
   @Nullable
   protected
   DFAState getExistingTargetState(@NotNull DFAState previousD, int t) {
   ...
   @NotNull
   protected
   DFAState computeTargetState(@NotNull DFA dfa, @NotNull DFAState previousD, 
                               int t) {
     ...
     if (debug) {
       Collection<BitSet> altSubSets =
         PredictionMode.getConflictingAltSubsets(reach);
       System.out.println("SLL altSubSets=" + altSubSets + ", configs=" + reach + ", predict=" + predictedAlt + ", allSubsetsConflict=" + 
     ...
     } else if (PredictionMode.hasSLLConflictTerminatingPrediction(mode,
                  reach)) {
   ...
   protected
   int execATNWithFullContext(DFA dfa, DFAState D, @NotNull ATNConfigSet s0, 
                              @NotNull TokenStream input, int startIndex, 
                              ParserRuleContext outerContext) {
     if (debug || debug_list_atn_decisions) {
     ...
     while (true) {
       ...
       Collection<BitSet> altSubSets =
         PredictionMode.getConflictingAltSubsets(reach);
       if (debug) {
         System.out.println("LL altSubSets=" + altSubSets + ", predict=" + 
     ...
     if (reach.uniqueAlt != ATN.INVALID_ALT_NUMBER) {
       reportContextSensitivity(dfa, predictedAlt, reach, startIndex,
         input.index());
       return predictedAlt;
     }
   ...
   protected
   ATNConfigSet computeReachSet(ATNConfigSet closure, int t, boolean fullCtx) {
     if (debug)
     ...
     if (t == IntStream.EOF) {
       reach =
       removeAllConfigsNotInRuleStopState(reach, reach == intermediate);
     }
   ...
   ATNConfigSet computeStartState(@NotNull ATNState p, 
                                  @Nullable RuleContext ctx, boolean fullCtx) {
   ...
   int getAltThatFinishedDecisionEntryRule(ATNConfigSet configs) {
     ...
     for (ATNConfig c : configs) {
       if (c.reachesIntoOuterContext > 0 || (c.state instanceof RuleStopState && 
   ...
   BitSet evalSemanticContext(@NotNull DFAState.PredPrediction[] predPredictions,
     ...
     for (DFAState.PredPrediction pair : predPredictions) {
       ...
       boolean predicateEvaluationResult =
         pair.pred.eval(parser, outerContext);
       if (debug || dfa_debug) {
   ...
   void closure(@NotNull ATNConfig config, @NotNull ATNConfigSet configs, 
     ...
     closureCheckingStopState(config, configs, closureBusy,
       collectPredicates, fullCtx, initialDepth);
     assert !fullCtx || !configs.dipsIntoOuterContext;
   }
   protected
   void closureCheckingStopState(@NotNull ATNConfig config, 
                                 @NotNull ATNConfigSet configs, 
                                 @NotNull Set<ATNConfig> closureBusy, 
                                 boolean collectPredicates, boolean fullCtx, 
                                 int depth) {
       ...
       if (!config.context.isEmpty()) {
         for (int i = 0; i < config.context.size(); i++) {
           ...
           ATNState returnState =
             atn.states.get(config.context.getReturnState(i));
           PredictionContext newContext = config.context.getParent(i);
           ATNConfig c =
             new ATNConfig(returnState, config.alt, newContext,
               config.semanticContext);
           c.reachesIntoOuterContext = config.reachesIntoOuterContext;
           assert depth > Integer.MIN_VALUE;
           closureCheckingStopState(c, configs, closureBusy,
             collectPredicates, fullCtx, depth - 1);
         }
         return;
       } else if (fullCtx) {
   ...
   protected
   void closure_(@NotNull ATNConfig config, @NotNull ATNConfigSet configs, 
                 @NotNull Set<ATNConfig> closureBusy, 
                 boolean collectPredicates, boolean fullCtx, int depth) {
     ...
     for (int i = 0; i < p.getNumberOfTransitions(); i++) {
       ...
       if (c != null) {
         ...
         closureCheckingStopState(c, configs, closureBusy,
           continueCollecting, fullCtx, newDepth);
       }
     }
   }
   @NotNull
     ...
     if (collectPredicates && (!pt.isCtxDependent || (pt.isCtxDependent && inContext))) {
       if (fullCtx) {
         ...
         boolean predSucceeds =
           pt.getPredicate().eval(parser, _outerContext);
   ...
   ATNConfig ruleTransition(@NotNull ATNConfig config, 
     ...
     PredictionContext newContext =
       SingletonPredictionContext.create(config.context,
   ...
   BitSet getConflictingAlts(ATNConfigSet configs) {
     Collection<BitSet> altsets =
       PredictionMode.getConflictingAltSubsets(configs);
   ...
   protected
   static
   int getUniqueAlt(@NotNull ATNConfigSet configs) {
   ...
   protected
   DFAState addDFAEdge(@NotNull DFA dfa, @Nullable DFAState from, int t, 
   ...
   @NotNull
   protected
   DFAState addDFAState(@NotNull DFA dfa, @NotNull DFAState D) {
     if (D == ERROR) {
       return D;
     }
   ...
   protected
   void reportAttemptingFullContext(DFA dfa, 
   ...
   protected
   void reportContextSensitivity(DFA dfa, int prediction, 
                                 @NotNull ATNConfigSet configs, 
                                 int startIndex, int stopIndex) {
     if (debug || retry_debug) {
       Interval interval = Interval.of(startIndex, stopIndex);
   ...
   public
   final
   void setPredictionMode(@NotNull PredictionMode mode) {
     this.mode = mode;
   }
   @NotNull
   public
