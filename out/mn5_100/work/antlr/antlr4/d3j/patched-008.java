...
import java.util.*;
public
class ParserATNSimulator<Symbol extends Token> extends ATNSimulator
{
   ...
   ParserATNSimulator(@Nullable Parser parser, @NotNull ATN atn) {
     ...
     this.parser = parser;
     decisionToDFA =
     new DFA[atn.getNumberOfDecisions() + (parser != null ? 1 : 0)];
   }
   @Override
   public
   void reset() {}
   public
   int adaptivePredict(@NotNull SymbolStream<? extends Symbol> input, 
                       int decision, 
   ...
   public
   int predictATN(@NotNull DFA dfa, 
   ...
   public
   int execDFA(@NotNull DFA dfa, @NotNull DFAState s0, 
     ...
     loop: while (true) {
       ...
       if (s.isCtxSensitive) {
         ...
         ATNConfigSet fullCtxSet =
           execATNWithFullContext(dfa, s, s0_closure, input, startIndex,
             outerContext, decState.getNumberOfTransitions(), greedy);
   ...
   public
   int execATN(@NotNull DFA dfa, @NotNull DFAState s0, 
               @NotNull SymbolStream<? extends Symbol> input, int startIndex, 
     ...
     while (true) {
       boolean loopsSimulateTailRecursion = false;
       ATNConfigSet reach =
         computeReachSet(previous, t, greedy, loopsSimulateTailRecursion);
       if (reach == null)
             ...
             if (outerContext == ParserRuleContext.EMPTY || !D.configset.dipsIntoOuterContext || k == 1) {
               ...
               ATNConfigSet s0_closure =
                 computeStartState(dfa.atnStartState, outerContext, greedy,
                   loopsSimulateTailRecursion);
               fullCtxSet =
               execATNWithFullContext(dfa, D, s0_closure, input, startIndex,
                 outerContext, decState.getNumberOfTransitions(), greedy);
       ...
       if (!greedy) {
         int exitAlt = 2;
         if (predictedAlt != ATN.INVALID_ALT_NUMBER && configWithAltAtStopState(reach,
                                                         1)) {
           if (debug)
   ...
   public
   ATNConfigSet execATNWithFullContext(DFA dfa, DFAState D, 
                                       @NotNull ATNConfigSet s0, 
                                       @NotNull SymbolStream<? extends Symbol> input,
                                       int startIndex, 
                                       ParserRuleContext<?> outerContext, 
                                       int nalts, boolean greedy) {
     retry_with_context++;
     ...
     if (reach.hasSemanticContext) {
       ...
       if (altToPred != null) {
         ...
         if (predPredictions.size() < nalts) {
           IntervalSet conflictingAlts =
             getConflictingAltsFromConfigSet(reach);
           DecisionState decState = atn.getDecisionState(dfa.decision);
           reportInsufficientPredicates(dfa, startIndex, input.index(),
             conflictingAlts, decState, altToPred, reach, true);
         }
         input.seek(startIndex);
     ...
     reportAmbiguity(dfa, D, startIndex, input.index(),
       reach.conflictingAlts, reach);
     reach.uniqueAlt = reach.conflictingAlts.getMinElement();
     return reach;
   }
   protected
   ATNConfigSet computeReachSet(ATNConfigSet closure, int t, boolean greedy, 
     ...
     for (ATNConfig c : closure) {
       ...
       for (int ti = 0; ti < n; ti++) {
         ...
         if (target != null) {
           ...
           closure(new ATNConfig(c, target), reach, closureBusy, false,
   ...
   @NotNull
   public
   ATNConfigSet computeStartState(@NotNull ATNState p, 
                                  @Nullable RuleContext ctx, boolean greedy, 
                                  boolean loopsSimulateTailRecursion) {
   ...
   @Nullable
   public
   ATNState getReachableTarget(@NotNull Transition trans, int ttype) {
   ...
   public
   List<DFAState.PredPrediction> predicateDFAState(DFAState D, 
   ...
   public
   SemanticContext[] getPredsForAmbigAlts(@NotNull IntervalSet ambigAlts, 
   ...
   public
   List<DFAState.PredPrediction> getPredicatePredictions(IntervalSet ambigAlts,
     ...
     if (pairs.isEmpty())
       pairs = null; else if (firstUnpredicated != ATN.INVALID_ALT_NUMBER) {
       pairs.add(new DFAState.PredPrediction(null, firstUnpredicated));
     }
     return pairs;
   }
   public
   int evalSemanticContext(List<DFAState.PredPrediction> predPredictions, 
                           ParserRuleContext<?> outerContext) {
     ...
     for (DFAState.PredPrediction pair : predPredictions) {
       ...
       if (debug || dfa_debug) {
         System.out.println("eval pred " + pair + "=" + pair.pred.eval(parser,
                                                          outerContext));
       }
   ...
   void closure(@NotNull ATNConfig config, @NotNull ATNConfigSet configs, 
                @NotNull Set<ATNConfig> closureBusy, 
                boolean collectPredicates, boolean greedy, 
                boolean loopsSimulateTailRecursion, int depth) {
     if (debug)
     ...
     if (config.state instanceof RuleStopState) {
       ...
       if (config.context != null && !config.context.isEmpty()) {
         ...
         ATNState invokingState =
           atn.states.get(config.context.invokingState);
         RuleTransition rt = (RuleTransition)invokingState.transition(0);
         ATNState retState = rt.followState;
         ATNConfig c =
           new ATNConfig(retState, config.alt, newContext,
       ...
       if (config.state.getClass() == StarLoopbackState.class || config.state.getClass() == PlusLoopbackState.class) {
         config.context =
         new RuleContext(config.context, config.state.stateNumber);
         if (debug)
   ...
   @NotNull
   public
   ATNConfig ruleTransition(@NotNull ATNConfig config, @NotNull Transition t) {
     if (debug) {
   ...
   @Nullable
   public
   IntervalSet getConflictingAlts(@NotNull ATNConfigSet configs, 
                                  boolean fullCtx) {
     if (debug)
     ...
     for (int state : stateToConfigListMap.keySet()) {
       ...
       for (int i = 0; i < size; i++) {
         ...
         for (int j = i + 1; j < size; j++) {
           ...
           if (c.alt != d.alt) {
             boolean conflicting =
               (fullCtx && c.context.equals(d.context)) || (!fullCtx && 
             c.context.conflictsWith(d.context));
             if (conflicting) {
               if (debug) {
                 System.out.println("we reach state " + c.state.stateNumber + " in rule " + (parser != null ? 
                   getRuleName(c.state.ruleIndex) : "n/a") + " alts " + c.alt + "," + d.alt + " from ctx " + 
                   c.context.toString(parser) + " and " + d.context.toString(parser));
               }
               ambigAlts.add(c.alt);
               ambigAlts.add(d.alt);
             }
   ...
   protected
   IntervalSet getConflictingAltsFromConfigSet(ATNConfigSet configs) {
     IntervalSet conflictingAlts;
   ...
   protected
   int resolveToMinAlt(@NotNull DFAState D, IntervalSet conflictingAlts) {
   ...
   protected
   int resolveNongreedyToExitBranch(@NotNull ATNConfigSet reach, 
   ...
   public
   String getLookaheadName(SymbolStream<? extends Symbol> input) {
   ...
   @NotNull
   public
   NoViableAltException noViableAlt(@NotNull SymbolStream<? extends Symbol> input,
   ...
   @NotNull
   protected
   DFAState addDFAEdge(@NotNull DFA dfa, @NotNull ATNConfigSet p, int t, 
   ...
   public
   void reportAttemptingFullContext(DFA dfa, ATNConfigSet configs, 
                                    int startIndex, int stopIndex) {
     if (debug || retry_debug) {
       System.out.println("reportAttemptingFullContext decision=" + dfa.decision + ":" + configs + ", input=" + 
         parser.getInputString(startIndex, stopIndex));
     }
     if (parser != null)
       parser.getErrorHandler().reportAttemptingFullContext(parser, dfa,
         startIndex, stopIndex, configs);
   }
   public
   void reportContextSensitivity(DFA dfa, ATNConfigSet configs, 
                                 int startIndex, int stopIndex) {
     if (debug || retry_debug) {
       System.out.println("reportContextSensitivity decision=" + dfa.decision + ":" + configs + ", input=" + 
         parser.getInputString(startIndex, stopIndex));
     }
     if (parser != null)
       parser.getErrorHandler().reportContextSensitivity(parser, dfa,
         startIndex, stopIndex, configs);
   }
   public
   void reportAmbiguity(@NotNull DFA dfa, DFAState D, int startIndex, 
     ...
     if (parser != null)
       parser.getErrorHandler().reportAmbiguity(parser, dfa, startIndex,
         stopIndex, ambigAlts, configs);
   }
   public
   void reportInsufficientPredicates(@NotNull DFA dfa, int startIndex, 
                                     int stopIndex, 
     ...
     if (debug || retry_debug) {
       System.out.println("reportInsufficientPredicates " + ambigAlts + ", decState=" + decState + ": " + 
         Arrays.toString(altToPred) + parser.getInputString(startIndex,
                                        stopIndex));
     }
     if (parser != null) {
       parser.getErrorHandler().reportInsufficientPredicates(parser, dfa,
         startIndex, stopIndex, ambigAlts, decState, altToPred, configs,
         fullContextParse);
     }
   }
}