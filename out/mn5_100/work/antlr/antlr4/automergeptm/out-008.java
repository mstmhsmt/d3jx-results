...
public class ParserATNSimulator<Symbol extends Token> extends ATNSimulator {
  ...
  public int execDFA(@NotNull DFA dfa, @NotNull DFAState s0, @NotNull SymbolStream<? extends Symbol> input, int startIndex, @Nullable ParserRuleContext<?> outerContext) {
    ...
    while (true) {
      ...
      if (s.isCtxSensitive) {
        ...
        ATNConfigSet fullCtxSet = execATNWithFullContext(dfa, s, s0_closure, input, startIndex, outerContext, decState.getNumberOfTransitions(), greedy);
      ...
      if (s.edges == null || t >= s.edges.length || t < -1 || s.edges[t + 1] == null) {
        ...
        int alt;
        if (dfa_debug) {
          System.out.println("ATN exec upon " + parser.getInputString(startIndex) + " at DFA state " + s.stateNumber);
        }
  ...
  @NotNull public ATNConfigSet computeStartState(@NotNull ATNState p, @Nullable RuleContext ctx, boolean greedy, boolean loopsSimulateTailRecursion) {
  ...
  @Nullable public ATNState getReachableTarget(@NotNull Transition trans, int ttype) {
    if (trans instanceof AtomTransition) {
      ...
      if (trans instanceof SetTransition) {
        ...
        if (trans instanceof RangeTransition) {
          RangeTransition rt = (RangeTransition) trans;
          if (ttype >= rt.from && ttype <= rt.to) {
            return rt.target;
          }
        } else {
          if (trans instanceof WildcardTransition && ttype != Token.EOF) {
            return trans.target;
          }
        }
      }
  ...
  public List<DFAState.PredPrediction> getPredicatePredictions(IntervalSet ambigAlts, SemanticContext[] altToPred) {
    ...
    if (pairs.isEmpty()) {
      ...
      if (firstUnpredicated != ATN.INVALID_ALT_NUMBER) {
  ...
  @Nullable public ATNConfig getEpsilonTarget(@NotNull ATNConfig config, @NotNull Transition t, boolean collectPredicates, boolean inContext) {
    if (t instanceof RuleTransition) {
  ...
  @Nullable public ATNConfig predTransition(@NotNull ATNConfig config, @NotNull PredicateTransition pt, boolean collectPredicates, boolean inContext) {
    if (debug) {
  ...
  /**
	 * From grammar:
	 ...
	 functions
	 */
  @Nullable public IntervalSet getConflictingAlts(@NotNull ATNConfigSet configs, boolean fullCtx) {
    if (debug) {
  ...
  @NotNull public NoViableAltException noViableAlt(@NotNull SymbolStream<? extends Symbol> input, @NotNull ParserRuleContext<?> outerContext, @NotNull ATNConfigSet configs, int startIndex) {
  ...
  @NotNull protected DFAState addDFAEdge(@NotNull DFA dfa, @NotNull ATNConfigSet p, int t, @NotNull ATNConfigSet q) {
  ...
  @NotNull protected DFAState addDFAState(@NotNull DFA dfa, @NotNull ATNConfigSet configs) {
