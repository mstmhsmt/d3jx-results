...
<<<<<<< commits-mn5_100/nasa/mct/3d41ee6180e43e3c2c430c1db843557c764aea94/LegendEntryPopupMenuFactory-6ee0f60.java
String name = legendEntry.getComputedBaseDisplayName();if (name.isEmpty())
                name = legendEntry.getFullBaseDisplayName();if (!manifestation.isLocked()) {
...
=======
Color assigned = legendEntry.getForeground();String name = legendEntry.getComputedBaseDisplayName();if (name.isEmpty())
                name = legendEntry.getFullBaseDisplayName();String subMenuText1 = String.format(BUNDLE.getString("SelectColor.label"), name);
...
if (!manifestation.isLocked()) {
                for (int i = 0; i < PlotConstants.MAX_NUMBER_OF_DATA_ITEMS_ON_A_PLOT; i++) {
                    JMenuItem item = new JRadioButtonMenuItem("", new SolidColorIcon(PlotLineColorPalette.getColor(i)), (assigned == PlotLineColorPalette.getColor(i)));
                    final int colorIndex = i;
                    item.addActionListener(new ActionListener() {
            ...
            public CharacterDialog() {
                ...
                if (doc instanceof AbstractDocument) {
                    ...
                    ad.setDocumentFilter(new DocumentFilter() {
                        ...
                        @Override
                        public void replace(FilterBypass bypass, int start, int length, String str, AttributeSet attr) throws BadLocationException {
                            if (doc.getLength() + str.length() < 2 && !str.trim().isEmpty()) {
