...
public class LegendEntryPopupMenuFactory {
  private static final ResourceBundle BUNDLE = ResourceBundle.getBundle(LegendEntryPopupMenuFactory.class.getName().substring(0, LegendEntryPopupMenuFactory.class.getName().lastIndexOf(".")) + ".Bundle");
  ...
  private class LegendEntryPopup extends JPopupMenu {
    ...
    public LegendEntryPopup(final PlotViewManifestation manifestation, final LegendEntry legendEntry) {
      ...
      String subMenuText1 = String.format(BUNDLE.getString("SelectColor.label"), name);

<<<<<<< Unknown file: This is a bug in JDime.
=======
      String subMenuText2 = String.format(BUNDLE.getString("RegressionPointsLabel"), name);
>>>>>>> commits-mn5_100/nasa/mct/c07a2d235b353a36a66d6d0befefb495ce0b22b6/LegendEntryPopupMenuFactory-27a01e5.java
...
>>>>>>> commits-mn5_100/nasa/mct/c07a2d235b353a36a66d6d0befefb495ce0b22b6/LegendEntryPopupMenuFactory-27a01e5.java
      ...
      final JFormattedTextField myTextField = ((NumberEditor) spinner.getEditor()).getTextField();
      spinner.addKeyListener(new KeyListener() {
        @Override public void keyTyped(KeyEvent e) {
          if (!(e.getKeyChar() == KeyEvent.CHAR_UNDEFINED) && (e.getKeyCode() == KeyEvent.VK_UNDEFINED) && (e.getKeyChar() == '0' || e.getKeyChar() == '1' || e.getKeyChar() == '2' || e.getKeyChar() == '3' || e.getKeyChar() == '4' || e.getKeyChar() == '5' || e.getKeyChar() == '6' || e.getKeyChar() == '7' || e.getKeyChar() == '8' || e.getKeyChar() == '9') && Integer.valueOf(myTextField.getValue() + String.valueOf(e.getKeyChar())).compareTo((Integer) ((SpinnerNumberModel) spinner.getModel()).getMinimum()) > 0 && Integer.valueOf(myTextField.getValue() + String.valueOf(e.getKeyChar())).compareTo((Integer) ((SpinnerNumberModel) spinner.getModel()).getMaximum()) < 0) {
      ...
      numberEditor.addKeyListener(new KeyListener() {
        ...
        @Override public void keyPressed(KeyEvent e) {
          if (e.getKeyCode() == KeyEvent.VK_LEFT && numberEditor.getTextField().getCaretPosition() == 0) {
      ...
      regressionMenu.addMenuKeyListener(new MenuKeyListener() {
        ...
        @Override public void menuKeyPressed(MenuKeyEvent e) {
      ...
      if (!manifestation.isLocked()) {
        ...
        for (int i = 0; i < PlotConstants.MAX_NUMBER_OF_DATA_ITEMS_ON_A_PLOT; i++) {
          JMenuItem item = new JRadioButtonMenuItem("", new SolidColorIcon(PlotLineColorPalette.getColor(i)), (currentColor == PlotLineColorPalette.getColor(i)));
        ...
        add(subMenu);
        final LineSettings settings = legendEntry.getLineSettings();String subMenuText = String.format(BUNDLE.getString("SelectColor.label"), name);JMenu subMenu = new JMenu(subMenuText);Color currentColor = legendEntry.getForeground();for (int i = 0; i < PlotConstants.MAX_NUMBER_OF_DATA_ITEMS_ON_A_PLOT; i++) {
          JMenuItem item = new JRadioButtonMenuItem("", new SolidColorIcon(PlotLineColorPalette.getColor(i)), (currentColor == PlotLineColorPalette.getColor(i)));
          final int colorIndex = i;
          item.addActionListener(new ActionListener() {
            @Override public void actionPerformed(ActionEvent e) {
          ...
          subMenu.add(item);
        }add(subMenu);subMenuText = String.format(BUNDLE.getString("SelectThickness.label"), name);subMenu = new JMenu(subMenuText);for (int i = 1; i <= PlotConstants.MAX_LINE_THICKNESS; i++) {
          JMenuItem item = new JRadioButtonMenuItem("" + i, (settings.getThickness() == i));
          ...
          subMenu.add(item);
        }add(subMenu);if (manifestation.getPlot() != null && manifestation.getPlot().getPlotLineDraw().drawMarkers()) {
          subMenuText = String.format(BUNDLE.getString("SelectMarker.label"), name);
          subMenu = new JMenu(subMenuText);
          for (int i = 0; i < PlotConstants.MAX_NUMBER_OF_DATA_ITEMS_ON_A_PLOT; i++) {
            JMenuItem item = new JRadioButtonMenuItem("", new PlotMarkerIcon(PlotLineShapePalette.getShape(i), false), (settings.getMarker() == i && !settings.getUseCharacter()));
          ...
          JMenuItem other = new JRadioButtonMenuItem(BUNDLE.getString("SelectCharacter.label"), settings.getUseCharacter());
          if (!settings.getCharacter().isEmpty()) {
            FontRenderContext frc = ((Graphics2D) manifestation.getGraphics()).getFontRenderContext();
            other.setIcon(new PlotMarkerIcon(PlotLineShapePalette.getShape(settings.getCharacter(), frc), PlotLineColorPalette.getColor(settings.getColorIndex()), false));
          }
        ...
        if (manifestation.getPlot() != null && manifestation.getPlot().getPlotLineDraw().drawMarkers()) {
          subMenuText = String.format(BUNDLE.getString("SelectMarker.label"), name);
          subMenu = new JMenu(subMenuText);
          for (int i = 0; i < PlotConstants.MAX_NUMBER_OF_DATA_ITEMS_ON_A_PLOT; i++) {
            JMenuItem item = new JRadioButtonMenuItem("", new PlotMarkerIcon(PlotLineShapePalette.getShape(i), false), (settings.getMarker() == i && !settings.getUseCharacter()));
...
>>>>>>> commits-mn5_100/nasa/mct/c07a2d235b353a36a66d6d0befefb495ce0b22b6/LegendEntryPopupMenuFactory-27a01e5.java
    ...
    private class SolidColorIcon implements Icon {
      ...
      @Override public void paintIcon(Component arg0, Graphics g, int x, int y) {
    ...
    private class CharacterDialog extends JDialog {
      ...
      public CharacterDialog() {
        ...
        setTitle(BUNDLE.getString("SelectCharacter.title") + " - " + f.getTitle() + " - " + manifestation.getInfo().getViewName());
        ...
        if (doc instanceof AbstractDocument) {
          ...
          ad.setDocumentFilter(new DocumentFilter() {
            @Override public void insertString(FilterBypass bypass, int offset, String str, AttributeSet attr) throws BadLocationException {
              if (doc.getLength() + str.length() < 2 && !str.trim().isEmpty()) {
                super.insertString(bypass, offset, str, attr);
              }
            }

            @Override public void replace(FilterBypass bypass, int start, int length, String str, AttributeSet attr) throws BadLocationException {
              if (doc.getLength() + str.length() < 2 && !str.trim().isEmpty()) {
