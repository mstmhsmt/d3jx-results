...
class TestQuinnCurtisPlot
{
   ...
   @Mock private AbbreviatingPlotLabelingAlgorithm plotLabelingAlgorithm =
     new AbbreviatingPlotLabelingAlgorithm();
   @BeforeSuite
   public
   void setupPlotsToCopyInTests() {
     ...
     Mockito.when(mockPlotViewManifestation.getCurrentMCTTime()).thenReturn(
       new GregorianCalendar().getTimeInMillis());
     originalPlotTimeOnXAxis = new PlotterPlot();
     originalPlotTimeOnXAxis.createChart(new Font("Arial", Font.PLAIN, 1), 1,
       Color.white, Color.white, 0, Color.white, Color.white, Color.white,
       "dd", Color.black, Color.white, 1, false, true, true, plotAbstraction,
       plotLabelingAlgorithm);
     originalPlotTimeOnXAxis.setCompressionEnabled(false);
     originalPlotTimeOnYAxis = new PlotterPlot();
     originalPlotTimeOnYAxis.createChart(new Font("Arial", Font.PLAIN, 1), 1,
       Color.white, Color.white, 0, Color.white, Color.white, Color.white,
       "dd", Color.black, Color.white, 1, false, true, true, plotAbstraction,
       plotLabelingAlgorithm);
     Mockito.when(plotAbstraction.getSubPlots()).thenReturn(Arrays.asList((AbstractPlottingPackage)originalPlotTimeOnXAxis,
                                                              originalPlotTimeOnYAxis));
     ...
     try {
       originalPlotTimeOnXAxis.addData(DATA_SET_X_1,
         timeOne.getTimeInMillis(), 10.0);
       originalPlotTimeOnXAxis.addData(DATA_SET_X_1,
         timeTwo.getTimeInMillis(), 11.0);
       originalPlotTimeOnXAxis.addData(DATA_SET_X_1,
         timeThree.getTimeInMillis(), 20.1);
       originalPlotTimeOnYAxis.addData(DATA_SET_Y_1,
         timeOne.getTimeInMillis(), 25.0);
       originalPlotTimeOnYAxis.addData(DATA_SET_Y_1,
         timeTwo.getTimeInMillis(), 107.0);
     } catch (Exception e) {
       e.printStackTrace();
     } 
   }
   @Test
   public
   void testcheckThatSetsAreEqualMethod() {
     PlotDataSeries dataSeries =
       originalPlotTimeOnXAxis.plotDataManager.dataSeries.get(DATA_SET_X_1);
     Assert.assertNotNull(dataSeries);
     Assert.assertNotNull(dataSeries.getData());
     checkThatSetsContainSameProcessVars(originalPlotTimeOnXAxis.plotDataManager.dataSeries,
       originalPlotTimeOnXAxis.plotDataManager.dataSeries);
   }
   @Test
   void testPlotBufferSize() {
     PlotterPlot testPlot = new PlotterPlot();
     testPlot.createChart(new Font("Arial", Font.PLAIN, 1), 1, Color.white,
       Color.white, 0, Color.white, Color.white, Color.white, "dd",
       Color.black, Color.white, 1, false, true, true, plotAbstraction,
       plotLabelingAlgorithm);
   }
   @Test
   public
   void testGetMinMaxNonTimeValues() {
     GregorianCalendar timeOne = new GregorianCalendar();
     GregorianCalendar timeTwo = new GregorianCalendar();
     GregorianCalendar timeThree = new GregorianCalendar();
     timeTwo.add(Calendar.SECOND, 100);
     ...
     Assert.assertEquals(testPlot.getNonTimeMaxDataValueCurrentlyDisplayed(),
       20.1);
     Assert.assertEquals(testPlot.getNonTimeMinDataValueCurrentlyDisplayed(),
       10.0);
   }
   @Test
   ...
   void testGetTimeAxisScrollMode() {
     ...
     testXAsTimePlot.createChart(new Font("Arial", Font.PLAIN, 1), 1,
       Color.white, Color.white, 0, Color.white, Color.white, Color.white,
       "dd", Color.black, Color.white, 1, false, true, true, plotAbstraction,
       plotLabelingAlgorithm);
     Assert.assertEquals(testXAsTimePlot.getTimeAxisSubsequentSetting(),
       TimeAxisSubsequentBoundsSetting.JUMP);
     PlotterPlot testYAsTimePlot = new PlotterPlot();
     testYAsTimePlot.createChart(new Font("Arial", Font.PLAIN, 1), 1,
       Color.white, Color.white, 0, Color.white, Color.white, Color.white,
       "dd", Color.black, Color.white, 1, false, true, true, plotAbstraction,
       plotLabelingAlgorithm);
     Assert.assertEquals(testYAsTimePlot.getTimeAxisSubsequentSetting(),
       TimeAxisSubsequentBoundsSetting.SCRUNCH);
   }
   private
   void checkThatSetsContainSameProcessVars(Map<String,PlotDataSeries> dataSeries1,
                                            
                                            Map<String,PlotDataSeries> dataSeries2) {
   ...
   void testPositionPanels() {
     ...
     timeThree.add(Calendar.SECOND, 200);
     PlotterPlot testPlot = new PlotterPlot();
     testPlot.createChart(new Font("Arial", Font.PLAIN, 12), 1, Color.gray,
       Color.black, 0, Color.white, Color.white, Color.white, "dd",
       Color.black, Color.black, 1, false, true, true, plotAbstraction,
       plotLabelingAlgorithm);
     ...
     Assert.assertEquals(legendPanel.getSize().getHeight(),
       legendPanel.getPreferredSize().getHeight());
     Assert.assertEquals(legendPanel.getSize().getWidth(),
       legendPanel.getPreferredSize().getWidth());
     frame.setSize((int)legendPanel.getPreferredSize().getWidth() + PlotConstants.MINIMUM_PLOT_WIDTH + PlotConstants.LOCAL_CONTROL_WIDTH + 20,
       frame.getHeight());
     try {
       Thread.sleep(500);
     } catch (Exception e) {} 
     Assert.assertTrue(legendPanel.getPreferredSize().getHeight() == 
       legendPanel.getSize().getHeight());
     ...
     frame.setSize((int)legendPanel.getPreferredSize().getWidth() + PlotConstants.MINIMUM_PLOT_WIDTH - 100,
       frame.getHeight());
     try {
       Thread.sleep(500);
     } catch (Exception e) {} 
   ...
   @Test
   public
   void testAddDataOffFixedPlotIsIgnored() {
     PlotSettings settings = new PlotSettings();
     settings.setMinTime(0);
     settings.setMaxTime(10000);
     PlotView testPlot =
       new PlotView.Builder(PlotterPlot.class).plotSettings(settings).isCompressionEnabled(true).build();
   ...
   @Test
   public
   void plotNoDeteGetNonTimeAxisMaxAndMin() {
     PlotAbstraction testPlot =
       new PlotView.Builder(PlotterPlot.class).build();
     PlotterPlot plot = (PlotterPlot)testPlot.returnPlottingPackage();
     Assert.assertEquals(plot.getMinNonTime(),
       PlotConstants.DEFAULT_NON_TIME_AXIS_MAX_VALUE);
     Assert.assertEquals(plot.getMaxNonTime(),
       PlotConstants.DEFAULT_NON_TIME_AXIS_MIN_VALUE);
   }
   @Test
   public
   void testAddAndRemoveObservors() {
     PlotAbstraction testPlot =
       new PlotView.Builder(PlotterPlot.class).build();
   ...
   @Test
   public
   void testUpdateFromStreamStateTransitions() {
     PlotAbstraction testPlot =
       new PlotView.Builder(PlotterPlot.class).build();
     PlotterPlot qcPlot = (PlotterPlot)testPlot.returnPlottingPackage();
