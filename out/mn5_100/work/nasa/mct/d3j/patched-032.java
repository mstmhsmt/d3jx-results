...
public
class PlotViewManifestation extends FeedView implements RenderingCallback
{
   private final static Logger logger =
     LoggerFactory.getLogger(PlotViewManifestation.class);
   private AbbreviatingPlotLabelingAlgorithm plotLabelingAlgorithm =
   ...
   private PlotDataFeedUpdateHandler plotDataFeedUpdateHandler =
     new PlotDataFeedUpdateHandler(this);
   private PlotPersistenceHandler plotPersistenceHandler =
     new PlotPersistenceHandler(this);
   private
   SwingWorker<Map<String,List<Map<String,String>>>,Map<String,List<Map<String,String>>>>
   currentDataRequest;
   private
   SwingWorker<Map<String,List<Map<String,String>>>,Map<String,List<Map<String,String>>>>
   ...
   public
   PlotViewManifestation(AbstractComponent component, ViewInfo vi) {
     super(component,
     vi);
     plotFrameBackground = getColor("plotFrame.background");
     if (plotFrameBackground == null)
       plotFrameBackground =
       PlotConstants.DEFAULT_PLOT_FRAME_BACKGROUND_COLOR;
   ...
   @Override
   public
   FeedProvider getFeedProvider(AbstractComponent component) {
     ...
     PlotConfiguration settings =
       plotPersistenceHandler.loadPlotSettingsFromPersistance();
     String persistedTimeSystem =
       settings != null ? settings.getTimeSystemSetting() : null;
     String persistedFeedType =
       settings != null ? settings.getFeedTypeSetting() : null;
     String assignedTimeSystem =
       (plotDataAssigner != null) ? plotDataAssigner.getTimeSystemDefaultChoice() : null;
     String assignedFeedType = null;
     String filterTimeSystem =
       (persistedTimeSystem != null && !persistedTimeSystem.isEmpty()) ? persistedTimeSystem : assignedTimeSystem;
     String filterFeedType =
       (persistedFeedType != null && !persistedFeedType.isEmpty()) ? persistedFeedType : assignedFeedType;
     if (filterTimeSystem != null && feedProviders != null && feedProviders.size() > 0) {
       for (FeedProvider fp : feedProviders) {
         if (matchFeedProvider(fp, feedInfoProvider, filterTimeSystem,
               filterFeedType)) {
           return fp;
         }
       }
   ...
   private
   boolean matchFeedProvider(FeedProvider fp, 
                             FeedInfoProvider feedInfoProvider, 
                             String timeSystem, String feedType) {
     ...
     FeedInfo feedInfo =
       feedInfoProvider != null ? feedInfoProvider.getFeedInfo(fp) : null;
     return feedInfo != null && feedType.equals(feedInfo.getTypeId());
   }
   @Override
   protected
   void handleNamingContextChange() {
     updateMonitoredGUI();
   }
   private
   ...
   @Override
   public
   void updateMonitoredGUI(PropertyChangeEvent evt) {
     updateMonitoredGUI();
   }
   @Override
   public
   void updateMonitoredGUI(AddChildEvent event) {
     setLabelingContext(plotLabelingAlgorithm, getNamingContext());
     respondToChildChangeEvent();
   }
   @Override
   public
   void updateMonitoredGUI(RemoveChildEvent event) {
     setLabelingContext(plotLabelingAlgorithm, getNamingContext());
     respondToChildChangeEvent();
   }
   private
   void respondToChildChangeEvent() {
     generatePlot();
   }
   private
   void respondToSettingsChange() {
     generatePlot();
   ...
   public
   Map<String,String> getFeedInfoChoices() {
   ...
   private
   long getPointTime(Map<String,String> data) {
     return Long.parseLong(data.get(FeedProvider.NORMALIZED_TIME_KEY));
   }
   private
   void expandData(Map<String,List<Map<String,String>>> expandedData, 
                   final long startTime, final long endTime) {
     for (FeedProvider fp : getVisibleFeedProviders()) {
       List<Map<String,String>> points =
         expandedData.get(fp.getSubscriptionId());
       if (points != null && !points.isEmpty()) {
         if (fp.isNonCODDataBuffer()) {
           continue;
         }
         List<Map<String,String>> expandedPoints =
         ...
         for (int i = 0; i < points.size(); i++) {
           ...
           long nextPointTime =
             (points.size() > i + 1) ? getPointTime(points.get(i + 1)) : 
           Math.min(now, endTime) + 1000;
           for (long currentTime = pointTime + 1000;
                currentTime <= nextPointTime - 1000; currentTime += 1000) {
             Map<String,String> newPoint = new HashMap<String,String>(point);
             newPoint.put(FeedProvider.NORMALIZED_TIME_KEY,
     ...
     return new DataTransformation()
            {
               @Override
               public
               void transform(Map<String,List<Map<String,String>>> data, 
   ...
   public
   void requestPredictiveData(GregorianCalendar startTime, 
     ...
     currentPredictionRequest =
     this.requestData(plotDataAssigner.getPredictiveFeedProviders(),
   ...
   public
   void requestDataRefresh(GregorianCalendar startTime, 
                           GregorianCalendar endTime) {
     if (plotDataAssigner.hasFeeds()) {
       cancelAnyOutstandingRequests();
       currentDataRequest =
       this.requestData(null, startTime.getTimeInMillis(),
         endTime.getTimeInMillis(), getTransformation(), this, true);
       currentDataRequest.addPropertyChangeListener(new PropertyChangeListener()
                                                    {
                                                       ...
                                                       void propertyChange(
                                                       java.beans.PropertyChangeEvent evt) {
                                                         if (currentDataRequest.getState() == SwingWorker.StateValue.STARTED && 
                                                         evt.getOldValue() == SwingWorker.StateValue.PENDING) {
   ...
   private
   void cancelOutstandingPredictionRequests() {
   ...
   private
   void cancelAnyOutstandingRequests() {
   ...
   @Override
   public
   void clear(Collection<FeedProvider> feedProviders) {
     updateMonitoredGUI();
   }
   @Override
   public
   void updateFromFeed(Map<String,List<Map<String,String>>> data) {
     ...
     if (manifestedComponent != null) {
       ...
       if (!feedproviders.isEmpty()) {
         ...
         if (currentTimeInMillis >= 0) {
           ...
           logger.error("FeedProvider currentTimeMillis() returned a time less than zero: {}",
             currentTimeInMillis);
         }
       } else {
         logger.debug("No feed providers. Returning cached time: {}",
           cachedTime);
       }
     }
     return cachedTime;
   ...
   private
   void createPlotAndAddItToPanel() {
     createPlot();
   ...
   void createPlot() {
     thePlot =
     PlotViewFactory.createPlot(plotPersistenceHandler.loadPlotSettingsFromPersistance(),
       getCurrentMCTTime(), this, plotDataAssigner.returnNumberOfSubPlots(),
       null, plotLabelingAlgorithm,
       plotDataAssigner.getTimeSystemDefaultChoice());
   }
   private
   void addPlotToPanel() {
     if (theView != null) {
   ...
   private
   void enforceBackgroundColor(final Color bg) {
     this.setBackground(bg);
     thePlot.getPlotPanel().setBackground(bg);
     ComponentTraverser.traverse(theView,
       new ComponentTraverser.ComponentProcedure()
       {
          @Override
          public
          void run(Component c) {
            if ((PlotConstants.DEFAULT_PLOT_FRAME_BACKGROUND_COLOR).equals(
                  c.getBackground())) {
              c.setBackground(bg);
            }
          }
   ...
   FeedFilter getFilter() {
     ...
     Boolean enabled =
       getPlot().getExtension(PlotConstants.FILTER_ENABLED, Boolean.class);
     String definition =
       getPlot().getExtension(PlotConstants.FILTER_VALUE, String.class);
   ...
   public
   FeedFilterProvider getFilterProvider() {
   ...
   private
   void setLabelingContext(AbbreviatingPlotLabelingAlgorithm plotLabelingAlgorithm,
     ...
     if (logger.isDebugEnabled()) {
       printTitleArrayLists("*** DEBUG 2 *** panelContextTitleList",
         panelContextTitleList);
       printTitleArrayLists("*** DEBUG 2 *** canvasContextTitleList",
         canvasContextTitleList);
     }
   }
   private
   void printTitleArrayLists(String name, List<String> arrayList) {
     for (int i = 0; i < arrayList.size(); i++) {
       logger.debug(name + ".get(" + i + ")=" + arrayList.get(i));
     }
   }
   public
   void addFeedCallback(Runnable r) {
     feedCallbacks.add(r);
   }
   public
   void removeFeedCallback(Runnable r) {
