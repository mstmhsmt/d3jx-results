...
public
class PlotterPlot implements AbstractPlottingPackage
{
   private final static Logger logger =
     LoggerFactory.getLogger(PlotterPlot.class);
   ...
   PlotLocalControlsManager localControlsManager =
     new PlotLocalControlsManager(this);
   ...
   boolean compressionIsEnabled =
     PlotConstants.COMPRESSION_ENABLED_BY_DEFAULT;
   ...
   public
   void createChart(AxisOrientationSetting theAxisOrientation, 
                    String theTimeSystem, String theTimeFormat, 
                    ...
                    Font theTimeAxisFont, int thePlotLineThickness, 
                    Color thePlotBackgroundFrameColor, 
                    Color thePlotAreaBackgroundColor, 
                    int theTimeAxisIntercept, Color theTimeAxisColor, 
                    Color theTimeAxisLabelColor, 
                    Color theNonTimeAxisLabelColor, 
                    String theTimeAxisDateFormat, Color theNonTimeAxisColor, 
                    Color theGridLineColor, int theMinSamplesForAutoScale, 
     ...
     nonTimeAxisMinPhysicalValue =
     logicalToPhysicalY(nonTimeVaribleAxisMinValue);
     nonTimeAxisMaxPhysicalValue =
     logicalToPhysicalY(nonTimeVaribleAxisMaxValue);
   }
   private
   void setupPlotObjects() {
   ...
   void setupLegends() {
     ...
     legendManager =
     new LegendManager(this, plotBackgroundFrameColor, plotLabelingAlgorithm);
     SpringLayout layout = (SpringLayout)plotView.getLayout();
     layout.putConstraint(SpringLayout.WEST, legendManager,
       PlotConstants.PLOT_LEGEND_OFFSET_FROM_LEFT_HAND_SIDE,
       SpringLayout.WEST, plotView);
     layout.putConstraint(SpringLayout.NORTH, legendManager, 0,
       SpringLayout.NORTH, plotView.getContents());
   }
   private
   void setupTimeSyncLine() {
     timeSyncLine = new PlotTimeSyncLine(this);
   }
   private
   void setupLimitManager() {
     limitManager.setupLimitButtons();
   }
   private
   void setupLocalControlManager() {
     getLocalControlsManager().setupLocalControlManager();
   }
   private
   void setupDataCursor() {
   ...
   @Override
   public
   void addDataSet(String dataSetName, Color plottingColor) {
   ...
   @Override
   public
   void addData(String feed, SortedMap<Long,Double> points) {
     plotDataManager.addData(feed, points);
     cornerResetButtonManager.updateButtons();
   }
   @Override
   public
   void addData(String feed, long time, double value) {
   ...
   @Override
   public
   LimitAlarmState getNonTimeMaxAlarmState() {
     return limitManager.nonTimeMaxAlarm;
   }
   @Override
   public
   LimitAlarmState getNonTimeMinAlarmState() {
     return limitManager.nonTimeMinAlarm;
   }
   @Override
   public
   long getCurrentTimeAxisMinAsLong() {
     return Math.min(theTimeAxis.getStartAsLong(),
              theTimeAxis.getEndAsLong());
   }
   @Override
   public
   GregorianCalendar getCurrentTimeAxisMin() {
   ...
   @Override
   public
   long getCurrentTimeAxisMaxAsLong() {
     return Math.max(theTimeAxis.getStartAsLong(),
              theTimeAxis.getEndAsLong());
   }
   @Override
   public
   GregorianCalendar getCurrentTimeAxisMax() {
   ...
   private
   void informTimeAxisPinned(boolean pinned) {
     minMaxChanged();
   }
   ...
   @Override
   public
   double getInitialNonTimeMaxSetting() {
   ...
   @Override
   public
   double getInitialNonTimeMinSetting() {
   ...
   @Override
   public
   long getInitialTimeMaxSetting() {
     return timeVariableAxisMaxValue;
   }
   @Override
   public
   long getInitialTimeMinSetting() {
   ...
   @Override
   public
   void setPlotView(PlotAbstraction plotView) {
     plotAbstraction = plotView;
   }
   @Override
   public
   void notifyGlobalTimeSyncFinished() {
     if (plotAbstraction != null) {
   ...
   @Override
   public
   void informUpdateCachedDataStreamStarted() {
   ...
   void calculatePlotAreaLayout() {
     ...
     double totalLegendPlusAxisLabelPlusBufferWidth =
       preferedLegendWidth + yAxisAxisLabelWidth + PlotConstants.LOCAL_CONTROL_WIDTH + PlotConstants.PLOT_LEGEND_BUFFER;
     boolean wasVisible = legendManager.isVisible();
     double spaceForPlot =
       totalPlotWindowWidth - totalLegendPlusAxisLabelPlusBufferWidth;
     int minWidth =
       (getAxisOrientationSetting() == AxisOrientationSetting.X_AXIS_AS_TIME) ? PlotConstants.MINIMUM_PLOT_WIDTH : PlotConstants.MINIMUM_PLOT_HEIGHT;
     ...
     if (wasVisible != visible) {
       ...
       if (getAxisOrientationSetting() == AxisOrientationSetting.X_AXIS_AS_TIME) {
         if (visible) {
           layout.putConstraint(SpringLayout.WEST, yAxis, 0,
             SpringLayout.EAST, legendManager);
         } else {
           layout.putConstraint(SpringLayout.WEST, yAxis, 0,
             SpringLayout.WEST, plotView);
         }
         ...
         if (yAxis.getComponentCount() > 0) {
           layout.putConstraint(SpringLayout.WEST, yAxis, 2,
             SpringLayout.EAST, legendManager);
         } else {
           layout.putConstraint(SpringLayout.WEST, legendManager, 5,
             SpringLayout.WEST, plotView);
           layout.putConstraint(SpringLayout.WEST, yAxis, 2,
             SpringLayout.EAST, legendManager);
         }
   ...
   @Override
   public
   void setCompressionEnabled(boolean state) {
     compressionIsEnabled = state;
   }
   @Override
   public
   boolean isCompresionEnabled() {
     return compressionIsEnabled;
   }
   public
   void setUserOperationLockedState(boolean state) {
     userOperationsLocked = state;
   }
   public
   boolean isUserOperationsLocked() {
     return userOperationsLocked;
   }
   public
   PlotDisplayState getPlotDisplayState() {
     return plotDisplayState;
   }
   public
   void setPlotDisplayState(PlotDisplayState state) {
   ...
   String toString() {
     ...
     stringRepresentation.append("Data in plot local buffer (size " + 
       plotDataManager.dataSeries.size() + ")\n");
   ...
   public
   void setTimeAxisStartAndStop(long startTime, long endTime) {
   ...
   @Override
   public
   void notifyObserversTimeChange() {
     for (PlotObserver o : observers) {
       o.updateTimeAxis(this, theTimeAxis.getStartAsLong(),
         theTimeAxis.getEndAsLong());
     }
   }
   @Override
   public
   void registerObservor(PlotObserver o) {
     observers.add(o);
   }
   @Override
   public
   void removeObserver(PlotObserver o) {
     observers.remove(o);
   }
   @Override
   public
   ...
   @Override
   public
   boolean getOrdinalPositionInStackedPlot() {
   ...
   public
   Pinnable getNonTimeAxisUserPin() {
   ...
   @Override
   public
   AbbreviatingPlotLabelingAlgorithm getPlotLabelingAlgorithm() {
     return plotLabelingAlgorithm;
   }
   @Override
   public
   void setPlotLabelingAlgorithm(AbbreviatingPlotLabelingAlgorithm thePlotLabelingAlgorithm) {
     plotLabelingAlgorithm = thePlotLabelingAlgorithm;
   }
   public
   Rectangle2D getContentRect() {
     return plotView.getContents().getBounds();
   }
   public
   LegendManager getLegendManager() {
     return legendManager;
   }
   void newPointPlotted(long time, double value) {
     if (((nonTimeAxisMaxSubsequentSetting == NonTimeAxisSubsequentBoundsSetting.SEMI_FIXED && value > 
     getCurrentNonTimeAxisMax() && !nonTimeMaxFixed) || (nonTimeAxisMinSubsequentSetting == NonTimeAxisSubsequentBoundsSetting.SEMI_FIXED && value < 
     getCurrentNonTimeAxisMin() && !nonTimeMinFixed))) {
       adjustAxis(Math.min(oldMinNonTime, getCurrentNonTimeAxisMin()),
         Math.max(oldMaxNonTime, getCurrentNonTimeAxisMax()));
     }
   }
   private
   void adjustAxisMin() {
     adjustAxis(calculateMinNonTimeWithPadding(oldMinNonTime,
                  getCurrentNonTimeAxisMax(), getCurrentNonTimeAxisMin()),
       getCurrentNonTimeAxisMax());
   }
   private
   void adjustAxisMax() {
     adjustAxis(getCurrentNonTimeAxisMin(),
       calculateMaxNonTimeWithPadding(oldMaxNonTime,
         getCurrentNonTimeAxisMin(), getCurrentNonTimeAxisMax()));
   }
   private
   void adjustAxis(double min, double max) {
     ...
     if (axisOrientation == AxisOrientationSetting.X_AXIS_AS_TIME) {
       inverted =
       yAxisSetting == YAxisMaximumLocationSetting.MAXIMUM_AT_BOTTOM;
     } else {
   ...
   private
   double calculateMaxNonTimeWithPadding(double maxNonTime, double min, 
                                         double originalMax) {
   ...
   private
   double calculateMinNonTimeWithPadding(double minNonTime, double max, 
   ...
   void minMaxChanged() {
     ...
     if ((minNonTime != oldMinNonTime || maxNonTime != oldMaxNonTime) && !isPaused && !
     nonTimeAxis.isPinned()) {
       double start = theNonTimeAxis.getStart();
       double end = theNonTimeAxis.getEnd();
       double min = Math.min(start, end);
       double max = Math.max(start, end);
       assert max >= min;
       if (minNonTime <= min || Math.abs(nonTimeAxisMinPhysicalValue - 
                                  logicalToPhysicalY(minNonTime)) <= 1) {
         if (!nonTimeMinFixed) {
           min = calculateMinNonTimeWithPadding(minNonTime, maxNonTime, min);
         }
       }
       if (maxNonTime >= min || Math.abs(nonTimeAxisMaxPhysicalValue - 
                                  logicalToPhysicalY(maxNonTime)) <= 1) {
         if (!nonTimeMaxFixed) {
   ...
   private
   double logicalToPhysicalY(double logicalValue) {
   ...
   @Override
   public
   void setTruncationPoint(double min) {
     for (PlotDataSeries d : plotDataManager.dataSeries.values()) {
       CompressingXYDataset data = d.getData();
   ...
   public
   PlotLocalControlsManager getLocalControlsManager() {
   ...
   public
   void setLocalControlsManager(PlotLocalControlsManager localControlsManager) {
     this.localControlsManager = localControlsManager;
   }
   public
   static
   NumberFormat getNumberFormatter(double value) {
