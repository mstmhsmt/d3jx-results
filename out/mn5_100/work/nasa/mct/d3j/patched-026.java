...
class LegendEntryView extends FeedView implements AbstractLegendEntry
{
   ...
   LegendEntryView(AbstractComponent ac, ViewInfo vi) {
     ...
     feedProviders =
     fp == null ? Collections.<FeedProvider>emptyList() : Collections.singleton(fp);
   ...
   public
   void setAppearance(Color c, Icon i) {
     ...
     for (FeedProvider fp : feedProviders) {
       if (data.containsKey(fp.getSubscriptionId())) {
         ...
         if (!series.isEmpty()) {
           ...
           RenderingInfo ri =
             fp.getRenderingInfo(series.get(series.size() - 1));
   ...
   private
   Font updateBaseFont(int style) {
     baseFont = label.getFont().deriveFont(9f).deriveFont(style);
     @SuppressWarnings("rawtypes") Map attrs = baseFont.getAttributes();
   ...
   private
   class LegendEntryMouseAdapter extends MouseAdapter
   {
      ...
      void mouseDragged(MouseEvent evt) {
        if (mousePressed) {
          label.getTransferHandler().exportAsDrag(label, evt,
            TransferHandler.COPY);
        }
      }
   }
   private
   class LegendEntryTransferHandler extends TransferHandler
   {
   ...
   @Override
   public
   void setBaseDisplayName(String displayName) {
