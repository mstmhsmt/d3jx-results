...
class EclipseHandlerUtil
{
   ...
   void error(CompilationUnitDeclaration cud, String message, 
     ...
     if (cud != null)
       EclipseAST.addProblemToCompilationResult(cud, false,
   ...
   private static Map<ASTNode,ASTNode> generatedNodes =
     new WeakHashMap<ASTNode,ASTNode>();
   public
   static
   ...
   boolean typeMatches(Class<?> type, EclipseNode node, TypeReference typeRef) {
     ...
     TypeResolver resolver =
       new TypeResolver(node.getPackageDeclaration(),
   ...
   TypeReference copyType(TypeReference ref, ASTNode source) {
     if (ref instanceof ParameterizedQualifiedTypeReference) {
       ...
       TypeReference typeRef =
         new ParameterizedQualifiedTypeReference(iRef.tokens, args,
           iRef.dimensions(), iRef.sourcePositions);
     ...
     if (ref instanceof QualifiedTypeReference) {
       ...
       TypeReference typeRef =
         new QualifiedTypeReference(iRef.tokens, iRef.sourcePositions);
       setGeneratedBy(typeRef, source);
       return typeRef;
     }
   ...
   generatedLazyGettersWithPrimitiveBoolean =
     new WeakHashMap<FieldDeclaration,Object>();
   ...
   GetterMethod findGetter(EclipseNode field) {
     ...
     boolean forceBool =
       generatedLazyGettersWithPrimitiveBoolean.containsKey(fieldDeclaration);
     TypeReference fieldType = fieldDeclaration.type;
     boolean isBoolean =
       forceBool || (nameEquals(fieldType.getTypeName(), "boolean") && 
     fieldType.dimensions() == 0);
   ...
   List<String> toAllGetterNames(EclipseNode field, boolean isBoolean) {
     return TransformationsUtil.toAllGetterNames(getAccessorsForField(field),
              field.getName(), isBoolean);
   }
   public
   static
   String toGetterName(EclipseNode field, boolean isBoolean) {
     return TransformationsUtil.toGetterName(getAccessorsForField(field),
              field.getName(), isBoolean);
   }
   public
   static
   java.util.List<String> toAllSetterNames(EclipseNode field, 
                                           boolean isBoolean) {
     return TransformationsUtil.toAllSetterNames(getAccessorsForField(field),
              field.getName(), isBoolean);
   }
   public
   static
   String toSetterName(EclipseNode field, boolean isBoolean) {
     return TransformationsUtil.toSetterName(getAccessorsForField(field),
              field.getName(), isBoolean);
   }
   public
   static
   java.util.List<String> toAllWitherNames(EclipseNode field, 
                                           boolean isBoolean) {
     return TransformationsUtil.toAllWitherNames(getAccessorsForField(field),
              field.getName(), isBoolean);
   }
   public
   static
   String toWitherName(EclipseNode field, boolean isBoolean) {
     return TransformationsUtil.toWitherName(getAccessorsForField(field),
              field.getName(), isBoolean);
   }
   public
   static
   ...
   public
   static
   void injectFieldSuppressWarnings(EclipseNode type, FieldDeclaration field) {
     field.annotations = createSuppressWarningsAll(field, field.annotations);
     injectField(type, field);
   }
   public
   static
   void injectField(EclipseNode type, FieldDeclaration field) {
   ...
   public
   static
   void injectMethod(EclipseNode type, AbstractMethodDeclaration method) {
     method.annotations =
   ...
   Statement generateNullCheck(AbstractVariableDeclaration variable, 
     ...
     IfStatement ifStatement =
       new IfStatement(equalExpression, throwStatement, 0, 0);
     ...
     try {
       if (castExpressionConstructorIsTypeRefBased) {
         ...
         if (castTo.getClass() == SingleTypeReference.class && !isPrimitive(castTo)) {
           ...
           castToConverted =
           new QualifiedNameReference(qtr.tokens, qtr.sourcePositions,
   ...
   private static final Annotation[] EMPTY_ANNOTATION_ARRAY =
     new Annotation[0];
   static
   Annotation[] getAndRemoveAnnotationParameter(Annotation annotation, 
     ...
     if (annotation instanceof NormalAnnotation) {
       ...
       if (!result.isEmpty()) {
         normalAnnotation.memberValuePairs =
         pairs.isEmpty() ? null : pairs.toArray(new MemberValuePair[0]);
         return result.toArray(EMPTY_ANNOTATION_ARRAY);
       }
     }
