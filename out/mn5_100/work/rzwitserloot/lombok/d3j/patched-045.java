...
public
class HandleSetter implements JavacAnnotationHandler<Setter>
{
   public
   boolean generateSetterForType(JavacNode typeNode, JavacNode errorNode, 
                                 AccessLevel level, 
     ...
     boolean notAClass =
       (modifiers & (Flags.INTERFACE | Flags.ANNOTATION | Flags.ENUM)) != 0;
   ...
   public
   void generateSetterForField(JavacNode fieldNode, DiagnosticPosition pos, 
                               AccessLevel level, 
                               List<JCExpression> onMethod, 
                               List<JCExpression> onParam) {
     ...
     createSetterForField(level, fieldNode, fieldNode, false, onMethod,
       onParam);
   }
   @Override
   public
   boolean handle(AnnotationValues<Setter> annotation, JCAnnotation ast, 
     ...
     if (node == null)
       return false;
     List<JCExpression> onParamList =
       getAndRemoveAnnotationParameter(ast, "onParam");
     List<JCExpression> onMethodList =
       getAndRemoveAnnotationParameter(ast, "onMethod");
     if (node.getKind() == Kind.FIELD) {
       return createSetterForFields(level, fields, annotationNode, true,
                onMethodList, onParamList);
     }
   ...
   private
   boolean createSetterForFields(AccessLevel level, 
                                 Collection<JavacNode> fieldNodes, 
                                 JavacNode errorNode, boolean whineIfExists, 
                                 List<JCExpression> onMethod, 
                                 List<JCExpression> onParam) {
   ...
   private
   boolean createSetterForField(AccessLevel level, JavacNode fieldNode, 
                                JavacNode errorNode, boolean whineIfExists, 
                                List<JCExpression> onMethod, 
                                List<JCExpression> onParam) {
     ...
     switch (methodExists(methodName, fieldNode, false)) {
       ...
       case EXISTS_BY_USER:
         if (whineIfExists)
           errorNode.addWarning(String.format("Not generating %s(%s %s): A method with that name already exists",
                                  methodName, fieldDecl.vartype,
                                  fieldDecl.name));
         return true;
       default:
       case NOT_EXISTS:
     ...
     long access =
       toJavacModifier(level) | (fieldDecl.mods.flags & Flags.STATIC);
     injectMethod(fieldNode.up(),
       createSetter(access, fieldNode, fieldNode.getTreeMaker(), onMethod,
         onParam));
     return true;
   }
   private
   JCMethodDecl createSetter(long access, JavacNode field, 
                             TreeMaker treeMaker, 
                             List<JCExpression> onMethod, 
                             List<JCExpression> onParam) {
     JCVariableDecl fieldDecl = (JCVariableDecl)field.get();
     JCExpression fieldRef =
       createFieldAccessor(treeMaker, field, FieldAccess.ALWAYS_FIELD);
     JCAssign assign =
       treeMaker.Assign(fieldRef, treeMaker.Ident(fieldDecl.name));
     List<JCStatement> statements;
     List<JCAnnotation> nonNulls =
       findAnnotations(field, TransformationsUtil.NON_NULL_PATTERN);
     List<JCAnnotation> nullables =
     ...
     if (nonNulls.isEmpty()) {
       ...
       if (nullCheck != null)
         statements = List.<JCStatement>of(nullCheck, treeMaker.Exec(assign)); else
         statements = List.<JCStatement>of(treeMaker.Exec(assign));
     }
     ...
     JCVariableDecl param =
       treeMaker.VarDef(treeMaker.Modifiers(Flags.FINAL, annsOnParam),
     ...
     return treeMaker.MethodDef(treeMaker.Modifiers(access,
                                  copyAnnotations(onMethod)),
              methodName, methodType, methodGenericParams, parameters,
