...
public
class HandleGetter extends EclipseAnnotationHandler<Getter>
{
   private static final Annotation[] EMPTY_ANNOTATIONS_ARRAY =
     new Annotation[0];
   public
   boolean generateGetterForType(EclipseNode typeNode, EclipseNode pos, 
                                 AccessLevel level, 
   ...
   public
   void generateGetterForField(EclipseNode fieldNode, ASTNode pos, 
                               AccessLevel level, boolean lazy) {
   ...
   public
   void handle(AnnotationValues<Getter> annotation, Annotation ast, 
               EclipseNode annotationNode) {
     handleFlagUsage(annotationNode, ConfigurationKeys.GETTER_FLAG_USAGE,
       "@Getter");
     ...
     List<Annotation> onMethod =
       unboxAndRemoveAnnotationParameter(ast, "onMethod",
         "@Getter(onMethod=", annotationNode);
     switch (node.getKind()) {
       case FIELD:
         createGetterForFields(level,
           annotationNode.upFromAnnotationToFields(), annotationNode,
           annotationNode.get(), true, lazy, onMethod);
         break;
       case TYPE:
         if (!onMethod.isEmpty()) {
   ...
   public
   void createGetterForFields(AccessLevel level, 
                              Collection<EclipseNode> fieldNodes, 
                              EclipseNode errorNode, ASTNode source, 
                              boolean whineIfExists, boolean lazy, 
                              List<Annotation> onMethod) {
     for (EclipseNode fieldNode : fieldNodes) {
       createGetterForField(level, fieldNode, errorNode, source,
         whineIfExists, lazy, onMethod);
     }
   }
   public
   void createGetterForField(AccessLevel level, EclipseNode fieldNode, 
                             EclipseNode errorNode, ASTNode source, 
                             boolean whineIfExists, boolean lazy, 
                             List<Annotation> onMethod) {
     ...
     int modifier =
       toEclipseModifier(level) | (field.modifiers & ClassFileConstants.AccStatic);
       ...
       switch (methodExists(altName, fieldNode, false, 0)) {
         ...
         case EXISTS_BY_USER:
           if (whineIfExists) {
             ...
             errorNode.addWarning(String.format("Not generating %s(): A method with that name already exists%s",
     ...
     MethodDeclaration method =
       createGetter((TypeDeclaration)fieldNode.up().get(), fieldNode,
         getterName, modifier, source, lazy, onMethod);
     injectMethod(fieldNode.up(), method);
   }
   public
   static
   Annotation[] findDelegatesAndMarkAsHandled(EclipseNode fieldNode) {
   ...
   public
   MethodDeclaration createGetter(TypeDeclaration parent, 
                                  EclipseNode fieldNode, String name, 
                                  int modifier, ASTNode source, boolean lazy, 
                                  List<Annotation> onMethod) {
     FieldDeclaration field = (FieldDeclaration)fieldNode.get();
     TypeReference returnType =
       copyType(((FieldDeclaration)fieldNode.get()).type, source);
     ...
     MethodDeclaration method =
       new MethodDeclaration(parent.compilationResult);
     ...
     EclipseHandlerUtil.registerCreatedLazyGetter((FieldDeclaration)fieldNode.get(),
       method.selector, returnType);
     {
       ...
       Annotation[] copiedAnnotations =
         copyAnnotations(source, onMethod.toArray(new Annotation[0]),
   ...
   public
   Statement[] createSimpleGetterBody(ASTNode source, EclipseNode fieldNode) {
     FieldDeclaration field = (FieldDeclaration)fieldNode.get();
     Expression fieldRef =
       createFieldAccessor(fieldNode, FieldAccess.ALWAYS_FIELD, source);
     Statement returnStatement =
       new ReturnStatement(fieldRef, field.sourceStart, field.sourceEnd);
     return new Statement[]{
     returnStatement
     };
   }
   private static final char[][] AR =
     fromQualifiedName("java.util.concurrent.atomic.AtomicReference");
   ...
   private static final int PARENTHESIZED =
     (1 << ASTNode.ParenthesizedSHIFT) & ASTNode.ParenthesizedMASK;
   public
   Statement[] createLazyGetterBody(ASTNode source, EclipseNode fieldNode) {
     FieldDeclaration field = (FieldDeclaration)fieldNode.get();
     ...
     if (field.type instanceof SingleTypeReference && !(field.type instanceof ArrayTypeReference)) {
       char[][] newType =
         TYPE_MAP.get(new String(((SingleTypeReference)field.type).token));
       if (newType != null) {
         boxedComponentType =
         new QualifiedTypeReference(newType, poss(source, 3));
         isPrimitive = true;
       }
     }
     ...
     {
       EqualExpression cond =
         new EqualExpression(new SingleNameReference(valueName, p),
       ...
       inner.statements = new Statement[2];
       {
         ...
         createFieldAccessor(fieldNode, FieldAccess.ALWAYS_FIELD, source);
         Assignment assign =
           new Assignment(new SingleNameReference(valueName, p), getter, pE);
         assign.sourceStart = pS;
         assign.statementEnd = assign.sourceEnd = pE;
         inner.statements[0] = assign;
       }
       {
         EqualExpression innerCond =
           new EqualExpression(new SingleNameReference(valueName, p),
             new NullLiteral(pS, pE), BinaryExpression.EQUAL_EQUAL);
         ...
         {
           LocalDeclaration actualValueDecl =
             new LocalDeclaration(actualValueName, pS, pE);
           actualValueDecl.type = rawComponentType;
           actualValueDecl.type.sourceStart = pS;
           actualValueDecl.type.sourceEnd =
           actualValueDecl.type.statementEnd = pE;
         ...
         {
           if (isPrimitive) {
             Assignment innerAssign =
               new Assignment(new SingleNameReference(valueName, p),
         ...
         {
           if (!isPrimitive) {
             EqualExpression avIsNull =
               new EqualExpression(new SingleNameReference(actualValueName,
                                     p),
                 new NullLiteral(pS, pE), BinaryExpression.EQUAL_EQUAL);
             avIsNull.sourceStart = pS;
             avIsNull.sourceEnd = avIsNull.statementEnd = pE;
             Expression fieldRef =
               createFieldAccessor(fieldNode, FieldAccess.ALWAYS_FIELD,
                 source);
             ConditionalExpression ternary =
               new ConditionalExpression(avIsNull, fieldRef,
                 new SingleNameReference(actualValueName, p));
             ternary.sourceStart = pS;
             ternary.sourceEnd = ternary.statementEnd = pE;
             Assignment innerAssign =
               new Assignment(new SingleNameReference(valueName, p), ternary,
                 pE);
             innerAssign.sourceStart = pS;
             innerAssign.statementEnd = innerAssign.sourceEnd = pE;
             innerThen.statements[1] = innerAssign;
       ...
       SynchronizedStatement sync =
         new SynchronizedStatement(lock, inner, pS, pE);
     ...
     }
     {
       if (isPrimitive) {
         CastExpression cast =
           makeCastExpression(new SingleNameReference(valueName, p),
             boxedComponentType, source);
         statements[2] = new ReturnStatement(cast, pS, pE);
       }
     }
     {
       if (!isPrimitive) {
         EqualExpression vIsThisFieldName =
           new EqualExpression(new SingleNameReference(valueName, p),
             createFieldAccessor(fieldNode, FieldAccess.ALWAYS_FIELD, source),
         ...
         ConditionalExpression ternary =
           new ConditionalExpression(vIsThisFieldName,
             new NullLiteral(pS, pE), new SingleNameReference(valueName, p));
         ternary.sourceStart = pS;
         ternary.sourceEnd = ternary.statementEnd = pE;
         ternary.bits |= PARENTHESIZED;
         CastExpression cast =
           makeCastExpression(ternary, boxedComponentType, source);
         statements[2] = new ReturnStatement(cast, pS, pE);
       }
     }
     {
       ...
       TypeReference type =
         new ParameterizedQualifiedTypeReference(AR, typeParams, 0,
           poss(source, 5));
       type.sourceStart = -1;
       type.sourceEnd = -2;
       field.type = type;
