...
class JavacHandlerUtil
{
   ...
   private static Map<JCTree,WeakReference<JCTree>> generatedNodes =
     new WeakHashMap<JCTree,WeakReference<JCTree>>();
   public
   static
   boolean inNetbeansEditor(JavacNode node) {
     return inNetbeansEditor(node.getContext());
   }
   private
   static
   boolean inNetbeansEditor(Context context) {
     Options options = Options.instance(context);
     return (options.keySet().contains("ide") && !options.keySet().contains("backgroundCompilation"));
   }
   public
   static
   JCTree getGeneratedBy(JCTree node) {
   ...
   public
   static
   boolean isGenerated(JCTree node) {
     return getGeneratedBy(node) != null;
   }
   public
   static
   <T extends JCTree>
   T recursiveSetGeneratedBy(T node, JCTree source, Context context) {
     if (node == null)
       return null;
   ...
   static
   <T extends JCTree>
   T setGeneratedBy(T node, JCTree source, Context context) {
     if (node == null)
       return null;
     synchronized (generatedNodes) {
       if (source == null)
         generatedNodes.remove(node); else
   ...
   public
   static
   boolean hasAnnotation(Class<? extends Annotation> type, JavacNode node) {
     return hasAnnotation(type, node, false);
   }
   public
   static
   boolean hasAnnotationAndDeleteIfNeccessary(Class<? extends Annotation> type,
                                              JavacNode node) {
   ...
   public
   static
   boolean annotationTypeMatches(Class<? extends Annotation> type, 
                                 JavacNode node) {
     if (node.getKind() != Kind.ANNOTATION)
       return false;
     return typeMatches(type, node,
              ((JCAnnotation)node.get()).annotationType);
   }
   public
   static
   boolean typeMatches(Class<?> type, JavacNode node, JCTree typeNode) {
   ...
   public
   static
   boolean isFieldDeprecated(JavacNode field) {
   ...
   public
   static
   boolean nodeHasDeprecatedFlag(JCTree node) {
   ...
   AnnotationValues<A> createAnnotation(Class<A> type, final JavacNode node) {
     ...
     for (Method m : type.getDeclaredMethods()) {
       ...
       final java.util.List<DiagnosticPosition> positions =
         new ArrayList<DiagnosticPosition>();
   ...
   public
   static
   void deleteAnnotationIfNeccessary(JavacNode annotation, 
   ...
   void deleteAnnotationIfNeccessary(JavacNode annotation, 
     ...
     deleteAnnotationIfNeccessary0(annotation, annotationType1,
       annotationType2);
   }
   private
   static
   void deleteAnnotationIfNeccessary0(JavacNode annotation, 
     ...
     switch (parentNode.getKind()) {
       ...
       case LOCAL:
         ...
         variable.mods.annotations =
         filterList(variable.mods.annotations, annotation.get());
         break;
       case METHOD:
         JCMethodDecl method = (JCMethodDecl)parentNode.get();
         method.mods.annotations =
         filterList(method.mods.annotations, annotation.get());
         break;
       case TYPE:
         try {
           JCClassDecl type = (JCClassDecl)parentNode.get();
           type.mods.annotations =
           filterList(type.mods.annotations, annotation.get());
   ...
   public
   static
   void deleteImportFromCompilationUnit(JavacNode node, String name) {
     ...
     for (JCTree def : unit.defs) {
       ...
       if (def instanceof JCImport) {
         ...
         delete =
         (!imp0rt.staticImport && imp0rt.qualid.toString().equals(name));
       }
       if (!delete)
         newDefs.append(def);
     }
   ...
   private
   static
   List<JCAnnotation> filterList(List<JCAnnotation> annotations, 
                                 JCTree jcTree) {
     ListBuffer<JCAnnotation> newAnnotations =
   ...
   public
   static
   java.util.List<String> toAllGetterNames(JavacNode field) {
     return HandlerUtil.toAllGetterNames(field.getAst(),
              getAccessorsForField(field), field.getName(), isBoolean(field));
   }
   public
   static
   String toGetterName(JavacNode field) {
     return HandlerUtil.toGetterName(field.getAst(),
              getAccessorsForField(field), field.getName(), isBoolean(field));
   }
   public
   static
   java.util.List<String> toAllSetterNames(JavacNode field) {
     return HandlerUtil.toAllSetterNames(field.getAst(),
              getAccessorsForField(field), field.getName(), isBoolean(field));
   }
   public
   static
   String toSetterName(JavacNode field) {
     return HandlerUtil.toSetterName(field.getAst(),
              getAccessorsForField(field), field.getName(), isBoolean(field));
   }
   public
   static
   java.util.List<String> toAllWitherNames(JavacNode field) {
     return HandlerUtil.toAllWitherNames(field.getAst(),
              getAccessorsForField(field), field.getName(), isBoolean(field));
   }
   public
   static
   String toWitherName(JavacNode field) {
     return HandlerUtil.toWitherName(field.getAst(),
              getAccessorsForField(field), field.getName(), isBoolean(field));
   }
   public
   static
   boolean shouldReturnThis(JavacNode field) {
     if ((((JCVariableDecl)field.get()).mods.flags & Flags.STATIC) != 0)
       return false;
     AnnotationValues<Accessors> accessors =
       JavacHandlerUtil.getAccessorsForField(field);
     return HandlerUtil.shouldReturnThis0(accessors, field.getAst());
   }
   public
   static
   JCExpression cloneSelfType(JavacNode field) {
   ...
   public
   static
   AnnotationValues<Accessors> getAccessorsForField(JavacNode field) {
     for (JavacNode node : field.down()) {
       if (annotationTypeMatches(Accessors.class, node)) {
   ...
   public
   static
   MemberExistsResult fieldExists(String fieldName, JavacNode node) {
   ...
   public
   static
   MemberExistsResult methodExists(String methodName, JavacNode node, 
                                   int params) {
   ...
   public
   static
   MemberExistsResult constructorExists(JavacNode node) {
     node = upToTypeNode(node);
     if (node != null && node.get() instanceof JCClassDecl) {
       for (JCTree def : ((JCClassDecl)node.get()).defs) {
         if (def instanceof JCMethodDecl) {
   ...
   public
   static
   boolean isConstructorCall(final JCStatement statement) {
   ...
   public
   static
   int toJavacModifier(AccessLevel accessLevel) {
     switch (accessLevel) {
       case MODULE:
       case PACKAGE:
         return 0;
   ...
   static
   boolean lookForGetter(JavacNode field, FieldAccess fieldAccess) {
   ...
   static
   JCExpression getFieldType(JavacNode field, FieldAccess fieldAccess) {
   ...
   static
   JCExpression createFieldAccessor(JavacTreeMaker maker, JavacNode field, 
   ...
   JCExpression createFieldAccessor(JavacTreeMaker maker, JavacNode field, 
     ...
     if (getter == null) {
       ...
       return receiver == null ? maker.Ident(fieldDecl.name) : maker.Select(receiver,
                                                                 fieldDecl.name);
     }
     if (receiver == null)
       receiver = maker.Ident(field.toName("this"));
   ...
   public
   static
   void injectFieldSuppressWarnings(JavacNode typeNode, JCVariableDecl field) {
     injectField(typeNode, field, true);
   }
   public
   static
   JavacNode injectField(JavacNode typeNode, JCVariableDecl field) {
     return injectField(typeNode, field, false);
   }
   private
   static
   JavacNode injectField(JavacNode typeNode, JCVariableDecl field, 
                         boolean addSuppressWarnings) {
     JCClassDecl type = (JCClassDecl)typeNode.get();
     if (addSuppressWarnings)
       addSuppressWarningsAll(field.mods, typeNode, field.pos,
         getGeneratedBy(field), typeNode.getContext());
   ...
   public
   static
   boolean isEnumConstant(final JCVariableDecl field) {
     return (field.mods.flags & Flags.ENUM) != 0;
   }
   public
   static
   void injectMethod(JavacNode typeNode, JCMethodDecl method) {
     JCClassDecl type = (JCClassDecl)typeNode.get();
     ...
     addSuppressWarningsAll(method.mods, typeNode, method.pos,
       getGeneratedBy(method), typeNode.getContext());
     type.defs = type.defs.append(method);
     typeNode.add(method, Kind.METHOD);
   }
   public
   static
   JavacNode injectType(JavacNode typeNode, final JCClassDecl type) {
     JCClassDecl typeDecl = (JCClassDecl)typeNode.get();
     addSuppressWarningsAll(type.mods, typeNode, type.pos,
       getGeneratedBy(type), typeNode.getContext());
     typeDecl.defs = typeDecl.defs.append(type);
     return typeNode.add(type, Kind.TYPE);
   }
   public
   static
   long addFinalIfNeeded(long flags, Context context) {
     boolean addFinal =
       LombokOptionsFactory.getDelombokOptions(context).getFormatPreferences().generateFinalParams();
     if (addFinal)
       flags |= Flags.FINAL;
     return flags;
   }
   public
   static
   JCExpression genJavaLangTypeRef(JavacNode node, String... simpleNames) {
   ...
   public
   static
   JCExpression genJavaLangTypeRef(JavacNode node, int pos, 
                                   String... simpleNames) {
     if (LombokOptionsFactory.getDelombokOptions(node.getContext()).getFormatPreferences().javaLangAsFqn()) {
   ...
   public
   static
   void addSuppressWarningsAll(JCModifiers mods, JavacNode node, int pos, 
                               JCTree source, Context context) {
     ...
     for (JCAnnotation ann : mods.annotations) {
       ...
       if (annType instanceof JCIdent)
         lastPart = ((JCIdent)annType).name; else if (annType instanceof JCFieldAccess)
         lastPart = ((JCFieldAccess)annType).name;
       if (lastPart != null && lastPart.contentEquals("SuppressWarnings"))
         return;
     }
     JavacTreeMaker maker = node.getTreeMaker();
     JCExpression suppressWarningsType =
       genJavaLangTypeRef(node, "SuppressWarnings");
     JCLiteral allLiteral = maker.Literal("all");
     suppressWarningsType.pos = pos;
     allLiteral.pos = pos;
     JCAnnotation annotation =
       recursiveSetGeneratedBy(maker.Annotation(suppressWarningsType,
                                 List.<JCExpression>of(allLiteral)),
         source, context);
     annotation.pos = pos;
   ...
   private
   static
   List<JCTree> addAllButOne(List<JCTree> defs, int idx) {
   ...
   public
   static
   JCExpression chainDots(JavacNode node, String elem1, String elem2, 
                          String... elems) {
     return chainDots(node, -1, elem1, elem2, elems);
   }
   public
   static
   JCExpression chainDots(JavacNode node, String[] elems) {
     return chainDots(node, -1, null, null, elems);
   }
   public
   static
   JCExpression chainDots(JavacNode node, int pos, String elem1, 
                          String elem2, String... elems) {
     assert elems != null;
     JavacTreeMaker maker = node.getTreeMaker();
     if (pos != -1)
       maker = maker.at(pos);
     ...
     if (elem2 != null)
       e =
       e == null ? maker.Ident(node.toName(elem2)) : maker.Select(e,
                                                       node.toName(elem2));
     for (int i = 0; i < elems.length; i++) {
       e =
       e == null ? maker.Ident(node.toName(elems[i])) : maker.Select(e,
                                                          node.toName(elems[i]));
     }
     assert e != null;
     return e;
   ...
   public
   static
   JCExpression chainDotsString(JavacNode node, String elems) {
     return chainDots(node, null, null, elems.split("\\."));
   }
   public
   static
   List<JCAnnotation> findAnnotations(JavacNode fieldNode, 
                                      Pattern namePattern) {
   ...
   public
   static
   JCStatement generateNullCheck(JavacTreeMaker maker, JavacNode variable) {
     JCVariableDecl varDecl = (JCVariableDecl)variable.get();
     if (isPrimitive(varDecl.vartype))
       return null;
     Name fieldName = varDecl.name;
     JCExpression npe = genJavaLangTypeRef(variable, "NullPointerException");
     JCExpression exception =
       maker.NewClass(null, List.<JCExpression>nil(), npe,
         List.<JCExpression>of(maker.Literal(fieldName.toString())), null);
     JCStatement throwStatement = maker.Throw(exception);
     JCBlock throwBlock = maker.Block(0, List.of(throwStatement));
     return maker.If(maker.Binary(CTC_EQUAL, maker.Ident(fieldName),
   ...
   static
   List<JCAnnotation> unboxAndRemoveAnnotationParameter(JCAnnotation ast, 
     ...
     outer: for (JCExpression param : ast.args) {
       ...
       int endPos =
         Javac.getEndPosition(param.pos(),
           (JCCompilationUnit)annotationNode.top().get());
       annotationNode.getAst().removeFromDeferredDiagnostics(param.pos,
         endPos);
       if (valueOfParam instanceof JCAnnotation) {
         String dummyAnnotationName =
           ((JCAnnotation)valueOfParam).annotationType.toString();
         dummyAnnotationName =
         dummyAnnotationName.replace("_", "").replace("$", "").replace("x",
           "").replace("X", "");
         ...
         if (valueOfParam instanceof JCNewArray && ((JCNewArray)valueOfParam).elems.isEmpty()) {} else {
           annotationNode.addError("The correct format is " + errorName + "@__({@SomeAnnotation, @SomeOtherAnnotation}))");
         }
       }
     }
   ...
   public
   static
   List<JCTypeParameter> copyTypeParams(JavacTreeMaker maker, 
   ...
   public
   static
   JCExpression namePlusTypeParamsToTypeReference(JavacTreeMaker maker, 
                                                  Name typeName, 
                                                  List<JCTypeParameter> params) {
     ListBuffer<JCExpression> typeArgs = new ListBuffer<JCExpression>();
     if (!params.isEmpty()) {
   ...
   public
   static
   void sanityCheckForMethodGeneratingAnnotationsOnBuilderClass(JavacNode typeNode,
   ...
   static
   List<JCAnnotation> copyAnnotations(List<? extends JCExpression> in) {
   ...
   static
   boolean isClass(JavacNode typeNode) {
     return isClassAndDoesNotHaveFlags(typeNode,
              Flags.INTERFACE | Flags.ENUM | Flags.ANNOTATION);
   }
   static
   boolean isClassOrEnum(JavacNode typeNode) {
     return isClassAndDoesNotHaveFlags(typeNode,
              Flags.INTERFACE | Flags.ANNOTATION);
   }
   public
   static
   boolean isClassAndDoesNotHaveFlags(JavacNode typeNode, int flags) {
     JCClassDecl typeDecl = null;
     if (typeNode.get() instanceof JCClassDecl)
       typeDecl = (JCClassDecl)typeNode.get(); else
       return false;
   ...
   public
   static
   JavacNode upToTypeNode(JavacNode node) {
     if (node == null)
   ...
   public
   static
   JCExpression cloneType(JavacTreeMaker maker, JCExpression in, 
                          JCTree source, Context context) {
   ...
   private
   static
   JCExpression cloneType0(JavacTreeMaker maker, JCTree in) {
     if (in == null)
       return null;
   ...
   private static final Pattern SECTION_FINDER =
     Pattern.compile("^\\s*\\**\\s*[-*][-*]+\\s*([GS]ETTER|WITHER)\\s*[-*][-*]+\\s*\\**\\s*$",
       Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
   public
   static
   String stripLinesWithTagFromJavadoc(String javadoc, String regexpFragment) {
     Pattern p =
       Pattern.compile("^\\s*\\**\\s*" + regexpFragment + "\\s*\\**\\s*$",
         Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
   ...
   public
   static
   String stripSectionsFromJavadoc(String javadoc) {
   ...
   public
   static
   String[] splitJavadocOnSectionIfPresent(String javadoc, String sectionName) {
     Matcher m = SECTION_FINDER.matcher(javadoc);
     ...
     if (getterSectionStart != -1) {
       if (getterSectionEnd != -1) {
         return new String[]{
         javadoc.substring(getterSectionStart, getterSectionEnd),
         javadoc.substring(0, getterSectionHeaderStart) + javadoc.substring(getterSectionEnd)
         };
       } else {
         return new String[]{
         javadoc.substring(getterSectionStart),
         javadoc.substring(0, getterSectionHeaderStart)
         };
       }
     }
     return null;
   ...
   enum CopyJavadoc
   {
      ...
      private
      static
      String[] splitForSetters(String javadoc, String sectionName) {
        String[] out = splitJavadocOnSectionIfPresent(javadoc, sectionName);
        if (out != null)
          return out;
        String copy = javadoc;
        javadoc =
        stripLinesWithTagFromJavadoc(javadoc, "@param(?:eter)?\\s+.*");
   ...
   public
   static
   void copyJavadoc(JavacNode from, JCTree to, CopyJavadoc copyMode) {
     if (copyMode == null)
   ...
   class CopyJavadoc_8
   {
      ...
      private
      static
      Comment createJavadocComment(final String text, final JavacNode field) {
        return new Comment()
               {
                  @Override
                  public
                  ...
                  boolean isDeprecated() {
                    return text.contains("@deprecated") && field.getKind() == Kind.FIELD && 
                    isFieldDeprecated(field);
                  }
               };
      }
   }
   ...
   private
   static
   void copyJavadoc_jdk6_7(JavacNode from, JCTree to, CopyJavadoc copyMode, 
                           Object dc) {
