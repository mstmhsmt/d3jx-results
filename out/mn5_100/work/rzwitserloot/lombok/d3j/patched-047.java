...
class AbstractRunTests
{
   ...
   public
   boolean compareFile(DirectoryRunner.TestParams params, File file)
     throws Throwable {
     ConfigurationKeysLoader.LoaderLoader.loadAllConfigurationKeys();
     final LombokTestSource sourceDirectives =
       LombokTestSource.readDirectives(file);
     if (sourceDirectives.isIgnore() || !sourceDirectives.versionWithinLimit(
                                           params.getVersion()))
       return false;
     String fileName = file.getName();
     LombokTestSource expected =
       LombokTestSource.read(params.getAfterDirectory(),
         params.getMessagesDirectory(), fileName);
     if (expected.isIgnore() || !expected.versionWithinLimit(params.getVersion()))
       return false;
     LinkedHashSet<CompilerMessage> messages =
     ...
     LombokConfiguration.overrideConfigurationResolverFactory(new ConfigurationResolverFactory()
                                                              {
                                                                 ...
                                                                 AST<?,?,?> ast) {
                                                                   return 
                                                                   sourceDirectives.getConfiguration();
                                                                 }
                                                              });
     transformCode(messages, writer, file);
     compare(file.getName(), expected, writer.toString(), messages,
       params.printErrors(),
       sourceDirectives.isSkipCompareContent() || expected.isSkipCompareContent());
     return true;
   }
   protected
   abstract
   void transformCode(Collection<CompilerMessage> messages, 
                      StringWriter result, File file)
     throws Throwable;
   protected
   ...
   private
   static
   File findPlaceToDumpActualFiles() {
   ...
   private
   static
   void dumpToFile(File file, String content) throws IOException {
   ...
   private
   static
   void dumpToFile(File file, Collection<CompilerMessage> content)
     throws IOException {
     FileOutputStream fos = new FileOutputStream(file);
     try {
   ...
   private
   void compare(String name, LombokTestSource expected, String actualFile, 
                LinkedHashSet<CompilerMessage> actualMessages, 
                boolean printErrors, boolean skipCompareContent)
     throws Throwable {
     ...
     try {
       ...
       if (dumpActualFilesHere != null) {
         dumpToFile(new File(dumpActualFilesHere, name + ".messages"),
           actualMessages);
       }
       throw e;
     } 
   ...
   private
   static
   void compareMessages(String name, 
     ...
     while (true) {
       ...
       if (exHasNext && acHasNext) {
         ...
         if (cmm.matches(cm))
           continue;
         fail(String.format("[%s] Expected message '%s' but got message '%s'",
                name, cmm, cm));
         throw new AssertionError("fail should have aborted already.");
       }
       if (exHasNext)
         fail(String.format("[%s] Expected message '%s' but ran out of actual messages",
                name, expectedIterator.next()));
       if (acHasNext)
         fail(String.format("[%s] Unexpected message: %s", name,
                actualIterator.next()));
       throw new AssertionError("fail should have aborted already.");
     }
   }
   ...
   void compareContent(String name, String expectedFile, String actualFile) {
     ...
     for (int i = 0; i < size; i++) {
       ...
       assertEquals(String.format("Difference in %s on line %d", name, i + 1),
         expected, actual);
     }
     if (expectedLines.length > actualLines.length) {
       fail(String.format("Missing line %d in generated %s: %s", size + 1,
              name, expectedLines[size]));
     }
     if (expectedLines.length < actualLines.length) {
       fail(String.format("Extra line %d in generated %s: %s", size + 1,
              name, actualLines[size]));
     }
   }
   private
   static
   String trimRight(String in) {
