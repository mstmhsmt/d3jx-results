...
public class HandleLog {
  ...
  public static void processAnnotation(LoggingFramework framework, AnnotationValues<?> annotation, JavacNode annotationNode, String loggerCategory) {
    ...
    switch (typeNode.getKind()) {
      ...
      createField(framework, typeNode, loggingType, annotationNode.get(), frameworktypeNodeloggingTypeannotationNode.get()logFieldNameuseStatic);
      break;
      default:
  ...
  static public JCFieldAccess selfType(JavacNode typeNode) {
    JavacTreeMaker maker = typeNode.getTreeMaker();
    Name name = ((JCClassDecl) typeNode.get()).name;
    return maker.Select(maker.Ident(name), typeNode.toName("class"));
  }

  static public boolean createField(LoggingFramework framework, JavacNode typeNode, JCFieldAccess loggingType, JCTree source, LoggingFramework frameworkJavacNode typeNodeJCFieldAccess loggingTypeJCTree sourceString logFieldNameboolean useStatic) {
    JavacTreeMaker maker = typeNode.getTreeMaker();
