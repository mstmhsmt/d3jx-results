...
public abstract class AbstractRunTests {
  ...
  public boolean compareFile(DirectoryRunner.TestParams params, File file) throws Throwable {
    ...
    if (sourceDirectives.isIgnore() || !sourceDirectives.versionWithinLimit(params.getVersion())) {
      return false;
    }
    String fileName = file.getName();
    LombokTestSource expected = LombokTestSource.read(params.getAfterDirectory(), params.getMessagesDirectory(), fileName);
    if (expected.isIgnore() || !expected.versionWithinLimit(params.getVersion())) {
      return false;
    }
  ...
  private static void compareContent(String name, String expectedFile, String actualFile) {
    ...
    for (int i = 0; i < expectedLines.length; i++) {
      if (expectedLines[i].isEmpty() || expectedLines[i].startsWith("//")) {
        expectedLines[i] = "";
      } else {
        break;
      }
    ...
    for (int i = 0; i < actualLines.length; i++) {
      if (actualLines[i].isEmpty() || actualLines[i].startsWith("//")) {
        actualLines[i] = "";
      } else {
        break;
      }
