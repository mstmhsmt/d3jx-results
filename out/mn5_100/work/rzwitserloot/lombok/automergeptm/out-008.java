...
@ProviderFor(value = JavacAnnotationHandler.class) public class HandleToString extends JavacAnnotationHandler<ToString> {
  ...
  static JCMethodDecl createToString(JavacNode typeNode, Collection<JavacNode> fields, boolean includeFieldNames, boolean callSuper, FieldAccess fieldAccess, JCTree source) {
    ...
    for (JavacNode fieldNode : fields) {
      ...
      @SuppressWarnings(value = { "unused" }) boolean fieldIsObject = !fieldIsPrimitive && !fieldIsPrimitiveArray && !fieldIsObjectArray;
    ...
    if (!first) {
      current = Javac.makeBinary(maker, CTC_PLUS, current, maker.Literal(suffix));
    }
