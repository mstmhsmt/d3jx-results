...
public class JavacHandlerUtil {
  ...
  public static <T extends JCTree> T setGeneratedBy(T node, JCTree source, Context context) {
    ...
    synchronized (generatedNodes) {
      if (source == null) {
        ...
        generatedNodes.put(node, new WeakReference<JCTree>(source));
      }
    }
    if (source != null && (!inNetbeansEditor(context) || (node instanceof JCVariableDecl && (((JCVariableDecl) node).mods.flags & Flags.PARAMETER) != 0))) {
      node.pos = source.pos;
    }
    return node;
  }
  ...
  /**
	 ...
	 */
  @SuppressWarnings(value = { "unchecked" }) public static void deleteAnnotationIfNeccessary(JavacNode annotation, Class<? extends Annotation> annotationType) {
  ...
  /**
	 ...
	 * Only does this if the DeleteLombokAnnotations class is in the context.
	 */
  @SuppressWarnings(value = { "unchecked" }) public static void deleteAnnotationIfNeccessary(JavacNode annotation, Class<? extends Annotation> annotationType1, Class<? extends Annotation> annotationType2) {
  ...
  public static boolean isConstructorCall(final JCStatement statement) {
    ...
    if (invocation instanceof JCFieldAccess) {
      ...
      if (invocation instanceof JCIdent) {
  ...
  static public boolean isEnumConstant(final JCVariableDecl field) {
  ...
  static public void addSuppressWarningsAll(JCModifiers mods, JavacNode node, int pos, JCTree source, Context context) {
    ...
    for (JCAnnotation ann : mods.annotations) {
      ...
      if (annType instanceof JCIdent) {
        lastPart = ((JCIdent) annType).name;
      } else {
        if (annType instanceof JCFieldAccess) {
          lastPart = ((JCFieldAccess) annType).name;
        }
      }
      if (lastPart != null && lastPart.contentEquals("SuppressWarnings")) {
        return;
      }
    }
  ...
  static public boolean isClassAndDoesNotHaveFlags(JavacNode typeNode, int flags) {
  ...
  static public String stripLinesWithTagFromJavadoc(String javadoc, String regexpFragment) {
    Pattern p = Pattern.compile("^\\s*\\**\\s*" + regexpFragment + "\\s*\\**\\s*$", Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
    Matcher m = p.matcher(javadoc);
    return m.replaceAll("");
  }

  static public String[] splitJavadocOnSectionIfPresent(String javadoc, String sectionName) {
  ...
  public static void copyJavadoc(JavacNode from, JCTree to, CopyJavadoc copyMode) {
    ...
    try {
      ...
      if (dc instanceof Map) {
        ...
        if (Javac.instanceOfDocCommentTable(dc)) {
  ...
  @SuppressWarnings(value = { "unchecked", "all" }) private static void copyJavadoc_jdk6_7(JavacNode from, JCTree to, CopyJavadoc copyMode, Object dc) {
