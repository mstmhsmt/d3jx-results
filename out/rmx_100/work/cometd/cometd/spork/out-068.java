...
public abstract class OortService<R, C> extends AbstractLifeCycle implements ServerChannel.MessageListener {
    ...
    private final ConcurrentMap<Long, Map<String, Object>> callbacks = new ConcurrentHashMap<Map<String, Object>>();
     ...
     *
     * @param targetOortURL
     * 		the owner node Oort URL
     * @param actionData
     * 		the action data that will be passed to {@link #onForward(Object)}
     * @param context
     * 		the opaque context passed to {@link #onForwardSucceeded(Object, Object)}
    ...
    protected boolean forward(String targetOortURL, Object parameter, C context) {
        ...
        if (targetOortURL == null) {
            ...
            return true;
        } else if (localOortURL.equals(targetOortURL)) {
            // Local case
     ...
     *
     * @param actionData
     * 		the action data passed from {@link #forward(String, Object, Object)}
