...
public class AsyncJSONTransport extends AbstractHttpTransport {
    ...
    @Override
    protected void write(HttpServletRequest request, HttpServletResponse response, ServerSessionImpl session, boolean scheduleExpiration, List<ServerMessage> messages, ServerMessage[] replies) {
        AsyncContext asyncContext = request.getAsyncContext();
        try {
    ...
    protected abstract class AbstractReader implements ReadListener {
        ...
        protected void process(String json) throws IOException {
            ...
            try {
                ServerMessage[] messages = parseMessages(json);
                if (_logger.isDebugEnabled()) {
    ...
    protected class Writer implements WriteListener {
        ...
        protected Writer(HttpServletRequest request, HttpServletResponse response, AsyncContext asyncContext, ServerSessionImpl session, boolean scheduleExpiration, List<ServerMessage> messages, ServerMessage[] replies) {
            this.request = request;
            this.response = response;
            this.asyncContext = asyncContext;
