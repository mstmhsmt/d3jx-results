...
public
abstract
class AbstractClientSession implements ClientSession, Dumpable
{
   ...
   private final ConcurrentMap<String,AbstractSessionChannel> _channels =
     new ConcurrentHashMap<>();
   private final Map<String,ClientSessionChannel.MessageListener>
   _callbacks = new ConcurrentHashMap<>();
   private final Map<String,ClientSessionChannel.MessageListener>
   _subscribers = new ConcurrentHashMap<>();
   private final Map<String,MessageListener> _remoteCalls =
     new ConcurrentHashMap<>();
   ...
   protected
   boolean extendSend(Message.Mutable message) {
     if (message.isMeta()) {
   ...
   protected
   boolean extendRcv(Message.Mutable message) {
     if (message.isMeta()) {
       for (Extension extension : _extensions) {
   ...
   protected
   abstract
   ChannelId newChannelId(String channelId);
   protected
   abstract
   AbstractSessionChannel newChannel(ChannelId channelId);
   @Override
   public
   ...
   public
   ClientSessionChannel getChannel(ChannelId channelId) {
     return getChannel(channelId.toString(), channelId);
   }
   private
   ClientSessionChannel getChannel(String channelName, ChannelId channelId) {
   ...
   protected
   ConcurrentMap<String,AbstractSessionChannel> getChannels() {
     return _channels;
   }
   @Override
   public
   ...
   protected
   abstract
   void send(Message.Mutable message);
   protected
   ...
   public
   void receive(final Message.Mutable message) {
     ...
     if (Channel.META_SUBSCRIBE.equals(channelName)) {
       ClientSessionChannel.MessageListener subscriber =
         unregisterSubscriber(message.getId());
       if (!message.isSuccessful()) {
         String subscription =
           (String)message.get(Message.SUBSCRIPTION_FIELD);
         MarkedReference<AbstractSessionChannel> channelRef =
           getReleasableChannel(subscription);
   ...
   private
   boolean handleRemoteCall(Message.Mutable message) {
   ...
   private
   void notifyMessageListener(MessageListener listener, 
   ...
   protected
   void notifyListeners(Message.Mutable message) {
     ...
     MarkedReference<AbstractSessionChannel> channelRef =
       getReleasableChannel(message.getChannel());
     AbstractSessionChannel channel = channelRef.getReference();
     ...
     for (String wildChannelName : channelId.getWilds()) {
       MarkedReference<AbstractSessionChannel> wildChannelRef =
         getReleasableChannel(wildChannelName);
   ...
   protected
   void notifyListener(ClientSessionChannel.MessageListener listener, 
                       Message.Mutable message) {
     MarkedReference<AbstractSessionChannel> channelRef =
       getReleasableChannel(message.getChannel());
     AbstractSessionChannel channel = channelRef.getReference();
   ...
   private
   MarkedReference<AbstractSessionChannel> getReleasableChannel(String id) {
     AbstractSessionChannel channel =
       ChannelId.isMeta(id) ? (AbstractSessionChannel)getChannel(id) : 
     getChannels().get(id);
     if (channel != null) {
   ...
   protected
   void registerCallback(String messageId, 
                         ClientSessionChannel.MessageListener callback) {
     if (callback != null) {
   ...
   protected
   ClientSessionChannel.MessageListener unregisterCallback(String messageId) {
     if (messageId == null)
       return null;
     return _callbacks.remove(messageId);
   }
   protected
   void registerSubscriber(String messageId, 
                           ClientSessionChannel.MessageListener subscriber) {
     if (subscriber != null) {
   ...
   protected
   ClientSessionChannel.MessageListener unregisterSubscriber(String messageId) {
     if (messageId == null)
       return null;
     ...
     children.add(new Dumpable()
                  {
                     ...
                     void dump(Appendable out, String indent)
                       throws IOException {
                       Collection<AbstractSessionChannel> channels =
                         getChannels().values();
                       ContainerLifeCycle.dumpObject(out,
   ...
   protected
   abstract
   class AbstractSessionChannel implements ClientSessionChannel, Dumpable
   {
      private final ChannelId _id;
      private final AttributesMap _attributes = new AttributesMap();
      private final CopyOnWriteArrayList<MessageListener> _subscriptions =
        new CopyOnWriteArrayList<>();
      private final AtomicInteger _subscriptionCount = new AtomicInteger();
      private final CopyOnWriteArrayList<ClientSessionChannelListener>
      ...
      @Override
      public
      void publish(Object data, MessageListener callback) {
        throwIfReleased();
        Message.Mutable message = newMessage();
        String messageId = newMessageId();
        message.setId(messageId);
      ...
      @Override
      public
      void subscribe(MessageListener listener, MessageListener callback) {
        throwIfReleased();
      ...
      protected
      void sendSubscribe(MessageListener listener, MessageListener callback) {
        Message.Mutable message = newMessage();
        String messageId = newMessageId();
        message.setId(messageId);
      ...
      @Override
      public
      void unsubscribe(MessageListener listener, MessageListener callback) {
      ...
      private
      boolean removeSubscription(MessageListener listener) {
        throwIfReleased();
      ...
      protected
      void sendUnSubscribe(MessageListener callback) {
        Message.Mutable message = newMessage();
        String messageId = newMessageId();
        message.setId(messageId);
      ...
      protected
      void notifyOnMessage(MessageListener listener, Message message) {
        throwIfReleased();
        try {
        ...
        children.add(new Dumpable()
                     {
                        ...
                        void dump(Appendable out, String indent)
                          throws IOException {
                          List<ClientSessionChannelListener> listeners =
                            getListeners();
                          ContainerLifeCycle.dumpObject(out,
