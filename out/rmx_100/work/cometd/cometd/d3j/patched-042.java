...
public class AsyncJSONTransport extends AbstractHttpTransport {
  ...
  public void handle(HttpServletRequest request, HttpServletResponse response)
    ...
    ReadListener reader =
        "UTF-8".equals(charset.name())
            ? new UTF8Reader(request, response, asyncContext)
            : new CharsetReader(request, response, asyncContext, charset);
    ServletInputStream input = request.getInputStream();
    input.setReadListener(reader);
  }
  protected HttpScheduler suspend(HttpServletRequest request,
                                  HttpServletResponse response,
                                  ServerSessionImpl session,
                                  ServerMessage.Mutable reply, String browserId,
                                  long timeout) {
    AsyncContext asyncContext = request.getAsyncContext();
    return newHttpScheduler(request, response, asyncContext, session, reply,
                            browserId, timeout);
  }
  protected HttpScheduler newHttpScheduler(HttpServletRequest request,
                                           HttpServletResponse response,
                                           AsyncContext asyncContext,
                                           ServerSessionImpl session,
                                           ServerMessage.Mutable reply,
                                           String browserId, long timeout) {
    return new AsyncLongPollScheduler(request, response, asyncContext, session,
                                      reply, browserId, timeout);
  }
  @Override
  protected void write(HttpServletRequest request, HttpServletResponse response,
                       ServerSessionImpl session, boolean scheduleExpiration,
    ...
    try {
      ...
      output.setWriteListener(new Writer(request, response, asyncContext,
                                         session, scheduleExpiration, messages,
                                         replies));
    } catch (Exception x) {
      if (_logger.isDebugEnabled()) {
        _logger.debug("Exception while writing messages", x);
      }
      error(request, response, asyncContext,
  ...
  protected abstract class AbstractReader implements ReadListener {
    ...
    protected AbstractReader(HttpServletRequest request,
                             HttpServletResponse response,
                             AsyncContext asyncContext) {
    ...
    protected void process(String json) throws IOException {
      ...
      try {
        ...
        if (_logger.isDebugEnabled()) {
          _logger.debug("Parsed {} messages",
                        messages == null ? -1 : messages.length);
        }
        if (messages != null) {
  ...
  protected class UTF8Reader extends AbstractReader {
    private final Utf8StringBuilder content =
        new Utf8StringBuilder(BUFFER_CAPACITY);
    protected UTF8Reader(HttpServletRequest request,
                         HttpServletResponse response,
                         AsyncContext asyncContext) {
  ...
  protected class CharsetReader extends AbstractReader {
    ...
    private int count;
    public CharsetReader(HttpServletRequest request,
                         HttpServletResponse response,
  ...
  protected class Writer implements WriteListener {
    ...
    protected Writer(HttpServletRequest request, HttpServletResponse response,
                     AsyncContext asyncContext, ServerSessionImpl session,
                     boolean scheduleExpiration, List<ServerMessage> messages,
                     ServerMessage.Mutable[] replies) {
      this.request = request;
      this.response = response;
      this.asyncContext = asyncContext;
    ...
    public void onWritePossible() throws IOException {
      ServletOutputStream output = response.getOutputStream();
<<<<<<< /root/work/cometd/cometd/d3j/patched-042-0.java
      if (_logger.isDebugEnabled())
        _logger.debug("Messages/replies {}/{} to write for session {}",
                      messages.size(), replies.length, session);
      while (true) {
        switch (state) {
        case BEGIN: {
    ...
    private boolean writeHandshakeReply(ServletOutputStream output)
        throws IOException {
      if (replies.length > 0) {
        ServerMessage.Mutable reply = replies[0];
        if (Channel.META_HANDSHAKE.equals(reply.getChannel())) {
          if (allowMessageDeliveryDuringHandshake(session) &&
              !messages.isEmpty())
      ...
      return output.isReady();
||||||| /root/work/cometd/cometd/d3j/base-042.java
      if (_logger.isDebugEnabled())
        _logger.debug("Messages to write for session {}: {}", session,
                      messages.size());
      if (!writeMessages(output))
        return;
      if (_logger.isDebugEnabled())
        _logger.debug("Replies to write for session {}: {}", session,
                      replies.length);
      if (!writeReplies(output))
        return;
      asyncContext.complete();
=======
      ...
      if (_logger.isDebugEnabled()) {
        _logger.debug("Messages/replies {}/{} to write for session {}", session,
                      replies.length);
      }
      if (!writeReplies(output)) {
        return;
      ...
      asyncContext.complete();
>>>>>>> /root/work/cometd/cometd/d3j/patched-042-1.java
    }
    private boolean writeBegin(ServletOutputStream output) throws IOException {
      output.write('[');
      return output.isReady();
    ...
    private boolean writeMessages(ServletOutputStream output)
        throws IOException {
      try {
    ...
    private void startExpiration() {
      if (scheduleExpiration && session != null &&
          (session.isHandshook() || session.isConnected())) {
        session.scheduleExpiration(getInterval());
      }
    }
    private boolean writeReplies(ServletOutputStream output)
        throws IOException {
  ...
  private enum State { BEGIN, HANDSHAKE, MESSAGES, REPLIES, END, COMPLETE }
  private class AsyncLongPollScheduler extends LongPollScheduler {
    private AsyncLongPollScheduler(HttpServletRequest request,
                                   HttpServletResponse response,
                                   AsyncContext asyncContext,
                                   ServerSessionImpl session,
                                   ServerMessage.Mutable reply,
                                   String browserId, long timeout) {
      super(request, response, asyncContext, session, reply, browserId,
            timeout);
    }
    @Override
    protected void dispatch() {
      resume(getRequest(), getResponse(), getAsyncContext(), getServerSession(),
             getMetaConnectReply());
    }
  }
}