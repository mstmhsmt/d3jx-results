...
public class UTF8StreamJsonParser extends JsonParserBase {
  ...
  private final static int FEAT_MASK_TRAILING_COMMA =
      JsonReadFeature.ALLOW_TRAILING_COMMA.getMask();
  private final static int FEAT_MASK_ALLOW_MISSING =
      JsonReadFeature.ALLOW_MISSING_VALUES.getMask();
  ...
  public UTF8StreamJsonParser(ObjectReadContext readCtxt, IOContext ctxt,
                              ...
                              InputStream in, ByteQuadsCanonicalizer sym,
    ...
    if (_tokenIncomplete) {
      try {
        ...
        throw _constructReadException(
            "Failed to decode VALUE_STRING as base64 (" + b64variant +
            "): " + iae.getMessage());
      }
      _tokenIncomplete = false;
    } else {
      ...
      if (bits < 0) {
        ...
        if (bits == Base64Variant.BASE64_VALUE_PADDING) {
          ...
          if (!b64variant.usesPaddingChar(ch)) {
            if (_decodeBase64Escape(b64variant, ch, 3) !=
                Base64Variant.BASE64_VALUE_PADDING) {
              _reportInvalidBase64Char(b64variant, ch, 3,
    ...
    if (_streamReadContext.expectComma()) {
      ...
      if ((_formatReadFeatures & FEAT_MASK_TRAILING_COMMA) != 0) {
        ...
        if (isEndObject || (i == INT_RBRACKET)) {
          _closeScope(i);
          return isEndObject ? PropertyNameMatcher.MATCH_END_OBJECT
  ...
  protected final int _matchName(PropertyNameMatcher matcher, int i)
      throws JacksonException {
    if (i != INT_QUOTE) {
      return -1;
    }
  ...
  protected final int _matchMediumName(PropertyNameMatcher matcher, int qptr,
                                       int q2) throws JacksonException {
    final byte[] input = _inputBuffer;
    final int[] codes = _icLatin1;
  ...
  protected final JsonToken _parseFloatThatStartsWithPeriod()
    ...
    return _parseFloat(_textBuffer.emptyAndGetCurrentSegment(), 0, INT_PERIOD,
                       false, 0);
  }
  ...
  private final JsonToken _parseSignedNumber(boolean negative)
      throws JacksonException {
    char[] outBuf = _textBuffer.emptyAndGetCurrentSegment();
    int outPtr = 0;
    if (negative) {
  ...
  private final JsonToken _parseNumber2(char[] outBuf, int outPtr,
                                        boolean negative, int intPartLength)
      throws JacksonException {
    while (true) {
  ...
  protected final String parseMediumName2(int q3, final int q2)
      throws JacksonException {
    final byte[] input = _inputBuffer;
    final int[] codes = _icLatin1;
    int i = input[_inputPtr++] & 0xFF;
  ...
  protected final String parseLongName(int q, final int q2, int q3)
      throws JacksonException {
    _quadBuffer[0] = _quad1;
    _quadBuffer[1] = q2;
    _quadBuffer[2] = q3;
  ...
  private final String parseName(int q1, int q2, int q3, int ch,
                                 int lastQuadBytes) throws JacksonException {
    _quadBuffer[0] = q1;
    _quadBuffer[1] = q2;
    return parseEscapedName(_quadBuffer, 2, q3, ch, lastQuadBytes);
  }
  protected final String parseEscapedName(int[] quads, int qlen, int currQuad,
                                          int ch, int currQuadBytes)
      throws JacksonException {
    final int[] codes = _icLatin1;
    while (true) {
      if (codes[ch] != 0) {
  ...
  protected String _handleOddName(int ch) throws JacksonException {
    ...
    if (!isEnabled(JsonReadFeature.ALLOW_UNQUOTED_PROPERTY_NAMES)) {
      ...
      _reportUnexpectedChar(
          c, "was expecting double-quote to start property name");
    }
    final int[] codes = CharTypes.getInputCodeUtf8JsNames();
    if (codes[ch] != 0) {
      _reportUnexpectedChar(
          ch, "was expecting either valid name character (for unquoted name) " +
              "or double-quote (for quoted) to start property name");
    }
  ...
  protected String _parseAposName() throws JacksonException {
    if (_inputPtr >= _inputEnd) {
      if (!_loadMore()) {
        _reportInvalidEOF(": was expecting closing '\'' for property name",
                          JsonToken.PROPERTY_NAME);
      }
    }
    int ch = _inputBuffer[_inputPtr++] & 0xFF;
  ...
  private final String findName(int[] quads, int qlen, int lastQuad,
                                int lastQuadBytes) throws StreamReadException {
    if (qlen >= quads.length) {
      _quadBuffer = quads = growArrayBy(quads, quads.length);
    }
  ...
  protected JsonToken _handleUnexpectedValue(int c) throws JacksonException {
    ...
    case 'N':
      ...
      _reportError("Non-standard token 'NaN': enable " +
    ...
    case 'I':
      ...
      _reportError("Non-standard token 'Infinity': enable " +
                   "`JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS` to allow");
      break;
    case '+':
      if (_inputPtr >= _inputEnd) {
...
<<<<<<< /root/work/FasterXML/jackson-core/d3j/patched-006-0.java
  protected JsonToken _handleInvalidNumberStart(int ch, final boolean neg)
      throws JacksonException {
||||||| /root/work/FasterXML/jackson-core/d3j/base-006.java
...
=======
  ...
  protected JsonToken _handleInvalidNumberStart(int ch, final boolean neg,
      ...
      throws IOException {
>>>>>>> /root/work/FasterXML/jackson-core/d3j/patched-006-1.java
    while (ch == 'I') {
      if (_inputPtr >= _inputEnd) {
        if (!_loadMore()) {
      ...
      _reportError("Non-standard token '%s': enable " +
                   "`JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS` to allow",
                   match);
    }
<<<<<<< /root/work/FasterXML/jackson-core/d3j/patched-006-0.java
...
>>>>>>> /root/work/FasterXML/jackson-core/d3j/patched-006-1.java
  ...
  protected final void _matchFalse() throws JacksonException {
    ...
    if ((ptr + 4) < _inputEnd) {
      byte[] buf = _inputBuffer;
      if ((buf[ptr++] == 'a') && (buf[ptr++] == 'l') && (buf[ptr++] == 's') &&
          (buf[ptr++] == 'e')) {
        int ch = buf[ptr] & 0xFF;
        if (ch < INT_0 || (ch == INT_RBRACKET) || (ch == INT_RCURLY)) {
          _inputPtr = ptr;
  ...
  private final int _skipWS2() throws JacksonException {
    ...
    throw _constructReadException("Unexpected end-of-input within/between " +
                                  _streamReadContext.typeDesc() + " entries");
  }
  ...
  private final int _skipColon2(boolean gotColon) throws JacksonException {
    while (_inputPtr < _inputEnd || _loadMore()) {
      ...
      if (i > INT_SPACE) {
        ...
        if (i != INT_COLON) {
          _reportUnexpectedChar(
              i, "was expecting a colon to separate property name and value");
        }
        gotColon = true;
      } else if (i != INT_SPACE) {
        if (i == INT_LF) {
    ...
    _reportInvalidEOF(
        " within/between " + _streamReadContext.typeDesc() + " entries", null);
    return -1;
  }
  private final void _skipComment() throws JacksonException {
    if (!isEnabled(JsonReadFeature.ALLOW_JAVA_COMMENTS)) {
      _reportUnexpectedChar(
          '/', "maybe a (non-standard) comment? (not recognized as one since " +
  ...
  protected char _decodeEscaped() throws JacksonException {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (digit < 0) {
        _reportUnexpectedChar(
            ch & 0xFF, "expected a hex-digit for character escape sequence");
      }
      value = (value << 4) | digit;
    }
    return (char)value;
  ...
  public JsonLocation currentTokenLocation() {
    if (_currToken == JsonToken.PROPERTY_NAME) {
      ...
      return new JsonLocation(_contentReference(), total, -1L, _nameStartRow,
                              _nameStartCol);
    }
    return new JsonLocation(_contentReference(), _tokenInputTotal - 1, -1L,
                            _tokenInputRow, _tokenInputCol);
  }
  @Override
  public JsonLocation currentLocation() {
    ...
    return new JsonLocation(_contentReference(),
                            _currInputProcessed + _inputPtr, -1L, _currInputRow,
                            col);
  }
