...
public UTF8StreamJsonParser(ObjectReadContext readCtxt, IOContext ctxt, int stdFeatures, int formatReadFeatures, InputStream in, ByteQuadsCanonicalizer sym, byte[] inputBuffer, int start, int end, int bytesPreProcessed, boolean bufferRecyclable) {
    ...
    final protected boolean _loadMore() throws IOException {
    ...
    final private JsonToken _nextTokenNotInObject(int i) throws IOException {
        if (i == INT_QUOTE) {
            _tokenIncomplete = true;
    ...
    final private int _skipColonFast(int ptr) throws IOException {
    ...
    final private void _isNextTokenNameYes(int i) throws IOException {
        _currToken = JsonToken.FIELD_NAME;
        _updateLocation();
        switch(i) {
            case '"':
    ...
    final private boolean _isNextTokenNameMaybe(int i, SerializableString str) throws IOException {
        String n = _parseName(i);
        _parsingContext.setCurrentName(n);
    ...
    final private JsonToken _parseNumber2(char[] outBuf, int outPtr, boolean negative, int intPartLength) throws IOException {
        while (true) {
    ...
    final private int _verifyNoLeadingZeroes() throws IOException {
        if (_inputPtr >= _inputEnd && !_loadMore()) {
            return INT_0;
        }
    ...
    final private JsonToken _parseFloat(char[] outBuf, int outPtr, int c, boolean negative, int integerPartLength) throws IOException {
        int fractLen = 0;
        boolean eof = false;
        if (c == INT_PERIOD) {
            if (outPtr >= outBuf.length) {
    ...
    final private void _verifyRootSpace(int ch) throws IOException {
        ++_inputPtr;
        switch(ch) {
            case ' ':
            case '\t':
    ...
    final protected String _parseName(int i) throws IOException {
        if (i != INT_QUOTE) {
    ...
    final protected String parseMediumName(int q2) throws IOException {
        final byte[] input = _inputBuffer;
        final int[] codes = _icLatin1;
    ...
    final protected String parseMediumName2(int q3, final int q2) throws IOException {
        final byte[] input = _inputBuffer;
        final int[] codes = _icLatin1;
        int i = input[_inputPtr++] & 0xFF;
        if (codes[i] != 0) {
    ...
    final protected String parseLongName(int q, final int q2, int q3) throws IOException {
    ...
    final private String parseName(int q1, int ch, int lastQuadBytes) throws IOException {
        return parseEscapedName(_quadBuffer, 0, q1, ch, lastQuadBytes);
    }

    final private String parseName(int q1, int q2, int ch, int lastQuadBytes) throws IOException {
        _quadBuffer[0] = q1;
        return parseEscapedName(_quadBuffer, 1, q2, ch, lastQuadBytes);
    }

    final private String parseName(int q1, int q2, int q3, int ch, int lastQuadBytes) throws IOException {
        _quadBuffer[0] = q1;
        _quadBuffer[1] = q2;
        return parseEscapedName(_quadBuffer, 2, q3, ch, lastQuadBytes);
    }

    final protected String parseEscapedName(int[] quads, int qlen, int currQuad, int ch, int currQuadBytes) throws IOException {
        final int[] codes = _icLatin1;
        while (true) {
            if (codes[ch] != 0) {
                if (ch == INT_QUOTE) {
    ...
    final private String findName(int q1, int lastQuadBytes) throws JsonParseException {
    ...
    final private String findName(int q1, int q2, int lastQuadBytes) throws JsonParseException {
    ...
    final private String findName(int q1, int q2, int q3, int lastQuadBytes) throws JsonParseException {
    ...
    final private String findName(int[] quads, int qlen, int lastQuad, int lastQuadBytes) throws JsonParseException {
        if (qlen >= quads.length) {
            _quadBuffer = quads = growArrayBy(quads, quads.length);
        }
    ...
    final private String addName(int[] quads, int qlen, int lastQuadBytes) throws JsonParseException {
    ...
    final private void _finishString2(char[] outBuf, int outPtr) throws IOException {
        int c;
        final int[] codes = _icUTF8;
        final byte[] inputBuffer = _inputBuffer;
    ...
    final protected void _matchTrue() throws IOException {
        int ptr = _inputPtr;
        if ((ptr + 3) < _inputEnd) {
            byte[] buf = _inputBuffer;
    ...
    final protected void _matchFalse() throws IOException {
        int ptr = _inputPtr;
        if ((ptr + 4) < _inputEnd) {
            byte[] buf = _inputBuffer;
    ...
    final protected void _matchNull() throws IOException {
        int ptr = _inputPtr;
        if ((ptr + 3) < _inputEnd) {
            byte[] buf = _inputBuffer;
    ...
    final protected void _matchToken(String matchStr, int i) throws IOException {
    ...
    final private void _matchToken2(String matchStr, int i) throws IOException {
        final int len = matchStr.length();
        do {
    ...
    final private void _checkMatchEnd(String matchStr, int i, int ch) throws IOException {
        char c = (char) _decodeCharForError(ch);
    ...
    final private int _skipWS() throws IOException {
    ...
    final private int _skipWS2() throws IOException {
        while (_inputPtr < _inputEnd || _loadMore()) {
            int i = _inputBuffer[_inputPtr++] & 0xFF;
            if (i > INT_SPACE) {
                if (i == INT_SLASH) {
    ...
    final private int _skipWSOrEnd() throws IOException {
        if (_inputPtr >= _inputEnd) {
            if (!_loadMore()) {
    ...
    final private int _skipWSOrEnd2() throws IOException {
        while ((_inputPtr < _inputEnd) || _loadMore()) {
            int i = _inputBuffer[_inputPtr++] & 0xFF;
            if (i > INT_SPACE) {
                if (i == INT_SLASH) {
    ...
    final private int _skipColon() throws IOException {
    ...
    final private int _skipColon2(boolean gotColon) throws IOException {
        while (_inputPtr < _inputEnd || _loadMore()) {
            int i = _inputBuffer[_inputPtr++] & 0xFF;
            if (i > INT_SPACE) {
                if (i == INT_SLASH) {
    ...
    final private void _skipComment() throws IOException {
    ...
    final private void _skipCComment() throws IOException {
    ...
    final private boolean _skipYAMLComment() throws IOException {
    ...
    final private void _skipLine() throws IOException {
        final int[] codes = CharTypes.getInputCodeComment();
        while ((_inputPtr < _inputEnd) || _loadMore()) {
            int i = (int) _inputBuffer[_inputPtr++] & 0xFF;
            int code = codes[i];
    ...
    final private int _decodeUtf8_2(int c) throws IOException {
        if (_inputPtr >= _inputEnd) {
            _loadMoreGuaranteed();
        }
    ...
    final private int _decodeUtf8_3(int c1) throws IOException {
        if (_inputPtr >= _inputEnd) {
            _loadMoreGuaranteed();
        }
        c1 &= 0x0F;
    ...
    final private int _decodeUtf8_3fast(int c1) throws IOException {
        c1 &= 0x0F;
        int d = (int) _inputBuffer[_inputPtr++];
        if ((d & 0xC0) != 0x080) {
            _reportInvalidOther(d & 0xFF, _inputPtr);
    ...
    final private int _decodeUtf8_4(int c) throws IOException {
        if (_inputPtr >= _inputEnd) {
            _loadMoreGuaranteed();
        }
        int d = (int) _inputBuffer[_inputPtr++];
    ...
    final private void _skipUtf8_2() throws IOException {
        if (_inputPtr >= _inputEnd) {
            _loadMoreGuaranteed();
        }
        int c = (int) _inputBuffer[_inputPtr++];
    ...
    final private void _skipUtf8_3() throws IOException {
        if (_inputPtr >= _inputEnd) {
            _loadMoreGuaranteed();
        }
        int c = (int) _inputBuffer[_inputPtr++];
    ...
    final private void _skipUtf8_4(int c) throws IOException {
        if (_inputPtr >= _inputEnd) {
            _loadMoreGuaranteed();
        }
        int d = (int) _inputBuffer[_inputPtr++];
    ...
    final protected void _skipCR() throws IOException {
        if (_inputPtr < _inputEnd || _loadMore()) {
    ...
    @SuppressWarnings("resource")
    final protected byte[] _decodeBase64(Base64Variant b64variant) throws IOException {
        ByteArrayBuilder builder = _getByteArrayBuilder();
        while (true) {
            int ch;
            do {
    ...
    final private JsonToken _closeScope(int i) throws JsonParseException {
        if (i == INT_RCURLY) {
            _closeObjectScope();
            return (_currToken = JsonToken.END_OBJECT);
        }
    ...
    final private void _closeArrayScope() throws JsonParseException {
        _updateLocation();
        if (!_parsingContext.inArray()) {
    ...
    final private void _closeObjectScope() throws JsonParseException {
        _updateLocation();
        if (!_parsingContext.inObject()) {
            _reportMismatchedEndMarker('}', ']');
        }
    ...
    final protected int _matchLongName(FieldNameMatcher matcher, int qptr, int q) throws IOException {
        final byte[] input = _inputBuffer;
        final int[] codes = _icLatin1;
        int qlen = 3;
    ...
    public UTF8StreamJsonParser(ObjectReadContext readCtxt, IOContext ctxt, int stdFeatures, int formatReadFeatures, InputStream in, ByteQuadsCanonicalizer sym, byte[] inputBuffer, int start, int end, boolean bufferRecyclable) {
        this(readCtxt, ctxt, stdFeatures, formatReadFeatures, in, sym, inputBuffer, start, end, 0, bufferRecyclable);
    }
    ...
    final protected int _matchMediumName2(FieldNameMatcher matcher, int qptr, int q3, final int q2) throws IOException {
        final byte[] input = _inputBuffer;
        final int[] codes = _icLatin1;
        int i = input[qptr++] & 0xFF;
        if (codes[i] != 0) {
    ...
    final protected int _matchMediumName(FieldNameMatcher matcher, int qptr, int q2) throws IOException {
        final byte[] input = _inputBuffer;
        final int[] codes = _icLatin1;
        int i = input[qptr++] & 0xFF;
        if (codes[i] != 0) {
    ...
    final protected int _matchName(FieldNameMatcher matcher, int i) throws IOException {
        if (i != INT_QUOTE) {
            return -1;
        }
        int qptr = _inputPtr;
