...
public class MultiLayerNetwork implements Serializable, Classifier {
  ...
  protected void doBackWard(INDArray input, INDArray labels) {
    ...
    this.labels = labels;
    if (!(getOutputLayer() instanceof OutputLayer)) {
      log.warn("Warning: final layer isn't output layer. You can ignore this " +
               "message if you just intend on using a a deep neural network " +
               "with no output layer.");
      return;
    }
    ...
    for (int i = 0;
         i < getLayerWiseConfigurations()
                 .getConf(getLayerWiseConfigurations().getConfs().size() - 1)
                 .getNumIterations();
         i++) {
      int numLayers = getnLayers();
      List<Gradient> gradientUpdates = new ArrayList<>();
<<<<<<< /root/work/deeplearning4j/deeplearning4j/d3j/patched-068-0.java
      List<INDArray> activations = feedForward();
||||||| /root/work/deeplearning4j/deeplearning4j/d3j/base-068.java
      Pair<List<INDArray>, List<INDArray>> activationsAndDeriv =
          feedForwardActivationsAndDerivatives();
      List<INDArray> activations = activationsAndDeriv.getFirst();
=======
      Pair<List<INDArray>, List<INDArray>> activationsAndDeriv =
          feedForwardActivationsAndDerivatives(true);
      List<INDArray> activations = feedForward();
>>>>>>> /root/work/deeplearning4j/deeplearning4j/d3j/patched-068-1.java
      ...
      outputLayerGradients.gradientForVariable().put(
          DefaultParamInitializer.WEIGHT_KEY,
          delta.mmul(outputLayerInput).transpose());
      outputLayerGradients.gradientForVariable().put(
          DefaultParamInitializer.BIAS_KEY, delta.transpose().sum(0));
      gradientUpdates.add(outputLayerGradients);
      INDArray nextEpsilon = output.getParam(DefaultParamInitializer.WEIGHT_KEY)
                                 .mmul(delta)
                                 .transpose();
      for (int j = numLayers - 2; j >= 0; j--) {
<<<<<<< /root/work/deeplearning4j/deeplearning4j/d3j/patched-068-0.java
        Pair<Gradient, INDArray> pair =
            getLayers()[j].backwardGradient(nextEpsilon);
...
||||||| /root/work/deeplearning4j/deeplearning4j/d3j/base-068.java
        ...
        nextGradients = getLayers()[j].backwardGradient(
            currDerivative, nextLayer, nextGradients, currActivation);
        gradientUpdates.add(nextGradients);
=======
        Layer nextLayer = getLayers()[j + 1];
        gradientUpdates.add(nextGradients, pair.getFirst());
        nextEpsilon = pair.getSecond();
>>>>>>> /root/work/deeplearning4j/deeplearning4j/d3j/patched-068-1.java
      }
      ...
      for (int k = 0; k < numLayers; k++) {
        ...
        for (String paramType :
             gradientUpdates.get(k).gradientForVariable().keySet()) {
          currLayer.getOptimizer().updateGradientAccordingToParams(
              gradientUpdates.get(k).getGradientFor(paramType), currLayer,
              input.size(0), paramType, i);
