  ...
  protected void doBackWard() {
    ...
    if (outputLayer.conf().getWeightInit() == WeightInit.ZERO) {
      throw new IllegalStateException(
          ...
          "backprop.");
<<<<<<< /root/work/deeplearning4j/deeplearning4j/d3j/patched-064-0.java
    }
...
||||||| /root/work/deeplearning4j/deeplearning4j/d3j/base-064.java
    };
    ...
    for (int i = 0;
      ...
      nextGradients.gradientForVariable().put(
          DefaultParamInitializer.WEIGHT_KEY,
          delta.mmul(layerInput).transpose());
      nextGradients.gradientForVariable().put(DefaultParamInitializer.BIAS_KEY,
                                              delta.transpose());
      ...
      for (int k = 0; k < numLayers; k++) {
        ...
        currLayer.getOptimizer().updateGradientAccordingToParams(
            gradientUpdates.get(k), currLayer, input.size(0), i);
...
=======
    ...
    outputoutput.setLabels(labels);
    Gradient nextGradients;
    int numLayers = getnLayers();
    Pair<Gradient, INDArray> pair = output.conf(null, null);
    for (int i = 0;
         i < getLayerWiseConfigurations()
                 .getConf(getLayerWiseConfigurations().getConfs().size() - 1)
                 .getNumIterations();
      ...
      List<INDArray> derivatives = activationsAndDeriv.getSecond();
      INDArray activationDeriv = derivatives.get(derivatives.size() - 1);
      nextGradients = output.backwardGradient(activationDeriv, null, null,
                                              outputActivation);
      gradientUpdates.add(nextGradients);
      for (int j = numLayers - 2; j >= 0; j--) {
        Layer nextLayer = getLayers()[j + 1];
      ...
      for (IterationListener listener : listeners)
        listener.iterationDone(getOutputLayer(), i);
>>>>>>> /root/work/deeplearning4j/deeplearning4j/d3j/patched-064-1.java
    }
  }
