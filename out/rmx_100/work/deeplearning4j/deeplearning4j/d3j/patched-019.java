...
public class AbstractCache<T extends SequenceElement> implements VocabCache<T> {
  ...
  private volatile Map<String, T> extendedVocabulary =
...
<<<<<<< /root/work/deeplearning4j/deeplearning4j/d3j/patched-019-0.java
    T element = extendedVocabulary.get(word);
    if (extendedVocabulary.containsKey(word)) {
      element.increaseElementFrequency(increment);
||||||| /root/work/deeplearning4j/deeplearning4j/d3j/base-019.java
...
=======
    ...
    if (element != null) {
      element.increaseElementFrequency(increment);
>>>>>>> /root/work/deeplearning4j/deeplearning4j/d3j/patched-019-1.java
...
>>>>>>> /root/work/deeplearning4j/deeplearning4j/d3j/patched-019-1.java
  ...
  public void putVocabWord(String word) {
    if (!containsWord(word))
      throw new IllegalStateException(
          "Specified label is not present in vocabulary");
  }
  @Override
  public int numWords() {
  ...
  public void incrementDocCount(String word, long howMuch) {
    if (extendedVocabulary.containsKey(word)) {
      extendedVocabulary.get(word).setSequencesCount(
          vocabulary.get(word).getSequencesCount() + 1);
    }
  }
  @Override
  ...
  public void addToken(T element) {
    if (!vocabulary.containsKey(element.getStorageId())) {
      ...
      vocabulary.get(element.getStorageId())
          .incrementSequencesCount(element.getSequencesCount());
      vocabulary.get(element.getStorageId())
