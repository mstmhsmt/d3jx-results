...
//		INDArray timeSeriesMaskArray = Nd4j.ones(miniBatchSize,timeSeriesLength);	//For now: assume that all data in mini-batch is of length 'timeSeriesLength'
	...
	private INDArray[] activateHelper(INDArray input, boolean training) {
		...
		for (int t = 0; t < timeSeriesLength; t++) {
			...
			NDArrayIndex[] iIndexes = (miniBatchSize == 1) ? new NDArrayIndex[]{ interval(0, hiddenLayerSize) }// Indexing: special case for miniBatchSize=nRows=1. can't use "NDArrayIndex.all(),interval(0,hiddenLayerSize)"
			// Indexing: special case for miniBatchSize=nRows=1. can't use "NDArrayIndex.all(),interval(0,hiddenLayerSize)"
			// Indexing: special case for miniBatchSize=nRows=1. can't use "NDArrayIndex.all(),interval(0,hiddenLayerSize)"
