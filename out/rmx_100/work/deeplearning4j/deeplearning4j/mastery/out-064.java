...
public class MultiLayerNetwork implements Serializable, Classifier {
    ...
    public INDArray params() {
        ...
        for (int i = 0; i < getnLayers(); i++) params.add(layers[i].params());
    ...
    public int numParams() {
        int length = 0;
        for (int i = 0; i < layers.length; i++) length += layers[i].numParams();
        return length;
    }
...
if (!(getOutputLayer() instanceof OutputLayer)) {
            ...
            return;
        }List<INDArray> activations = feedForward();OutputLayer outputLayer = (OutputLayer) getOutputLayer();if (labels == null)
            throw new IllegalStateException("No labels found");if (outputLayer.conf().getWeightInit() == WeightInit.ZERO) {
...
if (!(getOutputLayer() instanceof OutputLayer)) {
            ...
            return;
        }OutputLayer output = (OutputLayer) getOutputLayer();if (labels == null)
            throw new IllegalStateException("No labels found");if (output.conf().getWeightInit() == WeightInit.ZERO) {
            throw new IllegalStateException("Output layer weights cannot be initialized to zero when using backprop.");
        };Gradient nextGradients;for (int i = 0; i < getLayerWiseConfigurations().getConf(getLayerWiseConfigurations().getConfs().size() - 1).getNumIterations(); i++) {
