...
public class ClasspathFingerprintingStrategy extends AbstractFingerprintingStrategy {
  ...
  @Override public String normalizePath(CompleteFileSystemLocationSnapshot snapshot) {
    return "";
  }

  @Override public Map<String, FileSystemLocationFingerprint> collectFingerprints(Iterable<? extends FileSystemSnapshot> roots) {
  ...
  public enum NonJarFingerprintingStrategy {
    IGNORE() {
      @Nullable @Override public HashCode determineNonJarFingerprint(HashCode original) {
        return null;
      }
    },
    USE_FILE_HASH() {
      @Override public HashCode determineNonJarFingerprint(HashCode original) {
        return original;
      }
    }
    ;

    @Nullable public abstract HashCode determineNonJarFingerprint(HashCode original);
  }
  ...
  private class ClasspathFingerprintingVisitor implements RelativePathTrackingFileSystemSnapshotHierarchyVisitor {
    ...
    @Override public SnapshotVisitResult visitEntry(CompleteFileSystemLocationSnapshot snapshot, RelativePathSupplier relativePath) {
      snapshot.accept(new FileSystemLocationSnapshotVisitor() {
        @Override public void visitRegularFile(RegularFileSnapshot fileSnapshot) {
        ...
        @Override public void visitMissing(MissingFileSnapshot missingSnapshot) {
          if (!relativePath.isRoot()) {
    ...
    /**
         ...
         */
    @Nullable private HashCode hashContent(RegularFileSnapshot fileSnapshot, RelativePathSupplier relativePath) {
  ...
  @Override public FingerprintHashingStrategy getHashingStrategy() {
