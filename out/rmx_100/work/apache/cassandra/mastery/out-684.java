...
public class ColumnFamilyStore implements ColumnFamilyStoreMBean {
    ...
    final public Keyspace keyspace;

    final public String name;

    final public CFMetaData metadata;

    final public IPartitioner partitioner;
    ...
    final public OpOrder readOrdering = new OpOrder();

    private final AtomicInteger fileIndexGenerator = new AtomicInteger(0);

    final public SecondaryIndexManager indexManager;
    ...
    final public ColumnFamilyMetrics metric;

    public volatile long sampleLatencyNanos;

    private final ScheduledFuture<?> latencyCalculator;

    static public void shutdownPostFlushExecutor() throws InterruptedException {
    ...
    public void reload() {
        if (!minCompactionThreshold.isModified())
            for (ColumnFamilyStore cfs : concatWithIndexes()) cfs.minCompactionThreshold = new DefaultInteger(metadata.getMinCompactionThreshold());
        if (!maxCompactionThreshold.isModified())
            for (ColumnFamilyStore cfs : concatWithIndexes()) cfs.maxCompactionThreshold = new DefaultInteger(metadata.getMaxCompactionThreshold());
    ...
    public void setCrcCheckChance(double crcCheckChance) {
        try {
            for (SSTableReader sstable : keyspace.getAllSSTables()) if (sstable.compression)
    ...
    static public ColumnFamilyStore createColumnFamilyStore(Keyspace keyspace, String columnFamily, boolean loadSSTables) {
    ...
    static public ColumnFamilyStore createColumnFamilyStore(Keyspace keyspace, String columnFamily, IPartitioner partitioner, CFMetaData metadata) {
    ...
    static public void scrubDataDirectories(CFMetaData metadata) {
        Directories directories = new Directories(metadata);
    ...
    static public void removeUnfinishedCompactionLeftovers(CFMetaData metadata, Map<Integer, UUID> unfinishedCompactions) {
        Directories directories = new Directories(metadata);
    ...
    static public synchronized void loadNewSSTables(String ksName, String cfName) {
    ...
    static public void rebuildSecondaryIndex(String ksName, String cfName, String... idxNames) {
    ...
    static public class FlushLargestColumnFamily implements Runnable {
    ...
    static public ColumnFamily removeDeletedCF(ColumnFamily cf, int gcBefore) {
        cf.purgeTombstones(gcBefore);
        return !cf.hasColumns() && !cf.isMarkedForDelete() ? null : cf;
    }

    static public ColumnFamily removeDeleted(ColumnFamily cf, int gcBefore) {
        return removeDeleted(cf, gcBefore, SecondaryIndexManager.nullUpdater);
    }

    static public ColumnFamily removeDeleted(ColumnFamily cf, int gcBefore, SecondaryIndexManager.Updater indexer) {
        if (cf == null) {
            return null;
        }
    ...
    static public ColumnFamily removeDeletedColumnsOnly(ColumnFamily cf, int gcBefore, SecondaryIndexManager.Updater indexer) {
    ...
    private void removeDroppedColumns(ColumnFamily cf) {
        if (cf == null || cf.metadata.getDroppedColumns().isEmpty())
            return;
        Iterator<Cell> iter = cf.iterator();
        while (iter.hasNext()) if (isDroppedColumn(iter.next(), metadata))
            iter.remove();
    }
    ...
    public Function<DataTracker.View, List<SSTableReader>> viewFilter(final Collection<AbstractBounds<RowPosition>> rowBoundsCollection) {
        return new Function<DataTracker.View, List<SSTableReader>>() {
            ...
            public List<SSTableReader> apply(DataTracker.View view) {
                ...
                for (AbstractBounds<RowPosition> rowBounds : rowBoundsCollection) sstables.addAll(view.sstablesInBounds(rowBounds));
    ...
    static public abstract class AbstractScanIterator extends AbstractIterator<Row> implements CloseableIterator<Row> {
    ...
    @VisibleForTesting
    public List<Row> getRangeSlice(final AbstractBounds<RowPosition> range, List<IndexExpression> rowFilter, IDiskAtomFilter columnFilter, int maxResults) {
    ...
    static public Iterable<ColumnFamilyStore> all() {
    ...
    static public class ViewFragment {

        final public List<SSTableReader> sstables;

        final public Iterable<Memtable> memtables;
