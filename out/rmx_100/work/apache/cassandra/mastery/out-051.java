...
<<<<<<< commits-rmx_100/apache/cassandra/5f63b0ade1fe2d9fbd0cb475da936e27ec3ed64e/AbstractReadRepairTest-bb10c67.java
static ReplicaPlan.ForRead<?, ?> replicaPlan;
=======

>>>>>>> commits-rmx_100/apache/cassandra/c2041ba45b036e1d5963f8166f517c6d95d4eab1/AbstractReadRepairTest-83865b1.java
    ...
    static Message<ReadResponse> msg(InetAddressAndPort from, Cell<?>... cells) {
        UnfilteredPartitionIterator iter = new SingletonUnfilteredPartitionIterator(update(cells).unfilteredIterator());
        return Message.builder(INTERNAL_RSP, ReadResponse.createDataResponse(iter, command, command.executionController().getRepairedDataInfo())).from(from).build();
    }
...
<<<<<<< commits-rmx_100/apache/cassandra/5f63b0ade1fe2d9fbd0cb475da936e27ec3ed64e/AbstractReadRepairTest-bb10c67.java
String ksName = "ks";String ddl = String.format("CREATE TABLE tbl (k int primary key, v text) WITH read_repair='%s'", repairStrategy.toString().toLowerCase());cfm = CreateTableStatement.parse(ddl, ksName).build();assert cfm.params.readRepair == repairStrategy;KeyspaceMetadata ksm = KeyspaceMetadata.create(ksName, KeyspaceParams.simple(3), Tables.of(cfm));SchemaTestUtil.announceNewKeyspace(ksm);ks = Keyspace.open(ksName);cfs = ks.getColumnFamilyStore("tbl");cfs.sampleReadLatencyMicros = 0;cfs.additionalWriteLatencyMicros = 0;target1 = InetAddressAndPort.getByName("127.0.0.255");target2 = InetAddressAndPort.getByName("127.0.0.254");target3 = InetAddressAndPort.getByName("127.0.0.253");targets = ImmutableList.of(target1, target2, target3);replica1 = fullReplica(target1, FULL_RANGE);replica2 = fullReplica(target2, FULL_RANGE);replica3 = fullReplica(target3, FULL_RANGE);replicas = EndpointsForRange.of(replica1, replica2, replica3);StorageService.instance.getTokenMetadata().clearUnsafe();StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 0 })), replica1.endpoint());StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 1 })), replica2.endpoint());StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 2 })), replica3.endpoint());
=======
target1 = InetAddressAndPort.getByName("127.1.0.255");target2 = InetAddressAndPort.getByName("127.1.0.254");target3 = InetAddressAndPort.getByName("127.1.0.253");remote1 = InetAddressAndPort.getByName("127.2.0.255");remote2 = InetAddressAndPort.getByName("127.2.0.254");remote3 = InetAddressAndPort.getByName("127.2.0.253");targets = ImmutableList.of(target1, target2, target3);remotes = ImmutableList.of(remote1, remote2, remote3);replica1 = fullReplica(target1, FULL_RANGE);replica2 = fullReplica(target2, FULL_RANGE);replica3 = fullReplica(target3, FULL_RANGE);replicas = EndpointsForRange.of(replica1, replica2, replica3);remoteReplica1 = fullReplica(remote1, FULL_RANGE);remoteReplica2 = fullReplica(remote2, FULL_RANGE);remoteReplica3 = fullReplica(remote3, FULL_RANGE);remoteReplicas = EndpointsForRange.of(remoteReplica1, remoteReplica2, remoteReplica3);StorageService.instance.getTokenMetadata().clearUnsafe();StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 0 })), replica1.endpoint());StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 1 })), replica2.endpoint());StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 2 })), replica3.endpoint());StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 3 })), remoteReplica1.endpoint());StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 4 })), remoteReplica2.endpoint());StorageService.instance.getTokenMetadata().updateNormalToken(ByteOrderedPartitioner.instance.getToken(ByteBuffer.wrap(new byte[] { 5 })), remoteReplica3.endpoint());for (Replica replica : replicas) {
            ...
            Gossiper.instance.initializeNodeUnsafe(replica.endpoint(), hostId, 1);
            StorageService.instance.getTokenMetadata().updateHostId(hostId, replica.endpoint());
        }String ksName = "ks";String ddl = String.format("CREATE TABLE tbl (k int primary key, v text) WITH read_repair='%s'", repairStrategy.toString().toLowerCase());cfm = CreateTableStatement.parse(ddl, ksName).build();assert cfm.params.readRepair == repairStrategy;KeyspaceMetadata ksm = KeyspaceMetadata.create(ksName, KeyspaceParams.nts("datacenter1", 3, "datacenter2", 3), Tables.of(cfm));MigrationManager.announceNewKeyspace(ksm, true);ks = Keyspace.open(ksName);cfs = ks.getColumnFamilyStore("tbl");
>>>>>>> commits-rmx_100/apache/cassandra/c2041ba45b036e1d5963f8166f517c6d95d4eab1/AbstractReadRepairTest-83865b1.java
...
>>>>>>> commits-rmx_100/apache/cassandra/c2041ba45b036e1d5963f8166f517c6d95d4eab1/AbstractReadRepairTest-83865b1.java
    ...
    static ReplicaPlan.ForTokenWrite repairPlan(ReplicaPlan.ForRangeRead readPlan, EndpointsForRange liveAndDown) {
        Token token = readPlan.range().left.getToken();
        EndpointsForToken pending = EndpointsForToken.empty(token);
        return ReplicaPlans.forWrite(readPlan.keyspace(), readPlan.consistencyLevel(), liveAndDown.forToken(token), pending, replica -> true, ReplicaPlans.writeReadRepair(readPlan));
    }
