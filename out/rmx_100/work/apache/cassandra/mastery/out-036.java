...
public class BatchStatement implements CQLStatement {
    ...
    final private VariableSpecifications bindVariables;

    final private List<ModificationStatement> statements;

    final private Map<TableId, RegularAndStaticColumns> updatedColumns;

    final private RegularAndStaticColumns conditionColumns;

    final private boolean updatesRegularRows;

    final private boolean updatesStaticRow;

    final private Attributes attrs;

    final private boolean hasConditions;

    final private boolean updatesVirtualTables;

    final private static Logger logger = LoggerFactory.getLogger(BatchStatement.class);

    final private static String UNLOGGED_BATCH_WARNING = "Unlogged batch covering {} partitions detected " + "against table{} {}. You should use a logged batch for " + "atomicity, or asynchronous writes for performance.";

    final private static String LOGGED_BATCH_LOW_GCGS_WARNING = "Executing a LOGGED BATCH on table{} {}, configured with a " + "gc_grace_seconds of 0. The gc_grace_seconds is used to TTL " + "batchlog entries, so setting gc_grace_seconds too low on " + "tables involved in an atomic batch might cause batchlog " + "entries to expire before being replayed.";

    public final static BatchMetrics metrics = new BatchMetrics();
    ...
    private static void verifyBatchSize(Collection<? extends IMutation> mutations) throws InvalidRequestException {
        ...
        if (size > warnThreshold) {
            ...
            if (size > failThreshold) {
                Tracing.trace(format, tableNames, FBUtilities.prettyPrintMemory(size), FBUtilities.prettyPrintMemory(failThreshold), FBUtilities.prettyPrintMemory(size - failThreshold), " (see batch_size_fail_threshold)");
                logger.error(format, tableNames, FBUtilities.prettyPrintMemory(size), FBUtilities.prettyPrintMemory(failThreshold), FBUtilities.prettyPrintMemory(size - failThreshold), " (see batch_size_fail_threshold)");
    ...
    public ResultMessage execute(QueryState queryState, BatchQueryOptions options, Dispatcher.RequestTime requestTime) {
        ...
        Guardrails.writeConsistencyLevels.guard(EnumSet.of(options.getConsistency(), options.getSerialConsistency()), clientState);
        for (int i = 0; i < statements.size(); i++) statements.get(i).validateDiskUsage(options.forStatement(i), clientState);
        if (hasConditions)
        ...
        if (updatesVirtualTables)
            ...
            executeWithoutConditions(getMutations(clientState, options, false, timestamp, nowInSeconds, requestTime), options.getConsistency(), requestTime);
    ...
    private ResultMessage executeWithConditions(BatchQueryOptions options, QueryState state, Dispatcher.RequestTime requestTime) {
        ...
        try (RowIterator result = StorageProxy.cas(ksName, tableName, casRequest.key, casRequest, options.getSerialConsistency(), options.getConsistency(), state.getClientState(), options.getNowInSeconds(state), requestTime)) {
    ...
    private ResultMessage executeInternalWithoutCondition(QueryState queryState, BatchQueryOptions batchOptions, Dispatcher.RequestTime requestTime) {
        ...
        for (IMutation mutation : getMutations(queryState.getClientState(), batchOptions, true, timestamp, nowInSeconds, requestTime)) mutation.apply();
        return null;
    }
    ...
    public static class Parsed extends QualifiedStatement {
        ...
        final private Attributes.Raw attrs;

        final private List<ModificationStatement.Parsed> parsedStatements;
    ...
    private static class MultiTableColumnsBuilder {
        ...
        final private Map<TableId, RegularAndStaticColumns.Builder> perTableBuilders = new HashMap<>();
