...
public class SinglePartitionReadCommand extends ReadCommand {
    ...
    final private DecoratedKey partitionKey;

    final private ClusteringIndexFilter clusteringIndexFilter;
    ...
    static public SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, RowFilter rowFilter, DataLimits limits, DecoratedKey partitionKey, ClusteringIndexFilter clusteringIndexFilter) {
        return create(false, metadata, nowInSec, columnFilter, rowFilter, limits, partitionKey, clusteringIndexFilter);
    }

    static public SinglePartitionReadCommand create(boolean isForThrift, CFMetaData metadata, int nowInSec, ColumnFilter columnFilter, RowFilter rowFilter, DataLimits limits, DecoratedKey partitionKey, ClusteringIndexFilter clusteringIndexFilter) {
    ...
    static public SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, DecoratedKey key, ColumnFilter columnFilter, ClusteringIndexFilter filter) {
        return create(metadata, nowInSec, columnFilter, RowFilter.NONE, DataLimits.NONE, key, filter);
    }

    static public SinglePartitionReadCommand fullPartitionRead(CFMetaData metadata, int nowInSec, DecoratedKey key) {
        return SinglePartitionReadCommand.create(metadata, nowInSec, key, Slices.ALL);
    }

    static public SinglePartitionReadCommand fullPartitionRead(CFMetaData metadata, int nowInSec, ByteBuffer key) {
        return SinglePartitionReadCommand.create(metadata, nowInSec, metadata.decorateKey(key), Slices.ALL);
    }

    static public SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, DecoratedKey key, Slice slice) {
        return create(metadata, nowInSec, key, Slices.with(metadata.comparator, slice));
    }

    static public SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, DecoratedKey key, Slices slices) {
    ...
    static public SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, ByteBuffer key, Slices slices) {
        return create(metadata, nowInSec, metadata.decorateKey(key), slices);
    }

    static public SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, DecoratedKey key, NavigableSet<Clustering> names) {
        ClusteringIndexNamesFilter filter = new ClusteringIndexNamesFilter(names, false);
        return SinglePartitionReadCommand.create(metadata, nowInSec, ColumnFilter.all(metadata), RowFilter.NONE, DataLimits.NONE, key, filter);
    }

    static public SinglePartitionReadCommand create(CFMetaData metadata, int nowInSec, DecoratedKey key, Clustering name) {
    ...
    static private SinglePartitionPager getPager(SinglePartitionReadCommand command, PagingState pagingState, int protocolVersion) {
...
<<<<<<< commits-rmx_100/apache/cassandra/5692c59d14dbc81e0a2ee6d9d0fbd7505d503ab5/SinglePartitionReadCommand-67e0d45.java
...
>>>>>>> commits-rmx_100/apache/cassandra/21d8a7d3bd5b9ec49f486c3c7a816939c4040686/SinglePartitionReadCommand-23b02f3.java
    ...
    static public class Group implements ReadQuery {

        final public List<SinglePartitionReadCommand> commands;

        final private DataLimits limits;

        final private int nowInSec;
        ...
        public Group(List<SinglePartitionReadCommand> commands, DataLimits limits) {
            ...
            for (int i = 1; i < commands.size(); i++) assert commands.get(i).nowInSec() == nowInSec;
        }

        static public Group one(SinglePartitionReadCommand command) {
        ...
        private UnfilteredPartitionIterator executeLocally(ReadExecutionController executionController, boolean sort) {
            ...
            for (SinglePartitionReadCommand cmd : commands) partitions.add(Pair.of(cmd.partitionKey, cmd.executeLocally(executionController)));
            if (sort)
    ...
    static private class Deserializer extends SelectionDeserializer {
