...
final public class CFMetaData {

    private static final Logger logger = LoggerFactory.getLogger(CFMetaData.class);

    static final public double DEFAULT_READ_REPAIR_CHANCE = 0.1;

    static final public double DEFAULT_DCLOCAL_READ_REPAIR_CHANCE = 0.0;

    static final public boolean DEFAULT_REPLICATE_ON_WRITE = true;

    static final public int DEFAULT_GC_GRACE_SECONDS = 864000;

    static final public int DEFAULT_MIN_COMPACTION_THRESHOLD = 4;

    static final public int DEFAULT_MAX_COMPACTION_THRESHOLD = 32;

    static final public Class<? extends AbstractCompactionStrategy> DEFAULT_COMPACTION_STRATEGY_CLASS = SizeTieredCompactionStrategy.class;

    static final public Caching DEFAULT_CACHING_STRATEGY = Caching.KEYS_ONLY;

    static final public boolean DEFAULT_POPULATE_IO_CACHE_ON_FLUSH = false;

    
<<<<<<< commits-rmx_100/apache/cassandra/9a28df0d0f87244bde2eccdc3fb359b6f3bfa183/CFMetaData-2da2361.java

=======
static final public CFMetaData SchemaColumnFamiliesCf = compile(9, "CREATE TABLE " + SystemTable.SCHEMA_COLUMNFAMILIES_CF + "(" + "keyspace_name text," + "columnfamily_name text," + "id int," + "type text," + "comparator text," + "subcomparator text," + "comment text," + "read_repair_chance double," + "local_read_repair_chance double," + "replicate_on_write boolean," + "gc_grace_seconds int," + "default_validator text," + "key_validator text," + "min_compaction_threshold int," + "max_compaction_threshold int," + "key_alias text," + "key_aliases text," + "bloom_filter_fp_chance double," + "caching text," + "populate_io_cache_on_flush boolean," + "compaction_strategy_class text," + "compression_parameters text," + "value_alias text," + "column_aliases text," + "compaction_strategy_options text," + "PRIMARY KEY (keyspace_name, columnfamily_name)" + ") WITH COMMENT='ColumnFamily definitions' AND gc_grace_seconds=8640");
>>>>>>> commits-rmx_100/apache/cassandra/cb0a0cd2f8295a526068b7f6c52860e707e7665a/CFMetaData-33e3f00.java


    static final public CFMetaData TraceSessionsCf = compile("CREATE TABLE " + Tracing.SESSIONS_CF + " (" + "session_id uuid PRIMARY KEY," + "coordinator inet," + "request text," + "started_at timestamp," + "parameters map<text, text>," + "duration int" + ") WITH COMMENT='traced sessions'", Tracing.TRACE_KS);

    static final public CFMetaData TraceEventsCf = compile("CREATE TABLE " + Tracing.EVENTS_CF + " (" + "session_id uuid," + "event_id timeuuid," + "source inet," + "thread text," + "activity text," + "source_elapsed int," + "PRIMARY KEY (session_id, event_id)" + ")", Tracing.TRACE_KS);

    public enum Caching {

        ALL, KEYS_ONLY, ROWS_ONLY, NONE;

        static public Caching fromString(String cache) throws ConfigurationException {
            try {
    ...
    final public UUID cfId;

    final public String ksName;

    final public String cfName;

    final public ColumnFamilyType cfType;
    ...
    private static CFMetaData newSystemMetadata(String keyspace, String cfName, String comment, AbstractType<?> comparator, AbstractType<?> subcc) {
        ...
        return newCFMD.comment(comment).readRepairChance(0).dcLocalReadRepairChance(0).gcGraceSeconds(0);
    }

    static public CFMetaData newIndexMetadata(CFMetaData parent, ColumnDefinition info, AbstractType<?> columnComparator) {
    ...
    static public CFMetaData rename(CFMetaData cfm, String newName) {
    ...
    public double getBloomFilterFpChance() {
        return (bloomFilterFpChance == null || bloomFilterFpChance == 0) ? compactionStrategyClass == LeveledCompactionStrategy.class ? 0.1 : 0.01 : bloomFilterFpChance;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return new EqualsBuilder().append(ksName, rhs.ksName).append(cfName, rhs.cfName).append(cfType, rhs.cfType).append(comparator, rhs.comparator).append(comment, rhs.comment).append(readRepairChance, rhs.readRepairChance).append(dcLocalReadRepairChance, rhs.dcLocalReadRepairChance).append(replicateOnWrite, rhs.replicateOnWrite).append(gcGraceSeconds, rhs.gcGraceSeconds).append(defaultValidator, rhs.defaultValidator).append(keyValidator, rhs.keyValidator).append(minCompactionThreshold, rhs.minCompactionThreshold).append(maxCompactionThreshold, rhs.maxCompactionThreshold).append(cfId, rhs.cfId).append(column_metadata, rhs.column_metadata).append(compactionStrategyClass, rhs.compactionStrategyClass).append(compactionStrategyOptions, rhs.compactionStrategyOptions).append(compressionParameters, rhs.compressionParameters).append(bloomFilterFpChance, rhs.bloomFilterFpChance).append(memtableFlushPeriod, rhs.memtableFlushPeriod).append(caching, rhs.caching).append(defaultTimeToLive, rhs.defaultTimeToLive).append(indexInterval, rhs.indexInterval).append(speculativeRetry, rhs.speculativeRetry).append(populateIoCacheOnFlush, rhs.populateIoCacheOnFlush).append(droppedColumns, rhs.droppedColumns).append(triggers, rhs.triggers).isEquals();
    }

    public int hashCode() {
        return new HashCodeBuilder(29, 1597).append(ksName).append(cfName).append(cfType).append(comparator).append(comment).append(readRepairChance).append(dcLocalReadRepairChance).append(replicateOnWrite).append(gcGraceSeconds).append(defaultValidator).append(keyValidator).append(minCompactionThreshold).append(maxCompactionThreshold).append(cfId).append(column_metadata).append(compactionStrategyClass).append(compactionStrategyOptions).append(compressionParameters).append(bloomFilterFpChance).append(memtableFlushPeriod).append(caching).append(defaultTimeToLive).append(indexInterval).append(speculativeRetry).append(populateIoCacheOnFlush).append(droppedColumns).append(triggers).toHashCode();
    }
    ...
    public AbstractType<?> getValueValidator(ColumnDefinition columnDefinition) {
        return columnDefinition == null ? defaultValidator : columnDefinition.getValidator();
    }

    static public void applyImplicitDefaults(org.apache.cassandra.thrift.CfDef cf_def) {
    ...
    static public CFMetaData fromThrift(org.apache.cassandra.thrift.CfDef cf_def) throws InvalidRequestException, ConfigurationException {
    ...
    static public void validateCompactionOptions(Class<? extends AbstractCompactionStrategy> strategyClass, Map<String, String> options) throws ConfigurationException {
        try {
            if (options == null)
                return;
    ...
    static public Class<? extends AbstractCompactionStrategy> createCompactionStrategy(String className) throws ConfigurationException {
    ...
    public AbstractCompactionStrategy createCompactionStrategyInstance(ColumnFamilyStore cfs) {
        try {
            Constructor<? extends AbstractCompactionStrategy> constructor = compactionStrategyClass.getConstructor(new Class[] { ColumnFamilyStore.class, Map.class });
    ...
    static public String getDefaultIndexName(String cfName, AbstractType<?> comparator, ByteBuffer columnName) {
    ...
    static public boolean isIndexNameValid(String name) {
    ...
    public CFMetaData validate() throws ConfigurationException {
        ...
        if (defaultValidator instanceof CounterColumnType) {
            for (ColumnDefinition def : regularColumns) if (!(def.getValidator() instanceof CounterColumnType))
            ...
            for (ColumnDefinition def : column_metadata.values()) if (def.getValidator() instanceof CounterColumnType)
    ...
    private static Set<String> existingIndexNames(String cfToExclude) {
        ...
        for (ColumnFamilyStore cfs : ColumnFamilyStore.all()) {
            if (cfToExclude == null || !cfs.name.equals(cfToExclude))
                for (ColumnDefinition cd : cfs.metadata.allColumns()) indexNames.add(cd.getIndexName());
        }
        return indexNames;
    }
    ...
    private void validateCompactionThresholds() throws ConfigurationException {
        ...
        if (minCompactionThreshold > maxCompactionThreshold)
            throw new ConfigurationException(String.format("Min compaction threshold (got %d) cannot be greater than max compaction threshold (got %d)", minCompactionThreshold, maxCompactionThreshold));
    }
    ...
    public RowMutation dropFromSchema(long timestamp) {
        ...
        for (ColumnDefinition cd : column_metadata.values()) cd.deleteFromSchema(rm, cfName, timestamp);
    ...
    public void toSchema(RowMutation rm, long timestamp) {
        ...
        for (ColumnDefinition cd : column_metadata.values()) cd.toSchema(rm, cfName, getColumnDefinitionComparator(cd), timestamp);
    }
    ...
    static public CFMetaData fromSchema(UntypedResultSet.Row result) {
    ...
    private String aliasesToJson(List<ColumnDefinition> rawAliases) {
        ...
        for (ColumnDefinition rawAlias : rawAliases) aliases.add(UTF8Type.instance.compose(rawAlias.name));
        return json(aliases);
    }
    ...
    private static List<ByteBuffer> aliasesFromStrings(List<String> aliases) {
        ...
        for (String alias : aliases) rawAliases.add(UTF8Type.instance.decompose(alias));
        return rawAliases;
    }
    ...
    static CFMetaData addColumnDefinitionsFromSchema(CFMetaData cfDef, Row serializedColumnDefinitions) {
        for (ColumnDefinition cd : ColumnDefinition.fromSchema(serializedColumnDefinitions, cfDef)) cfDef.column_metadata.put(cd.name, cd);
    ...
    public String toString() {
        return new ToStringBuilder(this).append("cfId", cfId).append("ksName", ksName).append("cfName", cfName).append("cfType", cfType).append("comparator", comparator).append("comment", comment).append("readRepairChance", readRepairChance).append("dclocalReadRepairChance", dcLocalReadRepairChance).append("replicateOnWrite", replicateOnWrite).append("gcGraceSeconds", gcGraceSeconds).append("defaultValidator", defaultValidator).append("keyValidator", keyValidator).append("minCompactionThreshold", minCompactionThreshold).append("maxCompactionThreshold", maxCompactionThreshold).append("column_metadata", column_metadata).append("compactionStrategyClass", compactionStrategyClass).append("compactionStrategyOptions", compactionStrategyOptions).append("compressionOptions", compressionParameters.asThriftOptions()).append("bloomFilterFpChance", bloomFilterFpChance).append("memtable_flush_period_in_ms", memtableFlushPeriod).append("caching", caching).append("defaultTimeToLive", defaultTimeToLive).append("speculative_retry", speculativeRetry).append("indexInterval", indexInterval).append("populateIoCacheOnFlush", populateIoCacheOnFlush).append("droppedColumns", droppedColumns).append("triggers", triggers).toString();
    }

    static final public CFMetaData SchemaColumnsCf = compile("CREATE TABLE " + SystemKeyspace.SCHEMA_COLUMNS_CF + " (" + "keyspace_name text," + "columnfamily_name text," + "column_name text," + "validator text," + "index_type text," + "index_options text," + "index_name text," + "component_index int," + "type text," + "PRIMARY KEY(keyspace_name, columnfamily_name, column_name)" + ") WITH COMMENT='ColumnFamily column attributes' AND gc_grace_seconds=8640");
    ...
    static final public CFMetaData SchemaColumnFamiliesCf = compile("CREATE TABLE " + SystemKeyspace.SCHEMA_COLUMNFAMILIES_CF + " (" + "keyspace_name text," + "columnfamily_name text," + "type text," + "comparator text," + "subcomparator text," + "comment text," + "read_repair_chance double," + "local_read_repair_chance double," + "replicate_on_write boolean," + "gc_grace_seconds int," + "default_validator text," + "key_validator text," + "min_compaction_threshold int," + "max_compaction_threshold int," + "memtable_flush_period_in_ms int," + "key_alias text," + "key_aliases text," + "bloom_filter_fp_chance double," + "caching text," + "default_time_to_live int," + "compaction_strategy_class text," + "compression_parameters text," + "value_alias text," + "column_aliases text," + "compaction_strategy_options text," + "default_read_consistency text," + "default_write_consistency text," + "speculative_retry text," + "populate_io_cache_on_flush boolean," + "index_interval int," + "dropped_columns map<text, bigint>," + "PRIMARY KEY (keyspace_name, columnfamily_name)" + ") WITH COMMENT='ColumnFamily definitions' AND gc_grace_seconds=8640");

    static final public CFMetaData PaxosCf = compile("CREATE TABLE " + SystemKeyspace.PAXOS_CF + " (" + "row_key blob," + "cf_id UUID," + "in_progress_ballot timeuuid," + "proposal blob," + "most_recent_commit_at timeuuid," + "most_recent_commit blob," + "PRIMARY KEY (row_key, cf_id)" + ") WITH COMMENT='in-progress paxos proposals'");

    static final public CFMetaData CounterIdCf = compile("CREATE TABLE \"" + SystemKeyspace.COUNTER_ID_CF + "\" (" + "key text," + "id timeuuid," + "PRIMARY KEY (key, id)" + ") WITH COMPACT STORAGE AND COMMENT='counter node IDs'");

    static final public CFMetaData IndexCf = compile("CREATE TABLE \"" + SystemKeyspace.INDEX_CF + "\" (" + "table_name text," + "index_name text," + "PRIMARY KEY (table_name, index_name)" + ") WITH COMPACT STORAGE AND COMMENT='indexes that have been completed'");
    ...
    private void rebuildCQL3Metadata() {
        ...
        int nbCkCols = isDense ? comparator.componentsCount() : comparator.componentsCount() - (hasCollection() ? 2 : 1);
    ...
    static final public CFMetaData SchemaTriggersCf = compile("CREATE TABLE " + SystemKeyspace.SCHEMA_TRIGGERS_CF + " (" + "keyspace_name text," + "columnfamily_name text," + "trigger_name text," + "trigger_options map<text, text>," + "PRIMARY KEY (keyspace_name, columnfamily_name, trigger_name)" + ") WITH COMMENT='triggers metadata table'");
    ...
    static final public CFMetaData HintsCf = compile("CREATE TABLE " + SystemKeyspace.HINTS_CF + " (" + "target_id uuid," + "hint_id timeuuid," + "message_version int," + "mutation blob," + "PRIMARY KEY (target_id, hint_id, message_version)" + ") WITH COMPACT STORAGE " + "AND COMPACTION={'class' : 'SizeTieredCompactionStrategy', 'enabled' : false} " + "AND COMMENT='hints awaiting delivery'" + "AND gc_grace_seconds=0");

    public Collection<ColumnDefinition> allColumns() {
        return column_metadata.values();
    }

    static final public CFMetaData RangeXfersCf = compile("CREATE TABLE " + SystemKeyspace.RANGE_XFERS_CF + " (" + "token_bytes blob PRIMARY KEY," + "requested_at timestamp" + ") WITH COMMENT='ranges requested for transfer here'");

    public boolean isSuper() {
        return cfType == ColumnFamilyType.Super;
    }

    static final public CFMetaData PeerEventsCf = compile("CREATE TABLE " + SystemKeyspace.PEER_EVENTS_CF + " (" + "peer inet PRIMARY KEY," + "hints_dropped map<uuid, int>" + ") WITH COMMENT='cf contains events related to peers'");
    ...
    public void validateColumns(Iterable<Column> columns) {
        for (Column column : columns) column.validateFields(this);
    }
    ...
    static final public CFMetaData LocalCf = compile("CREATE TABLE " + SystemKeyspace.LOCAL_CF + " (" + "key text PRIMARY KEY," + "tokens set<varchar>," + "cluster_name text," + "gossip_generation int," + "bootstrapped text," + "host_id uuid," + "release_version text," + "thrift_version text," + "cql_version text," + "native_protocol_version text," + "data_center text," + "rack text," + "partitioner text," + "schema_version uuid," + "truncated_at map<uuid, blob>" + ") WITH COMMENT='information about the local node'");
    ...
    static final public String DEFAULT_VALUE_ALIAS = "value";

    static final public CFMetaData PeersCf = compile("CREATE TABLE " + SystemKeyspace.PEERS_CF + " (" + "peer inet PRIMARY KEY," + "host_id uuid," + "tokens set<varchar>," + "schema_version uuid," + "release_version text," + "rpc_address inet," + "preferred_ip inet," + "data_center text," + "rack text" + ") WITH COMMENT='known peers in the cluster'");

    static final public String DEFAULT_COLUMN_ALIAS = "column";
    ...
    private static Map<ByteBuffer, Long> convertDroppedColumns(Map<String, Long> raw) {
        ...
        for (Map.Entry<String, Long> entry : raw.entrySet()) converted.put(UTF8Type.instance.decompose(entry.getKey()), entry.getValue());
        return converted;
    }

    static final public SpeculativeRetry DEFAULT_SPECULATIVE_RETRY = new SpeculativeRetry(SpeculativeRetry.RetryType.NONE, 0);

    static final public String DEFAULT_KEY_ALIAS = "key";

    static final public int DEFAULT_INDEX_INTERVAL = 128;

    static final public CFMetaData CompactionLogCf = compile("CREATE TABLE " + SystemKeyspace.COMPACTION_LOG + " (" + "id uuid PRIMARY KEY," + "keyspace_name text," + "columnfamily_name text," + "inputs set<int>" + ") WITH COMMENT='unfinished compactions'");

    static final public int DEFAULT_DEFAULT_TIME_TO_LIVE = 0;

    static final public CFMetaData BatchlogCf = compile("CREATE TABLE " + SystemKeyspace.BATCHLOG_CF + " (" + "id uuid PRIMARY KEY," + "written_at timestamp," + "data blob" + ") WITH COMMENT='uncommited batches' AND gc_grace_seconds=0 " + "AND COMPACTION={'class' : 'SizeTieredCompactionStrategy', 'min_threshold' : 2}");

    static final public CFMetaData SchemaKeyspacesCf = compile("CREATE TABLE " + SystemKeyspace.SCHEMA_KEYSPACES_CF + " (" + "keyspace_name text PRIMARY KEY," + "durable_writes boolean," + "strategy_class text," + "strategy_options text" + ") WITH COMPACT STORAGE AND COMMENT='keyspace definitions' AND gc_grace_seconds=8640");
    ...
    private static void addTriggerDefinitionsFromSchema(CFMetaData cfDef, Row serializedTriggerDefinitions) {
        for (TriggerDefinition td : TriggerDefinition.fromSchema(serializedTriggerDefinitions)) cfDef.triggers.put(td.name, td);
    }

    static public class SpeculativeRetry {
        ...
        static public SpeculativeRetry fromString(String retry) throws ConfigurationException {
    ...
    static public CFMetaData fromThriftCqlRow(CqlRow row) {
        Map<String, ByteBuffer> columns = new HashMap<>();
        try {
            for (org.apache.cassandra.thrift.Column column : row.getColumns()) columns.put(ByteBufferUtil.string(column.bufferForName()), column.value);
    ...
    static final public String DEFAULT_COMPRESSOR = LZ4Compressor.class.getCanonicalName();
