...
public
class CommitLog implements CommitLogMBean
{
   private static final Logger logger =
     LoggerFactory.getLogger(CommitLog.class);
   ...
   CommitLog construct() {
     CommitLog log =
       new CommitLog(DatabaseDescriptor.getCommitLogLocation(),
         CommitLogArchiver.construct());
     MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();
     try {
       mbs.registerMBean(log,
   ...
   CommitLog(String location, CommitLogArchiver archiver) {
     ...
     this.configuration =
     new Configuration(DatabaseDescriptor.getCommitLogCompression());
     ...
     new BatchCommitLogService(this) : new PeriodicCommitLogService(this);
   ...
   public
   int recover(File... clogs) throws IOException {
   ...
   public
   void sync(boolean syncAllSegments) {
   ...
   ReplayPosition add(Mutation mutation) {
     ...
     int size =
       (int)Mutation.serializer.serializedSize(mutation,
     ...
     try (BufferedDataOutputStreamPlus dos =
            new DataOutputBufferFixed(buffer)) {
       dos.writeInt(size);
   ...
   public
   void discardCompletedSegments(final UUID cfId, 
                                 final ReplayPosition context) {
     logger.trace("discard completed log segments for {}, table {}", context,
       cfId);
     for (Iterator<CommitLogSegment> iter =
            allocator.getActiveSegments().iterator();
          iter.hasNext(); ) {
   ...
   Map<String,Double> getActiveSegmentCompressionRatios() {
     ...
     for (CommitLogSegment segment : allocator.getActiveSegments())
       segmentRatios.put(segment.getName(),
         1.0 * segment.onDiskSize() / segment.contentSize());
   ...
   boolean handleCommitError(String message, Throwable t) {
     ...
     switch (DatabaseDescriptor.getCommitFailurePolicy()) {
       ...
       case stop_commit:
         logger.error(String.format("%s. Commit disk failure policy is %s; terminating thread",
   ...
   class Configuration
   {
      ...
      Configuration(ParameterizedClass compressorClass) {
        ...
        this.compressor =
        compressorClass != null ? CompressionParams.createCompressor(compressorClass) : null;
      }
      public
      ...
      public
      ParameterizedClass getCompressorClass() {
