...
public final class CFMetaData {
  ...
  public final static SpeculativeRetry DEFAULT_SPECULATIVE_RETRY =
      new SpeculativeRetry(SpeculativeRetry.RetryType.NONE, 0);
  ...
  public static final CFMetaData IndexCf =
      compile("CREATE TABLE \"" + SystemKeyspace.INDEX_CF + "\" ("
              + "table_name text,"
              + "index_name text,"
              + "PRIMARY KEY (table_name, index_name)"
              + (") WITH COMPACT STORAGE AND COMMENT='indexes that have been " +
                 "completed'"));
  public static final CFMetaData CounterIdCf =
      compile("CREATE TABLE \"" + SystemKeyspace.COUNTER_ID_CF + "\" ("
              + "key text,"
              + "id timeuuid,"
              + "PRIMARY KEY (key, id)"
              + ") WITH COMPACT STORAGE AND COMMENT='counter node IDs'");
  public static final CFMetaData SchemaKeyspacesCf =
<<<<<<< /root/work/apache/cassandra/d3j/patched-879-0.java
      compile("CREATE TABLE " + SystemKeyspace.SCHEMA_KEYSPACES_CF + " ("
              + "keyspace_name text PRIMARY KEY,"
              + "durable_writes boolean,"
              + "strategy_class text,"
              + "strategy_options text"
              + (") WITH COMPACT STORAGE AND COMMENT='keyspace definitions' " +
                 "AND gc_grace_seconds=8640"));
  public static final CFMetaData SchemaColumnFamiliesCf = compile(
      "CREATE TABLE " + SystemKeyspace.SCHEMA_COLUMNFAMILIES_CF + " ("
      + "keyspace_name text,"
  ...
  public static final CFMetaData SchemaColumnsCf =
      compile("CREATE TABLE " + SystemKeyspace.SCHEMA_COLUMNS_CF + " ("
||||||| /root/work/apache/cassandra/d3j/base-879.java
      compile(8, "CREATE TABLE " + SystemTable.SCHEMA_KEYSPACES_CF + "("
                     + "keyspace_name text PRIMARY KEY,"
                     + "durable_writes boolean,"
                     + "strategy_class text,"
                     + "strategy_options text"
                     + (") WITH COMPACT STORAGE AND COMMENT='keyspace " +
  ...
  public static final CFMetaData SchemaColumnFamiliesCf =
      compile(9, "CREATE TABLE " + SystemTable.SCHEMA_COLUMNFAMILIES_CF + "("
                     ...
                     + "default_write_consistency text,"
                     + "PRIMARY KEY (keyspace_name, columnfamily_name)"
                     + (") WITH COMMENT='ColumnFamily definitions' AND " +
                        "gc_grace_seconds=8640"));
  public static final CFMetaData SchemaColumnsCf = compile(
      10, "CREATE TABLE " + SystemTable.SCHEMA_COLUMNS_CF + "("
=======
      compile("CREATE TABLE " + SystemKeyspace.SCHEMA_KEYSPACES_CF + " ("
              + "keyspace_name text PRIMARY KEY,"
              + "durable_writes boolean,"
  ...
  public static final CFMetaData SchemaColumnsCf =
      compile("CREATE TABLE " + SystemKeyspace.SCHEMA_COLUMNS_CF + " ("
>>>>>>> /root/work/apache/cassandra/d3j/patched-879-1.java
              + "keyspace_name text,"
              + "columnfamily_name text,"
              + "column_name text,"
              + "validator text,"
              ...
              + (") WITH COMMENT='ColumnFamily column attributes' AND " +
                 "gc_grace_seconds=8640"));
  public static final CFMetaData SchemaTriggersCf =
      compile("CREATE TABLE " + SystemKeyspace.SCHEMA_TRIGGERS_CF + " ("
              + "keyspace_name text,"
              + "columnfamily_name text,"
              + "trigger_name text,"
  ...
  public static final CFMetaData HintsCf =
      compile("CREATE TABLE " + SystemKeyspace.HINTS_CF + " ("
              + "target_id uuid,"
              + "hint_id timeuuid,"
              + "message_version int,"
              + "mutation blob,"
  ...
  public static final CFMetaData PeersCf =
      compile("CREATE TABLE " + SystemKeyspace.PEERS_CF + " ("
  ...
  public static final CFMetaData PeerEventsCf =
      compile("CREATE TABLE " + SystemKeyspace.PEER_EVENTS_CF + " ("
              + "peer inet PRIMARY KEY,"
              + "hints_dropped map<uuid, int>"
              + ") WITH COMMENT='cf contains events related to peers'");
  public static final CFMetaData LocalCf =
      compile("CREATE TABLE " + SystemKeyspace.LOCAL_CF + " ("
  ...
  public static final CFMetaData TraceSessionsCf =
      compile("CREATE TABLE " + Tracing.SESSIONS_CF + " ("
  ...
  public static final CFMetaData TraceEventsCf =
      compile("CREATE TABLE " + Tracing.EVENTS_CF + " ("
                  + "session_id uuid,"
                  + "event_id timeuuid,"
                  + "source inet,"
                  + "thread text,"
  ...
  public static final CFMetaData BatchlogCf =
      compile("CREATE TABLE " + SystemKeyspace.BATCHLOG_CF + " ("
              + "id uuid PRIMARY KEY,"
  ...
  public static final CFMetaData RangeXfersCf =
      compile("CREATE TABLE " + SystemKeyspace.RANGE_XFERS_CF + " ("
  ...
  public static final CFMetaData CompactionLogCf =
      compile("CREATE TABLE " + SystemKeyspace.COMPACTION_LOG + " ("
              + "id uuid PRIMARY KEY,"
              + "keyspace_name text,"
              + "columnfamily_name text,"
              + "inputs set<int>"
  ...
  public static final CFMetaData PaxosCf =
      compile("CREATE TABLE " + SystemKeyspace.PAXOS_CF + " ("
              + "row_key blob,"
              + "cf_id UUID,"
    ...
    public static Caching fromString(String cache)
      ...
      try {
        ...
        throw new ConfigurationException(
            String.format("%s not found, available types: %s.", cache,
    ...
    public static SpeculativeRetry fromString(String retry)
      ...
      try {
        if (name.endsWith(RetryType.PERCENTILE.toString())) {
          double value =
              Double.parseDouble(name.substring(0, name.length() - 10));
          ...
          double value =
              Double.parseDouble(name.substring(0, name.length() - 2));
  ...
  private volatile Map<ByteBuffer, ColumnDefinition> column_metadata =
      new HashMap<>();
  ...
  public volatile CompressionParameters compressionParameters =
      new CompressionParameters(null);
  ...
  public CFMetaData
  compactionStrategyClass(Class<? extends AbstractCompactionStrategy> prop) {
  ...
  private static CFMetaData compile(String cql, String keyspace) {
    try {
      CreateTableStatement statement =
          (CreateTableStatement)QueryProcessor.parseStatement(cql)
              .prepare()
              .statement;
  ...
  private static AbstractType<?> makeComparator(ColumnFamilyType cftype,
    ...
    return cftype == ColumnFamilyType.Super
        ? CompositeType.getInstance(comp,
                                    subcc == null ? BytesType.instance : subcc)
        : comp;
  }
  ...
  static UUID getId(String ksName, String cfName) {
    return UUID.nameUUIDFromBytes(
        ArrayUtils.addAll(ksName.getBytes(), cfName.getBytes()));
  }
  private static CFMetaData newSystemMetadata(String keyspace, String cfName,
    ...
    ColumnFamilyType type =
        subcc == null ? ColumnFamilyType.Standard : ColumnFamilyType.Super;
    CFMetaData newCFMD =
        new CFMetaData(keyspace, cfName, type, comparator, subcc);
  ...
  public static CFMetaData newIndexMetadata(CFMetaData parent,
    ...
    Caching indexCaching = parent.getCaching() == Caching.ALL ||
                               ...
                               ? Caching.KEYS_ONLY
                               : Caching.NONE;
    return new CFMetaData(parent.ksName, parent.indexColumnFamilyName(info),
                          ColumnFamilyType.Standard, columnComparator,
                          (AbstractType)null)
  ...
  public CFMetaData clone() {
    return copyOpts(new CFMetaData(ksName, cfName, cfType, comparator, cfId),
                    this);
  }
  public static CFMetaData rename(CFMetaData cfm, String newName) {
    return copyOpts(new CFMetaData(cfm.ksName, newName, cfm.cfType,
                                   cfm.comparator, cfm.cfId),
                    cfm);
  }
  static CFMetaData copyOpts(CFMetaData newCFMD, CFMetaData oldCFMD) {
    ...
    return newCFMD.comment(oldCFMD.comment)
        ...
        .compactionStrategyOptions(
            new HashMap<>(oldCFMD.compactionStrategyOptions))
  ...
  public String indexColumnFamilyName(ColumnDefinition info) {
    return cfName + Directories.SECONDARY_INDEX_NAME_SEPARATOR +
        (info.getIndexName() == null ? ByteBufferUtil.bytesToHex(info.name)
                                     : info.getIndexName());
  }
  public String getComment() { return comment; }
  public boolean isSuper() { return cfType == ColumnFamilyType.Super; }
  public double getReadRepairChance() { return readRepairChance; }
  public double getDcLocalReadRepair() { return dcLocalReadRepairChance; }
  ...
  public boolean getReplicateOnWrite() { return replicateOnWrite; }
  public boolean populateIoCacheOnFlush() { return populateIoCacheOnFlush; }
  public int getGcGraceSeconds() { return gcGraceSeconds; }
  public AbstractType<?> getDefaultValidator() { return defaultValidator; }
  public AbstractType<?> getKeyValidator() { return keyValidator; }
  public Integer getMinCompactionThreshold() { return minCompactionThreshold; }
  public Integer getMaxCompactionThreshold() { return maxCompactionThreshold; }
  ...
  public SpeculativeRetry getSpeculativeRetry() { return speculativeRetry; }
  ...
  public Set<ColumnDefinition> regularColumns() { return regularColumns; }
  public ColumnDefinition compactValueColumn() { return compactValueColumn; }
  public double getBloomFilterFpChance() {
    return (bloomFilterFpChance == null || bloomFilterFpChance == 0)
        ? compactionStrategyClass == LeveledCompactionStrategy.class ? 0.1
                                                                     : 0.01
        : bloomFilterFpChance;
  }
  public Caching getCaching() { return caching; }
  public int getIndexInterval() { return indexInterval; }
  public int getMemtableFlushPeriod() { return memtableFlushPeriod; }
  public int getDefaultTimeToLive() { return defaultTimeToLive; }
  public Map<ByteBuffer, Long> getDroppedColumns() { return droppedColumns; }
  public boolean equals(Object obj) {
    if (obj == this) {
      return true;
  ...
  public AbstractType<?> getValueValidator(ColumnDefinition columnDefinition) {
    return columnDefinition == null ? defaultValidator
  ...
  applyImplicitDefaults(org.apache.cassandra.thrift.CfDef cf_def) {
    ...
    if (!cf_def.isSetPopulate_io_cache_on_flush())
      cf_def.setPopulate_io_cache_on_flush(
          CFMetaData.DEFAULT_POPULATE_IO_CACHE_ON_FLUSH);
    if (!cf_def.isSetMin_compaction_threshold())
      cf_def.setMin_compaction_threshold(
    ...
    if (cf_def.compaction_strategy == null)
      cf_def.compaction_strategy =
          DEFAULT_COMPACTION_STRATEGY_CLASS.getSimpleName();
    ...
    if (!cf_def.isSetDclocal_read_repair_chance())
      cf_def.setDclocal_read_repair_chance(
  ...
  public static CFMetaData fromThrift(org.apache.cassandra.thrift.CfDef cf_def)
    ...
    try {
      CFMetaData newCFMD =
          new CFMetaData(cf_def.keyspace, cf_def.name, cfType,
      ...
      if (cf_def.isSetCompaction_strategy_options())
        newCFMD.compactionStrategyOptions(
            new HashMap<>(cf_def.compaction_strategy_options));
      ...
      if (cf_def.isSetSpeculative_retry())
        newCFMD.speculativeRetry(
            SpeculativeRetry.fromString(cf_def.speculative_retry));
      if (cf_def.isSetPopulate_io_cache_on_flush())
      ...
      if (cf_def.isSetKey_alias() &&
        ...
        newCFMD.column_metadata.put(
            cf_def.key_alias,
            ColumnDefinition.partitionKeyDef(cf_def.key_alias,
  ...
  public void reload() {
    Row cfDefRow = SystemKeyspace.readSchemaRow(
        SystemKeyspace.SCHEMA_COLUMNFAMILIES_CF, ksName, cfName);
    if (cfDefRow.cf == null || cfDefRow.cf.getColumnCount() == 0)
      throw new RuntimeException(
          String.format("%s not found in the schema definitions keyspace.",
                        ksName + ":" + cfName));
    try {
  ...
  public void validateCompatility(CFMetaData cfm)
    ...
    if (!cfm.ksName.equals(ksName))
      throw new ConfigurationException(String.format(
          "Keyspace mismatch (found %s; expected %s)", cfm.ksName, ksName));
    if (!cfm.cfName.equals(cfName))
      throw new ConfigurationException(
          String.format("Column family mismatch (found %s; expected %s)",
                        cfm.cfName, cfName));
    if (!cfm.cfId.equals(cfId))
      throw new ConfigurationException(String.format(
          "Column family ID mismatch (found %s; expected %s)", cfm.cfId, cfId));
  ...
  public static void validateCompactionOptions(
    ...
    try {
      ...
      Map<String, String> unknownOptions =
          (Map<String, String>)validateMethod.invoke(null, options);
      ...
      logger.warn("Compaction Strategy {} does not have a static " +
                  "validateOptions method. Validation ignored",
  ...
  createCompactionStrategy(String className) throws ConfigurationException {
    ...
    Class<AbstractCompactionStrategy> strategyClass =
        FBUtilities.classForName(className, "compaction strategy");
    if (!AbstractCompactionStrategy.class.isAssignableFrom(strategyClass))
      throw new ConfigurationException(
          String.format("Specified compaction strategy class (%s) is not " +
                        "derived from AbstractReplicationStrategy",
                        className));
  ...
  createCompactionStrategyInstance(ColumnFamilyStore cfs) {
    try {
      Constructor<? extends AbstractCompactionStrategy> constructor =
          compactionStrategyClass.getConstructor(
              new Class[] {ColumnFamilyStore.class, Map.class});
  ...
  public org.apache.cassandra.thrift.CfDef toThrift() {
    org.apache.cassandra.thrift.CfDef def =
        new org.apache.cassandra.thrift.CfDef(ksName, cfName);
    ...
    def.setDefault_validation_class(
    ...
    def.setCompaction_strategy_options(
        new HashMap<>(compactionStrategyOptions));
  ...
  getColumnDefinitionFromColumnName(ByteBuffer columnName) {
    if (!isSuper() && (comparator instanceof CompositeType)) {
      ...
      for (ColumnDefinition def : column_metadata.values()) {
        ByteBuffer toCompare = def.componentIndex == null
                                   ? columnName
  ...
  public void addDefaultIndexNames() throws ConfigurationException {
    ...
    if (cfId != null) {
      ...
      for (Map.Entry<ByteBuffer, ColumnDefinition> entry :
        ...
        if (newDef.getIndexName() != null &&
          ...
          throw new ConfigurationException("Can't modify index name: was '" +
                                           oldIndexName + "' changed to '" +
                                           newDef.getIndexName() + "'.");
    ...
    for (ColumnDefinition column : column_metadata.values()) {
      if (column.getIndexType() != null && column.getIndexName() == null) {
        String baseName = getDefaultIndexName(
            cfName, getColumnDefinitionComparator(column), column.name);
  ...
  public CFMetaData validate() throws ConfigurationException {
    ...
    if (!isNameValid(ksName))
      throw new ConfigurationException(String.format(
          "Keyspace name must not be empty, more than %s characters long, or " +
          "contain non-alphanumeric-underscore characters (got \"%s\")",
          Schema.NAME_LENGTH, ksName));
    if (!isNameValid(cfName))
      throw new ConfigurationException(
          String.format("ColumnFamily name must not be empty, more than %s " +
                        "characters long, or contain " +
                        "non-alphanumeric-underscore characters (got \"%s\")",
                        Schema.NAME_LENGTH, cfName));
    if (cfType == null)
      throw new ConfigurationException(
          String.format("Invalid column family type for %s", cfName));
      ...
      for (ColumnDefinition def : regularColumns)
        if (!(def.getValidator() instanceof CounterColumnType))
          throw new ConfigurationException(
              ...
              getColumnDefinitionComparator(def).getString(def.name) +
    ...
    for (ColumnDefinition c : column_metadata.values()) {
      ...
      try {
        ...
        throw new ConfigurationException(
            String.format("Column name %s is not valid for comparator %s",
    ...
    if (bloomFilterFpChance != null && bloomFilterFpChance == 0)
      throw new ConfigurationException(
          "Zero false positives is impossible; bloom filter false positive " +
          "chance bffpc must be 0 < bffpc <= 1");
    return this;
  }
  ...
  private void validateCompactionThresholds() throws ConfigurationException {
    if (maxCompactionThreshold == 0) {
      logger.warn(
          "Disabling compaction by setting max or min compaction has been " +
          "deprecated, "
    ...
    if (minCompactionThreshold > maxCompactionThreshold)
      throw new ConfigurationException(
          String.format("Min compaction threshold (got %d) cannot be greater " +
                        "than max compaction threshold (got %d)",
  ...
  public RowMutation toSchemaUpdate(CFMetaData newState,
    ...
    MapDifference<ByteBuffer, ColumnDefinition> columnDiff =
        Maps.difference(column_metadata, newState.column_metadata);
    for (ColumnDefinition cd : columnDiff.entriesOnlyOnLeft().values()) {
    ...
    MapDifference<String, TriggerDefinition> triggerDiff =
        Maps.difference(triggers, newState.triggers);
  ...
  public RowMutation dropFromSchema(long timestamp) {
    ...
    cf.addAtom(new RangeTombstone(builder.build(), builder.buildAsEndOfRange(),
                                  timestamp, ldt));
  ...
  private void toSchemaNoColumnsNoTriggers(RowMutation rm, long timestamp) {
    ...
    if (isSuper()) {
      CompositeType ct = (CompositeType)comparator;
      cf.addColumn(Column.create(ct.types.get(0).toString(), timestamp, cfName,
                                 "comparator"));
      cf.addColumn(Column.create(ct.types.get(1).toString(), timestamp, cfName,
                                 "subcomparator"));
    } else {
      cf.addColumn(Column.create(comparator.toString(), timestamp, cfName,
                                 "comparator"));
    }
    cf.addColumn(comment == null
                     ? DeletedColumn.create(ldt, timestamp, cfName, "comment")
    ...
    cf.addColumn(
        Column.create(gcGraceSeconds, timestamp, cfName, "gc_grace_seconds"));
    cf.addColumn(Column.create(defaultValidator.toString(), timestamp, cfName,
                               "default_validator"));
    ...
    cf.addColumn(bloomFilterFpChance == null
                     ? DeletedColumn.create(ldt, timestamp, cfName,
                                            "bloomFilterFpChance")
    ...
    cf.addColumn(
        Column.create(caching.toString(), timestamp, cfName, "caching"));
    cf.addColumn(Column.create(defaultTimeToLive, timestamp, cfName,
                               "default_time_to_live"));
    cf.addColumn(Column.create(compactionStrategyClass.getName(), timestamp,
                               cfName, "compaction_strategy_class"));
    ...
    cf.addColumn(
        Column.create(indexInterval, timestamp, cfName, "index_interval"));
    cf.addColumn(Column.create(speculativeRetry.toString(), timestamp, cfName,
                               "speculative_retry"));
    for (Map.Entry<ByteBuffer, Long> entry : droppedColumns.entrySet())
      cf.addColumn(new Column(makeDroppedColumnName(entry.getKey()),
                              LongType.instance.decompose(entry.getValue()),
                              timestamp));
    cf.addColumn(Column.create(aliasesToJson(partitionKeyColumns), timestamp,
                               cfName, "key_aliases"));
    cf.addColumn(Column.create(aliasesToJson(clusteringKeyColumns), timestamp,
                               cfName, "column_aliases"));
    cf.addColumn(
        compactValueColumn == null
            ? DeletedColumn.create(ldt, timestamp, cfName, "value_alias")
            : Column.create(compactValueColumn.name, timestamp, cfName,
                            "value_alias"));
  }
  static CFMetaData fromSchemaNoColumnsNoTriggers(UntypedResultSet.Row result) {
    try {
      CFMetaData cfm = new CFMetaData(
          result.getString("keyspace_name"),
          ...
          result.has("subcomparator")
              ? TypeParser.parse(result.getString("subcomparator"))
              : null);
      ...
      if (result.has("speculative_retry"))
        cfm.speculativeRetry(
            SpeculativeRetry.fromString(result.getString("speculative_retry")));
      cfm.compactionStrategyClass(createCompactionStrategy(
          result.getString("compaction_strategy_class")));
      cfm.compressionParameters(CompressionParameters.create(
          fromJsonMap(result.getString("compression_parameters"))));
      cfm.compactionStrategyOptions(
          fromJsonMap(result.getString("compaction_strategy_options")));
      ...
      if (result.has("populate_io_cache_on_flush"))
        cfm.populateIoCacheOnFlush(
            result.getBoolean("populate_io_cache_on_flush"));
      if (result.has("key_aliases"))
        cfm.addColumnMetadataFromAliases(
            aliasesFromStrings(fromJsonList(result.getString("key_aliases"))),
            cfm.keyValidator, ColumnDefinition.Type.PARTITION_KEY);
      else if (result.has("key_alias"))
        cfm.addColumnMetadataFromAliases(
            Collections.<ByteBuffer>singletonList(result.getBytes("key_alias")),
            cfm.keyValidator, ColumnDefinition.Type.PARTITION_KEY);
      cfm.addColumnMetadataFromAliases(
          aliasesFromStrings(fromJsonList(result.getString("column_aliases"))),
          cfm.comparator, ColumnDefinition.Type.CLUSTERING_KEY);
      if (result.has("value_alias"))
        cfm.addColumnMetadataFromAliases(Collections.<ByteBuffer>singletonList(
                                             result.getBytes("value_alias")),
                                         cfm.defaultValidator,
                                         ColumnDefinition.Type.COMPACT_VALUE);
      if (result.has("dropped_columns"))
        cfm.droppedColumns(convertDroppedColumns(result.getMap(
            "dropped_columns", UTF8Type.instance, LongType.instance)));
      return cfm;
    ...
    if (comparator instanceof CompositeType) {
      ...
      for (int i = 0; i < aliases.size(); ++i) {
        if (aliases.get(i) != null)
          column_metadata.put(
              aliases.get(i),
              new ColumnDefinition(aliases.get(i), ct.types.get(i), i, type));
      }
    } else {
      ...
      if (!aliases.isEmpty() && aliases.get(0) != null)
        column_metadata.put(
            aliases.get(0),
            new ColumnDefinition(aliases.get(0), comparator, null, type));
    }
  }
  public static CFMetaData fromSchema(UntypedResultSet.Row result) {
    CFMetaData cfDef = fromSchemaNoColumnsNoTriggers(result);
    Row serializedTriggers = SystemKeyspace.readSchemaRow(
        SystemKeyspace.SCHEMA_TRIGGERS_CF, cfDef.ksName, cfDef.cfName);
    addTriggerDefinitionsFromSchema(cfDef, serializedTriggers);
    Row serializedColumns = SystemKeyspace.readSchemaRow(
        SystemKeyspace.SCHEMA_COLUMNS_CF, cfDef.ksName, cfDef.cfName);
  ...
  private static CFMetaData fromSchema(Row row) {
    UntypedResultSet.Row result =
        QueryProcessor
            .resultify("SELECT * FROM system.schema_columnfamilies", row)
            .one();
  ...
  private static Map<ByteBuffer, Long>
    ...
    case REGULAR:
      AbstractType<?> cfComparator =
          cfType == ColumnFamilyType.Super
              ? ((CompositeType)comparator).types.get(1)
              : comparator;
  ...
  static CFMetaData
  addColumnDefinitionsFromSchema(CFMetaData cfDef,
                                 Row serializedColumnDefinitions) {
    for (ColumnDefinition cd :
         ColumnDefinition.fromSchema(serializedColumnDefinitions, cfDef))
  ...
  private static void
  addTriggerDefinitionsFromSchema(CFMetaData cfDef,
                                  Row serializedTriggerDefinitions) {
    for (TriggerDefinition td :
         TriggerDefinition.fromSchema(serializedTriggerDefinitions))
      cfDef.triggers.put(td.name, td);
  }
  public void addTriggerDefinition(TriggerDefinition def)
      throws ConfigurationException {
    if (triggers.containsKey(def.name))
      throw new ConfigurationException(
          String.format("Cannot create trigger %s, a trigger with the same " +
                        "name already exists",
                        def.name));
  ...
  public void renameColumn(ByteBuffer from, String strFrom, ByteBuffer to,
    ...
    if (def == null)
      throw new InvalidRequestException(String.format(
          "Cannot rename unknown column %s in keyspace %s", strFrom, cfName));
    if (column_metadata.get(to) != null)
      throw new InvalidRequestException(
          String.format("Cannot rename column %s to %s in keyspace %s; " +
    ...
    if (def.type == ColumnDefinition.Type.REGULAR) {
      ...
      throw new InvalidRequestException(String.format(
          "Cannot rename column %s because it is secondary indexed", strFrom));
    }
  ...
  private void rebuildCQL3Metadata() {
    ...
    for (ColumnDefinition def : column_metadata.values()) {
      switch (def.type) {
      case PARTITION_KEY:
        assert !(def.componentIndex == null && keyValidator instanceof
                                                   CompositeType);
        pkCols.set(def.componentIndex == null ? 0 : def.componentIndex, def);
        break;
      case CLUSTERING_KEY:
        assert !(def.componentIndex == null && comparator instanceof
                                                   CompositeType);
  ...
  addDefaultKeyAliases(List<ColumnDefinition> pkCols) {
    for (int i = 0; i < pkCols.size(); i++) {
      if (pkCols.get(i) == null) {
        ...
        ByteBuffer name = ByteBufferUtil.bytes(
            i == 0 ? DEFAULT_KEY_ALIAS : DEFAULT_KEY_ALIAS + (i + 1));
        ColumnDefinition newDef =
            ColumnDefinition.partitionKeyDef(name, type, idx);
        column_metadata.put(newDef.name, newDef);
        pkCols.set(i, newDef);
      }
    }
  ...
  private List<ColumnDefinition>
  addDefaultColumnAliases(List<ColumnDefinition> ckCols) {
    for (int i = 0; i < ckCols.size(); i++) {
      if (ckCols.get(i) == null) {
        ...
        ColumnDefinition newDef =
            ColumnDefinition.clusteringKeyDef(name, type, idx);
        column_metadata.put(newDef.name, newDef);
        ckCols.set(i, newDef);
      }
    }
  ...
  addDefaultValueAlias(ColumnDefinition compactValueDef, boolean isDense) {
    if (isDense) {
      if (compactValueDef != null)
        return compactValueDef;
      ColumnDefinition newDef = ColumnDefinition.compactValueDef(
          ByteBufferUtil.bytes(DEFAULT_VALUE_ALIAS), defaultValidator);
      column_metadata.put(newDef.name, newDef);
      return newDef;
    } else {
  ...
  private static boolean isDense(AbstractType<?> comparator,
    ...
    for (ColumnDefinition def : defs) {
      ...
      case CLUSTERING_KEY:
        maxClusteringIdx =
            Math.max(maxClusteringIdx,
                     def.componentIndex == null ? 0 : def.componentIndex);
        break;
      case REGULAR:
        hasRegular = true;
        break;
