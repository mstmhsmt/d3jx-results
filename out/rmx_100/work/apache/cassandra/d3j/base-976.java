...
public final class KSMetaData {
  ...
  private KSMetaData(String name,
                     ...
                     Map<String, String> strategyOptions, boolean durableWrites,
                     Iterable<CFMetaData> cfDefs) {
    this.name = name;
    this.strategyClass =
        strategyClass == null ? NetworkTopologyStrategy.class : strategyClass;
  ...
  public static KSMetaData cloneWith(KSMetaData ksm,
                                     Iterable<CFMetaData> cfDefs) {
    return new KSMetaData(ksm.name, ksm.strategyClass, ksm.strategyOptions,
                          ksm.durableWrites, cfDefs);
  }
  public static KSMetaData systemKeyspace() {
    List<CFMetaData> cfDefs = Arrays.asList(
        CFMetaData.StatusCf, CFMetaData.HintsCf, CFMetaData.MigrationsCf,
        CFMetaData.SchemaCf, CFMetaData.IndexCf, CFMetaData.NodeIdCf,
        CFMetaData.VersionCf, CFMetaData.SchemaKeyspacesCf,
        CFMetaData.SchemaColumnFamiliesCf, CFMetaData.SchemaColumnsCf);
  ...
  testMetadata(String name,
               Class<? extends AbstractReplicationStrategy> strategyClass,
               Map<String, String> strategyOptions, CFMetaData... cfDefs) {
    return new KSMetaData(name, strategyClass, strategyOptions, true,
                          Arrays.asList(cfDefs));
  }
  public static KSMetaData testMetadataNotDurable(
      String name, Class<? extends AbstractReplicationStrategy> strategyClass,
      Map<String, String> strategyOptions, CFMetaData... cfDefs) {
  ...
  public Map<String, CFMetaData> cfMetaData() { return cfMetaData; }
  @Override
  ...
  @Deprecated
  public static KSMetaData
  fromAvro(org.apache.cassandra.db.migration.avro.KsDef ks) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Could not create ReplicationStrategy of type " + ks.strategy_class,
          ex);
    }
    ...
    return new KSMetaData(ks.name.toString(), repStratClass, strategyOptions,
                          ks.durable_writes, cfMetaData);
  }
  ...
  public static KSMetaData fromThrift(KsDef ksd, CFMetaData... cfDefs)
    ...
    return new KSMetaData(
        ksd.name, AbstractReplicationStrategy.getClass(ksd.strategy_class),
        ksd.strategy_options == null ? Collections.<String, String>emptyMap()
                                     : ksd.strategy_options,
        ksd.durable_writes, Arrays.asList(cfDefs));
  }
  ...
  public RowMutation diff(KsDef newState, long modificationTimestamp) {
    ...
    RowMutation m =
        new RowMutation(Table.SYSTEM_TABLE, SystemTable.getSchemaKSKey(name));
    for (KsDef._Fields field : KsDef._Fields.values()) {
      ...
      if (Objects.equal(curValue, newValue))
        continue;
      m.add(new QueryPath(SystemTable.SCHEMA_KEYSPACES_CF, null,
                          AsciiType.instance.fromString(field.getFieldName())),
            valueAsBytes(newValue), modificationTimestamp);
    }
    return m;
  }
  public KSMetaData reloadAttributes() throws IOException {
    ...
    if (ksDefRow.cf == null || ksDefRow.cf.isEmpty())
      throw new IOException(
          String.format("%s not found in the schema definitions table (%s).",
  ...
  public List<RowMutation> dropFromSchema(long timestamp) {
    ...
    RowMutation ksMutation =
        new RowMutation(Table.SYSTEM_TABLE, SystemTable.getSchemaKSKey(name));
    ksMutation.delete(new QueryPath(SystemTable.SCHEMA_KEYSPACES_CF),
                      timestamp);
  ...
  public static RowMutation toSchema(KsDef ksDef, long timestamp)
      throws IOException {
    RowMutation mutation = new RowMutation(
        Table.SYSTEM_TABLE, SystemTable.getSchemaKSKey(ksDef.name));
    for (KsDef._Fields field : KsDef._Fields.values()) {
      if (field.equals(KsDef._Fields.CF_DEFS))
        continue;
      mutation.add(
          new QueryPath(SystemTable.SCHEMA_KEYSPACES_CF, null,
                        AsciiType.instance.fromString(field.getFieldName())),
          valueAsBytes(ksDef.getFieldValue(field)), timestamp);
    }
  ...
  public static KsDef fromSchema(ColumnFamily serializedKsDef)
    ...
    for (IColumn ksAttr : serializedKsDef.getSortedColumns()) {
      if (ksAttr == null || ksAttr.isMarkedForDelete())
        continue;
      KsDef._Fields field =
          KsDef._Fields.findByName(comparator.getString(ksAttr.name()));
      if (field == null)
        continue;
      ksDef.setFieldValue(
          field,
          deserializeValue(ksAttr.value(),
                           getValueClass(KsDef.class, field.getFieldName())));
    }
  ...
  public static Map<String, CfDef>
  deserializeColumnFamilies(ColumnFamily serializedColumnFamilies) {
    ...
    for (CfDef cfDef : cfs.values()) {
      for (ColumnDef columnDef : ColumnDefinition.fromSchema(
