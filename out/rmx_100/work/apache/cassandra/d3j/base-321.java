...
public class StreamReceiveTask extends StreamTask {
  ...
  public synchronized void received(SSTableMultiWriter sstable) {
    if (done) {
      logger.warn("[{}] Received sstable {} on already finished stream " +
                  "received task. Aborting sstable.",
  ...
  public int getTotalNumberOfFiles() { return totalFiles; }
        ...
        try (Refs<SSTableReader> refs = Refs.ref(readers)) {
          if (hasViews || hasCDC) {
              ...
              try (ISSTableScanner scanner = reader.getScanner()) {
                while (scanner.hasNext()) {
                  try (UnfilteredRowIterator rowIterator = scanner.next()) {
                    Mutation m = new Mutation(PartitionUpdate.fromIterator(
                        rowIterator, ColumnFilter.all(cfs.metadata)));
            ...
            logger.debug("[Stream #{}] Received {} sstables from {} ({})",
                         task.session.planId(), readers.size(),
                         task.session.peer, readers);
            cfs.addSSTables(readers);
            cfs.indexManager.buildAllIndexesBlocking(readers);
            if (cfs.isRowCacheEnabled() || cfs.metadata.isCounter()) {
              List<Bounds<Token>> boundsToInvalidate =
                  new ArrayList<>(readers.size());
              readers.forEach(
                  sstable
                  -> boundsToInvalidate.add(new Bounds<Token>(
  ...
  private synchronized void abortTransaction() { txn.abort(); }
  private synchronized void finishTransaction() { txn.finish(); }
}