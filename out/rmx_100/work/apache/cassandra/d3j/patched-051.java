...
<<<<<<< /root/work/apache/cassandra/d3j/patched-051-0.java
  static ReplicaPlan.ForRead<?, ?> remoteReplica1;
...
||||||| /root/work/apache/cassandra/d3j/base-051.java
...
=======
  ...
  static EndpointsForRange remoteReplicas;
  static long now = TimeUnit.NANOSECONDS.toMicros(nanoTime());
>>>>>>> /root/work/apache/cassandra/d3j/patched-051-1.java
  ...
  static void assertRowsEqual(Row expected, Row actual) {
    try {
      ...
      Assert.assertArrayEquals(Iterables.toArray(expected.cells(), Cell.class),
                               Iterables.toArray(expected.cells(), Cell.class));
    } catch (Throwable t) {
      throw new AssertionError(
          String.format("Row comparison failed, expected %s got %s", expected,
                        actual),
          t);
    }
  }
  ...
  static DecoratedKey dk(int v) {
    return DatabaseDescriptor.getPartitioner().decorateKey(
        ByteBufferUtil.bytes(v));
  }
  static Cell<?> cell(String name, String value, long timestamp) {
    return BufferCell.live(
        cfm.getColumn(ColumnIdentifier.getInterned(name, false)), timestamp,
  ...
  static PartitionIterator partition(Cell<?>... cells) {
    UnfilteredPartitionIterator iter = new SingletonUnfilteredPartitionIterator(
        update(cells).unfilteredIterator());
    return UnfilteredPartitionIterators.filter(
        iter, Ints.checkedCast(TimeUnit.MICROSECONDS.toSeconds(now)));
  }
  ...
  static Message<ReadResponse> msg(InetAddressAndPort from, Cell<?>... cells) {
    UnfilteredPartitionIterator iter = new SingletonUnfilteredPartitionIterator(
        update(cells).unfilteredIterator());
    return Message
        .builder(INTERNAL_RSP,
                 ReadResponse.createDataResponse(
                     iter, command,
                     command.executionController().getRepairedDataInfo()))
        .from(from)
        .build();
  }
  ...
  static void configureClass(ReadRepairStrategy repairStrategy)
      throws Throwable {
    ...
    remoteReplicas =
        EndpointsForRange.of(remoteReplica1, remoteReplica2, remoteReplica3);
    StorageService.instance.getTokenMetadata().clearUnsafe();
    StorageService.instance.getTokenMetadata().updateNormalToken(
        ByteOrderedPartitioner.instance.getToken(
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-051-0.java
    StorageService.instance.getTokenMetadata().updateNormalToken(
        ...
        remoteReplica1.endpoint());
    StorageService.instance.getTokenMetadata().updateNormalToken(
        ByteOrderedPartitioner.instance.getToken(
            ByteBuffer.wrap(new byte[] {4})),
        remoteReplica2.endpoint());
    StorageService.instance.getTokenMetadata().updateNormalToken(
        ByteOrderedPartitioner.instance.getToken(
            ByteBuffer.wrap(new byte[] {5})),
        remoteReplica3.endpoint());
    ...
    String ddl = String.format(
        "CREATE TABLE tbl (k int primary key, v text) WITH read_repair='%s'",
    ...
    KeyspaceMetadata ksm = KeyspaceMetadata.create(
        ksName, KeyspaceParams.nts("datacenter1", 3, "datacenter2", 3),
        Tables.of(cfm));
...
=======
    ...
    KeyspaceMetadata ksm = KeyspaceMetadata.create(
        ksName, KeyspaceParams.nts("datacenter1", 3, "datacenter2", 3),
        Tables.of(cfm));
    SchemaTestUtil.announceNewKeyspace(ksm, true);
    ks = Keyspace.open(ksName);
    cfs = ks.getColumnFamilyStore("tbl");
    cfs.sampleReadLatencyMicros = 0;
    cfs.additionalWriteLatencyMicros = 0;
>>>>>>> /root/work/apache/cassandra/d3j/patched-051-1.java
  ...
  public void setUp() {
    assert configured
        : "configureClass must be called in a @BeforeClass method";
    cfs.sampleReadLatencyMicros = 0;
    cfs.additionalWriteLatencyMicros = 0;
  }
  static ReplicaPlan.ForRangeRead replicaPlan(ConsistencyLevel consistencyLevel,
  ...
  static ReplicaPlan.ForWrite repairPlan(EndpointsForRange liveAndDown,
  ...
  static ReplicaPlan.ForTokenWrite repairPlan(ReplicaPlan.ForRangeRead readPlan,
    ...
    return ReplicaPlans.forWrite(
        readPlan.keyspace(), readPlan.consistencyLevel(),
        liveAndDown.forToken(token), pending,
        replica -> true, ReplicaPlans.writeReadRepair(readPlan));
  }
  static ReplicaPlan.ForWrite repairPlan(ReplicaPlan.ForRangeRead readPlan,
                                         EndpointsForRange liveAndDown) {
    Token token = readPlan.range().left.getToken();
  ...
  static ReplicaPlan.ForRangeRead replicaPlan(Keyspace keyspace,
                                              ...
                                              EndpointsForRange replicas,
                                              EndpointsForRange targets) {
    return new ReplicaPlan.ForRangeRead(
        keyspace, keyspace.getReplicationStrategy(), consistencyLevel,
        ReplicaUtils.FULL_BOUNDS, replicas, targets, 1);
  }
  public abstract InstrumentedReadRepair createInstrumentedReadRepair(
      ReadCommand command, ReplicaPlan.Shared<?, ?> replicaPlan,
  ...
  public void readSpeculationCycle() {
    InstrumentedReadRepair repair =
        createInstrumentedReadRepair(ReplicaPlan.shared(
            replicaPlan(replicas, EndpointsForRange.of(replica1, replica2))));
