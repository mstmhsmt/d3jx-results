...
public class Frame {
  ...
  public void retain() { body.retain(); }
  public boolean release() { return body.release(); }
  public static Frame create(Message.Type type, int streamId,
                             ProtocolVersion version,
                             EnumSet<Header.Flag> flags, ByteBuf body) {
    Header header =
        new Header(version, flags, streamId, type, body.readableBytes());
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-286-0.java
    public final long bodySizeInBytes;
    private Header(ProtocolVersion version, EnumSet<Flag> flags, int streamId,
                   Message.Type type, long bodySizeInBytes) {
||||||| /root/work/apache/cassandra/d3j/base-286.java
...
=======
    ...
    private Header(ProtocolVersion version, EnumSet<Flag> flags, int streamId,
                   Message.Type type, long bodySizeInBytes) {
>>>>>>> /root/work/apache/cassandra/d3j/patched-286-1.java
  ...
  public static class Decoder extends ByteToMessageDecoder {
    ...
    public Decoder(Connection.Factory factory) { this.factory = factory; }
    @Override
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer,
      ...
      if (version.isBeta() && !decodedFlags.contains(Header.Flag.USE_BETA))
        throw new ProtocolException(
            String.format("Beta version of the protocol used (%s), but " +
                          "USE_BETA flag is unset",
                          version),
            version);
      ...
      results.add(new Frame(
          new Header(version, decodedFlags, streamId, type, bodyLength), body));
    }
    private void fail() {
      ...
      throw ErrorMessage.wrap(new InvalidRequestException(msg),
                              tooLongStreamId);
    }
  }
  private static long discard(ByteBuf buffer, long remainingToDiscard) {
    int availableToDiscard =
        (int)Math.min(remainingToDiscard, buffer.readableBytes());
  ...
  public static class Encoder extends MessageToMessageEncoder<Frame> {
    public void encode(ChannelHandlerContext ctx, Frame frame,
                       List<Object> results) throws IOException {
      ByteBuf header = CBUtil.allocator.buffer(Header.LENGTH);
      Message.Type type = frame.header.type;
      header.writeByte(
          type.direction.addToVersion(frame.header.version.asInt()));
  ...
  public static class Decompressor extends MessageToMessageDecoder<Frame> {
    public void decode(ChannelHandlerContext ctx, Frame frame,
                       List<Object> results) throws IOException {
