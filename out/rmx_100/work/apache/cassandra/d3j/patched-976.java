...
<<<<<<< /root/work/apache/cassandra/d3j/patched-976-0.java
  public RowMutation dropFromSchema(long timestamp) {
    List<RowMutation> mutations = new ArrayList<RowMutation>(cfMetaData.size());
    RowMutation rm =
||||||| /root/work/apache/cassandra/d3j/base-976.java
...
=======
  public RowMutation dropFromSchema(long timestamp) {
    RowMutation rm =
>>>>>>> /root/work/apache/cassandra/d3j/patched-976-1.java
  ...
  public RowMutation toSchema(long timestamp) {
    ...
    cf.addColumn(
        Column.create(strategyClass.getName(), timestamp, "strategy_class"));
    cf.addColumn(
        Column.create(json(strategyOptions), timestamp, "strategy_options"));
    for (CFMetaData cfm : cfMetaData.values())
  ...
  public static KSMetaData fromSchema(Row row, Iterable<CFMetaData> cfms)
    ...
    UntypedResultSet.Row result =
        QueryProcessor.resultify("SELECT * FROM system.schema_keyspaces", row)
            .one();
    try {
      return new KSMetaData(result.getString("name"),
                            AbstractReplicationStrategy.getClass(
                                result.getString("strategy_class")),
                            fromJsonMap(result.getString("strategy_options")),
                            result.getBoolean("durable_writes"), cfms);
  ...
  public static Map<String, CFMetaData> deserializeColumnFamilies(Row row) {
    ...
    UntypedResultSet results = QueryProcessor.resultify(
        "SELECT * FROM system.schema_columnfamilies", row);
    ...
    for (CFMetaData cfm : cfms.values()) {
      ...
      for (ColumnDefinition cd :
           ColumnDefinition.fromSchema(columnRow, cfm.comparator))
