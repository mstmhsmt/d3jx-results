...
public class ActiveRepairService
  ...
  static {
    executor = new JMXConfigurableThreadPoolExecutor(
        ...
        new NamedThreadFactory("AntiEntropySessions"), "internal");
  }
  ...
  public RepairFuture submitRepairSession(UUID parentRepairSession,
                                          Range<Token> range, String keyspace,
    ...
    RepairSession session =
        new RepairSession(parentRepairSession, range, keyspace,
                          parallelismDegree, endpoints, cfnames);
  ...
  RepairFuture submitArtificialRepairSession(RepairJobDesc desc) {
    ...
    neighbours.addAll(ActiveRepairService.getNeighbors(
        desc.keyspace, keyspaceLocalRanges, desc.range, null, null));
    RepairSession session =
        new RepairSession(desc.parentSessionId, desc.sessionId, desc.range,
                          desc.keyspace, RepairParallelism.PARALLEL, neighbours,
  ...
  prepareForRepair(InetAddress coordinator, Set<InetAddress> endpoints,
    ...
    for (InetAddress neighbour : endpoints) {
      if (FailureDetector.instance.isAlive(neighbour)) {
        PrepareMessage message =
            new PrepareMessage(parentRepairSession, cfIds, ranges);
  ...
  private void cleanupOldParentRepairSessions() {
    ...
    for (Map.Entry<UUID, ParentRepairSession> entry :
      ...
      if (session.failed &&
          currentTime - session.repairedAt > TimeUnit.HOURS.toMillis(24))
  ...
  finishParentSession(UUID parentSession, Set<InetAddress> neighbors,
    ...
    for (InetAddress neighbor : neighbors) {
      AnticompactionTask task =
          new AnticompactionTask(parentSession, neighbor, doAntiCompaction);
      tasks.add(task);
      task.run();
    }
    if (doAntiCompaction) {
  ...
  public static class ParentRepairSession {
    public final Map<UUID, ColumnFamilyStore> columnFamilyStores =
        new HashMap<>();
      ...
      if (!marked.contains(cfId)) {
        List<SSTableReader> sstables =
            columnFamilyStores.get(cfId)
                .select(ColumnFamilyStore.UNREPAIRED_SSTABLES)
                .sstables;
