...
public class MetadataCollector implements PartitionStatisticsCollector {
  ...
  defaultTombstoneDropTimeHistogramBuilder() {
    return new StreamingHistogram.StreamingHistogramBuilder(
        SSTable.TOMBSTONE_HISTOGRAM_BIN_SIZE,
        SSTable.TOMBSTONE_HISTOGRAM_SPOOL_SIZE,
        SSTable.TOMBSTONE_HISTOGRAM_TTL_ROUND_SECONDS);
  }
  public static StatsMetadata defaultStatsMetadata() {
    return new StatsMetadata(
        defaultPartitionSizeHistogram(),
        defaultCellPerPartitionCountHistogram(), IntervalSet.empty(),
        Long.MIN_VALUE, Long.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE, 0,
        Integer.MAX_VALUE, NO_COMPRESSION_RATIO,
        defaultTombstoneDropTimeHistogramBuilder().build(), 0,
        Collections.<ByteBuffer>emptyList(),
        Collections.<ByteBuffer>emptyList(), true,
        ActiveRepairService.UNREPAIRED_SSTABLE, -1, -1);
  }
  ...
  protected final MinMaxIntTracker localDeletionTimeTracker =
      new MinMaxIntTracker(Cell.NO_DELETION_TIME, Cell.NO_DELETION_TIME);
  protected final MinMaxIntTracker ttlTracker =
      new MinMaxIntTracker(Cell.NO_TTL, Cell.NO_TTL);
  protected double compressionRatio = NO_COMPRESSION_RATIO;
  protected StreamingHistogram
      .StreamingHistogramBuilder estimatedTombstoneDropTime =
      defaultTombstoneDropTimeHistogramBuilder();
  protected int sstableLevel;
<<<<<<< /root/work/apache/cassandra/d3j/patched-330-0.java
  private ClusteringPrefix minClustering = ClusteringBound.TOP;
  private ClusteringPrefix maxClustering = ClusteringBound.BOTTOM;
||||||| /root/work/apache/cassandra/d3j/base-330.java
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-330-1.java
  ...
  public MetadataCollector(Iterable<SSTableReader> sstables,
                           ClusteringComparator comparator, int level) {
    this(comparator);
    IntervalSet.Builder<CommitLogPosition> intervals =
        new IntervalSet.Builder<>();
  ...
  public MetadataCollector addCompressionRatio(long compressed,
                                               long uncompressed) {
  ...
  public MetadataCollector updateClusteringValues(ClusteringPrefix clustering) {
    minClustering = minClustering == null ||
                            comparator.compare(clustering, minClustering) < 0
                        ? clustering
                        : minClustering;
    maxClustering = maxClustering == null ||
                            comparator.compare(clustering, maxClustering) > 0
                        ? clustering
                        : maxClustering;
    return this;
  }
  ...
  private static ByteBuffer max(ByteBuffer b1, ByteBuffer b2,
                                AbstractType<?> comparator) {
    if (b1 == null)
      return b2;
    if (b2 == null)
  ...
  public void updateHasLegacyCounterShards(boolean hasLegacyCounterShards) {
    this.hasLegacyCounterShards =
        this.hasLegacyCounterShards || hasLegacyCounterShards;
  }
  public Map<MetadataType, MetadataComponent>
  finalizeMetadata(String partitioner, double bloomFilterFPChance,
                   long repairedAt, SerializationHeader header) {
    Preconditions.checkState((minClustering == null && maxClustering == null) ||
                             comparator.compare(maxClustering, minClustering) >=
                                 0);
    ByteBuffer[] minValues =
        minClustering != null ? minClustering.getRawValues() : EMPTY_CLUSTERING;
    ByteBuffer[] maxValues =
        maxClustering != null ? maxClustering.getRawValues() : EMPTY_CLUSTERING;
    ...
    components.put(
        ...
        new StatsMetadata(
            ...
            commitLogIntervals, timestampTracker.min(), timestampTracker.max(),
            localDeletionTimeTracker.min(), localDeletionTimeTracker.max(),
            ttlTracker.min(), ttlTracker.max(), compressionRatio,
            estimatedTombstoneDropTime.build(), sstableLevel,
            makeList(minValues), makeList(maxValues), hasLegacyCounterShards,
            repairedAt, totalColumnsSet, totalRows));
    components.put(MetadataType.COMPACTION,
                   new CompactionMetadata(cardinality));
  ...
  public static class MinMaxLongTracker {
    ...
    private long min;
    private long max;
    public MinMaxLongTracker() { this(Long.MIN_VALUE, Long.MAX_VALUE); }
    ...
    public long min() { return isSet ? min : defaultMin; }
    public long max() { return isSet ? max : defaultMax; }
  }
  public static class MinMaxIntTracker {
    ...
    private int min;
    private int max;
    public MinMaxIntTracker() { this(Integer.MIN_VALUE, Integer.MAX_VALUE); }
    ...
    public int min() { return isSet ? min : defaultMin; }
    public int max() { return isSet ? max : defaultMax; }
  }
}