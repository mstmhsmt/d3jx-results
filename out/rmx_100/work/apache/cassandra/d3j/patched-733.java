...
class SecondaryIndexManager
{
   ...
   public static final Updater nullUpdater =
     new Updater()
     {
        ...
        public
        void update(Cell oldCell, Cell cell) {}
        public
        void remove(Cell current) {}
        public
   ...
   indexesByColumn;
   private final
   ConcurrentMap<Class<? extends SecondaryIndex>,SecondaryIndex>
   rowLevelIndexMap;
   ...
   SecondaryIndexManager(ColumnFamilyStore baseCfs) {
     ...
     allIndexes =
     Collections.newSetFromMap(new ConcurrentHashMap<SecondaryIndex,Boolean>());
   ...
   void reload() {
     ...
     for (ByteBuffer indexedColumn : indexedColumnNames) {
       ColumnDefinition def =
         baseCfs.metadata.getColumnDefinition(indexedColumn);
   ...
   public
   void maybeBuildSecondaryIndexes(Collection<SSTableReader> sstables, 
                                   Set<String> idxNames) {
     if (idxNames.isEmpty())
       return;
     ...
     SecondaryIndexBuilder builder =
       new SecondaryIndexBuilder(baseCfs, idxNames,
   ...
   public
   boolean indexes(CellName name, Set<SecondaryIndex> indexes) {
   ...
   public
   Set<SecondaryIndex> indexFor(CellName name, Set<SecondaryIndex> indexes) {
   ...
   public
   boolean indexes(Cell cell) {
     return indexes(cell.name());
   }
   public
   boolean indexes(CellName name) {
     return indexes(name, allIndexes);
   }
   public
   Set<SecondaryIndex> indexFor(CellName name) {
     return indexFor(name, allIndexes);
   }
   public
   boolean hasIndexFor(List<IndexExpression> clause) {
   ...
   public
   void removeIndexedColumn(ByteBuffer column) {
   ...
   public
   synchronized
   Future<?> addIndexedColumn(ColumnDefinition cdef) {
   ...
   public
   SecondaryIndex getIndexForColumn(ByteBuffer column) {
   ...
   public
   Set<ColumnFamilyStore> getIndexesBackedByCfs() {
   ...
   public
   Set<SecondaryIndex> getIndexesNotBackedByCfs() {
     Set<SecondaryIndex> indexes =
       Collections.newSetFromMap(new IdentityHashMap<SecondaryIndex,Boolean>());
     for (SecondaryIndex index : allIndexes)
       if (index.getIndexCfs() == null)
       indexes.add(index);
     return indexes;
   ...
   public
   void indexRow(ByteBuffer key, ColumnFamily cf, OpOrder.Group opGroup) {
     ...
     for (SecondaryIndex index : allIndexes) {
       if (index instanceof PerRowSecondaryIndex) {
         ...
         for (Cell cell : cf)
           if (cell.isLive(System.currentTimeMillis()) && index.indexes(
                                                            cell.name()))
   ...
   public
   void deleteFromIndexes(DecoratedKey key, List<Cell> indexedColumnsInRow, 
   ...
   public
   Updater updaterFor(DecoratedKey key, ColumnFamily cf, 
                      OpOrder.Group opGroup) {
     return (indexesByColumn.isEmpty() && rowLevelIndexMap.isEmpty()) ? nullUpdater : 
     new StandardUpdater(key, cf, opGroup);
   }
   public
   Updater gcUpdaterFor(DecoratedKey key) {
     return new GCUpdater(key);
   }
   public
   List<SecondaryIndexSearcher> getIndexSearchersForQuery(List<IndexExpression> clause) {
   ...
   public
   void validateIndexSearchersForQuery(List<IndexExpression> clause)
     throws InvalidRequestException {
     Map<String,Set<IndexExpression>> expressionsByIndexType =
       new HashMap<>();
   ...
   public
   Set<SecondaryIndex> getIndexesByNames(Set<String> idxNames) {
   ...
   public
   void setIndexBuilt(Set<String> idxNames) {
     for (SecondaryIndex index : getIndexesByNames(idxNames))
       index.setIndexBuilt();
   }
   public
   void setIndexRemoved(Set<String> idxNames) {
     for (SecondaryIndex index : getIndexesByNames(idxNames))
   ...
   static
   boolean shouldCleanupOldValue(Cell oldCell, Cell newCell) {
     return !oldCell.name().equals(newCell.name()) || !oldCell.value().equals(
                                                         newCell.value()) || 
   ...
   private
   final
   class GCUpdater implements Updater
   {
      private final DecoratedKey key;
      public
      GCUpdater(DecoratedKey key) {
        this.key = key;
      }
      public
      void insert(Cell cell) {
      ...
      public
      void update(Cell oldCell, Cell newCell) {
        throw new UnsupportedOperationException();
      }
      public
   ...
   private
   final
   class StandardUpdater implements Updater
   {
      private final DecoratedKey key;
      private final ColumnFamily cf;
      private final OpOrder.Group opGroup;
      public
      StandardUpdater(DecoratedKey key, ColumnFamily cf, 
                      OpOrder.Group opGroup) {
        this.key = key;
        this.cf = cf;
      ...
      void insert(Cell cell) {
        ...
        for (SecondaryIndex index : indexFor(cell.name()))
          if (index instanceof PerColumnSecondaryIndex)
          ((PerColumnSecondaryIndex)index).insert(key.getKey(), cell,
            opGroup);
      }
      public
      void update(Cell oldCell, Cell cell) {
        ...
        for (SecondaryIndex index : indexFor(cell.name())) {
          if (index instanceof PerColumnSecondaryIndex) {
            if (cell.isLive()) {
              ((PerColumnSecondaryIndex)index).update(key.getKey(), oldCell,
                cell, opGroup);
            } else {
