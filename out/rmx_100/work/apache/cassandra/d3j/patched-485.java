...
public class ActiveRepairService
  ...
  public ActiveRepairService(IFailureDetector failureDetector,
                             Gossiper gossiper) {
    this.failureDetector = failureDetector;
    this.gossiper = gossiper;
  }
  public RepairSession
  submitRepairSession(UUID parentRepairSession, Range<Token> range,
                      String keyspace, RepairParallelism parallelismDegree,
                      Set<InetAddress> endpoints, long repairedAt,
                      ListeningExecutorService executor, String... cfnames) {
    if (endpoints.isEmpty())
      return null;
    if (cfnames.length == 0)
      return null;
    final RepairSession session = new RepairSession(
        parentRepairSession, UUIDGen.getTimeUUID(), range, keyspace,
  ...
  public static Set<InetAddress>
  getNeighbors(String keyspaceName,
    ...
    for (Range<Token> range : keyspaceLocalRanges) {
      if (range.contains(toRepair)) {
        ...
        throw new IllegalArgumentException(
            "Requested range intersects a local range but is not fully " +
    ...
    if (dataCenters != null && !dataCenters.isEmpty()) {
      ...
      if (specifiedHost.size() <= 1) {
        String msg =
            "Repair requires at least two endpoints that are neighbours " +
            "before it can continue, the endpoint used for this repair is %s, "
            +
            "other available neighbours are %s but these neighbours were not " +
            "part of the supplied list of hosts to use during the repair (%s).";
        throw new IllegalArgumentException(
            String.format(msg, specifiedHost, neighbors, hosts));
      }
  ...
  prepareForRepair(UUID parentRepairSession, InetAddress coordinator,
                   Set<InetAddress> endpoints, RepairOption options,
                   List<ColumnFamilyStore> columnFamilyStores) {
    registerParentRepairSession(parentRepairSession, coordinator,
                                columnFamilyStores, options.getRanges(),
                                options.isIncremental(), options.isGlobal());
    final CountDownLatch prepareLatch = new CountDownLatch(endpoints.size());
    final AtomicBoolean status = new AtomicBoolean(true);
    final Set<String> failedNodes =
        Collections.synchronizedSet(new HashSet<String>());
    IAsyncCallbackWithFailure callback = new IAsyncCallbackWithFailure() {
      public void response(MessageIn msg) { prepareLatch.countDown(); }
    ...
    for (InetAddress neighbour : endpoints) {
      if (FailureDetector.instance.isAlive(neighbour)) {
        ...
        boolean isGlobal =
            options.isGlobal() && peerVersion != null &&
            peerVersion.compareTo(SUPPORTS_GLOBAL_PREPARE_FLAG_VERSION) >= 0;
        logger.debug(
            "Sending prepare message: options.isGlobal = {}, peerVersion = {}",
            options.isGlobal(), peerVersion);
        PrepareMessage message =
            new PrepareMessage(parentRepairSession, cfIds, options.getRanges(),
    ...
    try {
      ...
      throw new RuntimeException("Did not get replies from all endpoints. " +
                                 "List of failed endpoint(s): " +
                                     failedNodes.toString(),
                                 e);
    }
    if (!status.get()) {
      removeParentRepairSession(parentRepairSession);
  ...
  public void
  registerParentRepairSession(UUID parentRepairSession, InetAddress coordinator,
                              List<ColumnFamilyStore> columnFamilyStores,
    ...
    parentRepairSessions.put(
        parentRepairSession,
        new ParentRepairSession(coordinator, columnFamilyStores, ranges,
                                isIncremental, isGlobal,
                                System.currentTimeMillis()));
  }
  public Set<SSTableReader> currentlyRepairing(UUID cfId,
  ...
  finishParentSession(UUID parentSession, Set<InetAddress> neighbors,
                      Collection<Range<Token>> successfulRanges) {
    List<ListenableFuture<?>> tasks = new ArrayList<>(neighbors.size() + 1);
    for (InetAddress neighbor : neighbors) {
      AnticompactionTask task =
          new AnticompactionTask(parentSession, neighbor, successfulRanges);
      tasks.add(task);
      task.run();
    }
  ...
  public ParentRepairSession getParentRepairSession(UUID parentSessionId) {
    ...
    if (session == null)
      throw new RuntimeException("Parent repair session with id = " +
                                 parentSessionId + " has failed.");
    return session;
  }
  public synchronized ParentRepairSession
  removeParentRepairSession(UUID parentSessionId) {
  ...
  public ListenableFuture<List<Object>>
  doAntiCompaction(final UUID parentRepairSession,
    ...
    assert prs.ranges.containsAll(successfulRanges)
        : "Trying to perform anticompaction on unknown ranges";
    List<ListenableFuture<?>> futures = new ArrayList<>();
    if (!successfulRanges.isEmpty()) {
      for (Map.Entry<UUID, ColumnFamilyStore> columnFamilyStoreEntry :
           prs.columnFamilyStores.entrySet()) {
        Refs<SSTableReader> sstables =
            prs.getActiveRepairedSSTableRefsForAntiCompaction(
                columnFamilyStoreEntry.getKey(), parentRepairSession);
        ColumnFamilyStore cfs = columnFamilyStoreEntry.getValue();
        futures.add(CompactionManager.instance.submitAntiCompaction(
            cfs, successfulRanges, sstables, prs.repairedAt));
      }
    }
  ...
  public static class ParentRepairSession {
    private final Map<UUID, ColumnFamilyStore> columnFamilyStores =
        new HashMap<>();
    ...
    public synchronized void markSSTablesRepairing(UUID cfId,
                                                   UUID parentSessionId) {
      if (!marked.contains(cfId)) {
        List<SSTableReader> sstables =
            columnFamilyStores.get(cfId)
                .select(isIncremental ? ColumnFamilyStore.UNREPAIRED_SSTABLES
                                      : ColumnFamilyStore.CANONICAL_SSTABLES)
                .sstables;
        Set<SSTableReader> currentlyRepairing =
            ActiveRepairService.instance.currentlyRepairing(cfId,
                                                            parentSessionId);
        if (!Sets.intersection(currentlyRepairing, Sets.newHashSet(sstables))
                 .isEmpty()) {
          logger.error(
    ...
    getActiveRepairedSSTableRefsForAntiCompaction(UUID cfId,
      ...
      boolean isSnapshotRepair = columnFamilyStores.get(cfId).snapshotExists(
          parentSessionId.toString());
      ImmutableMap.Builder<SSTableReader, Ref<SSTableReader>> references =
          ImmutableMap.builder();
<<<<<<< /root/work/apache/cassandra/d3j/patched-485-0.java
      Iterable<SSTableReader> sstables = getActiveSSTables(cfId);
      if (sstables == null)
        throw new RuntimeException(
            "Not possible to get sstables for anticompaction for " + cfId);
      for (SSTableReader sstable :
           isSnapshotRepair
               ? getSSTablesForSnapshotRepair(cfId, parentSessionId)
               : getActiveSSTables(cfId)) {
||||||| /root/work/apache/cassandra/d3j/base-485.java
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-485-1.java
    ...
    getSSTablesForSnapshotRepair(UUID cfId, UUID parentSessionId) {
    ...
    public synchronized void maybeSnapshot(UUID cfId, UUID parentSessionId) {
      ...
      if (!columnFamilyStores.get(cfId).snapshotExists(snapshotName)) {
        Set<SSTableReader> snapshottedSSTables =
            columnFamilyStores.get(cfId).snapshot(
                snapshotName, new Predicate<SSTableReader>() {
                  public boolean apply(SSTableReader sstable) {
                    return sstable != null &&
                        (!isIncremental || !sstable.isRepaired()) &&
                        !(sstable.partitioner instanceof LocalPartitioner) &&
        ...
        if (isAlreadyRepairing(cfId, parentSessionId, snapshottedSSTables)) {
          columnFamilyStores.get(cfId).clearSnapshot(
              parentSessionId.toString());
          logger.error(
              "Cannot start multiple repair sessions over the same sstables");
          throw new RuntimeException(
    ...
    public String toString() {
      return "ParentRepairSession{"
          + "columnFamilyStores=" + columnFamilyStores + ", ranges=" + ranges +
          ", sstableMap=" + sstableMap + ", repairedAt=" + repairedAt + '}';
    }
  }
