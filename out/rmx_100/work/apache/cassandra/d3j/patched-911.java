...
public class OutboundTcpConnection extends Thread {
  ...
  private static boolean isLocalDC(InetAddress targetHost) {
    ...
    String localDC = DatabaseDescriptor.getEndpointSnitch().getDatacenter(
  ...
  void softCloseSocket() { enqueue(CLOSE_SENTINEL, null); }
  ...
  public void run() {
    while (true) {
      ...
      if (qm.timestamp < System.currentTimeMillis() - entry.getTimeout())
  ...
  public int getPendingMessages() { return active.size() + backlog.size(); }
  public long getCompletedMesssages() { return completed; }
  public long getDroppedMessages() { return dropped.get(); }
<<<<<<< /root/work/apache/cassandra/d3j/patched-911-0.java
  private boolean shouldCompressConnection() {
    return DatabaseDescriptor.internodeCompression() ==
        ...
        (DatabaseDescriptor.internodeCompression() ==
         ...
         !isLocalDC(poolReference.endPoint()));
  }
  private void writeConnected(QueuedMessage qm, Entry entry) {
    Message message = entry.message;
||||||| /root/work/apache/cassandra/d3j/base-911.java
...
=======
  ...
  private void writeConnected(Entry entry) {
    Message message = entry.message;
>>>>>>> /root/work/apache/cassandra/d3j/patched-911-1.java
    try {
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-911-1.java
  ...
  public static void write(MessageOut message, String id, long timestamp,
                           DataOutputStream out, int version)
      throws IOException {
  ...
  private static void writeHeader(DataOutputStream out, int version,
    ...
    if (socket != null) {
      try {
        ...
        if (logger.isTraceEnabled())
          logger.trace(
              "exception closing connection to " + poolReference.endPoint(), e);
      }
      out = null;
      socket = null;
    }
  ...
  private boolean connect() {
    ...
    while (System.currentTimeMillis() <
           start + DatabaseDescriptor.getRpcTimeout()) {
      try {
        ...
        if (DatabaseDescriptor.getInternodeSendBufferSize() != null) {
          try {
            socket.setSendBufferSize(
                DatabaseDescriptor.getInternodeSendBufferSize().intValue());
          } catch (SocketException se) {
            logger.warn("Failed to set send buffer size on internode socket.",
                        se);
          }
        }
        ...
        if (targetVersion >= MessagingService.VERSION_12) {
          ...
          if (targetVersion > maxTargetVersion) {
            logger.debug(
                "Target max version is {}; will reconnect with that version",
                maxTargetVersion);
          ...
          if (targetVersion < maxTargetVersion &&
              targetVersion < MessagingService.current_version) {
            logger.trace("Detected higher max version {} (using {}); will " +
                         "reconnect when queued messages are done",
          ...
          CompactEndpointSerializationHelper.serialize(
              FBUtilities.getBroadcastAddress(), out);
          if (shouldCompressConnection()) {
            out.flush();
            logger.trace("Upgrading OutputStream to be compressed");
            out = new DataOutputStream(new SnappyOutputStream(
  ...
  private void expireMessages() {
    while (true) {
      ...
      if (qm == null ||
          qm.timestamp >= System.currentTimeMillis() - qm.message.getTimeout())
        break;
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-911-0.java
  ...
  private static class RetriedEntry extends Entry {
    RetriedEntry(Entry e) { super(e.message, e.id); }
    boolean shouldRetry() { return false; }
  }
  private static class RetriedQueuedMessage extends QueuedMessage {
    RetriedQueuedMessage(QueuedMessage msg) { super(msg.message, msg.id); }
    boolean shouldRetry() { return false; }
||||||| /root/work/apache/cassandra/d3j/base-911.java
=======
    boolean shouldRetry() {
