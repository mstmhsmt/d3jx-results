...
public class SelectStatement implements CQLStatement {
  ...
  private static final Parameters defaultParameters = new Parameters(
      Collections.emptyMap(), Collections.emptyList(), false, false, false);
  public SelectStatement(TableMetadata table, int boundTerms,
                         Parameters parameters, Selection selection,
                         StatementRestrictions restrictions, boolean isReversed,
                         AggregationSpecification aggregationSpec,
                         Comparator<List<ByteBuffer>> orderingComparator,
                         Term limit, Term perPartitionLimit) {
    this.table = table;
  ...
  static SelectStatement forSelection(TableMetadata table,
                                      Selection selection) {
    return new SelectStatement(
        table, 0, defaultParameters, selection,
        StatementRestrictions.empty(StatementType.SELECT, table), false, null,
        null, null, null);
  }
  public ResultSet.ResultMetadata getResultMetadata() {
    return selection.getResultMetadata();
  }
  public int getBoundTerms() { return boundTerms; }
  public void checkAccess(ClientState state)
      throws InvalidRequestException, UnauthorizedException {
    if (table.isView()) {
      TableMetadataRef baseTable =
          View.findBaseTable(keyspace(), columnFamily());
      if (baseTable != null)
  ...
  public ResultMessage.Rows execute(QueryState state, QueryOptions options,
    ...
    ReadQuery query = getQuery(options, selectors.getColumnFilter(), nowInSec,
                               userLimit, userPerPartitionLimit, pageSize);
    if (aggregationSpec == null &&
        (pageSize <= 0 || (query.limits().count() <= pageSize)))
    ...
    return execute(Pager.forDistributedQuery(pager, cl, state.getClientState()),
                   options, selectors, pageSize, nowInSec, userLimit,
                   queryStartNanoTime);
  }
  public ReadQuery getQuery(QueryOptions options, int nowInSec)
      throws RequestValidationException {
    Selectors selectors = selection.newSelectors(options);
    return getQuery(options, selectors.getColumnFilter(), nowInSec,
                    getLimit(options), getPerPartitionLimit(options),
                    options.getPageSize());
  }
  public ReadQuery getQuery(QueryOptions options, ColumnFilter columnFilter,
                            int nowInSec, int userLimit, int perPartitionLimit,
                            int pageSize) {
    boolean isPartitionRangeQuery =
        restrictions.isKeyRange() || restrictions.usesSecondaryIndexing();
  ...
  private ResultMessage.Rows execute(ReadQuery query, QueryOptions options,
                                     QueryState state, Selectors selectors,
                                     int nowInSec, int userLimit,
                                     long queryStartNanoTime)
      throws RequestValidationException, RequestExecutionException {
    try (PartitionIterator data =
             query.execute(options.getConsistency(), state.getClientState(),
                           queryStartNanoTime)) {
  ...
  private static abstract class Pager {
    ...
    protected Pager(QueryPager pager) { this.pager = pager; }
    public static Pager
    forInternalQuery(QueryPager pager,
                     ReadExecutionController executionController) {
      return new InternalPager(pager, executionController);
    }
    public static Pager forDistributedQuery(QueryPager pager,
                                            ConsistencyLevel consistency,
                                            ClientState clientState) {
      return new NormalPager(pager, consistency, clientState);
    }
    public boolean isExhausted() { return pager.isExhausted(); }
    public PagingState state() { return pager.state(); }
  ...
  private ResultMessage.Rows execute(Pager pager, QueryOptions options,
                                     Selectors selectors, int pageSize,
                                     int nowInSec, int userLimit,
                                     long queryStartNanoTime)
      throws RequestValidationException, RequestExecutionException {
    if (aggregationSpec != null) {
      if (!restrictions.hasPartitionKeyRestrictions()) {
        ...
        warn("Aggregation query used on multiple partition keys (IN " +
             "restriction)");
      }
    }
    checkFalse(pageSize > 0 && needsPostQueryOrdering(),
               "Cannot page queries with both ORDER BY and a IN restriction " +
               "on the partition key;"
                   + " you must either remove the ORDER BY or the IN and " +
                     "sort client side, or disable paging for this query");
    ResultMessage.Rows msg;
    try (PartitionIterator page =
  ...
  private ResultMessage.Rows processResults(PartitionIterator partitions,
                                            ...
                                            Selectors selectors, int nowInSec,
                                            int userLimit)
      throws RequestValidationException {
    ResultSet rset =
        process(partitions, options, selectors, nowInSec, userLimit);
  ...
  public ResultMessage.Rows executeInternal(QueryState state,
                                            QueryOptions options, int nowInSec,
                                            long queryStartNanoTime)
      throws RequestExecutionException, RequestValidationException {
    int userLimit = getLimit(options);
    int userPerPartitionLimit = getPerPartitionLimit(options);
  ...
  private QueryPager getPager(ReadQuery query, QueryOptions options) {
    QueryPager pager =
        query.getPager(options.getPagingState(), options.getProtocolVersion());
  ...
  public String keyspace() { return table.keyspace; }
  public String columnFamily() { return table.name; }
  public Selection getSelection() { return selection; }
  public StatementRestrictions getRestrictions() { return restrictions; }
  private ReadQuery getSliceCommands(QueryOptions options,
    ...
    for (ByteBuffer key : keys) {
      ...
      commands.add(SinglePartitionReadCommand.create(
          table, nowInSec, columnFilter, rowFilter, limit, dk, filter));
    }
  ...
  public SinglePartitionReadCommand internalReadForView(DecoratedKey key,
    ...
    return SinglePartitionReadCommand.create(
        table, nowInSec, columnFilter, rowFilter, DataLimits.NONE, key, filter);
  }
  ...
  private ReadQuery getRangeCommand(QueryOptions options,
                                    ColumnFilter columnFilter, DataLimits limit,
                                    int nowInSec) {
    ...
    PartitionRangeReadCommand command = PartitionRangeReadCommand.create(
        table, nowInSec, columnFilter, rowFilter, limit,
        new DataRange(keyBounds, clusteringIndexFilter));
    command.maybeValidateIndex();
    return command;
  }
  private ClusteringIndexFilter
  makeClusteringIndexFilter(QueryOptions options, ColumnFilter columnFilter) {
    ...
    if (clusterings.isEmpty() &&
        columnFilter.fetchedColumns().statics.isEmpty())
      return null;
  ...
  private Slices makeSlices(QueryOptions options)
    ...
    Slices.Builder builder =
        new Slices.Builder(table.comparator, startBounds.size());
  ...
  private DataLimits getDataLimits(int userLimit, int perPartitionLimit,
    ...
    if (aggregationSpec != null &&
        aggregationSpec != AggregationSpecification.AGGREGATE_EVERYTHING) {
      if (parameters.isDistinct)
        return DataLimits.distinctLimits(cqlRowLimit);
      return DataLimits.groupByLimits(cqlRowLimit, cqlPerPartitionLimit,
                                      pageSize, aggregationSpec);
    }
    if (parameters.isDistinct)
      return cqlRowLimit == DataLimits.NO_LIMIT
          ? DataLimits.DISTINCT_NONE
          : DataLimits.distinctLimits(cqlRowLimit);
  ...
  private int getLimit(Term limit, QueryOptions options) {
    ...
    if (limit != null) {
      ByteBuffer b = checkNotNull(limit.bindAndGet(options),
  ...
  public RowFilter getRowFilter(QueryOptions options)
      throws InvalidRequestException {
    ColumnFamilyStore cfs =
        Keyspace.open(keyspace()).getColumnFamilyStore(columnFamily());
    SecondaryIndexManager secondaryIndexManager = cfs.indexManager;
    RowFilter filter =
        restrictions.getRowFilter(secondaryIndexManager, options);
    return filter;
  }
  private ResultSet process(PartitionIterator partitions, QueryOptions options,
                            Selectors selectors, int nowInSec, int userLimit)
      throws InvalidRequestException {
    GroupMaker groupMaker =
        aggregationSpec == null ? null : aggregationSpec.newGroupMaker();
    ResultSetBuilder result =
        new ResultSetBuilder(getResultMetadata(), selectors, groupMaker);
  ...
  void processPartition(RowIterator partition, QueryOptions options,
                        ResultSetBuilder result, int nowInSec)
      throws InvalidRequestException {
  ...
  private static void addValue(ResultSetBuilder result, ColumnMetadata def,
                               Row row, int nowInSec,
    ...
    if (def.isComplex()) {
      ...
      if (complexData == null)
        ...
        result.add(((CollectionType)def.type)
                       .serializeForNativeProtocol(complexData.iterator(),
                                                   protocolVersion));
      else
        result.add(((UserType)def.type)
                       .serializeForNativeProtocol(complexData.iterator(),
                                                   protocolVersion));
    } else {
  ...
  public static class RawStatement extends CFStatement {
    ...
    public RawStatement(CFName cfName, Parameters parameters,
                        List<RawSelector> selectClause, WhereClause whereClause,
                        Term.Raw limit, Term.Raw perPartitionLimit) {
      super(cfName);
      this.parameters = parameters;
    ...
    public ParsedStatement.Prepared prepare(boolean forView)
        throws InvalidRequestException {
      TableMetadata table =
      ...
      boolean containsOnlyStaticColumns =
          selectOnlyStaticColumns(table, selectables);
      StatementRestrictions restrictions = prepareRestrictions(
          table, boundNames, containsOnlyStaticColumns, forView);
      ...
      Selection selection =
          prepareSelection(table, selectables, boundNames,
                           resultSetOrderingColumns, restrictions);
      if (parameters.isDistinct) {
        checkNull(
            perPartitionLimit,
      ...
      AggregationSpecification aggregationSpec = getAggregationSpecification(
          table, selection, restrictions, parameters.isDistinct);
      checkFalse(aggregationSpec ==
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-343-0.java
        orderingComparator = getOrderingComparator(
            table, selection, restrictions, orderingColumns);
        isReversed = isReversed(table, orderingColumns, restrictions);
||||||| /root/work/apache/cassandra/d3j/base-343.java
        orderingComparator =
            getOrderingComparator(cfm, selection, restrictions);
        isReversed = isReversed(cfm);
=======
        orderingComparator = getOrderingComparator(
            table, selection, restrictions, parameters.isJson);
        isReversed = isReversed(table, orderingColumns, restrictions);
>>>>>>> /root/work/apache/cassandra/d3j/patched-343-1.java
        if (isReversed)
          orderingComparator = Collections.reverseOrder(orderingComparator);
      }
      checkNeedsFiltering(restrictions);
      SelectStatement stmt = new SelectStatement(
          table, boundNames.size(), parameters, selection, restrictions,
          isReversed, aggregationSpec, orderingComparator,
      ...
      return new ParsedStatement.Prepared(
          stmt, boundNames, boundNames.getPartitionKeyBindIndexes(table));
    }
    private Selection
    prepareSelection(TableMetadata table, List<Selectable> selectables,
                     VariableSpecifications boundNames,
                     Set<ColumnMetadata> resultSetOrderingColumns,
                     StatementRestrictions restrictions) {
      boolean hasGroupBy = !parameters.groups.isEmpty();
      if (selectables.isEmpty()) {
        return hasGroupBy ? Selection.wildcardWithGroupBy(table, boundNames,
                                                          parameters.isJson)
                          : Selection.wildcard(table, parameters.isJson);
      }
      return Selection.fromSelectors(table, selectables, boundNames,
                                     resultSetOrderingColumns,
                                     restrictions.nonPKRestrictedColumns(false),
                                     hasGroupBy, parameters.isJson);
    }
    private boolean selectOnlyStaticColumns(TableMetadata table,
      ...
      if (table.isStaticCompactTable() || !table.hasStaticColumns() ||
          selectables.isEmpty())
        return false;
      return Selectable.selectColumns(selectables,
                                      (column) -> column.isStatic()) &&
          !Selectable.selectColumns(
              selectables,
              (column) -> !column.isPartitionKey() && !column.isStatic());
    }
    private Map<ColumnMetadata, Boolean>
    getOrderingColumns(TableMetadata table) {
    ...
    private StatementRestrictions
    prepareRestrictions(TableMetadata metadata,
                        VariableSpecifications boundNames,

                        boolean selectsOnlyStaticColumns, boolean forView)
        throws InvalidRequestException {
      return new StatementRestrictions(
          StatementType.SELECT, metadata, whereClause, boundNames,
          selectsOnlyStaticColumns, parameters.allowFiltering, forView);
    }
    ...
    private static void
    verifyOrderingIsAllowed(StatementRestrictions restrictions)
        throws InvalidRequestException {
      checkFalse(restrictions.usesSecondaryIndexing(),
                 "ORDER BY with 2ndary indexes is not supported.");
      checkFalse(restrictions.isKeyRange(),
                 "ORDER BY is only supported when the partition key is " +
                 "restricted by an EQ or an IN.");
    }
    private static void
    validateDistinctSelection(TableMetadata metadata, Selection selection,
                              StatementRestrictions restrictions)
        throws InvalidRequestException {
      checkFalse(
          restrictions.hasClusteringColumnsRestrictions() ||
              (restrictions.hasNonPrimaryKeyRestrictions() &&
               !restrictions.nonPKRestrictedColumns(true).stream().allMatch(
                   ColumnMetadata::isStatic)),
          "SELECT DISTINCT with WHERE clause only supports restriction by " +
          "partition key and/or static columns.");
      ...
      for (ColumnMetadata def : requestedColumns)
        checkFalse(!def.isPartitionKey() && !def.isStatic(),
                   "SELECT DISTINCT queries must only request partition key " +
                   "columns and/or static columns (not %s)",
                   def.name);
      if (!restrictions.isKeyRange())
        return;
      for (ColumnMetadata def : metadata.partitionKeyColumns())
        checkTrue(requestedColumns.contains(def),
                  "SELECT DISTINCT queries must request all the partition " +
                  "key columns (missing %s)",
                  def.name);
    }
    ...
    getAggregationSpecification(TableMetadata metadata,
      ...
      for (ColumnMetadata.Raw raw : parameters.groups) {
        ...
        checkTrue(def.isPartitionKey() || def.isClusteringColumn(),
                  "Group by is currently only supported on the columns of " +
                  "the PRIMARY KEY, got %s",
                  def.name);
        while (true) {
          checkTrue(pkColumns.hasNext(),
                    "Group by currently only support groups of columns " +
      ...
      checkFalse(
          pkColumns.hasNext() && pkColumns.next().isPartitionKey(),
          "Group by is not supported on only a part of the partition key");
      checkFalse(clusteringPrefixSize > 0 && isDistinct,
                 "Grouping on clustering columns is not allowed for SELECT " +
                 "DISTINCT queries");
      return AggregationSpecification.aggregatePkPrefix(metadata.comparator,
                                                        clusteringPrefixSize);
    }
    private Comparator<List<ByteBuffer>>
<<<<<<< /root/work/apache/cassandra/d3j/patched-343-0.java
    getOrderingComparator(TableMetadata metadata, Selection selection,
                          StatementRestrictions restrictions,

                          Map<ColumnMetadata, Boolean> orderingColumns)
||||||| /root/work/apache/cassandra/d3j/base-343.java
    getOrderingComparator(CFMetaData cfm, Selection selection,
                          StatementRestrictions restrictions)
=======
    getOrderingComparator(TableMetadata metadata, Selection selection,
                          StatementRestrictions restrictions, boolean isJson)
>>>>>>> /root/work/apache/cassandra/d3j/patched-343-1.java
        throws InvalidRequestException {
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-343-0.java
      Map<ColumnDefinition, Integer> orderingIndexes =
          getOrderingIndex(metadata, selection, orderingColumns);
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-343-1.java
    ...
    private boolean isReversed(TableMetadata table,
                               Map<ColumnMetadata, Boolean> orderingColumns,
                               StatementRestrictions restrictions)
        throws InvalidRequestException {
      Boolean[] reversedMap = new Boolean[table.clusteringColumns().size()];
      int i = 0;
      for (Map.Entry<ColumnMetadata, Boolean> entry :
           orderingColumns.entrySet()) {
        ColumnMetadata def = entry.getKey();
        boolean reversed = entry.getValue();
        checkTrue(def.isClusteringColumn(),
                  "Order by is currently only supported on the clustered " +
                  "columns of the PRIMARY KEY, got %s",
                  def.name);
        while (i != def.position()) {
          checkTrue(
              restrictions.isColumnRestrictedByEq(
                  table.clusteringColumns().get(i++)),
              "Order by currently only supports the ordering of columns " +
              "following their declared order in the PRIMARY KEY");
        }
        i++;
  ...
  public static class Parameters {
    ...
    public Parameters(Map<ColumnMetadata.Raw, Boolean> orderings,
                      List<ColumnMetadata.Raw> groups, boolean isDistinct,
                      boolean allowFiltering, boolean isJson) {
  ...
  private static abstract class ColumnComparator<T> implements Comparator<T> {
    protected final int compare(Comparator<ByteBuffer> comparator,
                                ByteBuffer aValue, ByteBuffer bValue) {
      if (aValue == null)
  ...
  private static class SingleColumnComparator
    ...
    public SingleColumnComparator(int columnIndex,
                                  Comparator<ByteBuffer> orderer) {
      index = columnIndex;
      comparator = orderer;
    }
  ...
  private static class CompositeComparator
    ...
    private CompositeComparator(List<Comparator<ByteBuffer>> orderTypes,
