...
public
class CassandraAuthorizer implements IAuthorizer
{
   private static final Logger logger =
     LoggerFactory.getLogger(CassandraAuthorizer.class);
   private static final String USERNAME = "username";
   private static final String RESOURCE = "resource";
   private static final String PERMISSIONS = "permissions";
   private static final String PERMISSIONS_CF = "permissions";
   private static final String PERMISSIONS_CF_SCHEMA =
     String.format("CREATE TABLE %s.%s (" + "username text," + "resource text," + "permissions set<text>," + "PRIMARY KEY(username, resource)" + ") WITH gc_grace_seconds=%d",
       Auth.AUTH_KS, PERMISSIONS_CF, 90 * 24 * 60 * 60);
   ...
   Set<Permission> authorize(AuthenticatedUser user, IResource resource) {
     ...
     try {
       ResultMessage.Rows rows =
         authorizeStatement.execute(QueryState.forInternalCalls(),
       ...
       logger.warn("CassandraAuthorizer failed to authorize {} for {}", user,
         resource);
   ...
   public
   void grant(AuthenticatedUser performer, Set<Permission> permissions, 
              IResource resource, String to)
     throws RequestExecutionException {
     modify(permissions, resource, to, "+");
   }
   public
   void revoke(AuthenticatedUser performer, Set<Permission> permissions, 
               IResource resource, String from)
     throws RequestExecutionException {
     modify(permissions, resource, from, "-");
   }
   private
   void modify(Set<Permission> permissions, IResource resource, String user, 
               String op)
     throws RequestExecutionException {
     process(String.format("UPDATE %s.%s SET permissions = permissions %s {%s} WHERE username = '%s' AND resource = '%s'",
               Auth.AUTH_KS, PERMISSIONS_CF, op,
               "'" + StringUtils.join(permissions, "','") + "'",
               escape(user), escape(resource.getName())));
   }
   public
   Set<PermissionDetails> list(AuthenticatedUser performer, 
   ...
   private
   static
   String buildListQuery(IResource resource, String of) {
   ...
   void revokeAll(IResource droppedResource) {
     ...
     try {
       rows =
       process(String.format("SELECT username FROM %s.%s WHERE resource = '%s' ALLOW FILTERING",
   ...
   public
   Set<DataResource> protectedResources() {
     return ImmutableSet.of(DataResource.columnFamily(Auth.AUTH_KS,
                              PERMISSIONS_CF));
   }
   public
   ...
   void setup() {
     ...
     try {
       String query =
         String.format("SELECT permissions FROM %s.%s WHERE username = ? AND resource = ?",
           Auth.AUTH_KS, PERMISSIONS_CF);
       authorizeStatement =
