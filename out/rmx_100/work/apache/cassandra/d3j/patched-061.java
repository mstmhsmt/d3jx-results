...
public
class CompactionController extends AbstractCompactionController
{
   ...
   protected
   CompactionController(ColumnFamilyStore cfs, int maxValue) {
     this(cfs,
     null,
     maxValue);
   }
   public
   CompactionController(ColumnFamilyStore cfs, Set<SSTableReader> compacting, 
                        int gcBefore) {
     this(cfs,
     compacting,
     gcBefore,
   ...
   public
   CompactionController(ColumnFamilyStore cfs, Set<SSTableReader> compacting, 
                        int gcBefore, RateLimiter limiter, 
     ...
     compactingRepaired =
     compacting != null && compacting.stream().allMatch(SSTableReader::isRepaired);
     this.minTimestamp =
     compacting != null && !compacting.isEmpty() ? compacting.stream().mapToLong(
                                                     SSTableReader::getMinTimestamp).min().getAsLong() : 0;
     refreshOverlaps();
   ...
   void maybeRefreshOverlaps() {
     ...
     if (cfs.getNeverPurgeTombstones()) {
       logger.debug("not refreshing overlaps for {}.{} - neverPurgeTombstones is enabled",
   ...
   void refreshOverlaps() {
     ...
     if (compacting == null)
       overlappingSSTables =
       Refs.tryRef(Collections.<SSTableReader>emptyList()); else
       overlappingSSTables =
       cfs.getAndReferenceOverlappingLiveSSTables(compacting);
     this.overlapIterator =
     new OverlapIterator<>(buildIntervals(overlappingSSTables));
   }
   public
   Set<SSTableReader> getFullyExpiredSSTables() {
     return getFullyExpiredSSTables(cfs, compacting, overlappingSSTables,
              gcBefore, ignoreOverlaps());
   }
   public
   static
   Set<SSTableReader> getFullyExpiredSSTables(ColumnFamilyStore cfStore, 
   ...
   @Override
   public
   LongPredicate getPurgeEvaluator(DecoratedKey key) {
     ...
     for (SSTableReader sstable : filteredSSTables) {
       ...
       sstable.getBloomFilter().isPresent(key)) {
         minTimestampSeen =
         Math.min(minTimestampSeen, sstable.getMinTimestamp());
         hasTimestamp = true;
       }
     }
     for (Memtable memtable : memtables) {
       if (memtable.getMinTimestamp() != Memtable.NO_MIN_TIMESTAMP) {
         ...
         if (partition != null) {
           minTimestampSeen =
           Math.min(minTimestampSeen, partition.stats().minTimestamp);
           hasTimestamp = true;
         }
       }
     }
     if (!hasTimestamp)
       return time -> true; else {
   ...
   public
   Iterable<UnfilteredRowIterator> shadowSources(DecoratedKey key, 
     ...
     return Iterables.filter(Iterables.transform(overlapIterator.overlaps(),
                               reader -> getShadowIterator(reader, key,
                                           tombstoneOnly)),
              Predicates.notNull());
   }
   @SuppressWarnings("resource")
   private
   UnfilteredRowIterator getShadowIterator(SSTableReader reader, 
                                           DecoratedKey key, 
                                           boolean tombstoneOnly) {
     if (reader.isMarkedSuspect() || reader.getMaxTimestamp() <= minTimestamp || tombstoneOnly && !
     reader.mayHaveTombstones())
       return null;
     RowIndexEntry<?> position =
       reader.getPosition(key, SSTableReader.Operator.EQ);
     if (position == null)
       return null;
     FileDataInput dfile =
       openDataFiles.computeIfAbsent(reader, this::openDataFile);
   ...
   private
   FileDataInput openDataFile(SSTableReader reader) {
