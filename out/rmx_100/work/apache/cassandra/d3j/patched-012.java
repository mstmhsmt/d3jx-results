  ...
  implements CQLMessageHandler.MessageConsumer<Message.Request>
{
   ...
   @VisibleForTesting static final LocalAwareExecutorPlus requestExecutor =
     SHARED.newExecutor(DatabaseDescriptor.getNativeTransportMaxThreads(),
       DatabaseDescriptor::setNativeTransportMaxThreads, "transport",
       "Native-Transport-Requests");
   @VisibleForTesting static final LocalAwareExecutorService authExecutor =
     SHARED.newExecutor(Math.max(1,
                          DatabaseDescriptor.getNativeTransportMaxAuthThreads()),
       DatabaseDescriptor::setNativeTransportMaxAuthThreads, "transport",
   ...
   {
      FlushItem<?> toFlushItem(Channel channel, Message.Request request, 
   ...
   @Override
   public
   void dispatch(Channel channel, Message.Request request, 
                 FlushItemConverter forFlusher, Overload backpressure) {
     boolean isAuthQuery =
       DatabaseDescriptor.getNativeTransportMaxAuthThreads() > 0 && (request.type == Message.Type.AUTH_RESPONSE || request.type == Message.Type.CREDENTIALS);
     LocalAwareExecutorService executor =
       isAuthQuery ? authExecutor : requestExecutor;
   ...
   class RequestTime
   {
      ...
      public
      static
      RequestTime forImmediateExecution() {
      ...
      public
      long computeDeadline(long verbExpiresAfterNanos) {
      ...
      public
      long computeTimeout(long now, long verbExpiresAfterNanos) {
        return computeDeadline(verbExpiresAfterNanos) - now;
      }
      public
      boolean shouldSendHints() {
      ...
      public
      long timeSpentInQueueNanos() {
        return startedAtNanos - enqueuedAtNanos;
      }
   }
   public
   class RequestProcessor implements DebuggableTask.RunnableDebuggableTask
   {
      ...
      public
      RequestProcessor(Channel channel, Message.Request request, 
                       FlushItemConverter forFlusher, Overload backpressure) {
      ...
      void run() {
        ...
        processRequest(channel, request, forFlusher, backpressure,
          new RequestTime(request.createdAtNanos, startTimeNanos));
      }
      @Override
      public
   ...
   private
   static
   Message.Response processRequest(ServerConnection connection, 
     ...
     switch (backpressure) {
       ...
       case REQUESTS:
         {
           String message =
             String.format("Request breached global limit of %d requests/second and triggered backpressure.",
       ...
       case BYTES_IN_FLIGHT:
         {
           String message =
             String.format("Request breached limit(s) on bytes in flight (Endpoint: %d, Global: %d) and triggered backpressure.",
               ClientResourceLimits.getEndpointLimit(),
               ClientResourceLimits.getGlobalLimit());
           NoSpamLogger.log(logger, NoSpamLogger.Level.INFO, 1,
             TimeUnit.MINUTES, message);
           ClientWarn.instance.warn(message);
           break;
       ...
       case QUEUE_TIME:
         {
           String message =
             String.format("Request has spent over %s time of the maximum timeout %dms in the queue",
               DatabaseDescriptor.getNativeTransportQueueMaxItemAgeThreshold(),
   ...
   static
   Message.Response processRequest(Channel channel, Message.Request request, 
                                   Overload backpressure, 
                                   RequestTime requestTime) {
     try {
       return processRequest((ServerConnection)request.connection(), request,
   ...
   void processRequest(Channel channel, Message.Request request, 
                       ...
                       RequestTime requestTime) {
     Message.Response response =
       processRequest(channel, request, backpressure, requestTime);
     FlushItem<?> toFlush =
       forFlusher.toFlushItem(channel, request, response);
     Message.logger.trace("Responding: {}, v={}", response,
       request.connection().getVersion());
     flush(toFlush);
   }
   private
   void flush(FlushItem<?> item) {
     EventLoop loop = item.channel.eventLoop();
     Flusher flusher = flusherLookup.get(loop);
     if (flusher == null) {
       Flusher created =
         useLegacyFlusher ? Flusher.legacy(loop) : Flusher.immediate(loop);
   ...
   Consumer<EventMessage> eventDispatcher(final Channel channel, 
     ...
     return eventMessage -> flush(new FlushItem.Framed(channel,
                                    eventMessage.encode(version), null,
                                    allocator, f -> f.response.release()));
   }
}