...
public final class CFMetaData {
  ...
  public static final CFMetaData OldMigrationsCf =
      newSystemMetadata(Table.SYSTEM_KS, DefsTable.OLD_MIGRATIONS_CF, 2,
                        "unused", TimeUUIDType.instance, null);
  @Deprecated
  ...
  public static final CFMetaData IndexCf =
      compile(5, "CREATE TABLE \"" + SystemTable.INDEX_CF + "\" ("
                     + "table_name text,"
                     + "index_name text,"
                     + "PRIMARY KEY (table_name, index_name)"
                     + (") WITH COMPACT STORAGE AND COMMENT='indexes that " +
                        "have been completed'"));
  public static final CFMetaData CounterIdCf = compile(
      6, "CREATE TABLE \"" + SystemTable.COUNTER_ID_CF + "\" ("
             + "key text,"
             + "id timeuuid,"
             + "PRIMARY KEY (key, id)"
  ...
  public static final CFMetaData HintsCf =
      compile("CREATE TABLE " + SystemTable.HINTS_CF + " ("
              + "target_id uuid,"
              + "hint_id timeuuid,"
              + "message_version int,"
              + "mutation blob,"
  ...
  public static final CFMetaData BatchlogCf =
      compile("CREATE TABLE " + SystemTable.BATCHLOG_CF + " ("
              + "id uuid PRIMARY KEY,"
  ...
  public static final CFMetaData RangeXfersCf =
      compile("CREATE TABLE " + SystemTable.RANGE_XFERS_CF + " ("
  ...
  volatile Map<ByteBuffer, ColumnDefinition> column_metadata =
      new HashMap<ByteBuffer, ColumnDefinition>();
  ...
  private static CFMetaData compile(Integer id, String cql, String keyspace) {
    try {
      CreateColumnFamilyStatement statement =
          (CreateColumnFamilyStatement)QueryProcessor.parseStatement(cql)
              .prepare()
              .statement;
      CFMetaData cfmd = newSystemMetadata(keyspace, statement.columnFamily(),
                                          id, "", statement.comparator, null);
  ...
  private AbstractType<?> enforceSubccDefault(ColumnFamilyType cftype,
                                              AbstractType<?> subcc) {
    return (subcc == null) && (cftype == ColumnFamilyType.Super)
        ? BytesType.instance
        : subcc;
  }
  ...
  public List<ByteBuffer> getKeyAliases() { return keyAliases; }
  public List<ByteBuffer> getColumnAliases() { return columnAliases; }
  public ByteBuffer getValueAlias() { return valueAlias; }
  ...
  public static CFMetaData fromThrift(org.apache.cassandra.thrift.CfDef cf_def)
    ...
    try {
      ...
      if (cf_def.isSetKey_alias()) {
        newCFMD.keyAliases(
            Collections.<ByteBuffer>singletonList(cf_def.key_alias));
      }
      ...
      if (cf_def.isSetCompaction_strategy_options())
        newCFMD.compactionStrategyOptions(
            new HashMap<String, String>(cf_def.compaction_strategy_options));
  ...
  public void reload() {
    ...
    if (cfDefRow.cf == null || cfDefRow.cf.isEmpty())
      throw new RuntimeException(
          String.format("%s not found in the schema definitions table.",
                        ksName + ":" + cfName));
    try {
  ...
  public void validateCompatility(CFMetaData cfm)
    ...
    if (cfm.subcolumnComparator == null) {
      ...
      throw new ConfigurationException(
          "subcolumncomparators do not match or are note compatible.");
  }
  public static void validateCompactionOptions(
      Class<? extends AbstractCompactionStrategy> strategyClass,
      Map<String, String> options, boolean checkUnexpected)
      throws ConfigurationException {
    try {
      if (options == null)
        return;
  ...
  public org.apache.cassandra.thrift.CfDef toThrift() {
    ...
    if (subcolumnComparator != null) {
      assert cfType == ColumnFamilyType.Super
          : String.format("%s CF %s should not have subcomparator %s defined",
                          cfType, cfName, subcolumnComparator);
    ...
    List<org.apache.cassandra.thrift.ColumnDef> column_meta =
        new ArrayList<org.apache.cassandra.thrift.ColumnDef>(
    ...
    def.setCompaction_strategy_options(
        new HashMap<String, String>(compactionStrategyOptions));
  ...
  public CFMetaData validate() throws ConfigurationException {
    ...
    if (cfType == ColumnFamilyType.Super) {
      if (subcolumnComparator == null)
        throw new ConfigurationException(String.format(
            "Missing subcolumn comparator for super column family %s", cfName));
    } else {
      if (subcolumnComparator != null)
        throw new ConfigurationException(
            String.format("Subcolumn comparator (%s) is invalid for standard " +
                          "column family %s",
                          subcolumnComparator, cfName));
    }
    ...
    if (subcolumnComparator instanceof CounterColumnType)
      throw new ConfigurationException(
          "CounterColumnType is not a valid sub-column comparator");
    ...
    for (ColumnDefinition columndef : column_metadata.values()) {
      for (ByteBuffer alias : keyAliases)
        if (alias.equals(columndef.name))
          throw new ConfigurationException(
              "Cannot have key alias equals to a column name: " +
              UTF8Type.instance.compose(alias));
      for (ByteBuffer alias : columnAliases)
        if (alias.equals(columndef.name))
          throw new ConfigurationException(
              "Cannot have column alias equals to a column name: " +
              UTF8Type.instance.compose(alias));
      if (valueAlias != null && valueAlias.equals(columndef.name))
        throw new ConfigurationException(
            "Cannot have value alias equals to a column name: " +
            UTF8Type.instance.compose(valueAlias));
    }
    for (ByteBuffer alias : keyAliases)
  ...
  private static void validateAlias(ByteBuffer alias, String msg)
      throws ConfigurationException {
    if (alias != null) {
      try {
  ...
  public RowMutation toSchemaUpdate(CFMetaData newState,
    ...
    RowMutation rm =
        new RowMutation(Table.SYSTEM_KS, SystemTable.getSchemaKSKey(ksName));
  ...
  public RowMutation dropFromSchema(long timestamp) {
    ...
    cf.addColumn(
        DeletedColumn.create(ldt, timestamp, cfName, "read_repair_chance"));
    cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
                                      "local_read_repair_chance"));
    cf.addColumn(
        DeletedColumn.create(ldt, timestamp, cfName, "replicate_on_write"));
    cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
                                      "populate_io_cache_on_flush"));
    cf.addColumn(
        DeletedColumn.create(ldt, timestamp, cfName, "gc_grace_seconds"));
    cf.addColumn(
        DeletedColumn.create(ldt, timestamp, cfName, "default_validator"));
    ...
    cf.addColumn(
        DeletedColumn.create(ldt, timestamp, cfName, "bloom_filter_fp_chance"));
    cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName, "caching"));
    cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
                                      "compaction_strategy_class"));
    cf.addColumn(
        DeletedColumn.create(ldt, timestamp, cfName, "compression_parameters"));
    cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName, "value_alias"));
    cf.addColumn(
        DeletedColumn.create(ldt, timestamp, cfName, "column_aliases"));
    cf.addColumn(DeletedColumn.create(ldt, timestamp, cfName,
  ...
  private void toSchemaNoColumns(RowMutation rm, long timestamp) {
    ...
    cf.addColumn(
        Column.create(comparator.toString(), timestamp, cfName, "comparator"));
    if (subcolumnComparator != null)
      cf.addColumn(Column.create(subcolumnComparator.toString(), timestamp,
    ...
    cf.addColumn(Column.create(maxCompactionThreshold, timestamp, cfName,
                               "max_compaction_threshold"));
    cf.addColumn(Column.create(json(aliasesAsStrings(keyAliases)), timestamp,
                               cfName, "key_aliases"));
    ...
    cf.addColumn(Column.create(json(aliasesAsStrings(columnAliases)), timestamp,
  ...
  static CFMetaData fromSchemaNoColumns(UntypedResultSet.Row result) {
    try {
      ...
      if (result.has("key_aliases")) {
        ...
        cfm.keyAliases(Collections.<ByteBuffer>singletonList(
            result.getBytes("key_alias")));
      }
  ...
  public static CFMetaData fromSchema(UntypedResultSet.Row result) {
    ...
    Row serializedColumnDefinitions =
        ColumnDefinition.readSchema(cfDef.ksName, cfDef.cfName);
  ...
  public AbstractType<?> getColumnDefinitionComparator(Integer componentIndex) {
    AbstractType<?> cfComparator =
        cfType == ColumnFamilyType.Super ? subcolumnComparator : comparator;
