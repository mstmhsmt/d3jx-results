...
<<<<<<< /root/work/apache/cassandra/d3j/patched-276-0.java
import org.apache.cassandra.batchlog.BatchlogManager;
||||||| /root/work/apache/cassandra/d3j/base-276.java
import org.slf4j.LoggerFactory;
import ch.qos.logback.classic.LoggerContext;
=======
>>>>>>> /root/work/apache/cassandra/d3j/patched-276-1.java
...
public class Instance extends IsolatedExecutor implements IInvokableInstance {
  ...
  Instance(IInstanceConfig config, ClassLoader classLoader) {
    ...
    FBUtilities.setBroadcastInetAddress(
        config.broadcastAddressAndPort().address);
<<<<<<< /root/work/apache/cassandra/d3j/patched-276-0.java
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-276-1.java
  ...
  public IInstanceConfig config() { return config; }
  public ICoordinator coordinator() { return new Coordinator(this); }
  public IListen listen() { return new Listen(this); }
  @Override
  ...
  public Object[][] executeInternal(String query, Object... args) {
    return sync(() -> {
             ...
             ResultMessage result = prepared.statement.executeInternal(
  ...
  public void startup() { throw new UnsupportedOperationException(); }
    ...
    sync(() -> {
      try {
        ...
        CQLStatement statement =
            QueryProcessor.parseStatement(query, queryState).statement;
        ...
        throw new RuntimeException(
            "Error setting schema for test (query was: " + query + ")", e);
      }
    }).run();
  }
  private void registerMockMessaging(ICluster cluster) {
    BiConsumer<InetAddressAndPort, IMessage> deliverToInstance =
        (to, message) -> cluster.get(to).receiveMessage(message);
    BiConsumer<InetAddressAndPort, IMessage> deliverToInstanceIfNotFiltered =
        (to, message) -> {
    ...
    cluster.stream().forEach(instance -> {
      ...
      if (!addressAndPort.equals(instance.config().broadcastAddressAndPort()))
        throw new IllegalStateException(
            "addressAndPort mismatch: " + addressAndPort + " vs " +
            instance.config().broadcastAddressAndPort());
      InetAddressAndPort prev =
          addressAndPortMap.put(addressAndPort.address, addressAndPort);
      if (null != prev)
        throw new IllegalStateException(
            "This version of Cassandra does not support multiple nodes with " +
            "the same InetAddress: " +
            addressAndPort + " vs " + prev);
    });
    MessagingService.instance().addMessageSink(new MessageDeliverySink(
        deliverToInstanceIfNotFiltered, addressAndPortMap::get));
  }
  private void registerFilter(ICluster cluster) {
    ...
    MessagingService.instance().addMessageSink(new IMessageSink() {
      public boolean allowOutgoingMessage(MessageOut message, int id,
        ...
        IInstance to =
            cluster.get(InetAddressAndPort.getByAddressOverrideDefaults(
                toAddress, instance.config().broadcastAddressAndPort().port));
  ...
  private class MessageDeliverySink implements IMessageSink {
    ...
    private final Function<InetAddress, InetAddressAndPort>
        lookupAddressAndPort;
    MessageDeliverySink(
        BiConsumer<InetAddressAndPort, IMessage> deliver,
        Function<InetAddress, InetAddressAndPort> lookupAddressAndPort) {
    ...
    public boolean allowOutgoingMessage(MessageOut messageOut, int id,
      ...
      try (DataOutputBuffer out = new DataOutputBuffer(1024)) {
        ...
        deliver.accept(toFull,
                       new Message(messageOut.verb.ordinal(), out.toByteArray(),
                                   id, version, from));
      } catch (IOException e) {
  ...
  public void receiveMessage(IMessage imessage) {
    sync(() -> {
      try (DataInputBuffer input = new DataInputBuffer(imessage.bytes())) {
        ...
        throw new RuntimeException(
            "Exception occurred on node " + broadcastAddressAndPort(), t);
      }
    }).run();
  }
  ...
  public void setMessagingVersion(InetAddressAndPort endpoint, int version) {
    runOnInstance(()
                      -> MessagingService.instance().setVersion(
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-276-1.java
        ...
        if (!FBUtilities.getBroadcastAddress().equals(
                broadcastAddressAndPort().address))
          throw new IllegalStateException();
      ...
      for (int i = 0; i < tokens.size(); i++) {
        ...
        Gossiper.runInGossipStageBlocking(() -> {
          ...
          Gossiper.instance.injectApplicationState(
              ep.address, ApplicationState.TOKENS,
              new VersionedValue.VersionedValueFactory(partitioner)
                  .tokens(Collections.singleton(token)));
          storageService.onChange(
              ep.address, ApplicationState.STATUS,
              new VersionedValue.VersionedValueFactory(partitioner)
                  .normal(Collections.singleton(token)));
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-276-0.java
  public Future<Void> shutdown() { return shutdown(true); }
        ...
        async((ExecutorService executor) -> {
          ...
          if (config.has(GOSSIP) || config.has(NETWORK)) {
            ...
            error = parallelRun(error, executor,
                                ()
                                    -> NanoTimeToCurrentTimeMillis.shutdown(
                                        MINUTES.toMillis(1L)));
          }
          error = parallelRun(
              error, executor,
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-276-0.java
              ...
              Ref::shutdownReferenceReaper, Memtable.MEMORY_POOL::shutdown,
              ScheduledExecutors::shutdownAndWait,
              SSTableReader::shutdownBlocking);
          error = parallelRun(error, executor,
                              MessagingService.instance()::shutdown);
          error = parallelRun(error, executor, StageManager::shutdownAndWait,
...
=======
              ...
              ()
                  -> PendingRangeCalculatorService.instance.shutdownExecutor(
                      1L, MINUTES),
              ()
                  -> StorageService.instance.shutdownBGMonitorAndWait(1L,
                                                                      MINUTES),
              ()
          ...
          error =
              parallelRun(error, executor, CommitLog.instance::shutdownBlocking,
                          MessagingService.instance()::shutdown);
          error = parallelRun(
              error, executor,
              ()
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-276-1.java
    ...
    return CompletableFuture
        .runAsync(ThrowingRunnable.toRunnable(future::get), isolatedExecutor)
        .thenRun(super::shutdown);
  }
  CompactionManager.instance::forceShutdown
      CommitLog.instance::shutdownBlocking private static Throwable
      parallelRun(Throwable accumulate, ExecutorService runOn,
