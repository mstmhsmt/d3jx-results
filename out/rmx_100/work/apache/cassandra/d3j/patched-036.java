...
public
class BatchStatement implements CQLStatement
{
   public
   enum Type
   {
   ...
   private static final String UNLOGGED_BATCH_WARNING =
     "Unlogged batch covering {} partitions detected " + "against table{} {}. You should use a logged batch for " + "atomicity, or asynchronous writes for performance.";
   private static final String LOGGED_BATCH_LOW_GCGS_WARNING =
   ...
   public
   BatchStatement(Type type, VariableSpecifications bindVariables, 
     ...
     MultiTableColumnsBuilder regularBuilder =
       new MultiTableColumnsBuilder();
   ...
   @Override
   public
   List<ColumnSpecification> getBindVariables() {
   ...
   short[] getPartitionKeyBindVariableIndexes() {
     boolean affectsMultipleTables =
       !statements.isEmpty() && !statements.stream().map(s -> s.metadata().id).allMatch(
                                   isEqual(statements.get(0).metadata().id));
     return (affectsMultipleTables || statements.isEmpty()) ? null : 
     bindVariables.getPartitionKeyBindVariableIndexes(statements.get(0).metadata());
   }
   @Override
   public
   Iterable<org.apache.cassandra.cql3.functions.Function> getFunctions() {
     List<org.apache.cassandra.cql3.functions.Function> functions =
       new ArrayList<>();
   ...
   void validate() throws InvalidRequestException {
     ...
     for (ModificationStatement statement : statements) {
       ...
       if (statement.isCounter())
         hasCounters = true; else
         hasNonCounters = true;
       if (statement.isVirtual())
         hasVirtualTables = true; else
   ...
   public
   List<ModificationStatement> getStatements() {
     return statements;
   }
   @VisibleForTesting
   public
   List<? extends IMutation> getMutations(ClientState state, 
                                          BatchQueryOptions options, 
                                          boolean local, long batchTimestamp, 
                                          long nowInSeconds, 
                                          Dispatcher.RequestTime requestTime) {
     if (statements.isEmpty())
       return Collections.emptyList();
     List<List<ByteBuffer>> partitionKeys =
       new ArrayList<>(statements.size());
     Map<TableId,HashMultiset<ByteBuffer>> partitionCounts =
       new HashMap<>(updatedColumns.size());
     ...
     for (int i = 0, isize = statements.size(); i < isize; i++) {
       ...
       HashMultiset<ByteBuffer> perKeyCountsForTable =
         partitionCounts.computeIfAbsent(stmt.metadata.id,
           k -> HashMultiset.create());
     ...
     if (metadata != null)
       collector =
       new SingleTableUpdatesCollector(metadata,
         updatedColumns.get(metadata.id), partitionCounts.get(metadata.id)); else
     ...
     for (int i = 0, isize = statements.size(); i < isize; i++) {
       ...
       statement.addUpdates(collector, partitionKeys.get(i), state,
         statementOptions, local, timestamp, nowInSeconds, requestTime);
     }
     if (tablesWithZeroGcGs != null) {
       String suffix = tablesWithZeroGcGs.size() == 1 ? "" : "s";
       NoSpamLogger.log(logger, NoSpamLogger.Level.WARN, 1, TimeUnit.MINUTES,
         LOGGED_BATCH_LOW_GCGS_WARNING, suffix, tablesWithZeroGcGs);
   ...
   private
   static
   void verifyBatchSize(Collection<? extends IMutation> mutations)
     ...
     if (size > warnThreshold) {
       ...
       if (size > failThreshold) {
         Tracing.trace(format, tableNames,
           FBUtilities.prettyPrintMemory(size),
           FBUtilities.prettyPrintMemory(failThreshold),
           FBUtilities.prettyPrintMemory(size - failThreshold),
           " (see batch_size_fail_threshold)");
         logger.error(format, tableNames,
           FBUtilities.prettyPrintMemory(size),
           FBUtilities.prettyPrintMemory(failThreshold),
           FBUtilities.prettyPrintMemory(size - failThreshold),
           " (see batch_size_fail_threshold)");
         ...
         logger.warn(format, tableNames, FBUtilities.prettyPrintMemory(size),
           FBUtilities.prettyPrintMemory(warnThreshold),
           FBUtilities.prettyPrintMemory(size - warnThreshold), "");
       }
       ClientWarn.instance.warn(MessageFormatter.arrayFormat(format,
                                  new Object[]{ tableNames, size,
                                  warnThreshold, size - warnThreshold, "" }).getMessage());
     }
   }
   private
   void verifyBatchType(Collection<? extends IMutation> mutations) {
     if (!isLogged() && mutations.size() > 1) {
       ...
       if (keySet.size() > DatabaseDescriptor.getUnloggedBatchAcrossPartitionsWarnThreshold()) {
         NoSpamLogger.log(logger, NoSpamLogger.Level.WARN, 1,
           TimeUnit.MINUTES, UNLOGGED_BATCH_WARNING, keySet.size(),
         ...
         ClientWarn.instance.warn(MessageFormatter.arrayFormat(UNLOGGED_BATCH_WARNING,
                                    ...
                                    tableNames.size() == 1 ? "" : "s",
                                    tableNames }).getMessage());
       }
     }
   }
   public
   ResultMessage execute(QueryState queryState, QueryOptions options, 
                         Dispatcher.RequestTime requestTime) {
   ...
   public
   ResultMessage execute(QueryState queryState, BatchQueryOptions options, 
                         Dispatcher.RequestTime requestTime) {
     ...
     if (updatesVirtualTables)
       executeInternalWithoutCondition(queryState, options, requestTime); else
       executeWithoutConditions(getMutations(clientState, options, false,
                                  timestamp, nowInSeconds, requestTime),
         options.getConsistency(), requestTime);
     return new ResultMessage.Void();
   }
   private
   void executeWithoutConditions(List<? extends IMutation> mutations, 
                                 ConsistencyLevel cl, 
                                 Dispatcher.RequestTime requestTime)
   ...
   private
   ResultMessage executeWithConditions(BatchQueryOptions options, 
                                       QueryState state, 
                                       Dispatcher.RequestTime requestTime) {
     Pair<CQL3CasRequest,Set<ColumnMetadata>> p =
     ...
     try (RowIterator result =
            StorageProxy.cas(ksName, tableName, casRequest.key, casRequest,
              options.getSerialConsistency(), options.getConsistency(),
              state.getClientState(), options.getNowInSeconds(state),
              requestTime)) {
       return new ResultMessage.Rows(ModificationStatement.buildCasResultSet(ksName,
                                       tableName, result,
                                       columnsWithConditions, true, state,
                                       options.forStatement(0)));
     } 
   }
   private
   Pair<CQL3CasRequest,Set<ColumnMetadata>> makeCasRequest(BatchQueryOptions options,
                                                           QueryState state) {
     ...
     for (int i = 0; i < statements.size(); i++) {
       ...
       if (key == null) {
         ...
         casRequest =
         new CQL3CasRequest(statement.metadata(), key, conditionColumns,
       ...
       if (statement.hasSlices()) {
         ...
         for (Slice slice : slices) {
           casRequest.addRangeDeletion(slice, statement, statementOptions,
         ...
         if (statement.hasConditions()) {
           ...
           if (statement.hasIfNotExistCondition() || statement.hasIfExistCondition())
             columnsWithConditions = null; else if (columnsWithConditions != null)
   ...
   public
   ResultMessage executeLocally(QueryState queryState, QueryOptions options)
   ...
   private
   ResultMessage executeInternalWithoutCondition(QueryState queryState, 
                                                 BatchQueryOptions batchOptions,
     ...
     for (IMutation mutation : getMutations(queryState.getClientState(),
                                 batchOptions, true, timestamp, nowInSeconds,
                                 requestTime))
       mutation.apply();
     return null;
   }
   private
   ResultMessage executeInternalWithConditions(BatchQueryOptions options, 
                                               QueryState state) {
     Pair<CQL3CasRequest,Set<ColumnMetadata>> p =
       makeCasRequest(options, state);
     ...
     long nowInSeconds = options.getNowInSeconds(state);
     try (RowIterator result =
            ModificationStatement.casInternal(state.getClientState(),
              request, timestamp, nowInSeconds)) {
       ResultSet resultSet =
         ModificationStatement.buildCasResultSet(ksName, tableName, result,
           columnsWithConditions, true, state, options.forStatement(0));
   ...
   public
   static
   class Parsed extends QualifiedStatement
   {
      private final Type type;
      private final Attributes.Raw attrs;
      private final List<ModificationStatement.Parsed> parsedStatements;
      public
      Parsed(Type type, Attributes.Raw attrs, 
      ...
      @Override
      public
      void setKeyspace(ClientState state) throws InvalidRequestException {
        for (ModificationStatement.Parsed statement : parsedStatements)
      ...
      public
      BatchStatement prepare(ClientState state) {
        List<ModificationStatement> statements =
          new ArrayList<>(parsedStatements.size());
        ...
        BatchStatement batchStatement =
          new BatchStatement(type, bindVariables, statements, prepAttrs);
   ...
   private
   static
   class MultiTableColumnsBuilder
   {
      private final Map<TableId,RegularAndStaticColumns.Builder>
      perTableBuilders = new HashMap<>();
      public
      void addAll(TableMetadata table, RegularAndStaticColumns columns) {
        RegularAndStaticColumns.Builder builder =
          perTableBuilders.get(table.id);
        if (builder == null) {
      ...
      public
      Map<TableId,RegularAndStaticColumns> build() {
        Map<TableId,RegularAndStaticColumns> m =
