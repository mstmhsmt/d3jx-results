...
public class SelectStatement implements CQLStatement {
  ...
  public SelectStatement(CFMetaData cfm, int boundTerms, Parameters parameters,
                         Selection selection,
  ...
  public ColumnFilter queriedColumns() { return queriedColumns; }
  static SelectStatement forSelection(CFMetaData cfm, Selection selection) {
    return new SelectStatement(
        cfm, 0, defaultParameters, selection,
        StatementRestrictions.empty(StatementType.SELECT, cfm), false, null,
        null, null, null);
  }
  ...
  public ResultMessage.Rows execute(QueryState state, QueryOptions options,
    ...
    ReadQuery query =
        getQuery(options, nowInSec, userLimit, userPerPartitionLimit, pageSize);
  ...
  public ReadQuery getQuery(QueryOptions options, int nowInSec, int userLimit,
                            int perPartitionLimit, int pageSize) {
  ...
  private ResultMessage.Rows execute(Pager pager, QueryOptions options,
                                     int pageSize, int nowInSec, int userLimit,
                                     long queryStartNanoTime)
      throws RequestValidationException, RequestExecutionException {
    if (aggregationSpec != null) {
    ...
    try (ReadExecutionController executionController =
      ...
      if (aggregationSpec == null &&
        ...
        return execute(Pager.forInternalQuery(pager, executionController),
                       options, pageSize, nowInSec, userLimit,
                       queryStartNanoTime);
      }
    }
  }
  ...
  public String keyspace() { return cfm.ksName; }
  public String columnFamily() { return cfm.cfName; }
  public Selection getSelection() { return selection; }
  public StatementRestrictions getRestrictions() { return restrictions; }
  private ReadQuery getSliceCommands(QueryOptions options, DataLimits limit,
                                     int nowInSec)
      throws RequestValidationException {
    ...
    for (ByteBuffer key : keys) {
      ...
      ColumnFilter cf =
          (cfm.isSuper() && cfm.isDense())
              ? SuperColumnCompatibility.getColumnFilter(
                    cfm, options, restrictions.getSuperColumnRestrictions())
              : queriedColumns;
      commands.add(SinglePartitionReadCommand.create(
          cfm, nowInSec, cf, rowFilter, limit, dk, filter));
    }
  ...
  public SinglePartitionReadCommand internalReadForView(DecoratedKey key,
    ...
    return SinglePartitionReadCommand.create(
        cfm, nowInSec, queriedColumns, rowFilter, DataLimits.NONE, key, filter);
  }
  ...
  private ReadQuery getRangeCommand(QueryOptions options, DataLimits limit,
    ...
    PartitionRangeReadCommand command = PartitionRangeReadCommand.create(
        false, cfm, nowInSec, queriedColumns, rowFilter, limit,
  ...
  private ClusteringIndexFilter makeClusteringIndexFilter(QueryOptions options)
    ...
    if (restrictions.isColumnRange()) {
      ...
      if (clusterings.isEmpty() &&
          queriedColumns.fetchedColumns().statics.isEmpty())
        return null;
  ...
  private Slices makeSlices(QueryOptions options)
    ...
    Slices.Builder builder =
        new Slices.Builder(cfm.comparator, startBounds.size());
  ...
  private ResultSet process(PartitionIterator partitions, QueryOptions options,
    ...
    Selection.ResultSetBuilder result =
        selection.resultSetBuilder(options, parameters.isJson, aggregationSpec);
  ...
  void processPartition(RowIterator partition, QueryOptions options,
    ...
    if (cfm.isSuper() && cfm.isDense()) {
      SuperColumnCompatibility.processPartition(
          cfm, selection, partition, result, options.getProtocolVersion(),
          restrictions.getSuperColumnRestrictions(), options);
      return;
    }
    ...
    if (!partition.hasNext()) {
      if (!staticRow.isEmpty() &&
          (queriesFullPartitions() || cfm.isStaticCompactTable())) {
  ...
  private static void addValue(Selection.ResultSetBuilder result,
                               ColumnDefinition def, Row row, int nowInSec,
  ...
  public static class RawStatement extends CFStatement {
    ...
    public ParsedStatement.Prepared prepare(boolean forView,
                                            ClientState clientState)
        throws InvalidRequestException {
      CFMetaData cfm = ThriftValidation.validateColumnFamilyWithCompactMode(
          keyspace(), columnFamily(), clientState.isNoCompactMode());
      VariableSpecifications boundNames = getBoundVariables();
      Selection selection = prepareSelection(cfm, boundNames);
      StatementRestrictions restrictions =
          prepareRestrictions(cfm, boundNames, selection, forView);
      if (parameters.isDistinct) {
        checkNull(
            perPartitionLimit,
      ...
      AggregationSpecification aggregationSpec = getAggregationSpecification(
          cfm, selection, restrictions, parameters.isDistinct);
      ...
      SelectStatement stmt = new SelectStatement(
          cfm, boundNames.size(), parameters, selection, restrictions,
      ...
      return new ParsedStatement.Prepared(
          stmt, boundNames, boundNames.getPartitionKeyBindIndexes(cfm));
    }
    private Selection prepareSelection(CFMetaData cfm,
                                       VariableSpecifications boundNames) {
      boolean hasGroupBy = !parameters.groups.isEmpty();
      if (selectClause.isEmpty())
        return hasGroupBy ? Selection.wildcardWithGroupBy(cfm, boundNames)
                          : Selection.wildcard(cfm);
      return Selection.fromSelectors(cfm, selectClause, boundNames, hasGroupBy);
    }
    private StatementRestrictions
    prepareRestrictions(CFMetaData cfm, VariableSpecifications boundNames,
                        Selection selection, boolean forView)
        throws InvalidRequestException {
      return new StatementRestrictions(StatementType.SELECT, cfm, whereClause,
                                       boundNames,
                                       selection.containsOnlyStaticColumns(),
                                       selection.containsAComplexColumn(),
                                       parameters.allowFiltering, forView);
    }
    ...
    private static void
    validateDistinctSelection(CFMetaData cfm, Selection selection,
                              StatementRestrictions restrictions)
        throws InvalidRequestException {
      checkFalse(
    ...
    getAggregationSpecification(CFMetaData cfm, Selection selection,
  ...
  public static class Parameters {
    ...
    public Parameters(Map<ColumnDefinition.Raw, Boolean> orderings,
                      List<ColumnDefinition.Raw> groups, boolean isDistinct,
