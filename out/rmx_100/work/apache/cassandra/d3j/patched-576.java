...
public class Server implements CassandraDaemon.Server {
  static { InternalLoggerFactory.setDefaultFactory(new Slf4JLoggerFactory()); }
  private static final Logger logger = LoggerFactory.getLogger(Server.class);
  private static final boolean enableEpoll = Boolean.valueOf(
      System.getProperty("cassandra.native.epoll.enabled", "true"));
  ...
  public Server(int port) { this(new InetSocketAddress(port)); }
  public void start() {
    if (!isRunning()) {
      run();
    }
  ...
  private void run() {
    ...
    if (!bindFuture.awaitUninterruptibly().isSuccess())
      throw new IllegalStateException(
          String.format("Failed to bind port %d on %s.", socket.getPort(),
  ...
  private void registerMetrics() {
    ClientMetrics.instance.addCounter(
        "connectedNativeClients", new Callable<Integer>() {
          @Override
  ...
  public static class ConnectionTracker implements Connection.Tracker {
    ...
    public void closeAll() { allChannels.close().awaitUninterruptibly(); }
  ...
  private static class Initializer extends ChannelInitializer {
    ...
    public Initializer(Server server) { this.server = server; }
    protected void initChannel(Channel channel) throws Exception {
      ...
      pipeline.addLast("frameDecoder",
                       new Frame.Decoder(server.connectionFactory));
  ...
  private static class SecureInitializer extends Initializer {
    ...
    public SecureInitializer(Server server,
                             EncryptionOptions encryptionOptions) {
      super(server);
      this.encryptionOptions = encryptionOptions;
      try {
        this.sslContext = SSLFactory.createSSLContext(
            encryptionOptions, encryptionOptions.require_client_auth);
      } catch (IOException e) {
  ...
  private static class LatestEvent {
    ...
    private LatestEvent(Event.StatusChange.Status status,
                        Event.TopologyChange.Change topology) {
      this.status = status;
    ...
    public static LatestEvent forStatusChange(Event.StatusChange.Status status,
                                              LatestEvent prev) {
      return new LatestEvent(status, prev == null ? null : prev.topology);
    }
    public static LatestEvent
    forTopologyChange(Event.TopologyChange.Change change, LatestEvent prev) {
      return new LatestEvent(prev == null ? null : prev.status, change);
    }
  }
  private static class EventNotifier
      extends MigrationListener implements IEndpointLifecycleSubscriber {
    private final Server server;
    private final Map<InetAddress, LatestEvent> latestEvents =
    ...
    private EventNotifier(Server server) { this.server = server; }
    private InetAddress getRpcAddress(InetAddress endpoint) {
      try {
        InetAddress rpcAddress = InetAddress.getByName(
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
    private void send(InetAddress endpoint, Event.NodeEvent event) {
      if (!endpoint.equals(FBUtilities.getBroadcastAddress()) &&
          event.nodeAddress().equals(
              DatabaseDescriptor.getBroadcastRpcAddress()))
        return;
      send(event);
    }
||||||| /root/work/apache/cassandra/d3j/base-576.java
=======
    private void send(InetAddress endpoint, Event.NodeEvent event) {
      if (!endpoint.equals(FBUtilities.getBroadcastAddress()) &&
          event.nodeAddress().equals(
    ...
    private void send(Event event) { server.connectionTracker.send(event); }
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    public void onJoinCluster(InetAddress endpoint) {
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
      onTopologyChange(endpoint,
                       Event.TopologyChange.newNode(getRpcAddress(endpoint),
                                                    server.socket.getPort()));
||||||| /root/work/apache/cassandra/d3j/base-576.java
      server.connectionTracker.send(Event.TopologyChange.newNode(
          getRpcAddress(endpoint), server.socket.getPort()));
=======
      send(endpoint, Event.TopologyChange.newNode(getRpcAddress(endpoint),
                                                  server.socket.getPort()));
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    }
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
      onTopologyChange(
          endpoint, Event.TopologyChange.removedNode(getRpcAddress(endpoint),
                                                     server.socket.getPort()));
||||||| /root/work/apache/cassandra/d3j/base-576.java
      server.connectionTracker.send(Event.TopologyChange.removedNode(
          getRpcAddress(endpoint), server.socket.getPort()));
=======
      send(endpoint, Event.TopologyChange.removedNode(getRpcAddress(endpoint),
                                                      server.socket.getPort()));
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    }
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
      onTopologyChange(endpoint,
                       Event.TopologyChange.movedNode(getRpcAddress(endpoint),
                                                      server.socket.getPort()));
||||||| /root/work/apache/cassandra/d3j/base-576.java
      server.connectionTracker.send(Event.TopologyChange.movedNode(
          getRpcAddress(endpoint), server.socket.getPort()));
=======
      send(endpoint, Event.TopologyChange.movedNode(getRpcAddress(endpoint),
                                                    server.socket.getPort()));
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    }
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
      onStatusChange(endpoint,
                     Event.StatusChange.nodeUp(getRpcAddress(endpoint),
                                               server.socket.getPort()));
||||||| /root/work/apache/cassandra/d3j/base-576.java
      Event.StatusChange.Status prev =
          lastStatusChange.put(endpoint, Event.StatusChange.Status.UP);
      if (prev == null || prev != Event.StatusChange.Status.UP)
        server.connectionTracker.send(Event.StatusChange.nodeUp(
            getRpcAddress(endpoint), server.socket.getPort()));
=======
      if (prev == null || prev != Event.StatusChange.Status.UP)
        send(endpoint, Event.StatusChange.nodeUp(getRpcAddress(endpoint),
                                                 server.socket.getPort()));
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    }
...
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
      onStatusChange(endpoint,
                     Event.StatusChange.nodeDown(getRpcAddress(endpoint),
                                                 server.socket.getPort()));
    }
    private void onTopologyChange(InetAddress endpoint,
      ...
      if (prev == null || prev.topology != event.change) {
        LatestEvent ret = latestEvents.put(
            endpoint, LatestEvent.forTopologyChange(event.change, prev));
        if (ret == prev)
          server.connectionTracker.send(event);
      }
    }
    private void onStatusChange(InetAddress endpoint,
      ...
      if (prev == null || prev.status != event.status) {
        LatestEvent ret = latestEvents.put(
            endpoint, LatestEvent.forStatusChange(event.status, prev));
        if (ret == prev)
          server.connectionTracker.send(event);
      }
||||||| /root/work/apache/cassandra/d3j/base-576.java
      Event.StatusChange.Status prev =
          lastStatusChange.put(endpoint, Event.StatusChange.Status.DOWN);
      if (prev == null || prev != Event.StatusChange.Status.DOWN)
        server.connectionTracker.send(Event.StatusChange.nodeDown(
            getRpcAddress(endpoint), server.socket.getPort()));
=======
      if (prev == null || prev != Event.StatusChange.Status.DOWN)
        send(endpoint, Event.StatusChange.nodeDown(getRpcAddress(endpoint),
                                                   server.socket.getPort()));
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    }
    ...
    public void onCreateColumnFamily(String ksName, String cfName) {
      send(new Event.SchemaChange(Event.SchemaChange.Change.CREATED,
                                  Event.SchemaChange.Target.TABLE, ksName,
                                  cfName));
    }
    public void onCreateUserType(String ksName, String typeName) {
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
      send(new Event.SchemaChange(Event.SchemaChange.Change.CREATED,
                                  Event.SchemaChange.Target.TYPE, ksName,
                                  typeName));
    }
    public void onCreateFunction(String ksName, String functionName,
                                 List<AbstractType<?>> argTypes) {
      server.connectionTracker.send(new Event.SchemaChange(
          Event.SchemaChange.Change.CREATED, Event.SchemaChange.Target.FUNCTION,
    ...
    public void onCreateAggregate(String ksName, String aggregateName,
      ...
      server.connectionTracker.send(new Event.SchemaChange(
          ...
          Event.SchemaChange.Target.AGGREGATE, ksName, aggregateName,
          AbstractType.asCQLTypeStringList(argTypes)));
||||||| /root/work/apache/cassandra/d3j/base-576.java
      server.connectionTracker.send(new Event.SchemaChange(
          Event.SchemaChange.Change.CREATED, Event.SchemaChange.Target.TYPE,
          ksName, typeName));
=======
      send(new Event.SchemaChange(Event.SchemaChange.Change.CREATED,
                                  Event.SchemaChange.Target.TYPE, ksName,
...
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    ...
    public void onUpdateFunction(String ksName, String functionName,
      ...
      server.connectionTracker.send(new Event.SchemaChange(
<<<<<<< /root/work/apache/cassandra/d3j/patched-576-0.java
          Event.SchemaChange.Change.UPDATED, Event.SchemaChange.Target.FUNCTION,
          ksName, functionName, AbstractType.asCQLTypeStringList(argTypes)));
    }
    public void onUpdateAggregate(String ksName, String aggregateName,
      ...
      server.connectionTracker.send(new Event.SchemaChange(
          ...
          AbstractType.asCQLTypeStringList(argTypes)));
||||||| /root/work/apache/cassandra/d3j/base-576.java
          Event.SchemaChange.Change.UPDATED, Event.SchemaChange.Target.TYPE,
          ksName, typeName));
=======
          Event.SchemaChange.Change.UPDATED, Event.SchemaChange.Target.FUNCTION,
          ksName, functionName, AbstractType.asCQLTypeStringList(argTypes)));
>>>>>>> /root/work/apache/cassandra/d3j/patched-576-1.java
    ...
    public void onDropFunction(String ksName, String functionName,
                               List<AbstractType<?>> argTypes) {
      server.connectionTracker.send(new Event.SchemaChange(
          Event.SchemaChange.Change.DROPPED, Event.SchemaChange.Target.FUNCTION,
          ksName, functionName, AbstractType.asCQLTypeStringList(argTypes)));
    }
