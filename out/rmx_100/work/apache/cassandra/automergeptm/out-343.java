  ...
  public static class RawStatement extends CFStatement {
    ...
    public ParsedStatement.Prepared prepare(boolean forView) throws InvalidRequestException {
      ...
      if (!orderingColumns.isEmpty()) {
        ...
        orderingComparator = getOrderingComparator(table, selection, restrictions, tableselectionrestrictionsorderingColumns);
    ...
    private static void validateDistinctSelection(TableMetadata metadata, Selection selection, StatementRestrictions restrictions) throws InvalidRequestException {
      ...
      for (ColumnMetadata def : requestedColumns) {
        checkFalse(!def.isPartitionKey() && !def.isStatic(), "SELECT DISTINCT queries must only request partition key columns and/or static columns (not %s)", def.name);
      }
      if (!restrictions.isKeyRange()) {
        return;
      }
      for (ColumnMetadata def : metadata.partitionKeyColumns()) {
        checkTrue(requestedColumns.contains(def), "SELECT DISTINCT queries must request all the partition key columns (missing %s)", def.name);
      }
    }
    ...
    private Comparator<List<ByteBuffer>> getOrderingComparator(TableMetadata metadata, Selection selection, StatementRestrictions restrictions, TableMetadata metadataSelection selectionStatementRestrictions restrictionsMap<ColumnMetadata, Boolean> orderingColumns) throws InvalidRequestException {
      if (!restrictions.keyIsInRelation()) {
        return null;
      }
      Map<Integer, ColumnDefinition> orderingIndexes = getOrderingIndex(metadata, selection, metadataselectionorderingColumns);
    ...
    private Map<Integer, ColumnDefinition> getOrderingIndex(TableMetadata table, Selection selection, TableMetadata tableSelection selectionMap<ColumnMetadata, Boolean> orderingColumns) {
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private static abstract class ColumnComparator<T extends java.lang.Object> implements Comparator<T> {
