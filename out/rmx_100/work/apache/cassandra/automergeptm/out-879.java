...
public final class CFMetaData {
  ...
  public static final CFMetaData IndexCf = compile("CREATE TABLE \"" + SystemKeyspace.INDEX_CF + "\" (" + "table_name text," + "index_name text," + "PRIMARY KEY (table_name, index_name)" + ") WITH COMPACT STORAGE AND COMMENT=\'indexes that have been completed\'");

  public static final CFMetaData CounterIdCf = compile("CREATE TABLE \"" + SystemKeyspace.COUNTER_ID_CF + "\" (" + "key text," + "id timeuuid," + "PRIMARY KEY (key, id)" + ") WITH COMPACT STORAGE AND COMMENT=\'counter node IDs\'");

  public static final CFMetaData SchemaKeyspacesCf = compile("CREATE TABLE " + SystemKeyspace.SCHEMA_KEYSPACES_CF + " (" + "keyspace_name text PRIMARY KEY," + "durable_writes boolean," + "strategy_class text," + "strategy_options text" + ") WITH COMPACT STORAGE AND COMMENT=\'keyspace definitions\' AND gc_grace_seconds=8640");
...
<<<<<<< commits-rmx_100/apache/cassandra/9a28df0d0f87244bde2eccdc3fb359b6f3bfa183/CFMetaData-2da2361.java
  "CREATE TABLE " + SystemKeyspace.SCHEMA_COLUMNFAMILIES_CF + " (" + "keyspace_name text," + "columnfamily_name text," + "type text," + "comparator text," + "subcomparator text,"
=======
...
=======
  "compaction_strategy_options text,"
>>>>>>> commits-rmx_100/apache/cassandra/cb0a0cd2f8295a526068b7f6c52860e707e7665a/CFMetaData-33e3f00.java
   + "PRIMARY KEY (keyspace_name, columnfamily_name)" + ") WITH COMMENT=\'ColumnFamily definitions\' AND gc_grace_seconds=8640");

  public static final CFMetaData SchemaColumnsCf = compile("CREATE TABLE " + SystemKeyspace.SCHEMA_COLUMNS_CF + " (" + "keyspace_name text," + "columnfamily_name text," + "column_name text," + "validator text," + "index_type text," + "index_options text," + "index_name text," + "component_index int," + "type text," + "PRIMARY KEY(keyspace_name, columnfamily_name, column_name)" + ") WITH COMMENT=\'ColumnFamily column attributes\' AND gc_grace_seconds=8640");

  public static final CFMetaData SchemaTriggersCf = compile("CREATE TABLE " + SystemKeyspace.SCHEMA_TRIGGERS_CF + " (" + "keyspace_name text," + "columnfamily_name text," + "trigger_name text," + "trigger_options map<text, text>," + "PRIMARY KEY (keyspace_name, columnfamily_name, trigger_name)" + ") WITH COMMENT=\'triggers metadata table\'");
  ...
  public static final CFMetaData PeersCf = compile("CREATE TABLE " + SystemKeyspace.PEERS_CF + " (" + "peer inet PRIMARY KEY," + "host_id uuid," + "tokens set<varchar>," + "schema_version uuid," + "release_version text," + "rpc_address inet," + "preferred_ip inet," + "data_center text," + "rack text" + ") WITH COMMENT=\'known peers in the cluster\'");

  public static final CFMetaData PeerEventsCf = compile("CREATE TABLE " + SystemKeyspace.PEER_EVENTS_CF + " (" + "peer inet PRIMARY KEY," + "hints_dropped map<uuid, int>" + ") WITH COMMENT=\'cf contains events related to peers\'");

  public static final CFMetaData LocalCf = compile("CREATE TABLE " + SystemKeyspace.LOCAL_CF + " (" + "key text PRIMARY KEY," + "tokens set<varchar>," + "cluster_name text," + "gossip_generation int," + "bootstrapped text," + "host_id uuid," + "release_version text," + "thrift_version text," + "cql_version text," + "native_protocol_version text," + "data_center text," + "rack text," + "partitioner text," + "schema_version uuid," + "truncated_at map<uuid, blob>" + ") WITH COMMENT=\'information about the local node\'");

  public static final CFMetaData TraceSessionsCf = compile("CREATE TABLE " + Tracing.SESSIONS_CF + " (" + "session_id uuid PRIMARY KEY," + "coordinator inet," + "request text," + "started_at timestamp," + "parameters map<text, text>," + "duration int" + ") WITH COMMENT=\'traced sessions\'", Tracing.TRACE_KS);

  public static final CFMetaData TraceEventsCf = compile("CREATE TABLE " + Tracing.EVENTS_CF + " (" + "session_id uuid," + "event_id timeuuid," + "source inet," + "thread text," + "activity text," + "source_elapsed int," + "PRIMARY KEY (session_id, event_id)" + ")", Tracing.TRACE_KS);

  public static final CFMetaData BatchlogCf = compile("CREATE TABLE " + SystemKeyspace.BATCHLOG_CF + " (" + "id uuid PRIMARY KEY," + "written_at timestamp," + "data blob" + ") WITH COMMENT=\'uncommited batches\' AND gc_grace_seconds=0 " + "AND COMPACTION={\'class\' : \'SizeTieredCompactionStrategy\', \'min_threshold\' : 2}");

  public static final CFMetaData RangeXfersCf = compile("CREATE TABLE " + SystemKeyspace.RANGE_XFERS_CF + " (" + "token_bytes blob PRIMARY KEY," + "requested_at timestamp" + ") WITH COMMENT=\'ranges requested for transfer here\'");

  public static final CFMetaData CompactionLogCf = compile("CREATE TABLE " + SystemKeyspace.COMPACTION_LOG + " (" + "id uuid PRIMARY KEY," + "keyspace_name text," + "columnfamily_name text," + "inputs set<int>" + ") WITH COMMENT=\'unfinished compactions\'");

  public static final CFMetaData PaxosCf = compile("CREATE TABLE " + SystemKeyspace.PAXOS_CF + " (" + "row_key blob," + "cf_id UUID," + "in_progress_ballot timeuuid," + "proposal blob," + "most_recent_commit_at timeuuid," + "most_recent_commit blob," + "PRIMARY KEY (row_key, cf_id)" + ") WITH COMMENT=\'in-progress paxos proposals\'");
  ...
  @VisibleForTesting CFMetaData(String keyspace, String name, ColumnFamilyType type, AbstractType<?> comp, UUID id) {
    ksName = keyspace;
    cfName = name;
    cfType = type;
    comparator = comp;
  ...
  public static CFMetaData fromThrift(org.apache.cassandra.thrift.CfDef cf_def) throws InvalidRequestException, ConfigurationException {
    ...
    try {
      ...
      return newCFMD.comment(cf_def.comment).replicateOnWrite(cf_def.replicate_on_write).defaultValidator(TypeParser.parse(cf_def.default_validation_class)).columnMetadata(ColumnDefinition.fromThrift(cf_def.column_metadata, newCFMD.isSuper())).columnMetadata(ColumnDefinition.fromThrift(cf_def.column_metadata)).rebuild();
