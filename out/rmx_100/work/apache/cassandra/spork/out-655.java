...
public class DatabaseDescriptor {
    ...
    private static void applyConfig(Config config) throws ConfigurationException {
        ...
        } else /* evaluate the DiskAccessMode Config directive, which also affects indexAccessMode selection */
        ...
        if (conf.memtable_flush_writers == null) {
            conf.memtable_flush_writers = Math.min(8, Math.max(2, Math.min(FBUtilities.getAvailableProcessors(), DatabaseDescriptor.conf.data_file_directories.length)));
        }
        ...
        if (conf.concurrent_compactors == null) {
            conf.concurrent_compactors = Math.min(8, Math.max(2, Math.min(FBUtilities.getAvailableProcessors(), DatabaseDescriptor.conf.data_file_directories.length)));
        }
