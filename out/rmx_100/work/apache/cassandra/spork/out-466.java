    ...
    private UnfilteredRowIterator queryMemtableAndSSTablesInTimestampOrder(ColumnFamilyStore cfs, ClusteringIndexNamesFilter filter) {
        ...
        for (SSTableReader sstable : view.sstables) {
            ...
            if (!shouldInclude(sstable)) {
                ...
                try (final UnfilteredRowIterator iter = StorageHook.instance.makeRowIterator(cfs, sstable, partitionKey(), Slices.ALL, columnFilter(), filter.isReversed(), isForThrift())) {
            ...
            sstable.incrementReadCount();
            try (final UnfilteredRowIterator iter = StorageHook.instance.makeRowIterator(cfs, sstable, partitionKey(), filter.getSlices(metadata()), columnFilter(), filter.isReversed(), isForThrift())) {
                if (iter.isEmpty()) {
                    continue;
                }
                if (sstable.isRepaired()) {
