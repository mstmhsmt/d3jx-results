...
public class BatchStatement implements CQLStatement {
     ...
     * 		type of the batch
     * @param statements
     * 		the list of statements in the batch
     * @param attrs
    ...
    private ResultMessage executeWithConditions(BatchQueryOptions options, QueryState state, Dispatcher.RequestTime requestTime) {
        ...
        try (final RowIterator result = StorageProxy.cas(ksName, tableName, casRequest.key, casRequest, options.getSerialConsistency(), options.getConsistency(), state.getClientState(), options.getNowInSeconds(state), requestTime)) {
    ...
    private ResultMessage executeInternalWithConditions(BatchQueryOptions options, QueryState state) {
        ...
        long nowInSeconds = options.getNowInSeconds(state);
        try (final RowIterator result = ModificationStatement.casInternal(state.getClientState(), request, timestamp, nowInSeconds)) {
