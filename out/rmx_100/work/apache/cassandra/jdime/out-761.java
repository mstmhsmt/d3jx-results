...
public class CassandraAuthorizer implements IAuthorizer {
  ...
  private static final String PERMISSIONS_CF_SCHEMA = String.format("CREATE TABLE %s.%s (" + "username text," + "resource text," + "permissions set<text>," + "PRIMARY KEY(username, resource)" + ") WITH gc_grace_seconds=%d", Auth.AUTH_KS, PERMISSIONS_CF, 90 * 24 * 60 * 60);
  ...
  public Set<Permission> authorize(AuthenticatedUser user, IResource resource) {
    ...
    try {
      ResultMessage.Rows rows = authorizeStatement.execute(
<<<<<<< commits-rmx_100/apache/cassandra/e2c74d56551575c553927a9c0f31fe07f2614894/CassandraAuthorizer-deecfdb.java
      QueryState.forInternalCalls()
=======
      ConsistencyLevel.LOCAL_ONE
>>>>>>> commits-rmx_100/apache/cassandra/d48a1871d8cd79c3330b355e153567828b0bfbeb/CassandraAuthorizer-0c94f57.java
      , new QueryOptions(ConsistencyLevel.ONE, Lists.newArrayList(ByteBufferUtil.bytes(user.getName()), ByteBufferUtil.bytes(resource.getName()))));
  ...
  private void modify(Set<Permission> permissions, IResource resource, String user, String op) throws RequestExecutionException {
    process(String.format("UPDATE %s.%s SET permissions = permissions %s {%s} WHERE username = \'%s\' AND resource = \'%s\'", Auth.AUTH_KS, PERMISSIONS_CF, op, "\'" + StringUtils.join(permissions, "\',\'") + "\'", escape(user), escape(resource.getName())));
  }
