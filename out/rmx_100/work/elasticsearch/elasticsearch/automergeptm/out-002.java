...
public class S3BlobStoreContainerTests extends ESTestCase {
  ...
  public void testExecuteMultipartUpload() throws IOException {
    final String bucketName = randomAlphaOfLengthBetween(1, 10);final String blobName = randomAlphaOfLengthBetween(1, 10);final BlobPath blobPath = BlobPath.EMPTY;if (randomBoolean()) {
      IntStream.of(randomIntBetween(1, 5)).forEach((value) -> BlobPath.EMPTY.add("path_" + value));
    }final long blobSize = ByteSizeUnit.GB.toBytes(randomIntBetween(1, 128));final long bufferSize = ByteSizeUnit.MB.toBytes(randomIntBetween(5, 1024));final S3BlobStore blobStore = mock(S3BlobStore.class);when(blobStore.bucket()).thenReturn(bucketName);when(blobStore.bufferSizeInBytes()).thenReturn(bufferSize);final boolean serverSideEncryption = randomBoolean();when(blobStore.serverSideEncryption()).thenReturn(serverSideEncryption);final StorageClass storageClass = randomFrom(StorageClass.values());when(blobStore.getStorageClass()).thenReturn(storageClass);final ObjectCannedACL cannedAccessControlList = randomBoolean() ? randomFrom(ObjectCannedACL.values()) : null;if (cannedAccessControlList != null) {
      when(blobStore.getCannedACL()).thenReturn(cannedAccessControlList);
    }final S3Client client = configureMockClient(blobStore);final var uploadId = randomIdentifier();final ArgumentCaptor<CreateMultipartUploadRequest> createMultipartUploadRequestCaptor = ArgumentCaptor.forClass(CreateMultipartUploadRequest.class);when(client.createMultipartUpload(createMultipartUploadRequestCaptor.capture())).thenReturn(CreateMultipartUploadResponse.builder().uploadId(uploadId).build());final ArgumentCaptor<UploadPartRequest> uploadPartRequestCaptor = ArgumentCaptor.forClass(UploadPartRequest.class);final ArgumentCaptor<RequestBody> uploadPartBodyCaptor = ArgumentCaptor.forClass(RequestBody.class);final List<String> expectedEtags = new ArrayList<>();final long partSize = Math.min(bufferSize, blobSize);long totalBytes = 0;do {
      expectedEtags.add(randomAlphaOfLength(50));
      totalBytes += partSize;
    } while(totalBytes < blobSize);when(client.uploadPart(uploadPartRequestCaptor.capture(), uploadPartBodyCaptor.capture())).thenAnswer((invocationOnMock) -> {
    ...
    });final ArgumentCaptor<CompleteMultipartUploadRequest> completeMultipartUploadRequestCaptor = ArgumentCaptor.forClass(CompleteMultipartUploadRequest.class);when(client.completeMultipartUpload(completeMultipartUploadRequestCaptor.capture())).thenReturn(CompleteMultipartUploadResponse.builder().build());final ByteArrayInputStream inputStream = new ByteArrayInputStream(new byte[0]);final S3BlobContainer blobContainer = new S3BlobContainer(blobPath, blobStore);blobContainer.executeMultipartUpload(randomPurpose(), blobStore, blobName, inputStream, blobSize);final CreateMultipartUploadRequest initRequest = createMultipartUploadRequestCaptor.getValue();assertEquals(bucketName, initRequest.bucket());assertEquals(blobPath.buildAsString() + blobName, initRequest.key());assertEquals(storageClass, initRequest.storageClass());assertEquals(cannedAccessControlList, initRequest.acl());if (serverSideEncryption) {
    ...
    }final Tuple<Long, Long> numberOfParts = S3BlobContainer.numberOfMultiparts(blobSize, bufferSize);final List<UploadPartRequest> uploadPartRequests = uploadPartRequestCaptor.getAllValues();assertEquals(numberOfParts.v1().intValue(), uploadPartRequests.size());final List<RequestBody> uploadPartBodies = uploadPartBodyCaptor.getAllValues();assertEquals(numberOfParts.v1().intValue(), uploadPartBodies.size());for (int i = 0; i < uploadPartRequests.size(); i++) {
    ...
    }final CompleteMultipartUploadRequest compRequest = completeMultipartUploadRequestCaptor.getValue();assertEquals(bucketName, compRequest.bucket());assertEquals(blobPath.buildAsString() + blobName, compRequest.key());assertEquals(uploadId, compRequest.uploadId());final List<String> actualETags = compRequest.multipartUpload().parts().stream().map(CompletedPart::eTag).collect(Collectors.toList());assertEquals(expectedEtags, actualETags);closeMockClient(blobStore);
  }
