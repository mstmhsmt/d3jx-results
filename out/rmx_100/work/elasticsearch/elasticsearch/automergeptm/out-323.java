...
public class GeohashCellQuery {
  ...
  public static class Builder extends AbstractQueryBuilder<Builder> {
    ...
    @Override public QueryValidationException validate() {
      ...
      if (levels != null && levels <= 0) {
        errors = QueryValidationException.addValidationError(NAME, "precision must be greater than 0. Found [" + levels + "]", errors);
      }
      return errors;
    }
    ...
    @Override protected Query doToQuery(QueryShardContext context) throws IOException {
      ...
      if (fieldType == null) {
        throw new QueryShardException(context, "failed to parse [{}] query. missing [{}] field [{}]", NAME, GeoPointFieldMapper.CONTENT_TYPE, fieldName);
      }
    ...
    @Override protected Builder doReadFrom(StreamInput in) throws IOException {
    ...
    @Override protected boolean doEquals(Builder other) {
      return Objects.equals(fieldName, other.fieldName) && Objects.equals(geohash, other.geohash) && Objects.equals(levels, other.levels) && Objects.equals(neighbors, other.neighbors);
    }
  ...
  public static class Parser extends BaseQueryParser<Builder> {
    ...
    @Override public Builder fromXContent(QueryParseContext parseContext) throws IOException, QueryParsingException {
      ...
      while ((token = parser.nextToken()) != Token.END_OBJECT) {
        if (token == Token.FIELD_NAME) {
          ...
          if (parseContext.isDeprecatedSetting(field)) {
            ...
            if (parseContext.parseFieldMatcher().match(field, PRECISION_FIELD)) {
              ...
              if (parseContext.parseFieldMatcher().match(field, NEIGHBORS_FIELD)) {
                parser.nextToken();
                neighbors = parser.booleanValue();
              } else {
                if (parseContext.parseFieldMatcher().match(field, AbstractQueryBuilder.NAME_FIELD)) {
                  parser.nextToken();
                  queryName = parser.text();
                } else {
                  if (parseContext.parseFieldMatcher().match(field, AbstractQueryBuilder.BOOST_FIELD)) {
                    parser.nextToken();
    ...
    @Override public GeohashCellQuery.Builder getBuilderPrototype() {
...
>>>>>>> commits-rmx_100/elasticsearch/elasticsearch/c8d1f7aa673e20a8806826b86b348dd34f9b4864/GeohashCellQuery-6067bc6.java
  }
}