...
public class InternalClusterService extends AbstractLifecycleComponent<ClusterService> implements ClusterService {
    ...
    final TimeValue timeout, final TimeoutClusterStateListener listener) {
    ...
    public List<PendingClusterTask> pendingTasks() {
        PrioritizedEsThreadPoolExecutor[] pendings = updateTasksExecutor.getPending();
    ...
    private class ReconnectToNodes implements Runnable {
        ...
        public void run() {
            ...
            for (DiscoveryNode node : clusterState.nodes()) {
                ...
                // we double check existence of node since connectToNode might take time...
                    if (!transportService.nodeConnected(node)) {
                        try {
                            ...
                            if (clusterState.nodes().nodeExists(node.id())) {
                                // double check here as well, maybe its gone?
    ...
    private static class DelegetingAckListener implements Discovery.AckListener {
        ...
        @Override
        public void onNodeAck(DiscoveryNode node, @Nullable
        Throwable t) {
