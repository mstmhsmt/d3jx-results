...
public class TransportCloseIndexAction
  ...
  public static final ActionType<CloseIndexResponse> TYPE =
      new ActionType<>(NAME);
  private static final Logger logger =
      LogManager.getLogger(TransportCloseIndexAction.class);
  ...
  public static final Setting<Boolean> CLUSTER_INDICES_CLOSE_ENABLE_SETTING =
      Setting.boolSetting("cluster.indices.close.enable", true,
  ...
  public TransportCloseIndexAction(
    ...
    super(NAME, transportService, clusterService, threadPool, actionFilters,
          CloseIndexRequest::new, CloseIndexResponse::new,
          EsExecutors.DIRECT_EXECUTOR_SERVICE);
    this.indexStateService = indexStateService;
<<<<<<< /root/work/elasticsearch/elasticsearch/d3j/patched-022-0.java
    this.projectResolver = projectResolver;
||||||| /root/work/elasticsearch/elasticsearch/d3j/base-022.java
=======
    this.indexNameExpressionResolver = indexNameExpressionResolver;
>>>>>>> /root/work/elasticsearch/elasticsearch/d3j/patched-022-1.java
    this.destructiveOperations = destructiveOperations;
    this.closeIndexEnabled = CLUSTER_INDICES_CLOSE_ENABLE_SETTING.get(settings);
    clusterSettings.addSettingsUpdateConsumer(
        CLUSTER_INDICES_CLOSE_ENABLE_SETTING, this::setCloseIndexEnabled);
  }
  private void setCloseIndexEnabled(boolean closeIndexEnabled) {
    this.closeIndexEnabled = closeIndexEnabled;
  }
  @Override
  protected void doExecute(Task task, CloseIndexRequest request,
    ...
    if (closeIndexEnabled == false) {
      throw new IllegalStateException(
          ...
          (": true] to enable it. NOTE: closed indices still consume a " +
  ...
  protected ClusterBlockException checkBlock(CloseIndexRequest request,
    ...
    return state.blocks().indicesBlockedException(
        projectMetadata.id(), ClusterBlockLevel.METADATA_WRITE,
        indexNameExpressionResolver.concreteIndexNames(projectMetadata,
                                                       request));
  }
  @Override
  protected void
  masterOperation(final Task task, final CloseIndexRequest request,
    ...
    if (concreteIndices == null || concreteIndices.length == 0) {
      listener.onResponse(
          new CloseIndexResponse(true, false, Collections.emptyList()));
      return;
    }
    final CloseIndexClusterStateUpdateRequest closeRequest =
        new CloseIndexClusterStateUpdateRequest(
            request.masterNodeTimeout(), request.ackTimeout(),
            projectResolver.getProjectId(), task.getId(),
            request.waitForActiveShards(), concreteIndices);
    indexStateService.closeIndices(
        closeRequest, listener.delegateResponse((delegatedListener, t) -> {
          logger.debug(()
                           -> "failed to close indices [" +
