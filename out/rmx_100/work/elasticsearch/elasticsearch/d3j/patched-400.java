...
public
class ParentFieldMapper extends AbstractFieldMapper implements RootMapper
{
   public static final String NAME = "_parent";
   public static final String CONTENT_TYPE = "_parent";
   public
   static
   class Defaults extends AbstractFieldMapper.Defaults
   {
   ...
   static
   class Builder
     extends AbstractFieldMapper.Builder<Builder,ParentFieldMapper>
   {
      ...
      public
      Builder fieldDataSettings(Settings settings) {
        this.fieldDataSettings = settings;
        return builder;
      }
      @Override
      public
      ParentFieldMapper build(BuilderContext context) {
        if (type == null) {
          throw new MapperParsingException("[_parent] field mapping must contain the [type] option");
        }
        fieldType.setNames(new MappedFieldType.Names(name, indexName,
                             indexName, name));
        return new ParentFieldMapper(fieldType, type, fieldDataSettings,
                 context.indexSettings());
      }
   }
   public
   static
   class TypeParser implements Mapper.TypeParser
   {
      @Override
      public
      Mapper.Builder parse(String name, Map<String,Object> node, 
        ...
        for (Iterator<Map.Entry<String,Object>> iterator =
          ...
          if (fieldName.equals("type")) {
            ...
            Map<String,String> fieldDataSettings =
              SettingsLoader.Helper.loadNestedFromMap(nodeMapValue(fieldNode,
                                                        "fielddata"));
            if (fieldDataSettings.containsKey(MappedFieldType.Loading.KEY)) {
              Settings settings =
                settingsBuilder().put(MappedFieldType.Loading.KEY,
                  fieldDataSettings.get(MappedFieldType.Loading.KEY)).build();
   ...
   public
   static
   class ParentFieldType extends MappedFieldType
   {
      public
      ParentFieldType() {
      ...
      protected
      ParentFieldType(ParentFieldType ref) {
        super(ref);
      }
      @Override
      public
      ...
      Query termsQuery(List values, @Nullable QueryParseContext context) {
        ...
        List<String> types =
          new ArrayList<>(context.mapperService().types().size());
   ...
   protected
   ParentFieldMapper(MappedFieldType fieldType, String type, 
                     @Nullable Settings fieldDataSettings, 
                     Settings indexSettings) {
     super(fieldType,
     Version.indexCreated(indexSettings).onOrAfter(Version.V_2_0_0),
     fieldDataSettings,
     indexSettings);
     this.type = type;
   ...
   ParentFieldMapper(Settings indexSettings) {
     this(Defaults.FIELD_TYPE.clone(),
     null,
     null,
     indexSettings);
     this.fieldType = this.fieldType.clone();
     this.fieldType.setFieldDataType(new FieldDataType("_parent",
                                       settingsBuilder().put(MappedFieldType.Loading.KEY,
                                         MappedFieldType.Loading.LAZY_VALUE)));
   ...
   FieldDataType defaultFieldDataType() {
     return new FieldDataType("_parent",
              settingsBuilder().put(MappedFieldType.Loading.KEY,
                MappedFieldType.Loading.EAGER_VALUE));
   }
   @Override
   public
   ...
   @Override
   protected
   void parseCreateField(ParseContext context, List<Field> fields)
     throws IOException {
     ...
     if (context.parser().currentName() != null && context.parser().currentName().equals(Defaults.NAME)) {
       ...
       fields.add(new Field(fieldType.names().indexName(),
                    Uid.createUid(context.stringBuilder(), type, parentId),
                    fieldType));
       if (hasDocValues()) {
       ...
       if (context.sourceToParse().parent() != null) {
         ...
         if (parsedParentId == null) {
           ...
           fields.add(new Field(fieldType.names().indexName(),
                        Uid.createUid(context.stringBuilder(), type,
                          parentId),
                        fieldType));
           if (hasDocValues()) {
             fields.add(createJoinField(type, parentId));
           }
         } else if (parentId != null && !parsedParentId.equals(Uid.createUid(
   ...
   private
   SortedDocValuesField createJoinField(String parentType, String id) {
     String joinField = joinField(parentType);
     return new SortedDocValuesField(joinField, new BytesRef(id));
   }
   public
   static
   String joinField(String parentType) {
   ...
   XContentBuilder toXContent(XContentBuilder builder, Params params)
     ...
     boolean includeDefaults =
       params.paramAsBoolean("include_defaults", false);
     ...
     if (customFieldDataSettings != null) {
       ...
       builder.field("fielddata",
         (Map)fieldType.fieldDataType().getSettings().getAsMap());
     }
     builder.endObject();
     return builder;
   }
   ...
   void merge(Mapper mergeWith, MergeResult mergeResult)
     ...
     if (!mergeResult.simulate()) {
       ...
       if (fieldMergeWith.customFieldDataSettings != null) {
         if (!Objects.equal(fieldMergeWith.customFieldDataSettings,
                this.customFieldDataSettings)) {
           this.customFieldDataSettings =
