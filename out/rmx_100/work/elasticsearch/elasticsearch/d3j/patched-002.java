...
public class S3BlobStoreContainerTests extends ESTestCase {
  public void testExecuteSingleUploadBlobSizeTooLarge() {
    ...
    final S3BlobContainer blobContainer =
        new S3BlobContainer(mock(BlobPath.class), blobStore);
    final IllegalArgumentException e = expectThrows(
        IllegalArgumentException.class,
        ()
            -> blobContainer.executeSingleUpload(
                randomPurpose(), blobStore, randomAlphaOfLengthBetween(1, 10),
                null, blobSize));
    assertEquals("Upload request size [" + blobSize +
                     "] can't be larger than 5gb",
                 e.getMessage());
  }
  public void testExecuteSingleUploadBlobSizeLargerThanBufferSize() {
    ...
    final IllegalArgumentException e =
        expectThrows(IllegalArgumentException.class,
                     ()
                         -> blobContainer.executeSingleUpload(
                             randomPurpose(), blobStore, blobName,
                             new ByteArrayInputStream(new byte[0]),
                             ByteSizeUnit.MB.toBytes(2)));
    assertEquals(
        "Upload request size [2097152] can't be larger than buffer size",
        e.getMessage());
  }
  public void testExecuteSingleUpload() throws IOException {
    ...
    if (randomBoolean()) {
      IntStream.of(randomIntBetween(1, 5))
          .forEach(value -> blobPath.add("path_" + value));
    }
    ...
    final S3BlobContainer blobContainer =
        new S3BlobContainer(blobPath, blobStore);
    ...
    final ObjectCannedACL cannedAccessControlList =
        randomBoolean() ? randomFrom(ObjectCannedACL.values()) : null;
    ...
    when(client.putObject(requestCaptor.capture(), bodyCaptor.capture()))
        .thenReturn(PutObjectResponse.builder().build());
    final ByteArrayInputStream inputStream =
        new ByteArrayInputStream(new byte[blobSize]);
    blobContainer.executeSingleUpload(randomPurpose(), blobStore, blobName,
    ...
    final IllegalArgumentException e = expectThrows(
        ...
        ()
            -> blobContainer.executeMultipartUpload(
                randomPurpose(), blobStore, randomAlphaOfLengthBetween(1, 10),
                null, blobSize));
    assertEquals("Multipart upload request size [" + blobSize +
                     "] can't be larger than 5tb",
                 e.getMessage());
  }
    ...
    final IllegalArgumentException e = expectThrows(
        ...
        ()
            -> blobContainer.executeMultipartUpload(
                ...
                null, blobSize));
    assertEquals("Multipart upload request size [" + blobSize +
                     "] can't be smaller than 5mb",
                 e.getMessage());
  }
  public void testExecuteMultipartUpload() throws IOException {
<<<<<<< /root/work/elasticsearch/elasticsearch/d3j/patched-002-0.java
    final var uploadId = randomIdentifier();
    when(client.createMultipartUpload(
             createMultipartUploadRequestCaptor.capture()))
        .thenReturn(
            CreateMultipartUploadResponse.builder().uploadId(uploadId).build());
    final ArgumentCaptor<RequestBody> uploadPartBodyCaptor =
        ArgumentCaptor.forClass(RequestBody.class);
    when(client.uploadPart(uploadPartRequestCaptor.capture(),
                           uploadPartBodyCaptor.capture()))
        .thenAnswer(invocationOnMock -> {
          final UploadPartRequest request =
              (UploadPartRequest)invocationOnMock.getArguments()[0];
          final UploadPartResponse.Builder responseBuilder =
    ...
    when(client.completeMultipartUpload(
             completeMultipartUploadRequestCaptor.capture()))
        .thenReturn(CompleteMultipartUploadResponse.builder().build());
||||||| /root/work/elasticsearch/elasticsearch/d3j/base-002.java
    final String bucketName = randomAlphaOfLengthBetween(1, 10);
    final String blobName = randomAlphaOfLengthBetween(1, 10);
    final BlobPath blobPath = BlobPath.EMPTY;
    if (randomBoolean()) {
      IntStream.of(randomIntBetween(1, 5))
          .forEach(value -> BlobPath.EMPTY.add("path_" + value));
    }
    ...
    final CannedAccessControlList cannedAccessControlList =
        randomBoolean() ? randomFrom(CannedAccessControlList.values()) : null;
    if (cannedAccessControlList != null) {
      when(blobStore.getCannedACL()).thenReturn(cannedAccessControlList);
    }
    final AmazonS3 client = configureMockClient(blobStore);
    final ArgumentCaptor<InitiateMultipartUploadRequest> initArgCaptor =
    ...
    when(client.initiateMultipartUpload(initArgCaptor.capture()))
        .thenReturn(initResult);
    final ArgumentCaptor<UploadPartRequest> uploadArgCaptor =
    ...
    when(client.uploadPart(uploadArgCaptor.capture()))
        .thenAnswer(invocationOnMock -> {
          final UploadPartRequest request =
              (UploadPartRequest)invocationOnMock.getArguments()[0];
    ...
    final ArgumentCaptor<CompleteMultipartUploadRequest> compArgCaptor =
        ArgumentCaptor.forClass(CompleteMultipartUploadRequest.class);
    when(client.completeMultipartUpload(compArgCaptor.capture()))
        .thenReturn(new CompleteMultipartUploadResult());
    final ByteArrayInputStream inputStream =
        new ByteArrayInputStream(new byte[0]);
    final S3BlobContainer blobContainer =
        new S3BlobContainer(blobPath, blobStore);
    blobContainer.executeMultipartUpload(randomPurpose(), blobStore, blobName,
                                         inputStream, blobSize);
...
=======
  ...
  void testExecuteMultipart(boolean doCopy) throws IOException {
    ...
    if (randomBoolean()) {
      IntStream.of(randomIntBetween(1, 5))
          .forEach(value -> sourceBlobPath.add("path_" + value));
    }
    final long blobSize = ByteSizeUnit.GB.toBytes(randomIntBetween(1, 128));
    final long bufferSize = ByteSizeUnit.MB.toBytes(randomIntBetween(5, 1024));
    final S3BlobStore blobStore = mock(S3BlobStore.class);
    ...
    final S3Client client = configureMockClient(blobStore);
    final ArgumentCaptor<CreateMultipartUploadRequest>
        createMultipartUploadRequestCaptor =
    ...
    final List<String> expectedEtags = new ArrayList<>();
    final long partSize =
        Math.min(doCopy ? ByteSizeUnit.GB.toBytes(5) : bufferSize, blobSize);
    long totalBytes = 0;
    do {
      expectedEtags.add(randomAlphaOfLength(50));
      totalBytes += partSize;
    ...
    if (doCopy) {
      when(client.copyPart(copyArgCaptor.capture()))
          .thenAnswer(invocationOnMock -> {
            final CopyPartRequest request =
                (CopyPartRequest)invocationOnMock.getArguments()[0];
    ...
    final ArgumentCaptor<CompleteMultipartUploadRequest>
    ...
    final S3BlobContainer sourceContainer =
        new S3BlobContainer(sourceBlobPath, sourceBlobStore);
    if (doCopy) {
      blobContainer.executeMultipartCopy(randomPurpose(), sourceContainer,
                                         sourceBlobName, blobName, blobSize);
    } else {
      blobContainer.executeMultipartUpload(randomPurpose(), blobStore, blobName,
                                           inputStream, blobSize);
    }
    final CreateMultipartUploadRequest initRequest =
        createMultipartUploadRequestCaptor.getValue();
...
>>>>>>> /root/work/elasticsearch/elasticsearch/d3j/patched-002-1.java
    if (serverSideEncryption) {
<<<<<<< /root/work/elasticsearch/elasticsearch/d3j/patched-002-0.java
      assertEquals(PutObjectRequest.builder()
                       .serverSideEncryption("AES256")
...
||||||| /root/work/elasticsearch/elasticsearch/d3j/base-002.java
      assertEquals(ObjectMetadata.AES_256_SERVER_SIDE_ENCRYPTION,
                   initRequest.getObjectMetadata().getSSEAlgorithm());
=======
      assertEquals(initRequest.getObjectMetadata().build());
>>>>>>> /root/work/elasticsearch/elasticsearch/d3j/patched-002-1.java
    }
<<<<<<< /root/work/elasticsearch/elasticsearch/d3j/patched-002-0.java
    final List<RequestBody> uploadPartBodies =
        uploadPartBodyCaptor.getAllValues();
    assertEquals(numberOfParts.v1().intValue(), uploadPartBodies.size());
    for (int i = 0; i < uploadPartRequests.size(); i++) {
      ...
      assertEquals(uploadRequest.sdkPartType() + " at " + i + " of " +
                       uploadPartRequests.size(),
                   uploadRequest.sdkPartType() == SdkPartType.LAST,
                   i == uploadPartRequests.size() - 1);
      assertEquals("part " + i,
                   uploadRequest.sdkPartType() == SdkPartType.LAST
                       ? Optional.of(numberOfParts.v2())
                       : Optional.of(bufferSize),
                   uploadPartBodies.get(i).optionalContentLength());
||||||| /root/work/elasticsearch/elasticsearch/d3j/base-002.java
    final Tuple<Long, Long> numberOfParts =
        S3BlobContainer.numberOfMultiparts(blobSize, bufferSize);
    final List<UploadPartRequest> uploadRequests =
        uploadArgCaptor.getAllValues();
    ...
    for (int i = 0; i < uploadRequests.size(); i++) {
      ...
      if (i == (uploadRequests.size() - 1)) {
        ...
        assertEquals(numberOfParts.v2().longValue(),
                     uploadRequest.getPartSize());
      } else {
...
=======
    final Tuple<Long, Long> numberOfParts =
        S3BlobContainer.numberOfMultiparts(blobSize, partSize);
    if (doCopy) {
      ...
      for (int i = 0; i < copyRequests.size(); i++) {
        ...
        final long endOffset =
            Math.min(startOffset + partSize - 1, blobSize - 1);
        assertEquals(sourceBucketName, request.getSourceBucketName());
        assertEquals(sourceBlobPath.buildAsString() + sourceBlobName,
                     request.getSourceKey());
        assertEquals(bucketName, request.getDestinationBucketName());
        assertEquals(blobPath.buildAsString() + blobName,
      ...
      final List<UploadPartRequest> uploadPartRequests =
          uploadPartRequestCaptor.getAllValues();
      ...
      for (int i = 0; i < uploadPartRequests.size(); i++) {
        ...
        assertEquals(uploadRequest.sdkPartType());
        if (i == (uploadRequests.size() - 1)) {
          assertTrue(uploadRequest.isLastPart());
          assertEquals(numberOfParts.v2().longValue(),
                       uploadRequest.getPartSize());
          ...
          assertFalse(uploadRequest.isLastPart());
          assertEquals(bufferSize, uploadPartBodies.size());
        }
      }
>>>>>>> /root/work/elasticsearch/elasticsearch/d3j/patched-002-1.java
    }
...
=======
    final CompleteMultipartUploadRequest compRequest =
    ...
    assertEquals(compRequest.uploadId());
    final List<String> actualETags = compRequest.getPartETags()
>>>>>>> /root/work/elasticsearch/elasticsearch/d3j/patched-002-1.java
                                         .stream()
  ...
  public void testExecuteMultipartUploadAborted() {
    ...
    when(blobStore.bucket()).thenReturn(bucketName);
    when(blobStore.bufferSizeInBytes()).thenReturn(bufferSize);
    when(blobStore.getStorageClass())
        .thenReturn(randomFrom(StorageClass.values()));
    final S3Client client = mock(S3Client.class);
    final SdkHttpClient httpClient = mock(SdkHttpClient.class);
    final AmazonS3Reference clientReference =
        new AmazonS3Reference(client, httpClient);
    when(blobStore.clientReference()).then(invocation -> {
      clientReference.mustIncRef();
      return clientReference;
    });
    when(blobStore.getMetricPublisher(any(), any()))
        .thenReturn(new MetricPublisher() {
          @Override
    ...
    final List<AwsServiceException> exceptions =
        Arrays.asList(S3Exception.builder()
                          .message("Expected initialization request to fail")
                          .build(),
                      S3Exception.builder()
                          .message("Expected upload part request to fail")
                          .build(),
                      S3Exception.builder()
                          .message("Expected completion request to fail")
                          .build());
    if (stage == 0) {
      when(
          client.createMultipartUpload(any(CreateMultipartUploadRequest.class)))
          .thenThrow(exceptions.get(stage));
    } else if (stage == 1) {
      final CreateMultipartUploadResponse.Builder initResult =
          CreateMultipartUploadResponse.builder();
      initResult.uploadId(uploadId);
      when(
          client.createMultipartUpload(any(CreateMultipartUploadRequest.class)))
          .thenReturn(initResult.build());
      ...
      when(client.uploadPart(any(UploadPartRequest.class),
          ...
          .thenAnswer(invocationOnMock -> {
            final UploadPartRequest request =
                (UploadPartRequest)invocationOnMock.getArguments()[0];
            final UploadPartResponse.Builder response =
                UploadPartResponse.builder();
    ...
    final ArgumentCaptor<AbortMultipartUploadRequest> argumentCaptor =
        ArgumentCaptor.forClass(AbortMultipartUploadRequest.class);
    when(client.abortMultipartUpload(argumentCaptor.capture()))
        .thenReturn(AbortMultipartUploadResponse.builder().build());
    final IOException e = expectThrows(IOException.class, () -> {
      final S3BlobContainer blobContainer =
          new S3BlobContainer(BlobPath.EMPTY, blobStore);
      blobContainer.executeMultipartUpload(
          randomPurpose(), blobStore, blobName,
          new ByteArrayInputStream(new byte[0]), blobSize);
    });
    assertEquals("Unable to upload or copy object [" + blobName +
  ...
  public void testCopy() throws Exception {
    ...
    when(blobStore.maxCopySizeBeforeMultipart())
        .thenReturn(S3Repository.MIN_PART_SIZE_USING_MULTIPART.getBytes());
    final var sourceBlobPath =
        BlobPath.EMPTY.add(randomAlphaOfLengthBetween(1, 10));
    final var sourceBlobContainer =
        new S3BlobContainer(sourceBlobPath, blobStore);
    final var destinationBlobPath =
        BlobPath.EMPTY.add(randomAlphaOfLengthBetween(1, 10));
    final var destinationBlobContainer =
        new S3BlobContainer(destinationBlobPath, blobStore);
    final var client = configureMockClient(blobStore);
    final ArgumentCaptor<CopyObjectRequest> captor =
        ArgumentCaptor.forClass(CopyObjectRequest.class);
    when(client.copyObject(captor.capture()))
        .thenReturn(new CopyObjectResult());
    destinationBlobContainer.copyBlob(randomPurpose(), sourceBlobContainer,
    ...
    assertEquals(destinationBlobPath.buildAsString() + blobName,
                 request.getDestinationKey());
  ...
  public void testNumberOfMultiparts() {
    final ByteSizeUnit unit = randomFrom(ByteSizeUnit.BYTES, ByteSizeUnit.KB,
                                         ByteSizeUnit.MB, ByteSizeUnit.GB);
    ...
    final long remaining = randomIntBetween(
        1, (size > Integer.MAX_VALUE) ? Integer.MAX_VALUE : (int)size - 1);
    assertNumberOfMultiparts(factor + 1, remaining, (size * factor) + remaining,
                             size);
  }
  public void testInitCannedACL() {
    String[] aclList =
        new String[] {"private",           "public-read",
                      "public-read-write", "authenticated-read",
                      "bucket-owner-read", "bucket-owner-full-control"};
    assertThat(S3BlobStore.initCannedACL(null),
               equalTo(ObjectCannedACL.PRIVATE));
  ...
  public void testInvalidCannedACL() {
    BlobStoreException ex =
        expectThrows(BlobStoreException.class,
                     () -> S3BlobStore.initCannedACL("test_invalid"));
    assertThat(ex.getMessage(),
               equalTo("cannedACL is not valid: [test_invalid]"));
  }
  public void testInitStorageClass() {
    assertThat(S3BlobStore.initStorageClass(null),
               equalTo(StorageClass.STANDARD));
    assertThat(S3BlobStore.initStorageClass(""),
               equalTo(StorageClass.STANDARD));
    assertThat(S3BlobStore.initStorageClass("standard"),
               equalTo(StorageClass.STANDARD));
    assertThat(S3BlobStore.initStorageClass("standard_ia"),
               equalTo(StorageClass.STANDARD_IA));
    assertThat(S3BlobStore.initStorageClass("onezone_ia"),
               equalTo(StorageClass.ONEZONE_IA));
    assertThat(S3BlobStore.initStorageClass("reduced_redundancy"),
               equalTo(StorageClass.REDUCED_REDUNDANCY));
    assertThat(S3BlobStore.initStorageClass("intelligent_tiering"),
               equalTo(StorageClass.INTELLIGENT_TIERING));
  }
  ...
  public void testInvalidStorageClass() {
    ...
    assertThat(ex.getMessage(),
               equalTo("`whatever` is not a known S3 Storage Class."));
  }
  public void testRejectGlacierStorageClass() {
    ...
    assertThat(ex.getMessage(),
               equalTo("Glacier storage class is not supported"));
  }
  private static void assertNumberOfMultiparts(final int expectedParts,
                                               final long expectedRemaining,
                                               long totalSize, long partSize) {
    final Tuple<Long, Long> result =
        S3BlobContainer.numberOfMultiparts(totalSize, partSize);
    assertEquals("Expected number of parts [" + expectedParts + "] but got [" +
                     result.v1() + "]",
                 expectedParts, (long)result.v1());
    assertEquals("Expected remaining [" + expectedRemaining + "] but got [" +
                     result.v2() + "]",
                 expectedRemaining, (long)result.v2());
  }
}