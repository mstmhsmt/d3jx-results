...
>>>>>>> commits-rmx_100/elasticsearch/elasticsearch/4e454439bb3d9b9a4e949dfed98736303a584e65/Definition-f20393f.java
  ...
  private Map<String, List<String>> addStructs() {
    ...
    int currentLine = -1;
    try {
      try (InputStream stream = Definition.class.getResourceAsStream(DEFINITION_FILE); LineNumberReader reader = new LineNumberReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
        String line = null;
...
>>>>>>> Unknown file: This is a bug in JDime.
  ...
  private final void addMethodInternal(final String struct, final String name, final String alias, final Type rtn, final Type[] args) {
    final Struct owner = structsMap.get(struct);
    if (owner == null) {
  ...
  private final void addFieldInternal(final String struct, final String name, final String alias, final Type type) {
    final Struct owner = structsMap.get(struct);
    if (owner == null) {
  ...
  private void addRuntimeClass(final Struct struct) {
    ...
    for (final Map.Entry<MethodKey, Method> method : methods.entrySet()) {
      ...
      if (m.arguments.size() == 0 && name.startsWith("get") && name.length() > 3 && Character.isUpperCase(name.charAt(3))) {
        ...
        if (m.arguments.size() == 0 && name.startsWith("is") && name.length() > 2 && Character.isUpperCase(name.charAt(2))) {
          final StringBuilder newName = new StringBuilder();
...
<<<<<<< commits-rmx_100/elasticsearch/elasticsearch/ce9b36590aa884c9bcabe183a974175d41901314/Definition-9e665e8.java
  private final void addTransform(final Type from, final Type to, final String struct, final String name, final boolean statik, final boolean explicit) {
    final Struct owner = structsMap.get(struct);
    if (owner == null) {
