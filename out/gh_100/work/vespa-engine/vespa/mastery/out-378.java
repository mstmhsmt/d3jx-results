...
public class JRTConfigRequester implements RequestWaiter {
    ...
    static final public ConfigSourceSet defaultSourceSet = ConfigSourceSet.createDefault();
    ...
    static public JRTConfigRequester create(ConfigSourceSet sourceSet, TimingValues timingValues) {
    ...
    private <T extends ConfigInstance> void doRequest(JRTConfigSubscription<T> sub, JRTClientConfigRequest req) {
        ...
        log.log(FINE, () -> "Requesting config for " + sub + " on connection " + connection + " with client timeout " + jrtClientTimeout + (log.isLoggable(FINEST) ? (",defcontent=" + req.getDefContent().asString()) : ""));
    ...
    public void handleRequestDone(Request req) {
        ...
        try {
            ...
            if (sub != null) {
                ...
                log.log(SEVERE, "Failed to get subscription object from JRT config callback: " + Exceptions.toMessageString(e));
            }
        }
    }

    private void doHandle(JRTConfigSubscription<ConfigInstance> sub, JRTClientConfigRequest jrtReq, Connection connection) {
        if (subscriptionIsClosed(sub))
            return;
        ...
        switch(jrtReq.errorCode()) {
            ...
            case ErrorCode.UNKNOWN_VESPA_VERSION:
                if (noApplicationWarningLogged.isBefore(Instant.now().minus(delayBetweenWarnings))) {
                    log.log(WARNING, "Request callback failed: " + ErrorCode.getName(jrtReq.errorCode()) + ". Connection spec: " + connection.getAddress() + ", error message: " + jrtReq.errorMessage());
    ...
    private void handleFailedRequest(JRTClientConfigRequest jrtReq, JRTConfigSubscription<ConfigInstance> sub, Connection connection) {
        ...
        log.log(INFO, "Failure of config subscription to " + connection.getAddress() + ", clients will keep existing config until resolved: " + sub);
        ...
        log.log(logLevel, () -> "Request for config " + jrtReq.getShortDescription() + "' failed with error code " + jrtReq.errorCode() + " (" + jrtReq.errorMessage() + "), scheduling new request " + " in " + delay + " ms");
