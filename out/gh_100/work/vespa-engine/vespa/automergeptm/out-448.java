...
public class AutoscalingMaintainer extends NodeRepositoryMaintainer {
  ...
  public AutoscalingMaintainer(NodeRepository nodeRepository, Deployer deployer, Metric metric, Duration interval) {
  ...
  @Override protected double maintain() {
    if (!nodeRepository().nodes().isWorking()) {
      return 0.0;
    }
    if (!nodeRepository().zone().environment().isAnyOf(Environment.dev, Environment.prod)) {
      return 1.0;
    }
  ...
  private void autoscale(ApplicationId applicationId, ClusterSpec.Id clusterId, NodeList clusterNodes) {
    ...
    if (advice.isPresent() && !cluster.get().targetResources().equals(advice.target()) || (updatedCluster != cluster.get() || !advice.reason().equals(cluster.get().autoscalingStatus()))) {
      try (var lock = nodeRepository().nodes().lock(applicationId)) {
        ...
        updatedCluster = updateCompletion(cluster.get(), clusterNodes).with(advice.reason()).withTarget(advice.target());
  ...
  private Cluster updateCompletion(Cluster cluster, NodeList clusterNodes) {
    ...
    if (event.completion().isPresent()) {
      return cluster;
    }
    if (clusterNodes.retired().stream().anyMatch((node) -> node.history().hasEventAt(History.Event.Type.retired, event.at()))) {
      return cluster;
    }
    for (var nodeTimeseries : nodeRepository().metricsDb().getNodeTimeseries(Duration.between(event.at(), clock().instant()), clusterNodes)) {
      Optional<NodeMetricSnapshot> onNewGeneration = nodeTimeseries.asList().stream().filter((snapshot) -> snapshot.generation() >= event.generation()).findAny();
      if (onNewGeneration.isEmpty()) {
        return cluster;
      }
    }
  ...
  private void logAutoscaling(ClusterResources target, ApplicationId application, Cluster cluster, NodeList clusterNodes) {
