...
public class ApplicationRepository implements com.yahoo.config.provision.Deployer {
  ...
  public static class Builder {
    ...
    public ApplicationRepository build() {
      return new ApplicationRepository(tenantRepository, hostProvisioner, InfraDeployerProvider.empty().getInfraDeployer(), configConvergenceChecker, httpProxy, configserverConfig, orchestrator, logRetriever, clock, testerClient, metric, secretStoreValidator, ClusterReindexingStatusClient.DUMMY_INSTANCE, flagSource);
    }
  }
  ...
  @Override public boolean bootstrapping() {
  ...
  private PrepareResult deploy(File applicationDir, PrepareParams prepareParams, DeployHandlerLogger logger) {
    long sessionId = createSession(prepareParams.getApplicationId(), prepareParams.tags(), prepareParams.getTimeoutBudget(), applicationDir, logger);
    Deployment deployment = prepare(sessionId, prepareParams, logger);
  ...
  /**
     ...
     */
  @Override public Optional<com.yahoo.config.provision.Deployment> deployFromLocalActive(ApplicationId application) {
  ...
  /**
     ...
     *         node in the config server cluster)
     */
  @Override public Optional<com.yahoo.config.provision.Deployment> deployFromLocalActive(ApplicationId application, boolean bootstrap) {
    return deployFromLocalActive(application, Duration.ofSeconds(configserverConfig.zookeeper().barrierTimeout()).plus(Duration.ofSeconds(5)), bootstrap);
  }
  ...
  /**
     ...
     *         node in the config server cluster)
     */
  @Override public Optional<com.yahoo.config.provision.Deployment> deployFromLocalActive(ApplicationId application, Duration timeout, boolean bootstrap) {
    ...
    return Optional.of(Deployment.unprepared(newSession, this, hostProvisioner, tenant, logger, timeout, clock, false, bootstrap));
  }

  @Override public Optional<Instant> lastDeployTime(ApplicationId application) {
    Tenant tenant = tenantRepository.getTenant(application.tenant());
    if (tenant == null) {
      return Optional.empty();
    }
  ...
  static void checkIfActiveIsNewerThanSessionToBeActivated(long sessionId, long currentActiveSessionId) {
    if (sessionId < currentActiveSessionId) {
      throw new ActivationConflictException("Cannot activate session " + sessionId + ", because it is older than current active session (" + currentActiveSessionId + ")");
    }
  }
  ...
  public boolean delete(ApplicationId applicationId) {
    ...
    Optional<ApplicationTransaction> applicationTransaction = hostProvisioner.map((provisioner) -> provisioner.lock(applicationId)).map((lock) -> new ApplicationTransaction(lock, transaction));
    try (var applicationLock = tenantApplications.lock(applicationId)) {
      ...
      transaction.add(new ContainerEndpointsCache(tenant.getPath(), curator).delete(applicationId));
  ...
  public List<String> deleteUnusedFileDistributionReferences(File fileReferencesPath, Duration keepFileReferencesDuration, int numberToAlwaysKeep) {
  ...
  private Set<String> getFileReferencesInUse() {
    ...
    for (var applicationId : listApplications()) {
      ...
      fileReferencesInUse.addAll(app.getModel().fileReferences().stream().map(FileReference::value).collect(Collectors.toSet()));
    }
  ...
  private List<String> sortedUnusedFileReferences(File fileReferencesPath, Set<String> fileReferencesInUse, Duration keepFileReferences) {
    ...
    return fileReferencesOnDisk.stream().filter((fileReference) -> !fileReferencesInUse.contains(fileReference)).filter((fileReference) -> isLastFileAccessBefore(new File(fileReferencesPath, fileReference), instant)).sorted(Comparator.comparing((a) -> lastAccessed(new File(fileReferencesPath, a)))).collect(Collectors.toList());
  }
  ...
  private Application getApplication(ApplicationId applicationId, Optional<Version> version) {
    try {
      ...
      if (activeApplicationSet.isEmpty()) {
  ...
  public List<ApplicationId> listApplications() {
    return tenantRepository.getAllTenants().stream().flatMap((tenant) -> tenant.getApplicationRepo().activeApplications().stream()).collect(Collectors.toList());
  }
  ...
  public ServiceResponse checkServiceForConfigConvergence(ApplicationId applicationId, String hostAndPort, Duration timeout, Optional<Version> vespaVersion) {
    return convergeChecker.getServiceConfigGeneration(getApplication(applicationId, vespaVersion), hostAndPort, timeout);
  }

  public ServiceListResponse servicesToCheckForConfigConvergence(ApplicationId applicationId, Duration timeoutPerService, Optional<Version> vespaVersion) {
    return convergeChecker.checkConvergenceForAllServices(getApplication(applicationId, vespaVersion), timeoutPerService);
  }

  public ConfigConvergenceChecker configConvergenceChecker() {
  ...
  public Activation activate(Session session, ApplicationId applicationId, Tenant tenant, boolean force) {
    ...
    try (var sessionLock = tenant.getApplicationRepo().lock(applicationId)) {
      ...
      if (applicationTransaction.isPresent()) {
        hostProvisioner.get().activate(session.getAllocatedHosts().getHosts(), new ActivationContext(session.getSessionId()), applicationTransaction.get());
        applicationTransaction.get().nested().commit();
      } else {
        transaction.commit();
      }
  ...
  public long createSessionFromExisting(ApplicationId applicationId, boolean internalRedeploy, TimeoutBudget timeoutBudget, DeployLogger deployLogger) {
    Tenant tenant = getTenant(applicationId);
    SessionRepository sessionRepository = tenant.getSessionRepository();
  ...
  public long createSession(ApplicationId applicationId, Tags tags, TimeoutBudget timeoutBudget, InputStream in, String contentType, DeployLogger logger) {
    File tempDir = uncheck(() -> Files.createTempDirectory("deploy")).toFile();
    long sessionId;
    try {
  ...
  public void deleteExpiredLocalSessions() {
    ...
    sessionsPerTenant.values().forEach((sessionList) -> sessionList.stream().map(Session::getOptionalApplicationId).filter(Optional::isPresent).forEach((appId) -> applicationIds.add(appId.get())));
  ...
  public int deleteExpiredRemoteSessions(Clock clock) {
    return tenantRepository.getAllTenants().stream().map((tenant) -> tenant.getSessionRepository().deleteExpiredRemoteSessions(clock)).mapToInt((i) -> i).sum();
  }
  ...
  public Set<TenantName> deleteUnusedTenants(Duration ttlForUnusedTenant, Instant now) {
    return tenantRepository.getAllTenantNames().stream().filter((tenantName) -> activeApplications(tenantName).isEmpty()).filter((tenantName) -> !tenantName.equals(TenantName.defaultName())).filter((tenantName) -> !tenantName.equals(HOSTED_VESPA_TENANT)).filter((tenantName) -> getTenantMetaData(tenantRepository.getTenant(tenantName)).lastDeployTimestamp().isBefore(now.minus(ttlForUnusedTenant))).peek(tenantRepository::deleteTenant).collect(Collectors.toSet());
  }
  ...
  public ApplicationReindexing getReindexing(ApplicationId id) {
    return requireDatabase(id).readReindexingStatus(id).orElseThrow(() -> new NotFoundException("Reindexing status not found for " + id));
  }
  ...
  public ApplicationId getApplicationIdForHostname(String hostname) {
    Optional<ApplicationId> applicationId = tenantRepository.getAllTenantNames().stream().map((tenantName) -> tenantRepository.getTenant(tenantName).getApplicationRepo().getApplicationIdForHostName(hostname)).filter(Objects::nonNull).findFirst();
  ...
  public Application getActiveApplication(ApplicationId applicationId) {
    return getActiveApplicationSet(applicationId).map((a) -> a.getForVersionOrLatest(Optional.empty(), clock.instant())).orElseThrow(() -> new RuntimeException("Found no active application for " + applicationId));
  }

  private File decompressApplication(InputStream in, String contentType, File tempDir) {
    try (CompressedApplicationInputStream application = CompressedApplicationInputStream.createFromCompressedStream(in, contentType, configserverConfig.maxApplicationPackageSize())) {
  ...
  public double getQuotaUsageRate(ApplicationId applicationId) {
    ...
    return application.getModel().provisioned().all().values().stream().map(Capacity::maxResources).mapToDouble((resources) -> resources.nodes() * resources.nodeResources().cost()).sum();
  }

  @Override public Duration serverDeployTimeout() {
  ...
  private void logConfigChangeActions(ConfigChangeActions actions, DeployLogger logger) {
    ...
    if (!refeedActions.isEmpty()) {
      logger.logApplicationPackage(Level.WARNING, "Change(s) between active and new application that may require re-feed:\n" + refeedActions.format());
    }
  ...
  public Zone zone() {
    return new Zone(SystemName.from(configserverConfig.system()), Environment.from(configserverConfig.environment()), RegionName.from(configserverConfig.region()));
  }
  ...
  public static class ActionTimer implements AutoCloseable {
    ...
    @Override public void close() {
      metric.set(name, Duration.between(start, clock.instant()).toMillis(), metric.createContext(Map.of("applicationId", id.toFullString(), "tenantName", id.tenant().value(), "app", id.application().value() + "." + id.instance().value(), "zone", environment + "." + region)));
    }
  }
  ...
  private static class NoopCompletionWaiter implements CompletionWaiter {
    @Override public void awaitCompletion(Duration timeout) {
    }

    @Override public void notifyCompletion() {
    }
  }
}