...
public class SearchService extends AbstractLifecycleComponent<SearchService> implements IndexEventListener {
    ...
    final private IndicesWarmer indicesWarmer;

    final private ScriptService scriptService;

    final private PageCacheRecycler pageCacheRecycler;

    final private BigArrays bigArrays;

    final private DfsPhase dfsPhase;

    final private QueryPhase queryPhase;

    final private FetchPhase fetchPhase;

    final private IndicesRequestCache indicesQueryCache;

    final private long defaultKeepAlive;

    private volatile TimeValue defaultSearchTimeout;

    final private ScheduledFuture<?> keepAliveReaper;

    final private AtomicLong idGenerator = new AtomicLong();

    final private ConcurrentMapLong<SearchContext> activeContexts = ConcurrentCollections.newConcurrentMapLongWithAggressiveConcurrency();

    final private Map<String, SearchParseElement> elementParsers;

    final private ParseFieldMatcher parseFieldMatcher;
    ...
    static final private int[] EMPTY_DOC_IDS = new int[0];
