...
@FxmlView public class ProposalsView extends ActivatableView<GridPane, Void> implements BsqBalanceListener, DaoStateListener {
  ...
  @Inject private ProposalsView(DaoFacade daoFacade, BsqWalletService bsqWalletService, PhasesView phasesView, DaoStateService daoStateService, BsqFormatter bsqFormatter, BSFormatter btcFormatter) {
  ...
  @Override protected void activate() {
    ...
    onUpdateBalances(bsqWalletService.getAvailableBalance(), bsqWalletService.getAvailableNonBsqBalance(), bsqWalletService.getUnverifiedBalance(), bsqWalletService.getLockedForVotingBalance(), bsqWalletService.getLockupBondsBalance(), bsqWalletService.getUnlockingBondsBalance());
    updateListItems();
    updateViews();
  }
  ...
  @Override public void onUpdateBalances(Coin confirmedBalance, Coin availableNonBsqBalance, Coin pendingBalance, Coin lockedForVotingBalance, Coin lockupBondsBalance, Coin unlockingBondsBalance) {
    if (isBlindVotePhaseButNotLastBlock()) {
      stakeInputTextField.setPromptText(Res.get("dao.proposal.myVote.stake.prompt", bsqFormatter.formatCoinWithCode(confirmedBalance)));
    } else {
      stakeInputTextField.setPromptText("");
    }
  }

  @Override public void onNewBlockHeight(int blockHeight) {
  }

  @Override public void onParseTxsComplete(Block block) {
    updateViews();
  }

  @Override public void onParseBlockChainComplete() {
    updateListItems();
    applyMerit();
  }
  ...
  private void fillListItems() {
    if (daoFacade.phaseProperty().get().ordinal() < DaoPhase.Phase.BLIND_VOTE.ordinal()) {
      ...
      listItems.setAll(list.stream().map((proposal) -> new ProposalsListItem(proposal, daoFacade, bsqFormatter)).collect(Collectors.toSet()));
    } else {
      List<Ballot> ballotList = daoFacade.getBallotsOfCycle();
      listItems.setAll(ballotList.stream().map((ballot) -> new ProposalsListItem(ballot, daoFacade, bsqFormatter)).collect(Collectors.toSet()));
    }
    updateViews();
  }
  ...
  private void createAllFieldsOnProposalDisplay(Proposal proposal, @Nullable Ballot ballot, @Nullable EvaluatedProposal evaluatedProposal) {
    ...
    Tuple3<Button, Button, Button> tuple = add3ButtonsAfterGroup(proposalDisplayGridPane, proposalDisplay.incrementAndGetGridRow(), Res.get("dao.proposal.myVote.accept"), Res.get("dao.proposal.myVote.reject"), Res.get("dao.proposal.myVote.removeMyVote"));
  ...
  private void onPhaseChanged(DaoPhase.Phase phase) {
    ...
    if (removeProposalButton != null) {
      boolean doShowRemoveButton = phase == DaoPhase.Phase.PROPOSAL && selectedItem != null && daoFacade.isMyProposal(selectedItem.getProposal());
  ...
  private void onSelectProposal(ProposalsListItem item) {
    ...
    if (selectedItem != null) {
      EvaluatedProposal evaluatedProposal = daoStateService.getEvaluatedProposalList().stream().filter((e) -> daoFacade.isTxInCorrectCycle(e.getProposal().getTxId(), daoFacade.getChainHeight())).filter((e) -> e.getProposalTxId().equals(selectedItem.getProposal().getTxId())).findAny().orElse(null);
  ...
  private void applyMerit() {
    boolean hasConfirmedVoteTxInCycle = daoFacade.getMyVoteListForCycle().stream().map((myVote) -> daoFacade.getTx(myVote.getTxId())).findAny().isPresent();
    long merit;
  ...
  private void onVote() {
    ...
    try {
      ...
      GUIUtil.showBsqFeeInfoPopup(blindVoteFee, miningFee, txSize, bsqFormatter, btcFormatter, Res.get("dao.blindVote"), () -> publishBlindVote(stake));
  ...
  private void updateViews() {
    ...
    voteButton.setDisable(!hasVotedOnProposal || !stakeInputTextField.getValidator().validate(stakeInputTextField.getText()).isValid);
    ...
    if (hasAlreadyVoted) {
      ...
      if (myVoteListForCycle.size() == 1) {
        Optional<MyVote> optionalMyVote = myVoteListForCycle.stream().filter((myVote) -> daoFacade.isTxInCorrectCycle(myVote.getHeight(), daoFacade.getChainHeight())).findAny();
  ...
  private boolean hasVotedOnProposal() {
    return listItems.stream().filter((e) -> e.getBallot() != null).map(ProposalsListItem::getBallot).anyMatch((e) -> e.getVote() != null);
  }
  ...
  private void createVoteView() {
    voteTitledGroupBg = addTitledGroupBg(root, ++gridRow, 4, Res.get("dao.proposal.votes.header"), 20);
    voteFields.add(voteTitledGroupBg);
    Tuple2<Label, TextField> meritTuple = addLabelTextField(root, gridRow, Res.getWithCol("dao.proposal.myVote.merit"), 40);
    ...
    stakeInputTextField = addInputTextField(root, ++gridRow, Res.getWithCol("dao.proposal.myVote.stake"));
    stakeInputTextField.setValidator(new BsqValidator(bsqFormatter));
    voteFields.add(stakeInputTextField);
    Tuple2<Label, TxIdTextField> blindVoteTxIdTuple = addLabelTxIdTextField(root, ++gridRow, Res.getWithCol("dao.proposal.myVote.blindVoteTxId"));
    ...
    Tuple2<Label, TxIdTextField> revealTxIdTuple = addLabelTxIdTextField(root, ++gridRow, Res.getWithCol("dao.proposal.myVote.revealTxId"));
    ...
    Tuple3<Button, BusyAnimation, Label> voteButtonTuple = addButtonBusyAnimationLabelAfterGroup(root, ++gridRow, Res.get("dao.proposal.myVote.button"));
  ...
  private void createProposalColumns() {
    ...
    column.setCellFactory(new Callback<TableColumn<ProposalsListItem, ProposalsListItem>, TableCell<ProposalsListItem, ProposalsListItem>>() {
      @Override public TableCell<ProposalsListItem, ProposalsListItem> call(TableColumn<ProposalsListItem, ProposalsListItem> column) {
        return new TableCell<ProposalsListItem, ProposalsListItem>() {
          @Override public void updateItem(final ProposalsListItem item, boolean empty) {
