...
public class PendingTradesDataModel extends ActivatableDataModel {
  ...
  @Inject public PendingTradesDataModel(TradeManager tradeManager, BtcWalletService btcWalletService, TradeWalletService tradeWalletService, FeeService feeService, User user, KeyRing keyRing, DisputeManager disputeManager, Preferences preferences, P2PService p2PService, Navigation navigation, WalletPasswordWindow walletPasswordWindow, NotificationCenter notificationCenter) {
  ...
  public void onWithdrawRequest(String toAddress, Coin amount, Coin fee, KeyParameter aesKey, ResultHandler resultHandler, FaultHandler faultHandler) {
    ...
    if (toAddress != null && toAddress.length() > 0) {
      tradeManager.onWithdrawRequest(toAddress, amount, fee, aesKey, getTrade(), () -> {
  ...
  @Nullable public PaymentAccountContractData getSellersPaymentAccountContractData() {
  ...
  private void selectBestItem() {
    if (list.size() == 1) {
      ...
      if (list.size() > 1 && (selectedItemProperty.get() == null || !list.contains(selectedItemProperty.get()))) {
        doSelectItem(list.get(0));
      } else {
        if (list.size() == 0) {
          doSelectItem(null);
      ...
      if (depositTx != null) {
        ...
        transactions.stream().forEach((transaction) -> {
          ...
          if (!valueSentFromMe.isZero()) {
            candidates.addAll(transaction.getOutputs().stream().filter((transactionOutput) -> !btcWalletService.isTransactionOutputMine(transactionOutput)).filter((transactionOutput) -> transactionOutput.getScriptPubKey().isPayToScriptHash()).map((transactionOutput) -> transaction).collect(Collectors.toList()));
          }
        });
  ...
  private void doOpenDispute(boolean isSupportTicket, Transaction depositTx) {
    ...
    if (trade != null) {
      ...
      if (p2PService.isBootstrapped()) {
        disputeManager.sendOpenNewDisputeMessage(dispute, () -> navigation.navigateTo(MainView.class, DisputesView.class), (errorMessage) -> new Popup().warning(errorMessage).show());
      } else {
