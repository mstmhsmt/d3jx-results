...
public class DisputeManager {
  ...
  @Inject public DisputeManager(P2PService p2PService, TradeWalletService tradeWalletService, BtcWalletService walletService, TradeManager tradeManager, ClosedTradableManager closedTradableManager, OpenOfferManager openOfferManager, KeyRing keyRing, @Named(value = Storage.DIR_KEY) File storageDir) {
  ...
  public void onAllServicesInitialized() {
    if (p2PService.isBootstrapped()) {
      ...
      p2PService.addP2PServiceListener(new BootstrapListener() {
        @Override public void onBootstrapComplete() {
          applyMessages();
        }
      });
    }
  ...
  private void dispatchMessage(DisputeMessage message) {
    if (message instanceof OpenNewDisputeMessage) {
      ...
      if (message instanceof PeerOpenedDisputeMessage) {
        ...
        if (message instanceof DisputeCommunicationMessage) {
          ...
          if (message instanceof DisputeResultMessage) {
            onDisputeResultMessage((DisputeResultMessage) message);
          } else {
            if (message instanceof PeerPublishedPayoutTxMessage) {
  ...
  public void sendOpenNewDisputeMessage(Dispute dispute, boolean reOpen, ResultHandler resultHandler, FaultHandler faultHandler) {
    if (!disputes.contains(dispute)) {
      ...
      if (!storedDisputeOptional.isPresent() || reOpen) {
        ...
        p2PService.sendEncryptedMailboxMessage(dispute.getContract().arbitratorNodeAddress, dispute.getArbitratorPubKeyRing(), new OpenNewDisputeMessage(dispute, p2PService.getAddress(), UUID.randomUUID().toString()), new SendMailboxMessageListener() {
          @Override public void onArrived() {
            disputeCommunicationMessage.setArrived(true);
            resultHandler.handleResult();
          }

          @Override public void onStoredInMailbox() {
            disputeCommunicationMessage.setStoredInMailbox(true);
            resultHandler.handleResult();
          }

          @Override public void onFault(String errorMessage) {
  ...
  private void sendPeerPublishedPayoutTxMessage(Transaction transaction, Dispute dispute, Contract contract) {
    ...
    p2PService.sendEncryptedMailboxMessage(peerNodeAddress, peersPubKeyRing, new PeerPublishedPayoutTxMessage(transaction.bitcoinSerialize(), dispute.getTradeId(), p2PService.getAddress(), UUID.randomUUID().toString()), new SendMailboxMessageListener() {
      @Override public void onArrived() {
      }
      ...
      if (disputeOptional.isPresent()) {
        ...
        if (disputeResult.isLoserPublisher()) {
          if (publisher == DisputeResult.Winner.BUYER) {
            ...
            if (publisher == DisputeResult.Winner.SELLER) {
  ...
  private boolean isArbitrator(DisputeResult disputeResult) {
    return Arrays.equals(disputeResult.getArbitratorPubKey(), walletService.getOrCreateAddressEntry(AddressEntry.Context.ARBITRATOR).getPubKey());
  }

  public String getNrOfDisputes(boolean isBuyer, Contract contract) {
    return String.valueOf(getDisputesAsObservableList().stream().filter((e) -> {
