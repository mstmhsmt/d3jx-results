...
class DisputeManager
{
   private static final Logger log =
     LoggerFactory.getLogger(DisputeManager.class);
   ...
   private final CopyOnWriteArraySet<DecryptedMsgWithPubKey>
   decryptedMailboxMessageWithPubKeys = new CopyOnWriteArraySet<>();
   private final CopyOnWriteArraySet<DecryptedMsgWithPubKey>
   decryptedDirectMessageWithPubKeys = new CopyOnWriteArraySet<>();
   ...
   @Inject
   public
   DisputeManager(P2PService p2PService, 
                  TradeWalletService tradeWalletService, 
                  BtcWalletService walletService, TradeManager tradeManager, 
                  ClosedTradableManager closedTradableManager, 
                  OpenOfferManager openOfferManager, KeyRing keyRing, 
     ...
     p2PService.addDecryptedDirectMessageListener((decryptedMessageWithPubKey,
                                                   senderAddress) -> {
         decryptedDirectMessageWithPubKeys.add(decryptedMessageWithPubKey);
         if (p2PService.isBootstrapped())
           applyMessages();
       });
     p2PService.addDecryptedMailboxListener((decryptedMessageWithPubKey, 
                                             senderAddress) -> {
   ...
   public
   void cleanupDisputes() {
     disputes.stream().forEach(dispute -> {
         dispute.setStorage(getDisputeStorage());
         if (dispute.isClosed())
           closedDisputes.put(dispute.getTradeId(), dispute); else
   ...
   private
   void dispatchMessage(DisputeMessage message) {
     if (message instanceof OpenNewDisputeMessage)
       onOpenNewDisputeMessage((OpenNewDisputeMessage)message); else if (message instanceof PeerOpenedDisputeMessage)
       onPeerOpenedDisputeMessage((PeerOpenedDisputeMessage)message); else if (message instanceof DisputeCommunicationMessage)
       onDisputeDirectMessage((DisputeCommunicationMessage)message); else if (message instanceof DisputeResultMessage)
       onDisputeResultMessage((DisputeResultMessage)message); else if (message instanceof PeerPublishedPayoutTxMessage)
       onDisputedPayoutTxMessage((PeerPublishedPayoutTxMessage)message); else
       log.warn("Unsupported message at dispatchMessage.\nmessage=" + message);
   }
   public
   void sendOpenNewDisputeMessage(Dispute dispute, boolean reOpen, 
                                  ResultHandler resultHandler, 
                                  FaultHandler faultHandler) {
     if (!disputes.contains(dispute)) {
       final Optional<Dispute> storedDisputeOptional =
         findDispute(dispute.getTradeId(), dispute.getTraderId());
       if (!storedDisputeOptional.isPresent() || reOpen) {
         String sysMsg =
           dispute.isSupportTicket() ? Res.get("support.youOpenedTicket") : 
         Res.get("support.youOpenedDispute", disputeInfo);
         DisputeCommunicationMessage disputeCommunicationMessage =
           new DisputeCommunicationMessage(dispute.getTradeId(),
             keyRing.getPubKeyRing().hashCode(), false,
             Res.get("support.systemMsg", sysMsg), null,
             p2PService.getAddress(), new Date().getTime(), false, false,
         ...
         p2PService.sendEncryptedMailboxMessage(dispute.getContract().arbitratorNodeAddress,
           ...
           new SendMailboxMessageListener()
           {
              ...
              void onFault(String errorMessage) {
                ...
                faultHandler.handleFault("Sending dispute message failed: " + errorMessage,
                  new MessageDeliveryFailedException());
              }
           });
       } else {
         final String msg =
           "We got a dispute already open for that trade and trading peer.\n" + "TradeId = " + 
         dispute.getTradeId();
         log.warn(msg);
       ...
       final String msg =
         "We got a dispute msg what we have already stored. TradeId = " + 
       dispute.getTradeId();
       log.warn(msg);
       faultHandler.handleFault(msg, new DisputeAlreadyOpenException());
     }
   ...
   private
   void sendPeerOpenedDisputeMessage(Dispute disputeFromOpener) {
     Contract contractFromOpener = disputeFromOpener.getContract();
     PubKeyRing pubKeyRing =
       disputeFromOpener.isDisputeOpenerIsBuyer() ? contractFromOpener.getSellerPubKeyRing() : 
     contractFromOpener.getBuyerPubKeyRing();
     Dispute dispute =
       new Dispute(disputeStorage, disputeFromOpener.getTradeId(),
         pubKeyRing.hashCode(), !disputeFromOpener.isDisputeOpenerIsBuyer(),
         !disputeFromOpener.isDisputeOpenerIsMaker(), pubKeyRing,
         disputeFromOpener.getTradeDate(), contractFromOpener,
     ...
     if (!storedDisputeOptional.isPresent()) {
       String sysMsg =
         dispute.isSupportTicket() ? Res.get("support.peerOpenedTicket") : 
       ...
       PubKeyRing peersPubKeyRing =
         dispute.isDisputeOpenerIsBuyer() ? contract.getBuyerPubKeyRing() : 
       contract.getSellerPubKeyRing();
       NodeAddress peerNodeAddress =
         dispute.isDisputeOpenerIsBuyer() ? contract.getBuyerNodeAddress() : 
   ...
   public
   DisputeCommunicationMessage sendDisputeDirectMessage(Dispute dispute, 
                                                        String text, 
                                                        ArrayList<Attachment> attachments) {
     DisputeCommunicationMessage disputeCommunicationMessage =
       new DisputeCommunicationMessage(dispute.getTradeId(),
         dispute.getTraderPubKeyRing().hashCode(), isTrader(dispute), text,
         null, p2PService.getAddress(), new Date().getTime(), false, false,
         UUID.randomUUID().toString());
     ...
     if (isTrader(dispute)) {
       ...
       if (contract.getBuyerPubKeyRing().equals(receiverPubKeyRing))
         peerNodeAddress = contract.getBuyerNodeAddress(); else
     ...
     if (receiverPubKeyRing != null) {
       ...
       p2PService.sendEncryptedMailboxMessage(peerNodeAddress,
         receiverPubKeyRing, disputeCommunicationMessage,
         new SendMailboxMessageListener()
         {
            @Override
            public
   ...
   public
   void sendDisputeResultMessage(DisputeResult disputeResult, 
                                 Dispute dispute, String text) {
     DisputeCommunicationMessage disputeCommunicationMessage =
       new DisputeCommunicationMessage(dispute.getTradeId(),
         dispute.getTraderPubKeyRing().hashCode(), false, text, null,
         p2PService.getAddress(), new Date().getTime(), false, false,
         UUID.randomUUID().toString());
     dispute.addDisputeMessage(disputeCommunicationMessage);
   ...
   private
   void sendPeerPublishedPayoutTxMessage(Transaction transaction, 
                                         Dispute dispute, Contract contract) {
     PubKeyRing peersPubKeyRing =
       dispute.isDisputeOpenerIsBuyer() ? contract.getSellerPubKeyRing() : 
     contract.getBuyerPubKeyRing();
     NodeAddress peerNodeAddress =
       dispute.isDisputeOpenerIsBuyer() ? contract.getSellerNodeAddress() : 
     contract.getBuyerNodeAddress();
     log.trace("sendPeerPublishedPayoutTxMessage to peerAddress " + peerNodeAddress);
     p2PService.sendEncryptedMailboxMessage(peerNodeAddress, peersPubKeyRing,
       new PeerPublishedPayoutTxMessage(transaction.bitcoinSerialize(),
         dispute.getTradeId(), p2PService.getAddress(),
         UUID.randomUUID().toString()),
       new SendMailboxMessageListener()
       {
          @Override
   ...
   void onOpenNewDisputeMessage(OpenNewDisputeMessage openNewDisputeMessage) {
     ...
     if (isArbitrator(dispute)) {
       if (!disputes.contains(dispute)) {
         ...
         log.warn("We got a dispute msg what we have already stored. TradeId = " + 
           dispute.getTradeId());
       }
     } else {
       ...
       if (!disputes.contains(dispute)) {
         ...
         if (!storedDisputeOptional.isPresent()) {
           Optional<Trade> tradeOptional =
             tradeManager.getTradeById(dispute.getTradeId());
   ...
   private
   void onDisputeDirectMessage(DisputeCommunicationMessage disputeCommunicationMessage) {
     ...
     Optional<Dispute> disputeOptional =
       findDispute(tradeId, disputeCommunicationMessage.getTraderId());
     ...
     if (disputeOptional.isPresent()) {
       ...
       if (!dispute.getDisputeCommunicationMessagesAsObservableList().contains(disputeCommunicationMessage))
         dispute.addDisputeMessage(disputeCommunicationMessage); else
       ...
       if (!delayMsgMap.containsKey(uid)) {
         Timer timer =
           UserThread.runAfter(() -> onDisputeDirectMessage(disputeCommunicationMessage),
             1);
   ...
   void onDisputeResultMessage(DisputeResultMessage disputeResultMessage) {
     ...
     if (!isArbitrator(disputeResult)) {
       ...
       Optional<Dispute> disputeOptional =
         findDispute(tradeId, disputeResult.traderId);
       ...
       if (disputeOptional.isPresent()) {
         ...
         if (!dispute.getDisputeCommunicationMessagesAsObservableList().contains(disputeCommunicationMessage))
           dispute.addDisputeMessage(disputeCommunicationMessage); else
           log.warn("We got a dispute mail msg what we have already stored. TradeId = " + 
             disputeCommunicationMessage.getTradeId());
         ...
         boolean isBuyer =
           keyRing.getPubKeyRing().equals(contract.getBuyerPubKeyRing());
         ...
         if (disputeResult.isLoserPublisher()) {
           if (publisher == DisputeResult.Winner.BUYER)
             publisher = DisputeResult.Winner.SELLER; else if (publisher == DisputeResult.Winner.SELLER)
         ...
         if ((isBuyer && publisher == DisputeResult.Winner.BUYER) || (!isBuyer && publisher == DisputeResult.Winner.SELLER)) {
           final Optional<Trade> tradeOptional =
             tradeManager.getTradeById(tradeId);
           Transaction payoutTx = null;
           if (tradeOptional.isPresent()) {
             payoutTx = tradeOptional.get().getPayoutTx();
           } else {
             final Optional<Tradable> tradableOptional =
               closedTradableManager.getTradableById(tradeId);
           ...
           if (payoutTx == null) {
             if (dispute.getDepositTxSerialized() != null) {
               try {
                 ...
                 byte[] multiSigPubKey =
                   isBuyer ? contract.getBuyerMultiSigPubKey() : contract.getSellerMultiSigPubKey();
                 DeterministicKey multiSigKeyPair =
                   walletService.getMultiSigKeyPair(dispute.getTradeId(),
                     multiSigPubKey);
                 Transaction signedDisputedPayoutTx =
                   tradeWalletService.traderSignAndFinalizeDisputedPayoutTx(
                     ...
                     multiSigKeyPair, contract.getBuyerMultiSigPubKey(),
                     contract.getSellerMultiSigPubKey(),
                     disputeResult.getArbitratorPubKey());
                 Transaction committedDisputedPayoutTx =
                   tradeWalletService.addTxToWallet(signedDisputedPayoutTx);
                 ...
                 tradeWalletService.broadcastTx(committedDisputedPayoutTx,
                   new FutureCallback<Transaction>()
                   {
                      ...
                      void onSuccess(Transaction transaction) {
                        ...
                        sendPeerPublishedPayoutTxMessage(transaction,
                          dispute, contract);
                        if (tradeManager.getTradeById(dispute.getTradeId()).isPresent())
                          tradeManager.closeDisputedTrade(dispute.getTradeId()); else {
                      ...
                      void onFailure(@NotNull Throwable t) {
                        log.error(t.getMessage());
                          ...
                          e) {
                 e.printStackTrace();
                 log.error("Error at traderSignAndFinalizeDisputedPayoutTx " + 
                   e.getMessage());
               } 
             } else {
         ...
         if (!delayMsgMap.containsKey(uid)) {
           Timer timer =
             UserThread.runAfter(() -> onDisputeResultMessage(disputeResultMessage),
               2);
           delayMsgMap.put(uid, timer);
         } else {
   ...
   private
   void onDisputedPayoutTxMessage(PeerPublishedPayoutTxMessage peerPublishedPayoutTxMessage) {
     ...
     if (disputeOptional.isPresent()) {
       ...
       Transaction walletTx =
         tradeWalletService.addTxToWallet(peerPublishedPayoutTxMessage.transaction);
       disputeOptional.get().setDisputePayoutTxId(walletTx.getHashAsString());
       BtcWalletService.printTx("Disputed payoutTx received from peer",
         walletTx);
       ...
       if (!delayMsgMap.containsKey(uid)) {
         Timer timer =
           UserThread.runAfter(() -> onDisputedPayoutTxMessage(peerPublishedPayoutTxMessage),
             3);
         delayMsgMap.put(uid, timer);
       } else {
   ...
   public
   Storage<DisputeList> getDisputeStorage() {
     ...
     return String.valueOf(getDisputesAsObservableList().stream().filter(e -> {
                               ...
                               if (isBuyer) {
                                 NodeAddress buyerNodeAddress =
                                   contract1.getBuyerNodeAddress();
                                 return buyerNodeAddress != null && buyerNodeAddress.equals(
                                                                    contract.getBuyerNodeAddress());
                               } else {
                                 NodeAddress sellerNodeAddress =
                                   contract1.getSellerNodeAddress();
   ...
   Optional<Dispute> findDispute(String tradeId, int traderId) {
     return disputes.stream().filter(e -> e.getTradeId().equals(tradeId) && 
              e.getTraderId() == traderId).findAny();
   }
   public
   Optional<Dispute> findOwnDispute(String tradeId) {
   ...
   private
   void cleanupRetryMap(String uid) {
     if (delayMsgMap.containsKey(uid)) {
