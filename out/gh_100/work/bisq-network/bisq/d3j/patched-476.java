...
@FxmlView
public
class ProposalsView extends ActivatableView<GridPane,Void>
  implements BsqBalanceListener, DaoStateListener
{
   ...
   private final ObservableList<ProposalsListItem> listItems =
     FXCollections.observableArrayList();
   private final SortedList<ProposalsListItem> sortedList =
     new SortedList<>(listItems);
   ...
   private Button removeProposalButton, acceptButton, rejectButton,
   ignoreButton, voteButton;
   ...
   @Inject
   private
   ProposalsView(DaoFacade daoFacade, BsqWalletService bsqWalletService, 
                 PhasesView phasesView, DaoStateService daoStateService, 
   ...
   void activate() {
     phasesView.activate();
     phaseSubscription =
     EasyBind.subscribe(daoFacade.phaseProperty(), this::onPhaseChanged);
     selectedProposalSubscription =
   ...
   @Override
   public
   void onUpdateBalances(Coin confirmedBalance, Coin availableNonBsqBalance, 
                         Coin pendingBalance, Coin lockedForVotingBalance, 
                         Coin lockupBondsBalance, Coin unlockingBondsBalance) {
     if (isBlindVotePhaseButNotLastBlock())
       stakeInputTextField.setPromptText(Res.get("dao.proposal.myVote.stake.prompt",
                                           bsqFormatter.formatCoinWithCode(confirmedBalance))); else
   ...
   void fillListItems() {
     if (daoFacade.phaseProperty().get().ordinal() < DaoPhase.Phase.BLIND_VOTE.ordinal()) {
       ...
       listItems.setAll(list.stream().map(proposal -> new ProposalsListItem(proposal,
                                                        daoFacade,
                                                        bsqFormatter)).collect(
                          Collectors.toSet()));
     } else {
       List<Ballot> ballotList = daoFacade.getBallotsOfCycle();
       listItems.setAll(ballotList.stream().map(ballot -> new ProposalsListItem(ballot,
                                                            daoFacade,
                                                            bsqFormatter)).collect(
                          Collectors.toSet()));
     }
   ...
   private
   void createAllFieldsOnProposalDisplay(Proposal proposal, 
                                         @Nullable Ballot ballot, 
                                         @Nullable EvaluatedProposal evaluatedProposal) {
     proposalDisplayView.setVisible(true);
     proposalDisplayView.setManaged(true);
     proposalDisplay.createAllFields(Res.get("dao.proposal.selectedProposal"),
       0, 0, proposal.getType(), false);
     ...
     Tuple2<Long,Long> meritAndStakeTuple =
       daoFacade.getMeritAndStakeForProposal(proposal.getTxId());
     ...
     addButtonAfterGroup(proposalDisplayGridPane,
       proposalDisplay.incrementAndGetGridRow(), Res.get("shared.remove"));
     removeProposalButton.setOnAction(event -> onRemoveProposal());
     onPhaseChanged(daoFacade.phaseProperty().get());
     Tuple3<Button,Button,Button> tuple =
       add3ButtonsAfterGroup(proposalDisplayGridPane,
   ...
   private
   void onPhaseChanged(DaoPhase.Phase phase) {
     ...
     if (removeProposalButton != null) {
       boolean doShowRemoveButton =
         phase == DaoPhase.Phase.PROPOSAL && selectedItem != null && 
   ...
   private
   void onSelectProposal(ProposalsListItem item) {
     selectedItem = item;
     if (selectedItem != null) {
       EvaluatedProposal evaluatedProposal =
         daoStateService.getEvaluatedProposalList().stream().filter(e -> 
           daoFacade.isTxInCorrectCycle(e.getProposal().getTxId(),
             daoFacade.getChainHeight())).filter(e -> e.getProposalTxId().equals(
                                                        selectedItem.getProposal().getTxId())).findAny().orElse(null);
       createAllFieldsOnProposalDisplay(selectedItem.getProposal(),
         selectedItem.getBallot(), evaluatedProposal);
       applyMerit();
     } else {
       hideProposalDisplay();
     }
   ...
   private
   void applyMerit() {
     boolean hasConfirmedVoteTxInCycle =
       daoFacade.getMyVoteListForCycle().stream().map(myVote -> daoFacade.getTx(
                                                                  myVote.getTxId())).findAny().isPresent();
     long merit;
     if (selectedItem != null && hasConfirmedVoteTxInCycle) {
       merit =
       daoFacade.getMeritAndStakeForProposal(selectedItem.getProposal().getTxId()).first;
     } else {
   ...
   void onVote() {
     ...
     try {
       Tuple2<Coin,Integer> miningFeeAndTxSize =
         daoFacade.getMiningFeeAndTxSize(stake);
       ...
       GUIUtil.showBsqFeeInfoPopup(blindVoteFee, miningFee, txSize,
         bsqFormatter, btcFormatter, Res.get("dao.blindVote"),
   ...
   private
   void publishBlindVote(Coin stake) {
   ...
   private
   void updateStateAfterVote() {
     updateViews();
     tableView.refresh();
   }
   private
   ProposalsListItem getBallotListItem() {
     return selectedItem;
   }
   private
   Optional<Vote> getVote(@Nullable Ballot ballot) {
     if (ballot == null)
   ...
   private
   void updateViews() {
     boolean isBlindVotePhaseButNotLastBlock =
       isBlindVotePhaseButNotLastBlock();
     boolean hasVotedOnProposal = hasVotedOnProposal();
     voteButton.setDisable(!hasVotedOnProposal || !stakeInputTextField.getValidator().validate(
                                                     stakeInputTextField.getText()).isValid);
     ...
     if (hasAlreadyVoted) {
       ...
       if (myVoteListForCycle.size() == 1) {
         Optional<MyVote> optionalMyVote =
           myVoteListForCycle.stream().filter(myVote -> daoFacade.isTxInCorrectCycle(
                                                          myVote.getHeight(),
                                                          daoFacade.getChainHeight())).findAny();
         ...
         String msg =
           "We found multiple MyVote entries in that cycle. That is not supported by the UI.";
         log.warn(msg);
   ...
   private
   boolean hasVotedOnProposal() {
     return listItems.stream().filter(e -> e.getBallot() != null).map(
              ProposalsListItem::getBallot).anyMatch(e -> e.getVote() != null);
   }
   private
   boolean isBlindVotePhaseButNotLastBlock() {
   ...
   void createProposalsTableView() {
     TableGroupHeadline proposalsHeadline =
       new TableGroupHeadline(Res.get("dao.proposal.active.header"));
     GridPane.setRowIndex(proposalsHeadline, ++gridRow);
     GridPane.setMargin(proposalsHeadline,
       new Insets(Layout.GROUP_DISTANCE, -10, -10, -10));
     ...
     GridPane.setMargin(tableView,
       new Insets(Layout.FIRST_ROW_AND_GROUP_DISTANCE, -10, 5, -10));
     GridPane.setColumnSpan(tableView, 2);
     root.getChildren().add(tableView);
     tableView.setItems(sortedList);
   }
   private
   void createEmptyProposalDisplay() {
     proposalDisplay =
     new ProposalDisplay(proposalDisplayGridPane, bsqFormatter,
       bsqWalletService, daoFacade);
   ...
   void createVoteView() {
     ...
     Tuple2<Label,TextField> meritTuple =
     ...
     stakeInputTextField =
     addInputTextField(root, ++gridRow,
     ...
     Tuple2<Label,TxIdTextField> blindVoteTxIdTuple =
       addLabelTxIdTextField(root, ++gridRow,
     ...
     Tuple2<Label,TxIdTextField> revealTxIdTuple =
       addLabelTxIdTextField(root, ++gridRow,
     ...
     Tuple3<Button,BusyAnimation,Label> voteButtonTuple =
       addButtonBusyAnimationLabelAfterGroup(root, ++gridRow,
   ...
   void createProposalColumns() {
     ...
     column.setCellValueFactory((item) -> new ReadOnlyObjectWrapper<>(
                                            item.getValue()));
     column.setCellFactory(new Callback<TableColumn<ProposalsListItem,ProposalsListItem>,TableCell<ProposalsListItem,ProposalsListItem>>()
                           {
                              @Override
                              public
                              TableCell<ProposalsListItem,ProposalsListItem> call(
                              TableColumn<ProposalsListItem,ProposalsListItem> column) {
                                return new TableCell<ProposalsListItem,ProposalsListItem>()
                                       {
                                          @Override
                                          public
                                          void updateItem(final ProposalsListItem item,
                                                          boolean empty) {
                                            super.updateItem(item, empty);
                                            if (item != null)
                                              setText(bsqFormatter.formatDateTime(
                                                        item.getProposal().getCreationDate())); else
                                              setText("");
                                          }
                                       };
                              }
                                ...
                                return new TableCell<ProposalsListItem,ProposalsListItem>()
                                       {
                                          ...
                                          void updateItem(final ProposalsListItem item,
                                            ...
                                            super.updateItem(item, empty);
                                            if (item != null)
                                              setText(item.getProposal().getName()); else
                                              setText("");
                                          }
                                       };
                              }
     ...
     tableView.getColumns().add(column);
     column =
     new AutoTooltipTableColumn<>(Res.get("dao.proposal.table.header.link"));
                                          ...
                                          void updateItem(final ProposalsListItem item,
                                            ...
                                            if (item != null && !empty) {
                                              final Proposal proposal =
                                                item.getProposal();
                                              field =
                                              new HyperlinkWithIcon(proposal.getLink());
                                              field.setOnAction(event -> 
                                                GUIUtil.openWebPage(Res.get("shared.openURL",
                                                                    proposal.getLink())));
     ...
     tableView.getColumns().add(column);
     column =
     new AutoTooltipTableColumn<>(Res.get("dao.proposal.table.header.proposalType"));
     column.setMinWidth(60);
     column.setCellValueFactory((item) -> new ReadOnlyObjectWrapper<>(
                                            item.getValue()));
     column.setCellFactory(new Callback<TableColumn<ProposalsListItem,ProposalsListItem>,TableCell<ProposalsListItem,ProposalsListItem>>()
                                ...
                                return new TableCell<ProposalsListItem,ProposalsListItem>()
                                       {
                                          ...
                                          void updateItem(final ProposalsListItem item,
                                            ...
                                            super.updateItem(item, empty);
                                            if (item != null)
                                              setText(Res.get("dao.proposal.type." + 
                                                        item.getProposal().getType().name())); else
                                              setText("");
                                          }
                                       };
                              }
