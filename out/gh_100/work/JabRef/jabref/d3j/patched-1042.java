...
class JabRef
{
   ...
   void start(String[] args) {
     ...
     RemotePreferences remotePreferences =
       new RemotePreferences(Globals.prefs);
     if (remotePreferences.useRemoteServer()) {
       Globals.remoteListener.open(new JabRefMessageHandler(this),
         remotePreferences.getPort());
       if (Globals.remoteListener.isOpen()) {
         Globals.remoteListener.start();
       } else {
         if (RemoteListenerClient.sendToActiveJabRefInstance(args,
               remotePreferences.getPort())) {
   ...
   public
   Vector<ParserResult> processArguments(String[] args, 
       ...
       if ("all".equals(value.trim())) {
         ...
         for (String key : keys) {
           if (Globals.prefs.hasKey(key.trim())) {
             System.out.println(Localization.lang("Resetting preference key '%0'",
                                  key.trim()));
             Globals.prefs.clear(key.trim());
           } else {
             System.out.println(Localization.lang("Unknown preference key '%0'",
                                  key.trim()));
           }
         }
       }
     ...
     if (cli.isExportMatches()) {
       if (!loaded.isEmpty()) {
         ...
         SearchManagerNoGUI smng =
           new SearchManagerNoGUI(searchTerm, dataBase);
         ...
         if ((newBase != null) && (newBase.getEntryCount() > 0)) {
           ...
           if (format != null) {
             try {
               ...
               format.performExport(newBase, pr.getMetaData(), data[1],
                 pr.getEncoding(), null);
             } catch (Exception ex) {
     ...
     if (cli.isFileExport()) {
       if (!loaded.isEmpty()) {
         ...
         if (data.length == 1) {
           ...
           if (format != null) {
             try {
               format.performExport(pr.getDatabase(), pr.getMetaData(),
     ...
     if (!cli.isBlank() && cli.isAuxImport()) {
       ...
       if (!loaded.isEmpty()) {
         ...
         if (data.length == 2) {
           ...
           AuxCommandLine acl =
             new AuxCommandLine(data[0], pr.getDatabase());
           BibtexDatabase newBase = acl.perform();
           boolean notSavedMsg = false;
           if (newBase != null) {
             if (newBase.getEntryCount() > 0) {
               String subName =
                 StringUtil.getCorrectFileName(data[1], "bib");
               try {
                 System.out.println(Localization.lang("Saving") + ": " + subName);
                 SaveSession session =
                   FileActions.saveDatabase(newBase, new MetaData(),
   ...
   private
   ParserResult fetch(String fetchCommand) {
     ...
     for (EntryFetcher e : EntryFetchers.INSTANCE.getEntryFetchers()) {
       if (engine.toLowerCase().equals(e.getClass().getSimpleName().replaceAll("Fetcher",
                                         "").toLowerCase())) {
         fetcher = e;
       }
     }
     if (fetcher == null) {
       System.out.println(Localization.lang("Could not find fetcher '%0'",
                            engine));
       System.out.println(Localization.lang("The following fetchers are available:"));
       for (EntryFetcher e : EntryFetchers.INSTANCE.getEntryFetchers()) {
         System.out.println("  " + e.getClass().getSimpleName().replaceAll("Fetcher",
                                     "").toLowerCase());
       }
       return null;
     }
     System.out.println(Localization.lang("Running Query '%0' with fetcher '%1'.",
                          query, engine) + " " + Localization.lang("Please wait..."));
     Collection<BibtexEntry> result =
       new ImportInspectionCommandLine().query(query, fetcher);
     if ((result == null) || result.isEmpty()) {
       System.out.println(Localization.lang("Query '%0' with fetcher '%1' did not return any results.",
                            query, engine));
       return null;
     }
   ...
   void setLookAndFeel() {
       ...
       if ("javax.swing.plaf.metal.MetalLookAndFeel".equals(lookFeel)) {
         ...
         try {
           ...
           JOptionPane.showMessageDialog(JabRef.jrf,
             Localization.lang("Unable to find the requested Look & Feel and thus the default one is used."),
     ...
     boolean overrideDefaultFonts =
       Globals.prefs.getBoolean(JabRefPreferences.OVERRIDE_DEFAULT_FONTS);
     if (overrideDefaultFonts) {
       ...
       while (keys.hasMoreElements()) {
         ...
         if ((key instanceof String) && ((String)key).endsWith(".font")) {
           ...
           font =
           new FontUIResource(font.getName(), font.getStyle(), fontSize);
   ...
   private
   void openWindow(Vector<ParserResult> loaded) {
     ...
     Globals.prefs.get(JabRefPreferences.LAST_EDITED) != null)) {
       String[] names =
         Globals.prefs.getStringArray(JabRefPreferences.LAST_EDITED);
         ...
         if (fileToOpen.exists()) {
           ...
           if (pr != null) {
             if (pr == ParserResult.INVALID_FORMAT) {
               System.out.println(Localization.lang("Error opening file") + " '" + 
                 fileToOpen.getPath() + "'");
             } else if (pr != ParserResult.FILE_LOCKED) {
               loaded.add(pr);
             }
     ...
     new Font(Globals.prefs.get(JabRefPreferences.FONT_FAMILY),
       Globals.prefs.getInt(JabRefPreferences.FONT_STYLE),
     ...
     if (cli.isLoadSession()) {
       JabRef.jrf.loadSessionAction.actionPerformed(new java.awt.event.ActionEvent(JabRef.jrf,
                                                      0, ""));
     }
     ...
     for (ParserResult pr : failed) {
       String message =
         "<html>" + Localization.lang("Error opening file '%0'.",
                      pr.getFile().getName()) + "<p>" + pr.getErrorMessage() + "</html>";
       JOptionPane.showMessageDialog(JabRef.jrf, message,
     ...
     for (int i = 0; i < loaded.size(); i++) {
       ...
       pr.hasWarnings()) {
         ...
         for (int j = 0;
              j < Math.min(JabRef.MAX_DIALOG_WARNINGS, wrns.length); j++) {
           wrn.append(j + 1).append(". ").append(wrns[j]).append("\n");
         }
         if (wrns.length > JabRef.MAX_DIALOG_WARNINGS) {
           wrn.append("... ");
           wrn.append(Localization.lang("%0 warnings",
                        String.valueOf(wrns.length)));
     ...
     for (int i = 0;
          (i < loaded.size()) && (i < JabRef.jrf.getBasePanelCount()); i++) {
       ParserResult pr = loaded.elementAt(i);
     ...
     if (!postponed.isEmpty()) {
       AutosaveStartupPrompter asp =
         new AutosaveStartupPrompter(JabRef.jrf, postponed);
   ...
   public
   static
   ParserResult openBibFile(String name, boolean ignoreAutosave) {
   ...
   private
   static
   ParserResult importFile(String argument) {
     ...
     try {
       if ((data.length > 1) && !"*".equals(data[1])) {
         ...
         try {
           ...
           if (OS.WINDOWS) {
             entries =
             Globals.importFormatReader.importFromFile(data[1], data[0],
               JabRef.jrf);
           } else {
             entries =
         ...
         if (OS.WINDOWS) {
           ...
           importResult =
           Globals.importFormatReader.importUnknownFormat(data[0].replaceAll("~",
       ...
       System.err.println(Localization.lang("Error opening file") + " '" + data[0] + "': " + 
         ex.getLocalizedMessage());
     } 
     return null;
   }
   private
   static
   ParserResult importToOpenBase(String argument) {
