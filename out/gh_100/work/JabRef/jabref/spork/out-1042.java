                    ...
                    if (newBase != null) {
                        if (newBase.getEntryCount() > 0) {
                            ...
                            try {
                                ...
                                SaveSession session = // no Metadata
                                FileActions.saveDatabase(newBase, new MetaData(), new File(subName), Globals.prefs, false, false, Globals.prefs.get(JabRefPreferences.DEFAULT_ENCODING), false);
                                // Show just a warning message if encoding didn't work for all characters:
                                if (!session.getWriter().couldEncodeAll()) {
                                    System.err.println(((Localization.lang("Warning") + ": ") + Localization.lang("The chosen encoding '%0' could not encode the following characters: ", session.getEncoding())) + session.getWriter().getProblemCharacters());
                                }
        ...
        try {
            ...
            if ("javax.swing.plaf.metal.MetalLookAndFeel".equals(lookFeel)) {
                ...
                try {
                    ...
                    // javax.swing.UnsupportedLookAndFeelException (sure; see bug #1278) or ClassNotFoundException (unsure) may be thrown
