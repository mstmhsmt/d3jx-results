...
class EventIndexUtils
{
   private static final Logger logger =
     LoggerFactory.getLogger(EventIndexUtils.class);
   ...
   public
   static
   Event toRecordingEvent(SearchMetadataCollection metadata, 
                          Unmarshaller unmarshaller)
     throws IOException {
     Map<String,SearchMetadata<?>> metadataMap = metadata.toMap();
     String eventJson =
       (String)metadataMap.get(EventIndexSchema.OBJECT).getValue();
   ...
   public
   static
   SearchMetadataCollection toSearchMetadata(Event event) {
     SearchMetadataCollection metadata =
       new SearchMetadataCollection(event.getIdentifier().concat(event.getOrganization()),
         Event.DOCUMENT_TYPE);
     metadata.addField(EventIndexSchema.UID, event.getIdentifier(), true);
     metadata.addField(EventIndexSchema.ORGANIZATION,
       event.getOrganization(), false);
     ...
     if (StringUtils.isNotBlank(event.getDescription())) {
       metadata.addField(EventIndexSchema.DESCRIPTION,
     ...
     if (StringUtils.isNotBlank(event.getWorkflowState())) {
       metadata.addField(EventIndexSchema.WORKFLOW_STATE,
         event.getWorkflowState(), true);
     }
     ...
     if (StringUtils.isNotBlank(event.getWorkflowDefinitionId())) {
       metadata.addField(EventIndexSchema.WORKFLOW_DEFINITION_ID,
         event.getWorkflowDefinitionId(), true);
     }
     if (StringUtils.isNotBlank(event.getRecordingStartDate())) {
       metadata.addField(EventIndexSchema.START_DATE,
         event.getRecordingStartDate(), true);
     }
     if (StringUtils.isNotBlank(event.getRecordingEndDate())) {
       metadata.addField(EventIndexSchema.END_DATE,
         event.getRecordingEndDate(), true);
     }
     ...
     if (event.getArchiveVersion() != null) {
       metadata.addField(EventIndexSchema.ARCHIVE_VERSION,
         event.getArchiveVersion(), true);
     }
     if (event.getRecordingStatus() != null) {
       metadata.addField(EventIndexSchema.RECORDING_STATUS,
         event.getRecordingStatus(), true);
     }
     ...
     metadata.addField(EventIndexSchema.HAS_COMMENTS, event.hasComments(),
       true);
     metadata.addField(EventIndexSchema.HAS_OPEN_COMMENTS,
       event.hasOpenComments(), true);
         ...
         HashMap<String,Object> myMap =
           new HashMap<String,Object>()
           {
              
              {
                 ...
                 put(CommentIndexSchema.RESOLVED_STATUS,
     ...
     if (event.getPublications() != null) {
       ...
       HashMap<String,Object>[] publicationsArray =
         new HashMap[publications.size()];
     ...
     if (event.getPresenters() != null) {
       ...
       metadata.addField(EventIndexSchema.PRESENTER,
         presenters.toArray(new String[presenters.size()]), true);
     }
     if (event.getContributors() != null) {
       List<String> contributors = event.getContributors();
       metadata.addField(EventIndexSchema.CONTRIBUTOR,
         contributors.toArray(new String[contributors.size()]), true);
     }
     ...
     if (StringUtils.isNotBlank(event.getAccessPolicy())) {
       metadata.addField(EventIndexSchema.ACCESS_POLICY,
         event.getAccessPolicy(), true);
     ...
     if (StringUtils.isNotBlank(event.getTechnicalStartTime())) {
       metadata.addField(EventIndexSchema.TECHNICAL_START,
         event.getTechnicalStartTime(), true);
     }
     if (StringUtils.isNotBlank(event.getTechnicalEndTime())) {
       metadata.addField(EventIndexSchema.TECHNICAL_END,
         event.getTechnicalEndTime(), true);
     }
   ...
   private
   static
   void addObjectStringtToMap(HashMap<String,Object> map, String key, 
                              Object value) {
     if (value == null) {
       map.put(key, "");
     } else {
   ...
   private
   static
   HashMap<String,Object> generatePublicationDoc(Publication publication) {
     HashMap<String,Object> pMap = new HashMap<String,Object>();
     pMap.put(PublicationIndexSchema.CHANNEL, publication.getChannel());
     addObjectStringtToMap(pMap, PublicationIndexSchema.MIMETYPE,
       publication.getMimeType());
     Attachment[] attachments = publication.getAttachments();
     HashMap<String,Object>[] attachmentsArray =
       new HashMap[attachments.length];
     for (int i = 0; i < attachmentsArray.length; i++) {
       Attachment attachment = attachments[i];
       HashMap<String,Object> element = new HashMap<String,Object>();
       element.put(PublicationIndexSchema.ELEMENT_ID,
         attachment.getIdentifier());
       addObjectStringtToMap(element,
         PublicationIndexSchema.ELEMENT_MIMETYPE, attachment.getMimeType());
       addObjectStringtToMap(element, PublicationIndexSchema.ELEMENT_TYPE,
         attachment.getElementType());
       element.put(PublicationIndexSchema.ELEMENT_TAG, attachment.getTags());
       addObjectStringtToMap(element, PublicationIndexSchema.ELEMENT_URL,
         attachment.getURI());
     ...
     for (int i = 0; i < catalogsArray.length; i++) {
       ...
       addObjectStringtToMap(element,
         PublicationIndexSchema.ELEMENT_MIMETYPE, catalog.getMimeType());
       addObjectStringtToMap(element, PublicationIndexSchema.ELEMENT_TYPE,
         catalog.getElementType());
     ...
     for (int i = 0; i < tracksArray.length; i++) {
       ...
       addObjectStringtToMap(element,
         PublicationIndexSchema.ELEMENT_MIMETYPE, track.getMimeType());
       addObjectStringtToMap(element, PublicationIndexSchema.ELEMENT_TYPE,
         track.getElementType());
       ...
       element.put(PublicationIndexSchema.TRACK_DURATION,
         track.getDuration());
   ...
   private
   static
   void addExtendedMetadata(SearchMetadataCollection doc, 
                            Map<String,Map<String,List<String>>> extendedMetadata) {
     for (String type : extendedMetadata.keySet()) {
       Map<String,List<String>> extendedMetadataByType =
         extendedMetadata.get(type);
       for (String name : extendedMetadataByType.keySet()) {
         List<String> values = extendedMetadataByType.get(name);
         String fieldName =
           SeriesIndexSchema.EXTENDED_METADATA_PREFIX.concat(type + "_" + name);
   ...
   private
   static
   void addAuthorization(SearchMetadataCollection doc, String aclString) {
     ...
     for (AccessControlEntry entry : acl.getEntries()) {
       if (!entry.isAllow()) {
         logger.info("Event index does not support denial via ACL, ignoring {}",
           entry);
         continue;
       }
     ...
     for (Map.Entry<String,List<String>> entry : permissions.entrySet()) {
       String fieldName =
         EventIndexSchema.ACL_PERMISSION_PREFIX.concat(entry.getKey());
       doc.addField(fieldName, entry.getValue(), false);
     }
   }
   public
   static
   Event updateEventExtendedMetadata(Event event, DublinCoreCatalog dc, 
   ...
   public
   static
   Event updateEvent(Event event, DublinCore dc) {
     ...
     try {
       if (StringUtils.isNotBlank(strPeriod)) {
         DCMIPeriod period =
           EncodingSchemeUtils.decodeMandatoryPeriod(strPeriod);
       ...
       logger.warn("Invalid start and end date/time for event {}: {}",
         event.getIdentifier(), strPeriod);
       event.setRecordingStartDate(DateTimeSupport.toUTC(created.getTime()));
     } 
     updateTechnicalDate(event);
     event.setContributors(dc.get(DublinCore.PROPERTY_CONTRIBUTOR,
                             DublinCore.LANGUAGE_ANY));
   ...
   public
   static
   Event updateTechnicalDate(Event event) {
   ...
   public
   static
   Event updateEvent(Event event, MediaPackage mp) {
   ...
   public
   static
   void updateSeriesName(Event event, String organization, User user, 
                         ElasticsearchIndex searchIndex)
     throws SearchIndexException {
     updateSeriesName(event, organization, user, searchIndex,
       DEFAULT_ATTEMPTS, DEFAULT_SLEEP);
   }
   public
   static
   void updateSeriesName(Event event, String organization, User user, 
                         ElasticsearchIndex searchIndex, int tries, 
                         long sleep)
     throws SearchIndexException {
     if (event.getSeriesId() != null) {
       for (int i = 1; i <= tries; i++) {
         SearchResult<Series> result =
           searchIndex.getByQuery(new SeriesSearchQuery(organization, user).withoutActions().withIdentifier(
                                    event.getSeriesId()));
         if (result.getHitCount() > 0) {
           ...
           try {
             ...
             logger.warn("Interrupted while sleeping before checking for the series being added to the index",
               e);
           } 
         }
       }
   ...
   private
   static
   String[] getPublicationFlavors(List<Publication> publications) {
   ...
   public
   static
   Boolean subflavorMatches(List<Publication> publications, 
