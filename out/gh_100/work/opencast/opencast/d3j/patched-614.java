...
@Path("/")
@RestService(name="localdistributionservice",
             title="Local Distribution Service",
             ...
             notes={"All paths above are relative to the REST endpoint base (something like http://your.server/files)",
                   ...
                   "A status code 500 means a general failure has occurred which is not recoverable and was not anticipated. In " + "other words, there is a bug! You should file an error report with your server logs from the time when the " + "error occurred: <a href=\"https://opencast.jira.com\">Opencast Issue Tracker</a>"})
public
class StreamingDistributionRestService extends AbstractJobProducerEndpoint
{
   private static final Logger logger =
     LoggerFactory.getLogger(StreamingDistributionRestService.class);
   ...
   public
   void setService(StreamingDistributionService service) {
   ...
   @RestQuery(name="distribute",
              ...
              restParameters={@RestParameter(name="mediapackage",
                                             isRequired=true,
              ...
              reponses={@RestResponse(responseCode=SC_OK,
                       ...
                       @RestResponse(responseCode=SC_NO_CONTENT,
                                     description="There is no streaming distribution service available")})
   public
   Response distribute(@FormParam("mediapackage") String mediaPackageXml, 
     ...
     try {
       Gson gson = new Gson();
       Set<String> setElementIds =
         gson.fromJson(elementIds, new TypeToken<Set<String>>(){}.getType());
       MediaPackage mediapackage =
         MediaPackageParser.getFromXml(mediaPackageXml);
   ...
   @RestQuery(name="retract",
              ...
              reponses={@RestResponse(responseCode=SC_OK,
                       ...
                       @RestResponse(responseCode=SC_NO_CONTENT,
                                     description="There is no streaming distribution service available")})
   public
   Response retract(@FormParam("mediapackage") String mediaPackageXml, 
                    @FormParam("channelId") String channelId, 
                    @FormParam("elementIds") String elementIds)
     throws Exception {
     Job job = null;
   ...
   @Override
   public
   JobProducer getService() {
     if (service instanceof JobProducer)
       return (JobProducer)service; else
       return null;
   }
   @Override
   public
