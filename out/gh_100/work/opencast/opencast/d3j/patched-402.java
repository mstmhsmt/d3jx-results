  ...
  implements UserProvider, RoleProvider, CachingUserProviderMXBean
{
   ...
   private static final Logger logger =
     LoggerFactory.getLogger(MoodleUserProviderInstance.class);
   ...
   public
   MoodleUserProviderInstance(String pid, MoodleWebService client, 
                              Organization organization, 
                              String coursePattern, String userPattern, 
                              String groupPattern, boolean groupRoles, 
     ...
     logger.info("Creating new MoodleUserProviderInstance(pid={}, url={}, cacheSize={}, cacheExpiration={})",
       pid, client.getURL(), cacheSize, cacheExpiration);
     cache =
     CacheBuilder.newBuilder().maximumSize(cacheSize).expireAfterWrite(cacheExpiration,
       TimeUnit.MINUTES).build(new CacheLoader<String,Object>()
                               {
                                  @Override
                                  public
                                  Object load(String username) {
   ...
   float getCacheHitRatio() {
     if (loadUserRequests.get() == 0)
       return 0;
     return (float)(loadUserRequests.get() - moodleWebServiceRequests.get()) / 
     loadUserRequests.get();
   }
   private
   void registerMBean(String pid) {
   ...
   Iterator<User> findUsers(String query, int offset, int limit) {
     ...
     try {
       ...
       logger.warn("Invalid regular expression for user pattern {} - disabling checks",
         userPattern);
       userPattern = null;
     } 
   ...
   List<Role> getRolesForUser(String username) {
     ...
     logger.debug("Return empty role set for {} - not found in Moodle",
       username);
     return new LinkedList<>();
   }
   @Override
   public
   Iterator<Role> findRoles(String query, Role.Target target, int offset, 
                            int limit) {
     ...
     query.endsWith("_" + GROUP_ROLE_SUFFIX))
       return Collections.emptyIterator();
     boolean findGroupRole =
       groupRoles && query.startsWith(GROUP_ROLE_PREFIX);
     String moodleId =
       findGroupRole ? query.substring(GROUP_ROLE_PREFIX.length()) : query;
     if (query.endsWith("_" + LEARNER_ROLE_SUFFIX)) {
       moodleId =
       query.substring(0, query.lastIndexOf("_" + LEARNER_ROLE_SUFFIX));
       ltirole = true;
     } else if (query.endsWith("_" + INSTRUCTOR_ROLE_SUFFIX)) {
       moodleId =
       query.substring(0, query.lastIndexOf("_" + INSTRUCTOR_ROLE_SUFFIX));
       ltirole = true;
     } else if (query.endsWith("_" + GROUP_ROLE_SUFFIX)) {
       moodleId =
       query.substring(0, query.lastIndexOf("_" + GROUP_ROLE_SUFFIX));
       ltirole = true;
     }
     String pattern = findGroupRole ? groupPattern : coursePattern;
     try {
       if ((pattern != null) && !moodleId.matches(pattern)) {
         logger.debug("Verify Moodle ID {} failed regexp {}", moodleId,
           pattern);
         return Collections.emptyIterator();
       }
     } catch (PatternSyntaxException e) {
       logger.warn("Invalid regular expression for pattern {} - disabling checks",
         pattern);
       if (findGroupRole) {
         groupPattern = null;
       } else {
     ...
     List<Role> roles = new LinkedList<>();
     JaxbOrganization jaxbOrganization =
       JaxbOrganization.fromOrganization(organization);
     if (ltirole) {
       roles.add(new JaxbRole(query, jaxbOrganization, "Moodle Site Role",
                   Role.Type.EXTERNAL));
     } else if (findGroupRole) {
       roles.add(new JaxbRole(GROUP_ROLE_PREFIX + moodleId + "_" + GROUP_ROLE_SUFFIX,
                   jaxbOrganization, "Moodle Group Learner Role",
                   Role.Type.EXTERNAL));
     } else {
       roles.add(new JaxbRole(moodleId + "_" + INSTRUCTOR_ROLE_SUFFIX,
                   jaxbOrganization, "Moodle Course Instructor Role",
                   Role.Type.EXTERNAL));
       roles.add(new JaxbRole(moodleId + "_" + LEARNER_ROLE_SUFFIX,
                   jaxbOrganization, "Moodle Course Learner Role",
                   Role.Type.EXTERNAL));
     }
     return roles.iterator();
   }
   private
   User loadUserFromMoodle(String username) {
     ...
     try {
       List<MoodleUser> moodleUsers =
         client.coreUserGetUsersByField(CoreUserGetUserByFieldFilters.username,
       ...
       List<String> courseIdsInstructor =
         client.toolOpencastGetCoursesForInstructor(username);
       List<String> courseIdsLearner =
         client.toolOpencastGetCoursesForLearner(username);
       List<String> groupIdsLearner =
         groupRoles ? client.toolOpencastGetGroupsForLearner(username) : 
       Collections.emptyList();
       Set<JaxbRole> roles = new HashSet<>();
       roles.add(new JaxbRole(Group.ROLE_PREFIX + "MOODLE", jaxbOrganization,
                   "Moodle Users", Role.Type.EXTERNAL_GROUP));
       for (String courseId : courseIdsInstructor) {
         roles.add(new JaxbRole(courseId + "_" + INSTRUCTOR_ROLE_SUFFIX,
                     jaxbOrganization, "Moodle Course Instructor Role",
                     Role.Type.EXTERNAL));
       }
       for (String courseId : courseIdsLearner) {
         roles.add(new JaxbRole(courseId + "_" + LEARNER_ROLE_SUFFIX,
                     jaxbOrganization, "Moodle Course Learner Role",
                     Role.Type.EXTERNAL));
       }
       for (String groupId : groupIdsLearner) {
         roles.add(new JaxbRole(GROUP_ROLE_PREFIX + groupId + "_" + GROUP_ROLE_SUFFIX,
                     jaxbOrganization, "Moodle Group Learner Role",
                     Role.Type.EXTERNAL));
       }
       return new JaxbUser(moodleUser.getUsername(), null,
                moodleUser.getFullname(), moodleUser.getEmail(),
                this.getName(), jaxbOrganization, roles);
     } catch (Exception e) {
       logger.warn("Exception loading Moodle user {} at {}: {}", username,
         client.getURL(), e.getMessage());
     } finally{
