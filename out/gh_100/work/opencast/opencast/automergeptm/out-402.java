...
public class MoodleUserProviderInstance implements UserProvider, RoleProvider, CachingUserProviderMXBean {
  ...
  public MoodleUserProviderInstance(String pid, MoodleWebService client, Organization organization, String coursePattern, String userPattern, String groupPattern, boolean groupRoles, int cacheSize, int cacheExpiration, String adminUserName) {
    ...
    cache = CacheBuilder.newBuilder().maximumSize(cacheSize).expireAfterWrite(cacheExpiration, TimeUnit.MINUTES).build(new CacheLoader<String, Object>() {
      @Override public Object load(String username) {
   ...
   */
  @Override public float getCacheHitRatio() {
   ...
   */
  @Override public Iterator<User> getUsers() {
   ...
   */
  @Override public User loadUser(String userName) {
   ...
   */
  @Override public Iterator<User> findUsers(String query, int offset, int limit) {
    if (query == null) {
   ...
   */
  @Override public void invalidate(String userName) {
   ...
   */
  @Override public Iterator<Role> getRoles() {
    return Collections.emptyIterator();
  }
   ...
   */
  @Override public List<Role> getRolesForUser(String username) {
   ...
   */
  @Override public Iterator<Role> findRoles(String query, Role.Target target, int offset, int limit) {
    ...
    if (query.endsWith("_" + LEARNER_ROLE_SUFFIX)) {
      ...
      if (query.endsWith("_" + INSTRUCTOR_ROLE_SUFFIX)) {
        moodleId = query.substring(0, query.lastIndexOf("_" + INSTRUCTOR_ROLE_SUFFIX));
        ltirole = true;
      } else {
        if (query.endsWith("_" + GROUP_ROLE_SUFFIX)) {
