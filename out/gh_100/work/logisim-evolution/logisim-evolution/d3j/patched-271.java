...
class Multiplexer extends InstanceFactory
{
   ...
   static
   void drawSelectCircle(Graphics g, Bounds bds, Location loc) {
     ...
     if (bds.getHeight() >= bds.getWidth()) {
       ...
       if (loc.getX() >= bds.getX() + bds.getWidth())
         loc.translate(-4, 0); else if (loc.getX() <= bds.getX())
         loc.translate(4, 0);
     } else {
       ...
       if (loc.getY() >= bds.getY() + bds.getHeight())
         loc.translate(0, -4); else if (loc.getY() <= bds.getY())
         loc.translate(0, 4);
     }
   ...
   Multiplexer() {
     ...
     setKeyConfigurator(JoinedConfigurator.create(new BitWidthConfigurator(Plexers.ATTR_SELECT,
                                                    1, 5, 0),
   ...
   @Override
   protected
   void configureNewInstance(Instance instance) {
   ...
   Bounds getOffsetBounds(AttributeSet attrs) {
     ...
     if (inputs == 2) {
       ...
       if (!wide && (dir == Direction.SOUTH || dir == Direction.WEST))
         offs += 5;
       return Bounds.create(-w, offs, w, length).rotate(Direction.EAST, dir,
                0, 0);
     }
   }
   @Override
   ...
   @Override
   protected
   void instanceAttributeChanged(Instance instance, Attribute<?> attr) {
   ...
   @Override
   public
   void paintGhost(InstancePainter painter) {
   ...
   void paintInstance(InstancePainter painter) {
     ...
     if (inputs == 2 || (!wide && oddside)) {
       ...
       int len = (wide ? 2 : 1);
       g.drawLine(pt.getX() - len * dx, pt.getY() - len * dy, pt.getX(),
         pt.getY());
     }
     if (enable) {
     ...
     Multiplexer.drawSelectCircle(g, bds,
       painter.getInstance().getPortLocation(inputs));
     int x0;
     int y0;
     int halign;
   ...
   void propagate(InstanceState state) {
     ...
     if (en == Value.FALSE) {
       ...
       Value base =
         opt == Plexers.DISABLED_ZERO ? Value.FALSE : Value.UNKNOWN;
   ...
   void updatePorts(Instance instance) {
     ...
     if (inputs == 2) {
       ...
       ps[0] =
       new Port(end0.getX(), end0.getY(), Port.INPUT, data.getWidth());
       ps[1] =
       new Port(end1.getX(), end1.getY(), Port.INPUT, data.getWidth());
     } else {
     ...
     if (!wide && !vertical && botLeft)
       sel = sel.translate(-10, 0); else if (!wide && vertical && !botLeft)
       sel = sel.translate(0, -10);
     Location en = sel.translate(dir, 10);
     ps[inputs] =
     new Port(sel.getX(), sel.getY(), Port.INPUT, select.getWidth());
     if (enable) {
       ps[inputs + 1] =
       new Port(en.getX(), en.getY(), Port.INPUT, BitWidth.ONE);
     }
