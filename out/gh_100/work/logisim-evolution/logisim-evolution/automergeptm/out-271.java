...
public class Multiplexer extends InstanceFactory {
  ...
  public Multiplexer() {
    ...
    setAttributes(new Attribute[] { StdAttr.FACING, Plexers.ATTR_SIZE, Plexers.ATTR_SELECT_LOC, Plexers.ATTR_SELECT, StdAttr.WIDTH, Plexers.ATTR_DISABLED, Plexers.ATTR_ENABLE }, new Object[] { Direction.EAST, Plexers.SIZE_WIDE, Plexers.SELECT_BOTTOM_LEFT, Plexers.DEFAULT_SELECT, BitWidth.ONE, Plexers.DISABLED_ZERO, Plexers.DEFAULT_ENABLE });
    setKeyConfigurator(JoinedConfigurator.create(new BitWidthConfigurator(Plexers.ATTR_SELECT, 1, 5, 0), new BitWidthConfigurator(StdAttr.WIDTH)));
  ...
  @Override public Object getDefaultAttributeValue(Attribute<?> attr, LogisimVersion ver) {
  ...
  @Override public String getHDLName(AttributeSet attrs) {
  ...
  @Override public Bounds getOffsetBounds(AttributeSet attrs) {
  ...
  @Override public boolean HasThreeStateDrivers(AttributeSet attrs) {
    return (attrs.getValue(Plexers.ATTR_DISABLED) == Plexers.DISABLED_FLOATING);
  }

  @Override public boolean HDLSupportedComponent(AttributeSet attrs) {
  ...
  @Override protected void instanceAttributeChanged(Instance instance, Attribute<?> attr) {
    if (attr == StdAttr.FACING || attr == Plexers.ATTR_SELECT_LOC || attr == Plexers.ATTR_SELECT || attr == Plexers.ATTR_SIZE) {
      instance.recomputeBounds();
      updatePorts(instance);
    } else {
      if (attr == StdAttr.WIDTH || attr == Plexers.ATTR_ENABLE) {
        instance.recomputeBounds();
        updatePorts(instance);
      } else {
        if (attr == Plexers.ATTR_DISABLED) {
  ...
  @Override public void paintGhost(InstancePainter painter) {
  ...
  @Override public void propagate(InstanceState state) {
    ...
    if (en == Value.FALSE) {
      ...
      if (en == Value.ERROR && state.isPortConnected(inputs + 1)) {
