...
abstract
class BasePlayer
  implements Player.EventListener, PlaybackListener, ImageLoadingListener
{
   ...
   @NonNull public static final String PLAYBACK_SKIP_SILENCE =
   ...
   protected static final float[] PLAYBACK_SPEEDS = { 0.5f, 0.75f, 1f, 1.25f,
   1.5f, 1.75f, 2f };
   ...
   public
   BasePlayer(@NonNull final Context context) {
     ...
     this.dataSource =
     new PlayerDataSource(context, userAgent, bandwidthMeter);
     final TrackSelection.Factory trackSelectionFactory =
       PlayerHelper.getQualitySelector(context);
   ...
   void initPlayer(final boolean playOnReady) {
     ...
     mediaSessionManager =
     new MediaSessionManager(context, simpleExoPlayer,
   ...
   void handleIntent(Intent intent) {
     ...
     final PlayQueue queue =
       SerializedCache.getInstance().take(intentCacheKey, PlayQueue.class);
     if (queue == null)
       return;
     if (intent.getBooleanExtra(APPEND_ONLY, false) && playQueue != null) {
       ...
       if ((intent.getBooleanExtra(SELECT_ON_APPEND, false) || getCurrentState() == STATE_COMPLETED) && 
     ...
     final float playbackSpeed =
       intent.getFloatExtra(PLAYBACK_SPEED, getPlaybackSpeed());
     final float playbackPitch =
       intent.getFloatExtra(PLAYBACK_PITCH, getPlaybackPitch());
     final boolean playbackSkipSilence =
       intent.getBooleanExtra(PLAYBACK_SKIP_SILENCE,
         getPlaybackSkipSilence());
     if (simpleExoPlayer != null && queue.size() == 1 && playQueue != null && 
     playQueue.getItem() != null && queue.getItem().getUrl().equals(playQueue.getItem().getUrl()) && 
     queue.getItem().getRecoveryPosition() != PlayQueueItem.RECOVERY_UNSET) {
       simpleExoPlayer.seekTo(playQueue.getIndex(),
         queue.getItem().getRecoveryPosition());
       return;
       ...
       if (item != null && item.getRecoveryPosition() == PlayQueueItem.RECOVERY_UNSET) {
         stateLoader =
         recordManager.loadStreamState(item).observeOn(AndroidSchedulers.mainThread()).doFinally(() -> 
           initPlayback(queue, repeatMode, playbackSpeed, playbackPitch,
             playbackSkipSilence, true)).subscribe(state -> queue.setRecovery(
                                                              queue.getIndex(),
     ...
     initPlayback(queue, repeatMode, playbackSpeed, playbackPitch,
       playbackSkipSilence, true);
   }
   protected
   void initPlayback(@NonNull final PlayQueue queue, 
   ...
   private
   void initThumbnail(final String url) {
     if (DEBUG)
     ...
     ImageLoader.getInstance().loadImage(url,
       ImageDisplayConstants.DISPLAY_THUMBNAIL_OPTIONS, this);
   }
   @Override
   public
   void onLoadingStarted(String imageUri, View view) {
     if (DEBUG)
       Log.d(TAG,
         "Thumbnail - onLoadingStarted() called on: " + "imageUri = [" + imageUri + "], view = [" + view + "]");
   }
   @Override
   public
   void onLoadingFailed(String imageUri, View view, FailReason failReason) {
     Log.e(TAG,
       "Thumbnail - onLoadingFailed() called on imageUri = [" + imageUri + "]",
       failReason.getCause());
   ...
   void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
     if (DEBUG)
       Log.d(TAG,
         "Thumbnail - onLoadingComplete() called with: " + "imageUri = [" + imageUri + "], view = [" + view + "], " + "loadedImage = [" + loadedImage + "]");
   ...
   void onLoadingCancelled(String imageUri, View view) {
     if (DEBUG)
       Log.d(TAG,
         "Thumbnail - onLoadingCancelled() called with: " + "imageUri = [" + imageUri + "], view = [" + view + "]");
     currentThumbnail = null;
   }
   protected
   void setupBroadcastReceiver(IntentFilter intentFilter) {
   ...
   void unregisterBroadcastReceiver() {
     try {
       ...
       Log.w(TAG,
         "Broadcast receiver already unregistered (" + unregisteredException.getMessage() + ")");
     } 
   }
   ...
   public
   abstract
   void onUpdateProgress(int currentProgress, int duration, int bufferPercent);
   protected
   ...
   private
   Disposable getProgressReactor() {
     return Observable.interval(PROGRESS_LOOP_INTERVAL_MILLIS,
              TimeUnit.MILLISECONDS, AndroidSchedulers.mainThread()).observeOn(
              AndroidSchedulers.mainThread()).subscribe(ignored -> triggerProgressUpdate(),
   ...
   void onTimelineChanged(Timeline timeline, Object manifest, 
     ...
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onTimelineChanged() called with " + (manifest == null ? "no manifest" : "available manifest") + ", " + "timeline size = [" + 
         timeline.getWindowCount() + "], " + "reason = [" + reason + "]");
     maybeUpdateCurrentMetadata();
   }
   @Override
   public
   void onTracksChanged(TrackGroupArray trackGroups, 
                        TrackSelectionArray trackSelections) {
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onTracksChanged(), " + "track group size = " + trackGroups.length);
     maybeUpdateCurrentMetadata();
   }
   @Override
   public
   ...
   @Override
   public
   void onLoadingChanged(final boolean isLoading) {
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onLoadingChanged() called with: " + "isLoading = [" + isLoading + "]");
   ...
   void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onPlayerStateChanged() called with: " + "playWhenReady = [" + playWhenReady + "], " + "playbackState = [" + playbackState + "]");
     if (getCurrentState() == STATE_PAUSED_SEEK) {
       if (DEBUG)
         Log.d(TAG,
           "ExoPlayer - onPlayerStateChanged() is currently blocked");
       return;
     }
   ...
   void maybeCorrectSeekPosition() {
     ...
     final long presetStartPositionMillis =
       currentInfo.getStartPosition() * 1000;
     if (presetStartPositionMillis > 0L) {
       if (DEBUG)
         Log.d(TAG,
           "Playback - Seeking to preset start " + "position=[" + presetStartPositionMillis + "]");
   ...
   void onPlayerError(ExoPlaybackException error) {
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onPlayerError() called with: " + "error = [" + error + "]");
     if (errorToast != null) {
       errorToast.cancel();
       errorToast = null;
     }
   ...
   private
   void processSourceError(final IOException error) {
   ...
   @Override
   public
   void onPositionDiscontinuity(@Player.DiscontinuityReason final int reason) {
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onPositionDiscontinuity() called with " + "reason = [" + reason + "]");
     if (playQueue == null)
       return;
     final int newWindowIndex = simpleExoPlayer.getCurrentWindowIndex();
     switch (reason) {
       case DISCONTINUITY_REASON_PERIOD_TRANSITION:
         if (getRepeatMode() == Player.REPEAT_MODE_ONE && newWindowIndex == 
         playQueue.getIndex()) {
           registerView();
           break;
         }
   ...
   @Override
   public
   void onRepeatModeChanged(@Player.RepeatMode final int reason) {
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onRepeatModeChanged() called with: " + "mode = [" + reason + "]");
   }
   @Override
   public
   void onShuffleModeEnabledChanged(final boolean shuffleModeEnabled) {
     if (DEBUG)
       Log.d(TAG,
         "ExoPlayer - onShuffleModeEnabledChanged() called with: " + "mode = [" + shuffleModeEnabled + "]");
     if (playQueue == null)
       return;
   ...
   void onPlaybackSynchronize(@NonNull final PlayQueueItem item) {
     if (DEBUG)
       Log.d(TAG,
         "Playback - onPlaybackSynchronize() called with " + "item=[" + 
         item.getTitle() + "], url=[" + item.getUrl() + "]");
     if (simpleExoPlayer == null || playQueue == null)
       return;
     ...
     final int currentPlaylistSize =
       simpleExoPlayer.getCurrentTimeline().getWindowCount();
     if (!hasPlayQueueItemChanged)
       return;
     currentItem = item;
     if (currentPlayQueueIndex != playQueue.getIndex()) {
       Log.e(TAG,
         "Playback - Play Queue may be desynchronized: item " + "index=[" + currentPlayQueueIndex + "], " + "queue index=[" + 
         playQueue.getIndex() + "]");
     } else if ((currentPlaylistSize > 0 && currentPlayQueueIndex >= currentPlaylistSize) || currentPlayQueueIndex < 0) {
       Log.e(TAG,
         "Playback - Trying to seek to invalid " + "index=[" + currentPlayQueueIndex + "] with " + "playlist length=[" + currentPlaylistSize + "]");
     } else if (currentPlaylistIndex != currentPlayQueueIndex || onPlaybackInitial || !
     isPlaying()) {
       if (DEBUG)
         Log.d(TAG,
           "Playback - Rewinding to correct" + " index=[" + currentPlayQueueIndex + "]," + " from=[" + currentPlaylistIndex + "], size=[" + currentPlaylistSize + "].");
       if (item.getRecoveryPosition() != PlayQueueItem.RECOVERY_UNSET) {
         simpleExoPlayer.seekTo(currentPlayQueueIndex,
   ...
   protected
   void onMetadataChanged(@NonNull final MediaSourceTag tag) {
     final StreamInfo info = tag.getMetadata();
     if (DEBUG) {
       Log.d(TAG,
         "Playback - onMetadataChanged() called, playing: " + info.getName());
     }
   ...
   public
   void showStreamError(Exception exception) {
     exception.printStackTrace();
     if (errorToast == null) {
       errorToast =
       Toast.makeText(context, R.string.player_stream_failure,
         Toast.LENGTH_SHORT);
       errorToast.show();
     }
   }
   public
   void showRecoverableError(Exception exception) {
     exception.printStackTrace();
     if (errorToast == null) {
       errorToast =
       Toast.makeText(context, R.string.player_recoverable_failure,
         Toast.LENGTH_SHORT);
       errorToast.show();
     }
   }
   public
   void showUnrecoverableError(Exception exception) {
     exception.printStackTrace();
     if (errorToast != null) {
       errorToast.cancel();
     }
     errorToast =
     Toast.makeText(context, R.string.player_unrecoverable_failure,
       Toast.LENGTH_SHORT);
     errorToast.show();
   }
   public
   void onPrepared(boolean playWhenReady) {
     if (DEBUG)
       Log.d(TAG,
         "onPrepared() called with: playWhenReady = [" + playWhenReady + "]");
     if (playWhenReady)
   ...
   public
   void onSelected(final PlayQueueItem item) {
   ...
   void seekTo(long positionMillis) {
     if (DEBUG)
       Log.d(TAG,
         "seekBy() called with: position = [" + positionMillis + "]");
     if (simpleExoPlayer != null)
       simpleExoPlayer.seekTo(positionMillis);
   }
   public
   void seekBy(long offsetMillis) {
     if (DEBUG)
       Log.d(TAG,
         "seekBy() called with: offsetMillis = [" + offsetMillis + "]");
   ...
   void registerView() {
     if (currentMetadata == null)
       return;
     final StreamInfo currentInfo = currentMetadata.getMetadata();
     final Disposable viewRegister =
       recordManager.onViewed(currentInfo).onErrorComplete().subscribe(ignored -> {},
   ...
   private
   void savePlaybackState(final StreamInfo info, final long progress) {
     if (info == null)
       return;
     if (DEBUG)
     ...
     final SharedPreferences prefs =
       PreferenceManager.getDefaultSharedPreferences(context);
     if (prefs.getBoolean(context.getString(R.string.enable_watch_history_key),
           true)) {
       final Disposable stateSaver =
         recordManager.saveStreamState(info, progress).observeOn(AndroidSchedulers.mainThread()).doOnError((
           e) -> {
             if (DEBUG)
               e.printStackTrace();
   ...
   private
   void resetPlaybackState(final PlayQueueItem queueItem) {
     if (queueItem == null)
       return;
     final SharedPreferences prefs =
       PreferenceManager.getDefaultSharedPreferences(context);
     if (prefs.getBoolean(context.getString(R.string.enable_watch_history_key),
           true)) {
       final Disposable stateSaver =
         queueItem.getStream().flatMapCompletable(info -> recordManager.saveStreamState(info,
                                                            0)).observeOn(
   ...
   public
   void resetPlaybackState(final StreamInfo info) {
   ...
   private
   void maybeUpdateCurrentMetadata() {
     if (simpleExoPlayer == null)
       return;
   ...
   private
   void maybeAutoQueueNextStream(@NonNull final MediaSourceTag currentMetadata) {
     if (playQueue == null || playQueue.getIndex() != playQueue.size() - 1 || 
     getRepeatMode() != Player.REPEAT_MODE_OFF || !PlayerHelper.isAutoQueueEnabled(context))
       return;
     final PlayQueue autoQueue =
       PlayerHelper.autoQueueOf(currentMetadata.getMetadata(),
   ...
   @Nullable
   public
   MediaSourceTag getCurrentMetadata() {
     return currentMetadata;
   }
   @NonNull
   public
   String getVideoUrl() {
     return currentMetadata == null ? context.getString(R.string.unknown_content) : 
   ...
   public
   Bitmap getThumbnail() {
     return currentThumbnail == null ? BitmapFactory.decodeResource(context.getResources(),
                                         R.drawable.dummy_thumbnail) : currentThumbnail;
   }
   ...
   boolean isLiveEdge() {
     ...
     if (currentTimeline.isEmpty() || currentWindowIndex < 0 || currentWindowIndex >= 
   ...
   public
   void setRepeatMode(@Player.RepeatMode final int repeatMode) {
     if (simpleExoPlayer != null)
   ...
   public
   boolean getPlaybackSkipSilence() {
   ...
   public
   void setPlaybackParameters(float speed, float pitch, boolean skipSilence) {
   ...
   public
   PlayQueueAdapter getPlayQueueAdapter() {
   ...
   public
   void setRecovery(final int queuePos, final long windowPos) {
     if (playQueue.size() <= queuePos)
       return;
     if (DEBUG)
       Log.d(TAG,
         "Setting recovery, queue: " + queuePos + ", pos: " + windowPos);
     playQueue.setRecovery(queuePos, windowPos);
   }
   public
   boolean gotDestroyed() {
     return simpleExoPlayer == null;
   }
   private
   boolean isPlaybackResumeEnabled() {
     final SharedPreferences prefs =
       PreferenceManager.getDefaultSharedPreferences(context);
     return prefs.getBoolean(context.getString(R.string.enable_watch_history_key),
              true) && prefs.getBoolean(context.getString(R.string.enable_playback_resume_key),
                         true);
   }
}