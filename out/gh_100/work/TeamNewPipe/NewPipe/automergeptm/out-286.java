 ...
 */
@SuppressWarnings(value = { "WeakerAccess" }) public abstract class BasePlayer implements Player.EventListener, PlaybackListener, ImageLoadingListener {
  ...
  @NonNull public static final String RESUME_PLAYBACK = "resume_playback";
  ...
  protected final static int PLAY_PREV_ACTIVATION_LIMIT_MILLIS = 5000;

  protected final static int PROGRESS_LOOP_INTERVAL_MILLIS = 500;

  protected final static int RECOVERY_SKIP_THRESHOLD_MILLIS = 3000;
  ...
  public void handleIntent(Intent intent) {
    ...
    if (simpleExoPlayer != null && queue.size() == 1 && playQueue != null && playQueue.getItem() != null && queue.getItem().getUrl().equals(playQueue.getItem().getUrl()) && queue.getItem().getRecoveryPosition() != PlayQueueItem.RECOVERY_UNSET) {
      simpleExoPlayer.seekTo(playQueue.getIndex(), queue.getItem().getRecoveryPosition());
      return;
    } else {
      if (intent.getBooleanExtra(RESUME_PLAYBACK, false) && isPlaybackResumeEnabled()) {
        final PlayQueueItem item = queue.getItem();
        if (item != null && item.getRecoveryPosition() == PlayQueueItem.RECOVERY_UNSET) {
          stateLoader = recordManager.loadStreamState(item).observeOn(AndroidSchedulers.mainThread()).doFinally(() -> initPlayback(queue, repeatMode, playbackSpeed, playbackPitch, playbackSkipSilence, true)).subscribe((state) -> queue.setRecovery(queue.getIndex(), state.getProgressTime()), (error) -> {
            if (DEBUG) {
  ...
  protected void initPlayback(@NonNull final PlayQueue queue, @Player.RepeatMode final int repeatMode, final float playbackSpeed, final float playbackPitch, final boolean playbackSkipSilence, final boolean playOnReady) {
    destroyPlayer();
  ...
  @Override public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
    Log.e(TAG, "Thumbnail - onLoadingFailed() called on imageUri = [" + imageUri + "]", failReason.getCause());
  ...
  private Disposable getProgressReactor() {
    return Observable.interval(PROGRESS_LOOP_INTERVAL_MILLIS, TimeUnit.MILLISECONDS, AndroidSchedulers.mainThread()).observeOn(AndroidSchedulers.mainThread()).subscribe((ignored) -> triggerProgressUpdate(), (error) -> Log.e(TAG, "Progress update failure: ", error));
  }

  @Override public void onTimelineChanged(Timeline timeline, Object manifest, @Player.TimelineChangeReason final int reason) {
    if (DEBUG) {
      Log.d(TAG, "ExoPlayer - onTimelineChanged() called with " + (manifest == null ? "no manifest" : "available manifest") + ", " + "timeline size = [" + timeline.getWindowCount() + "], " + "reason = [" + reason + "]");
    }
  ...
  @Override public void onLoadingChanged(final boolean isLoading) {
    ...
    if (!isLoading && getCurrentState() == STATE_PAUSED && isProgressLoopRunning()) {
      ...
      if (isLoading && !isProgressLoopRunning()) {
        startProgressLoop();
      }
    }
    maybeUpdateCurrentMetadata();
      ...
      if (cause instanceof UnknownHostException) {
        ...
        if (isCurrentWindowValid()) {
          ...
          if (cause instanceof FailedMediaSource.MediaSourceResolutionException) {
            ...
            if (cause instanceof FailedMediaSource.StreamInfoLoadException) {
              playQueue.error(false);
            } else {
              playQueue.error(true);
            }
  ...
  @Override public void onPositionDiscontinuity(@Player.DiscontinuityReason final int reason) {
    if (DEBUG) {
  ...
  @Override public void onRepeatModeChanged(@Player.RepeatMode final int reason) {
    if (DEBUG) {
  ...
  @Override public boolean isApproachingPlaybackEdge(final long timeToEndMillis) {
  ...
  @Override public void onPlaybackBlock() {
    if (simpleExoPlayer == null) {
      return;
    }
    if (DEBUG) {
  ...
  @Override public void onPlaybackUnblock(final MediaSource mediaSource) {
    if (simpleExoPlayer == null) {
      return;
    }
    if (DEBUG) {
  ...
  public void onPlaybackSynchronize(@NonNull final PlayQueueItem item) {
    if (DEBUG) {
      Log.d(TAG, "Playback - onPlaybackSynchronize() called with " + "item=[" + item.getTitle() + "], url=[" + item.getUrl() + "]");
    }
    if (simpleExoPlayer == null || playQueue == null) {
      return;
    }
    ...
    if (currentPlayQueueIndex != playQueue.getIndex()) {
      ...
      if ((currentPlaylistSize > 0 && currentPlayQueueIndex >= currentPlaylistSize) || currentPlayQueueIndex < 0) {
  ...
  @Override public void onPlaybackShutdown() {
    if (DEBUG) {
  ...
  private void registerView() {
    ...
    final StreamInfo currentInfo = currentMetadata.getMetadata();
    final Disposable viewRegister = recordManager.onViewed(currentInfo).onErrorComplete().subscribe((ignored) -> {
  ...
  private void savePlaybackState(final StreamInfo info, final long progress) {
    ...
    if (prefs.getBoolean(context.getString(R.string.enable_watch_history_key), true)) {
      final Disposable stateSaver = recordManager.saveStreamState(info, progress).observeOn(AndroidSchedulers.mainThread()).doOnError((e) -> {
        if (DEBUG) {
          e.printStackTrace();
        }
  ...
  private void resetPlaybackState(final PlayQueueItem queueItem) {
    ...
    final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
    if (prefs.getBoolean(context.getString(R.string.enable_watch_history_key), true)) {
      final Disposable stateSaver = queueItem.getStream().flatMapCompletable((info) -> recordManager.saveStreamState(info, 0)).observeOn(AndroidSchedulers.mainThread()).doOnError((e) -> {
        if (DEBUG) {
          e.printStackTrace();
        }
      }).onErrorComplete().subscribe();
  ...
  @Nullable public MediaSourceTag getCurrentMetadata() {
  ...
  @NonNull public String getVideoTitle() {
  ...
  @Nullable public Bitmap getThumbnail() {
    return currentThumbnail == null ? BitmapFactory.decodeResource(context.getResources(), R.drawable.dummy_thumbnail) : currentThumbnail;
  }

  /**
     * Checks if the current playback is a livestream AND is playing at or beyond the live edge
     */
  @SuppressWarnings(value = { "BooleanMethodIsAlwaysInverted" }) public boolean isLiveEdge() {
  ...
  public boolean isLive() {
    ...
    try {
      ...
      if (DEBUG) {
        Log.d(TAG, "Could not update metadata: " + ignored.getMessage());
      }
      if (DEBUG) {
  ...
  private boolean isPlaybackResumeEnabled() {
    final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
    return prefs.getBoolean(context.getString(R.string.enable_watch_history_key), true) && prefs.getBoolean(context.getString(R.string.enable_playback_resume_key), true);
  }
