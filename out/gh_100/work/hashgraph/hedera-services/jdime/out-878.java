...
public class BackedSystemAccountsCreator implements SystemAccountsCreator {
  ...
  public BackedSystemAccountsCreator(HederaNumbers hederaNums, AccountNumbers accountNums, PropertySource properties, LegacyEd25519KeyReader b64KeyReader) {
  ...
  @Override public void createSystemAccounts(FCMap<MerkleEntityId, MerkleAccount> accounts, AddressBook addressBook) {
    throw new UnsupportedOperationException();
  }

  @Override public void ensureSystemAccounts(BackingStore<AccountID, MerkleAccount> accounts, AddressBook addressBook) {
    ...
    long treasuryBalance = tinyBarFloat - (nodeBalance * nodeAccountNums.size()) - (defaultBalance * (N - nodeAccountNums.size() - 1));
    for (long num = 1; num <= N; num++) {
      ...
      if (num == accountNums.treasury()) {
        ...
        if (nodeAccountNums.contains(num)) {
  ...
  private MerkleAccount accountWith(long balance, long amount, long expiry) {
    var account = new HederaAccountCustomizer().fundsSentRecordThreshold(amount).fundsReceivedRecordThreshold(amount).isReceiverSigRequired(false).proxy(MISSING_ENTITY_ID).isAccountDeleted(false).expiry(expiry).memo("").isSmartContract(false).key(getGenesisKey()).autoRenewPeriod(expiry).customizing(new MerkleAccount());
    try {
  ...
  private JKey getGenesisKey() {
    if (genesisKey == null) {
      try {
        genesisKey = asFcKeyUnchecked(Key.newBuilder().setKeyList(KeyList.newBuilder().addKeys(Key.newBuilder().setEd25519(ByteString.copyFrom(commonsHexToBytes(getHexedABytes()))))).build());
  ...
  private String getHexedABytes() {
    if (hexedABytes == null) {
      hexedABytes = b64KeyReader.hexedABytesFrom(properties.getStringProperty("bootstrap.genesisB64Keystore.path"), properties.getStringProperty("bootstrap.genesisB64Keystore.keyName"));
    }
    return hexedABytes;
  }
