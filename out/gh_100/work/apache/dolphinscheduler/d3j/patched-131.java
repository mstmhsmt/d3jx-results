...
class ProjectServiceImpl extends BaseService implements ProjectService
{
   ...
   Map<String,Object> createProject(User loginUser, String name, String desc) {
     ...
     try {
       project =
       Project.newBuilder().name(name).code(SnowFlakeUtils.getInstance().nextId()).description(desc).userId(
         loginUser.getId()).userName(loginUser.getUserName()).createTime(now).updateTime(now).build();
   ...
   public
   boolean hasProjectAndPerm(User loginUser, Project project, 
   ...
   public
   Map<String,Object> queryProjectListPaging(User loginUser, 
                                             Integer pageSize, 
   ...
   private
   Map<String,Object> getCheckResult(User loginUser, Project project) {
   ...
   public
   Map<String,Object> update(User loginUser, Integer projectId, 
     ...
     boolean hasProjectAndPerm =
       hasProjectAndPerm(loginUser, project, result);
   ...
   private
   List<Project> getUnauthorizedProjects(Set<Project> projectSet, 
                                         List<Project> authedProjectList) {
   ...
   private
   boolean checkReadPermission(User user, Project project) {
     int permissionId = queryPermission(user, project);
     return (permissionId & Constants.READ_PERMISSION) != 0;
   }
   private
   int queryPermission(User user, Project project) {
