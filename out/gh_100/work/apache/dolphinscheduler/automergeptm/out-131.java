 ...
 **/
@Service public class ProjectServiceImpl extends BaseService implements ProjectService {
  @Autowired private ProjectMapper projectMapper;

  @Autowired private ProjectUserMapper projectUserMapper;

  @Autowired private ProcessDefinitionMapper processDefinitionMapper;
  ...
  public Map<String, Object> createProject(User loginUser, String name, String desc) {
    ...
    Date now = new Date();
    try {
      project = Project.newBuilder().name(name).code(SnowFlakeUtils.getInstance().nextId()).description(desc).userId(loginUser.getId()).userName(loginUser.getUserName()).createTime(now).updateTime(now).build();
  ...
  public Map<String, Object> checkProjectAndAuth(User loginUser, Project project, String projectName) {
    ...
    if (project == null) {
      ...
      if (!checkReadPermission(loginUser, project)) {
