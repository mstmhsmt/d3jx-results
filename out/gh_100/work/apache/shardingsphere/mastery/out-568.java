...
public final class OrchestrationMasterSlaveDataSourceFactory {
    ...
    static private YamlOrchestrationMasterSlaveRuleConfiguration unmarshal(final File yamlFile) throws IOException {
    ...
    static private YamlOrchestrationMasterSlaveRuleConfiguration unmarshal(final byte[] yamlByteArray) throws IOException {
    ...
    public static DataSource createDataSource(final File yamlFile) throws SQLException, IOException {
        ...
        return createDataSource(config.getDataSources(), config.getMasterSlaveRule().getMasterSlaveRuleConfiguration(), config.getMasterSlaveRule().getConfigMap(), config.getOrchestration().getOrchestrationConfiguration());
    }

    public static DataSource createDataSource(final Map<String, DataSource> dataSourceMap, final File yamlFile) throws SQLException, IOException {
        YamlOrchestrationMasterSlaveRuleConfiguration config = unmarshal(yamlFile);
        return createDataSource(dataSourceMap, config.getMasterSlaveRule().getMasterSlaveRuleConfiguration(), config.getMasterSlaveRule().getConfigMap(), config.getOrchestration().getOrchestrationConfiguration());
    }
