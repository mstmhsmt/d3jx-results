...
public class TcpIpConnectionManager implements ConnectionManager, PacketHandler {
  ...
  @Probe(name = "connectionListenerCount") final Set<ConnectionListener> connectionListeners = new CopyOnWriteArraySet<ConnectionListener>();
  ...
  @Probe(name = "count", level = MANDATORY) private final ConcurrentHashMap<Address, Connection> connectionsMap = new ConcurrentHashMap<Address, Connection>(100);

  @Probe(name = "monitorCount") private final ConcurrentHashMap<Address, TcpIpConnectionErrorHandler> monitors = new ConcurrentHashMap<Address, TcpIpConnectionErrorHandler>(100);

  @Probe(name = "inProgressCount") private final Set<Address> connectionsInProgress = Collections.newSetFromMap(new ConcurrentHashMap<Address, Boolean>());

  @Probe(name = "acceptedSocketCount", level = MANDATORY) private final Set<Channel> acceptedSockets = Collections.newSetFromMap(new ConcurrentHashMap<Channel, Boolean>());

  @Probe(name = "activeCount", level = MANDATORY) private final Set<TcpIpConnection> activeConnections = Collections.newSetFromMap(new ConcurrentHashMap<TcpIpConnection, Boolean>());

  @Probe(name = "textCount", level = MANDATORY) private final AtomicInteger allTextConnections = new AtomicInteger();
  ...
  @Probe private final MwCounter openedCount = newMwCounter();

  @Probe private final MwCounter closedCount = newMwCounter();
  ...
  public TcpIpConnectionManager(IOService ioService, ServerSocketChannel serverSocketChannel, LoggingService loggingService, MetricsRegistry metricsRegistry, EventLoopGroup eventLoopGroup) {
    ...
    this.scheduler = new ScheduledThreadPoolExecutor(SCHEDULER_POOL_SIZE, new ThreadFactoryImpl(createThreadPoolName(ioService.getHazelcastName(), "TcpIpConnectionManager")));
  ...
  @Override public int getActiveConnectionCount() {
  ...
  @Override public int getConnectionCount() {
  ...
  @Override public synchronized boolean registerConnection(final Address remoteEndPoint, final Connection connection) {
    try {
  ...
  synchronized TcpIpConnection newConnection(Channel channel, Address endpoint) {
    try {
      ...
      activeConnections.add(connection);
      logger.info("Established socket connection between " + channel.getLocalSocketAddress() + " and " + channel.getRemoteSocketAddress());
      openedCount.inc();
  ...
  @Override public Connection getOrConnect(Address address) {
    return getOrConnect(address, false);
  }

  @Override public Connection getOrConnect(final Address address, final boolean silent) {
  ...
  /**
     ...
     */
  @Override public void onConnectionClose(Connection connection) {
    closedCount.inc();
  ...
  @Probe(name = "clientCount", level = MANDATORY) @Override public int getCurrentClientConnections() {
    int count = 0;
    for (TcpIpConnection conn : activeConnections) {
  ...
  private final class SendTask implements Runnable {
    ...
    @SuppressFBWarnings(value = "VO_VOLATILE_INCREMENT", justification = "single-writer, many-reader") @Override public void run() {
      retries++;
      if (logger.isFinestEnabled()) {
