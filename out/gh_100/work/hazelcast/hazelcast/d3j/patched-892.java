...
class AbstractXmlConfigHelper
{
   private static final ILogger LOGGER =
     Logger.getLogger(AbstractXmlConfigHelper.class);
   ...
   class IterableNodeList implements Iterable<Node>
   {
      ...
      public
      IterableNodeList(final Node node) {
        this(node.getChildNodes());
      }
      public
      IterableNodeList(final NodeList list) {
        this(list,
        (short)0);
      }
      public
      IterableNodeList(final Node node, short nodeType) {
        this(node.getChildNodes(),
        nodeType);
      }
      public
      IterableNodeList(final NodeList parent, short nodeType) {
        this.parent = parent;
   ...
   protected
   String xmlToJavaName(final String name) {
   ...
   protected
   void appendToken(final StringBuilder builder, final StringBuilder token) {
   ...
   protected
   String getTextContent(final Node node) {
     if (node != null) {
       final String text;
       if (domLevel3) {
   ...
   private
   String getTextContentOld(final Node node) {
   ...
   private
   void appendTextContents(final Node node, final StringBuilder buf) {
   ...
   protected
   boolean checkTrue(final String value) {
     return "true".equalsIgnoreCase(value) || "yes".equalsIgnoreCase(value) || 
     "on".equalsIgnoreCase(value);
   }
   protected
   int getIntegerValue(final String parameterName, final String value, 
   ...
   protected
   long getLongValue(final String parameterName, final String value, 
   ...
   protected
   String getAttribute(org.w3c.dom.Node node, String attName) {
   ...
   protected
   SocketInterceptorConfig parseSocketInterceptorConfig(final org.w3c.dom.Node node) {
     SocketInterceptorConfig socketInterceptorConfig =
       new SocketInterceptorConfig();
     final NamedNodeMap atts = node.getAttributes();
     final Node enabledNode = atts.getNamedItem("enabled");
     final boolean enabled =
       enabledNode != null && checkTrue(getTextContent(enabledNode).trim());
   ...
   protected
   void fillProperties(final org.w3c.dom.Node node, Properties properties) {
     if (properties == null) {
       return;
     }
     for (org.w3c.dom.Node n : new IterableNodeList(node.getChildNodes())) {
   ...
   protected
   SerializationConfig parseSerialization(final Node node) {
   ...
   protected
   void fillDataSerializableFactories(Node node, 
                                      SerializationConfig serializationConfig) {
     for (org.w3c.dom.Node child : new IterableNodeList(node.getChildNodes())) {
       final String name = cleanNodeName(child);
       if ("data-serializable-factory".equals(name)) {
         final String value = getTextContent(child);
         final Node factoryIdNode =
           child.getAttributes().getNamedItem("factory-id");
         ...
         serializationConfig.addDataSerializableFactoryClass(factoryId,
           value);
       }
     }
   }
   protected
   void fillPortableFactories(Node node, 
                              SerializationConfig serializationConfig) {
     for (org.w3c.dom.Node child : new IterableNodeList(node.getChildNodes())) {
       final String name = cleanNodeName(child);
   ...
   protected
   void fillSerializers(final Node node, 
                        SerializationConfig serializationConfig) {
     for (org.w3c.dom.Node child : new IterableNodeList(node.getChildNodes())) {
       final String name = cleanNodeName(child);
       final String value = getTextContent(child);
   ...
   protected
   void fillNativeMemoryConfig(Node node, 
     ...
     if (allocType != null && !"".equals(allocType)) {
       nativeMemoryConfig.setAllocatorType(NativeMemoryConfig.MemoryAllocatorType.valueOf(
                                             upperCaseInternal(allocType)));
     }
     for (org.w3c.dom.Node n : new IterableNodeList(node.getChildNodes())) {
       final String nodeName = cleanNodeName(n.getNodeName());
       if ("size".equals(nodeName)) {
         ...
         final MemoryUnit unit =
           MemoryUnit.valueOf(getTextContent(attrs.getNamedItem("unit")));
