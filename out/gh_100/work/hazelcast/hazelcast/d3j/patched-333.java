...
public
class TcpIpConnectionManager implements ConnectionManager, PacketHandler
{
   private static final int RETRY_NUMBER = 5;
   private static final int DELAY_FACTOR = 100;
   private static final int SCHEDULER_POOL_SIZE = 4;
   final LoggingService loggingService;
   @Probe(name="connectionListenerCount") final Set<ConnectionListener>
   connectionListeners = new CopyOnWriteArraySet<ConnectionListener>();
   private final IOService ioService;
   private final ConstructorFunction<Address,TcpIpConnectionErrorHandler>
   monitorConstructor =
     new ConstructorFunction<Address,TcpIpConnectionErrorHandler>()
     {
        public
        TcpIpConnectionErrorHandler createNew(Address endpoint) {
          return new TcpIpConnectionErrorHandler(TcpIpConnectionManager.this,
                   endpoint);
        }
     };
   ...
   @Probe(name="count",
          level=MANDATORY)
   private final ConcurrentHashMap<Address,Connection> connectionsMap =
     new ConcurrentHashMap<Address,Connection>(100);
   @Probe(name="monitorCount") private final
   ConcurrentHashMap<Address,TcpIpConnectionErrorHandler> monitors =
     new ConcurrentHashMap<Address,TcpIpConnectionErrorHandler>(100);
   @Probe(name="inProgressCount") private final Set<Address>
   ...
   TcpIpConnectionManager(IOService ioService, 
     ...
     this.scheduler =
     new ScheduledThreadPoolExecutor(SCHEDULER_POOL_SIZE,
       new ThreadFactoryImpl(createThreadPoolName(ioService.getHazelcastName(),
   ...
   public
   Set<TcpIpConnection> getActiveConnections() {
   ...
   void handle(Packet packet) throws Exception {
     ...
     bind((TcpIpConnection)packet.getConn(), bind.getLocalAddress(),
       bind.getTargetAddress(), bind.shouldReply());
   }
   private
   synchronized
   boolean bind(TcpIpConnection connection, Address remoteEndPoint, 
                Address localEndpoint, boolean reply) {
     ...
     if (ioService.isSocketBindAny() && !connection.isClient() && !thisAddress.equals(localEndpoint)) {
       String msg =
         "Wrong bind request from " + remoteEndPoint + "! This node is not requested endpoint: " + localEndpoint;
       logger.warning(msg);
   ...
   public
   synchronized
   boolean registerConnection(final Address remoteEndPoint, 
     ...
     try {
       ...
       if (connection instanceof TcpIpConnection) {
         ...
         if (!connection.isClient()) {
           TcpIpConnectionErrorHandler connectionMonitor =
             getErrorHandler(remoteEndPoint, true);
   ...
   private
   boolean checkAlreadyConnected(TcpIpConnection connection, 
                                 Address remoteEndPoint) {
     final Connection existingConnection =
       connectionsMap.get(remoteEndPoint);
   ...
   void sendBindRequest(TcpIpConnection connection, Address remoteEndPoint, 
                        boolean reply) {
     connection.setEndPoint(remoteEndPoint);
     ioService.onSuccessfulConnection(remoteEndPoint);
     if (logger.isFinestEnabled()) {
     ...
     BindMessage bind =
       new BindMessage(ioService.getThisAddress(), remoteEndPoint, reply);
   ...
   Channel createChannel(SocketChannel socketChannel, boolean client)
     throws Exception {
     Channel wrapper =
       channelFactory.create(socketChannel, client,
         ioService.useDirectSocketBuffer());
   ...
   TcpIpConnection newConnection(Channel channel, Address endpoint) {
     try {
       ...
       TcpIpConnection connection =
         new TcpIpConnection(this, connectionIdGen.incrementAndGet(),
           channel);
       connection.setEndPoint(endpoint);
       activeConnections.add(connection);
       logger.info("Established socket connection between " + channel.getLocalSocketAddress() + " and " + 
   ...
   @Override
   public
   Connection getOrConnect(final Address address, final boolean silent) {
   ...
   private
   TcpIpConnectionErrorHandler getErrorHandler(Address endpoint, 
                                               boolean reset) {
     TcpIpConnectionErrorHandler monitor =
       ConcurrencyUtil.getOrPutIfAbsent(monitors, endpoint,
         monitorConstructor);
     if (reset) {
       monitor.reset();
     }
   ...
   private
   void fireConnectionRemovedEvent(final Connection connection, 
   ...
   private
   void destroySilently(Connection conn, String reason) {
     if (conn == null) {
       return;
     }
     try {
   ...
   private
   boolean send(Packet packet, Address target, SendTask sendTask) {
     ...
     if (retries < RETRY_NUMBER && ioService.isActive()) {
       ...
       scheduler.schedule(sendTask, (retries + 1) * DELAY_FACTOR,
   ...
   class SendTask implements Runnable
   {
      ...
      private
      SendTask(Packet packet, Address target) {
        this.packet = packet;
        this.target = target;
      }
      @SuppressFBWarnings(value="VO_VOLATILE_INCREMENT",
                          justification="single-writer, many-reader")
      @Override
      public
      void run() {
        retries++;
