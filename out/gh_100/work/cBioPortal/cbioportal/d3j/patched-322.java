...
class FileUtilsImpl implements org.mskcc.cbio.importer.FileUtils
{
   ...
   @Override
   public
   Collection<File> listFiles(File directory, String[] extensions, 
                              boolean recursive)
     throws Exception {
   ...
   @Override
   public
   Collection<String> listFiles(File directory, String wildcard)
     throws Exception {
     ArrayList toReturn = new ArrayList<String>();
     IOFileFilter filter = new WildcardFileFilter(wildcard);
     for (File file : org.apache.commons.io.FileUtils.listFiles(directory,
                        filter, null)) {
   ...
   @Override
   public
   List<DataMatrix> getDataMatrices(ImportDataRecord importDataRecord, 
                                    DataMatrix methylationCorrelation)
     throws Exception {
     ...
     if (GzipUtils.isCompressedFilename(fileCanonicalPath.toLowerCase())) {
       ...
       dataMatrices.addAll(getDataMatricesFromArchive(importDataRecord,
       ...
       InputStream is =
         org.apache.commons.io.FileUtils.openInputStream(dataFile);
       DataMatrix m =
         getDataMatrix(dataFile.getName(), is, methylationCorrelation);
       if (m != null) {
         dataMatrices.add(m);
       }
       IOUtils.closeQuietly(is);
   ...
   @Override
   public
   List<String> getMissingCaseListFilenames(String rootDirectory, 
                                            CancerStudyMetadata cancerStudyMetadata)
     throws Exception {
     ArrayList toReturn = new ArrayList<String>();
     String caseListDirectory =
       (rootDirectory + File.separator + cancerStudyMetadata.getStudyPath() + File.separator + "case_lists");
     for (CaseListMetadata caseListMetadata : config.getCaseListMetadata(Config.ALL)) {
       String caseListFilename =
         caseListDirectory + File.separator + caseListMetadata.getCaseListFilename();
   ...
   @Override
   public
   void generateCaseLists(boolean overwrite, boolean strict, 
                          String stagingDirectory, 
                          CancerStudyMetadata cancerStudyMetadata)
     throws Exception {
     for (CaseListMetadata caseListMetadata : config.getCaseListMetadata(Config.ALL)) {
       if (LOG.isInfoEnabled()) {
         LOG.info("generateCaseLists(), processing cancer study: " + cancerStudyMetadata + ", case list: " + 
           caseListMetadata.getCaseListFilename());
       }
       File caseListFile =
         org.apache.commons.io.FileUtils.getFile(stagingDirectory,
           cancerStudyMetadata.getStudyPath(), "case_lists",
           caseListMetadata.getCaseListFilename());
       ...
       if (unionCaseList) {
         stagingFilenames =
         caseListMetadata.getStagingFilenames().split("\\" + CaseListMetadata.CASE_LIST_UNION_DELIMITER);
       } else if (intersectionCaseList) {
         stagingFilenames =
         caseListMetadata.getStagingFilenames().split("\\" + CaseListMetadata.CASE_LIST_INTERSECTION_DELIMITER);
       } else {
         stagingFilenames = new String[]{
         caseListMetadata.getStagingFilenames() };
       }
       if (LOG.isInfoEnabled()) {
       ...
       for (String stagingFilename : stagingFilenames) {
         ...
         List<String> caseList =
           getCaseListFromStagingFile(strict, caseIDs, cancerStudyMetadata,
             stagingDirectory, stagingFilename);
         if (caseList.size() == 0) {
           if (LOG.isInfoEnabled()) {
       ...
       if (caseSet.size() > 0) {
         ...
         writeCaseListFile(stagingDirectory, cancerStudyMetadata,
           caseListMetadata, caseSet.toArray(new String[0]));
       ...
       if (overwrite && caseSet.size() > 0 && caseListMetadata.getCaseListFilename().equals(CaseListMetadata.ALL_CASES_FILENAME)) {
         ...
         writeCancerStudyMetadataFile(stagingDirectory, cancerStudyMetadata,
           caseSet.size());
       }
     }
   }
   @Override
   public
   List<String> getCaseListFromStagingFile(boolean strict, CaseIDs caseIDs, 
                                           CancerStudyMetadata cancerStudyMetadata,
                                           String stagingDirectory, 
     ...
     try {
       ...
       while (it.hasNext()) {
         ...
         List<String> thisRow =
           Arrays.asList(line.split(Converter.VALUE_DELIMITER));
         if (processHeader) {
           mafCaseIDColumnIndex =
           thisRow.indexOf(Converter.MUTATION_CASE_ID_COLUMN_HEADER);
   ...
   @Override
   public
   File createTmpFileWithContents(String filename, String fileContent)
     throws Exception {
     File tmpFile =
       org.apache.commons.io.FileUtils.getFile(org.apache.commons.io.FileUtils.getTempDirectoryPath(),
         filename);
     org.apache.commons.io.FileUtils.writeStringToFile(tmpFile, fileContent,
       false);
     return tmpFile;
   }
   @Override
   public
   File createFileWithContents(String filename, String fileContent)
     throws Exception {
   ...
   @Override
   public
   File createFileFromStream(String filename, InputStream is)
     throws Exception {
     File file = org.apache.commons.io.FileUtils.getFile(filename);
   ...
   @Override
   public
   void downloadFile(String urlSource, String urlDestination)
     throws Exception {
     if (urlSource == null || urlSource.length() == 0 || urlDestination == null || 
     ...
     if (GzipUtils.isCompressedFilename(urlSource)) {
       ...
       org.apache.commons.io.FileUtils.moveFile(org.apache.commons.io.FileUtils.getFile(
                                                  GzipUtils.getUncompressedFilename(
                                                    tempDestinationFile.getCanonicalPath())),
         destinationFile);
   ...
   @Override
   public
   void writeCancerStudyMetadataFile(String stagingDirectory, 
                                     CancerStudyMetadata cancerStudyMetadata, 
                                     int numCases)
     throws Exception {
     File metaFile =
     ...
     PrintWriter writer =
       new PrintWriter(org.apache.commons.io.FileUtils.openOutputStream(metaFile,
                         false));
     ...
     String name =
       (cancerStudyMetadata.getName().length() > 0) ? cancerStudyMetadata.getName() : 
     cancerStudyMetadata.getTumorTypeMetadata().getName();
     name =
     name.replaceAll(CancerStudyMetadata.TUMOR_TYPE_NAME_TAG,
       cancerStudyMetadata.getTumorTypeMetadata().getName());
     writer.print("name: " + name + "\n");
     writer.print("short_name: " + cancerStudyMetadata.getShortName() + "\n");
     String description = cancerStudyMetadata.getDescription();
     description =
     description.replaceAll(CancerStudyMetadata.NUM_CASES_TAG,
       Integer.toString(numCases));
     description =
     description.replaceAll(CancerStudyMetadata.TUMOR_TYPE_TAG,
       cancerStudyMetadata.getTumorTypeMetadata().getType().toUpperCase());
     description =
     description.replaceAll(CancerStudyMetadata.TUMOR_TYPE_NAME_TAG,
       cancerStudyMetadata.getTumorTypeMetadata().getName());
   ...
   public
   void writeMetadataFile(String stagingDirectory, 
                          CancerStudyMetadata cancerStudyMetadata, 
                          DatatypeMetadata datatypeMetadata, int numCases)
     throws Exception {
     String filename =
       stagingDirectory + File.separator + datatypeMetadata.getMetaFilename();
     ...
     stableID =
     stableID.replaceAll(DatatypeMetadata.CANCER_STUDY_TAG,
     ...
     String profileDescription =
       datatypeMetadata.getMetaProfileDescription();
     if (numCases < 0) {
       numCases = 0;
     }
     profileDescription =
     profileDescription.replaceAll(DatatypeMetadata.NUM_CASES_TAG,
       Integer.toString(numCases));
     profileDescription =
     profileDescription.replaceAll(DatatypeMetadata.TUMOR_TYPE_TAG,
   ...
   void writeMetadataFile(String stagingDirectory, 
     ...
     if (dataMatrix != null) {
       profileDescription =
       profileDescription.replaceAll(DatatypeMetadata.NUM_GENES_TAG,
         Integer.toString(dataMatrix.getGeneIDs().size()));
       profileDescription =
       profileDescription.replaceAll(DatatypeMetadata.NUM_CASES_TAG,
         Integer.toString(dataMatrix.getCaseIDs().size()));
     }
     profileDescription =
     profileDescription.replaceAll(DatatypeMetadata.TUMOR_TYPE_TAG,
       cancerStudyMetadata.getTumorType());
   ...
   @Override
   public
   void writeCopyNumberSegmentMetadataFile(String stagingDirectory, 
                                           CancerStudyMetadata cancerStudyMetadata,
                                           DatatypeMetadata datatypeMetadata, 
                                           DataMatrix dataMatrix)
     throws Exception {
     String metaFilename =
       datatypeMetadata.getMetaFilename().replaceAll(DatatypeMetadata.CANCER_STUDY_TAG,
         cancerStudyMetadata.toString());
     File metaFile =
       org.apache.commons.io.FileUtils.getFile(stagingDirectory,
         cancerStudyMetadata.getStudyPath(), metaFilename);
     if (LOG.isInfoEnabled()) {
     ...
     String cnaSegFilename =
       datatypeMetadata.getStagingFilename().replaceAll(DatatypeMetadata.CANCER_STUDY_TAG,
         cancerStudyMetadata.toString());
   ...
   @Override
   public
   void writeStagingFile(String stagingDirectory, 
                         CancerStudyMetadata cancerStudyMetadata, 
                         DatatypeMetadata datatypeMetadata, 
                         DataMatrix dataMatrix)
     throws Exception {
     ...
     stagingFilename =
     stagingFilename.replaceAll(DatatypeMetadata.CANCER_STUDY_TAG,
       cancerStudyMetadata.toString());
     File stagingFile =
       org.apache.commons.io.FileUtils.getFile(stagingDirectory,
         cancerStudyMetadata.getStudyPath(), stagingFilename);
     ...
     FileOutputStream out =
       org.apache.commons.io.FileUtils.openOutputStream(stagingFile, false);
     dataMatrix.write(out);
     IOUtils.closeQuietly(out);
   }
   @Override
   public
   void writeMutationStagingFile(String stagingDirectory, 
                                 CancerStudyMetadata cancerStudyMetadata, 
                                 DatatypeMetadata datatypeMetadata, 
                                 DataMatrix dataMatrix)
     throws Exception {
     ...
     FileOutputStream out =
       org.apache.commons.io.FileUtils.openOutputStream(oncotatorInputFile);
     dataMatrix.write(out);
     IOUtils.closeQuietly(out);
     String stagingFilename = datatypeMetadata.getStagingFilename();
     stagingFilename =
     ...
     File stagingFile =
       org.apache.commons.io.FileUtils.getFile(stagingDirectory,
         cancerStudyMetadata.getStudyPath(), stagingFilename);
     oncotateMAF(org.mskcc.cbio.importer.FileUtils.FILE_URL_PREFIX + 
   ...
   @Override
   public
   boolean writeZScoresStagingFile(String stagingDirectory, 
                                   CancerStudyMetadata cancerStudyMetadata, 
                                   DatatypeMetadata datatypeMetadata, 
     ...
     if (LOG.isInfoEnabled()) {
       LOG.info("writeZScoresStagingFile(), checking for existence of dependencies: " + 
     ...
     if (!cnaFile.exists()) {
       if (LOG.isInfoEnabled()) {
         LOG.info("writeZScoresStagingFile(), cannot find cna file dependency: " + 
     ...
     if (!expressionFile.exists()) {
       if (LOG.isInfoEnabled()) {
         LOG.info("writeZScoresStagingFile(), cannot find expression file dependency: " + 
     ...
     String[] args = { cnaFile.getCanonicalPath(),
     expressionFile.getCanonicalPath(), zScoresFile.getCanonicalPath(),
     NormalizeExpressionLevels.TCGA_NORMAL_SUFFIX };
     if (LOG.isInfoEnabled()) {
       LOG.info("writingZScoresStagingFlie(), calling NormalizeExpressionLevels: " + 
   ...
   @Override
   public
   File getOverrideFile(PortalMetadata portalMetadata, 
                        CancerStudyMetadata cancerStudyMetadata, 
                        String filename)
     throws Exception {
     File overrideFile =
       org.apache.commons.io.FileUtils.getFile(portalMetadata.getOverrideDirectory(),
         cancerStudyMetadata.getStudyPath(), filename);
     return (overrideFile.exists()) ? overrideFile : null;
   }
   @Override
   public
   void applyOverride(String overrideDirectory, String stagingDirectory, 
                      CancerStudyMetadata cancerStudyMetadata, 
                      String overrideFilename, String stagingFilename)
     throws Exception {
     File overrideFile =
       org.apache.commons.io.FileUtils.getFile(overrideDirectory,
         cancerStudyMetadata.getStudyPath(), overrideFilename);
     if (overrideFile.exists()) {
       ...
       if (overrideFile.isFile()) {
         ...
         org.apache.commons.io.FileUtils.copyDirectory(overrideFile,
           stagingFile);
       }
     }
   }
   @Override
   public
   void writeCaseListFile(String stagingDirectory, 
                          CancerStudyMetadata cancerStudyMetadata, 
     ...
     PrintWriter writer =
       new PrintWriter(org.apache.commons.io.FileUtils.openOutputStream(caseListFile,
                         false));
     writer.print("cancer_study_identifier: " + cancerStudyMetadata.getStableId() + "\n");
     ...
     String caseListDescription =
       caseListMetadata.getMetaCaseListDescription();
     caseListDescription =
     caseListDescription.replaceAll(DatatypeMetadata.NUM_CASES_TAG,
       Integer.toString(caseList.length));
   ...
   @Override
   public
   void oncotateAllMAFs(DataSourcesMetadata dataSourcesMetadata)
     throws Exception {
     String[] extensions = new String[]{ DatatypeMetadata.MAF_FILE_EXT };
     for (File maf : listFiles(new File(dataSourcesMetadata.getDownloadDirectory()),
                       extensions, true)) {
       File oncotatorInputFile =
         org.apache.commons.io.FileUtils.getFile(org.apache.commons.io.FileUtils.getTempDirectory(),
           "" + System.currentTimeMillis() + ".oncotatorInputFile");
   ...
   void oncotateMAF(String inputMAFURL, String outputMAFURL) throws Exception {
     ...
     if (ixNcbiBuild != -1 && it.hasNext()) {
       ...
       if (parts[ixNcbiBuild].contains("36") || parts[ixNcbiBuild].equals("hg18")) {
         ...
         org.apache.commons.io.FileUtils.copyFile(oncotatorInputFile,
           liftoverInputFile);
         oncotatorInputFile = new File(inputMAF.getFile());
         if (LOG.isInfoEnabled()) {
           LOG.info("oncotateMAF(), calling Hg18ToHg19...");
         }
         Hg18ToHg19.driver(liftoverInputFile.getCanonicalPath(),
           oncotatorInputFile.getCanonicalPath(), getLiftOverBinary(),
           getLiftOverChain());
     ...
     OncotateTool.driver(oncotatorInputFile.getCanonicalPath(),
       oncotatorOutputFile.getCanonicalPath(), true, true, true);
     if (LOG.isInfoEnabled()) {
     ...
     MutationAssessorTool.driver(oncotatorOutputFile.getCanonicalPath(),
       outputMAFFile.getCanonicalPath(), false, true, true);
   ...
   @Override
   public
   void copySegFiles(PortalMetadata portalMetadata, 
                     DatatypeMetadata datatypeMetadata, String remoteUserName)
     throws Exception {
     if (LOG.isInfoEnabled()) {
     ...
     Collection<DataSourcesMetadata> dataSourcesMetadata =
       config.getDataSourcesMetadata(Config.ALL);
     for (CancerStudyMetadata cancerStudyMetadata : config.getCancerStudyMetadata(
                                                      portalMetadata.getName())) {
       String rootDirectory =
         MetadataUtils.getCancerStudyRootDirectory(portalMetadata,
           dataSourcesMetadata, cancerStudyMetadata);
       ...
       String sourceFilename =
         (rootDirectory + File.separator + cancerStudyMetadata.getStudyPath() + File.separator + 
       datatypeMetadata.getStagingFilename());
       sourceFilename =
       sourceFilename.replaceAll(DatatypeMetadata.CANCER_STUDY_TAG,
         cancerStudyMetadata.toString());
       String destinationFilename =
         datatypeMetadata.getStagingFilename().replaceAll(DatatypeMetadata.CANCER_STUDY_TAG,
           cancerStudyMetadata.toString());
       String[] command = new String[]{ "scp", sourceFilename,
       remoteUserName + "@" + segFileLocation.getHost() + ":" + segFileLocation.getFile() + destinationFilename
       };
       if (LOG.isInfoEnabled()) {
   ...
   private
   List<DataMatrix> getDataMatricesFromArchive(ImportDataRecord importDataRecord,
     ...
     try {
       ...
       if (GzipUtils.isCompressedFilename(importDataRecord.getCanonicalPathToData().toLowerCase())) {
         ...
         TarArchiveInputStream tis =
           new TarArchiveInputStream(unzippedContent);
         ...
         while ((entry = tis.getNextTarEntry()) != null) {
           ...
           if (dataFilename.contains(DatatypeMetadata.TUMOR_TYPE_TAG)) {
             dataFilename =
             dataFilename.replaceAll(DatatypeMetadata.TUMOR_TYPE_TAG,
               importDataRecord.getTumorTypeLabel());
           }
           if (dataFilename.contains(DatatypeMetadata.CLINICAL_FOLLOWUP_VERSION)) {
             Matcher clinicalPatientFollowupMatcher =
               DatatypeMetadata.CLINICAL_FOLLOWUP_FILE_REGEX.matcher(entryName);
             if (clinicalPatientFollowupMatcher.find()) {
               dataFilename =
               dataFilename.replace(DatatypeMetadata.CLINICAL_FOLLOWUP_VERSION,
           ...
           if (entryName.contains(dataFilename)) {
             ...
             DataMatrix m =
               getDataMatrix(entryName, tis, methylationCorrelation);
             if (m != null) {
               toReturn.add(m);
             }
           }
         ...
         DataMatrix m =
           getDataMatrix(dataFile.getName(), unzippedContent,
             methylationCorrelation);
         if (m != null) {
           toReturn.add(m);
         }
   ...
   private
   DataMatrix getDataMatrix(String dataFilename, InputStream data, 
                            DataMatrix methylationCorrelation)
     throws Exception {
     ...
     while (it.hasNext()) {
       if (++count == 0) {
         columnNames =
         new LinkedList(Arrays.asList(it.nextLine().split(Converter.VALUE_DELIMITER,
                                        -1)));
       } else {
         rowData =
         (rowData == null) ? new LinkedList<LinkedList<String>>() : rowData;
         LinkedList<String> thisRow =
           new LinkedList(Arrays.asList(it.nextLine().split(Converter.VALUE_DELIMITER,
                                          -1)));
     ...
     if (LOG.isInfoEnabled()) {
       LOG.info("creating new DataMatrix(), from file data, num rows: " + 
         rowData.size());
     }
   ...
   private
   static
   void gunzip(String gzipFile) throws Exception {
   ...
   private
   static
   Map initProbMap(DataMatrix methylationCorrelation) {
   ...
   private
   List<String> getCaseListFromSequencedSamplesFile(File sequencedSamplesFile)
     throws Exception {
     if (LOG.isInfoEnabled()) {
     ...
     LinkedHashSet<String> caseSet = new LinkedHashSet<String>();
     org.apache.commons.io.LineIterator it =
       org.apache.commons.io.FileUtils.lineIterator(sequencedSamplesFile);
     try {
       while (it.hasNext()) {
   ...
   private
   boolean processingBCRClinicalFile(String dataFilename) {
     return (dataFilename.startsWith(DatatypeMetadata.BCR_CLINICAL_FILENAME_PREFIX));
   }
   private
   boolean skipClinicalDataRow(LinkedList<String> row) {
