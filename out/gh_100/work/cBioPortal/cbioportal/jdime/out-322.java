...
public class FileUtilsImpl implements org.mskcc.cbio.importer.FileUtils {
  ...
  @Value(value = "${liftover_binary}") public void setLiftOverBinary(String property) {
    this.liftoverBinary = property;
  }

  public String getLiftOverBinary() {
    return MetadataUtils.getCanonicalPath(liftoverBinary);
  }

  private String liftoverChain;

  @Value(value = "${liftover_chain_file}") public void setLiftOverChain(String property) {
    this.liftoverChain = property;
  }

  public String getLiftOverChain() {
    return MetadataUtils.getCanonicalPath(liftoverChain);
  }
  ...
  @Override public String getMD5Digest(File file) throws Exception {
  ...
  @Override public String getPrecomputedMD5Digest(File file) throws Exception {
    if (LOG.isInfoEnabled()) {
  ...
  @Override public void makeDirectory(File directory) throws Exception {
    org.apache.commons.io.FileUtils.forceMkdir(directory);
  }

  @Override public boolean directoryIsEmpty(File directory) throws Exception {
    return (listFiles(directory, null, true).isEmpty());
  }

  @Override public void deleteDirectory(File directory) throws Exception {
    org.apache.commons.io.FileUtils.deleteDirectory(directory);
  }

  @Override public void deleteFile(File file) throws Exception {
    if (file.exists()) {
      org.apache.commons.io.FileUtils.forceDelete(file);
    }
  }

  @Override public Collection<File> listFiles(File directory, String[] extensions, boolean recursive) throws Exception {
    return org.apache.commons.io.FileUtils.listFiles(directory, extensions, recursive);
  }

  @Override public Collection<String> listFiles(File directory, String wildcard) throws Exception {
  ...
  @Override public List<DataMatrix> getDataMatrices(ImportDataRecord importDataRecord, DataMatrix methylationCorrelation) throws Exception {
  ...
  @Override public List<String> getMissingCaseListFilenames(String rootDirectory, CancerStudyMetadata cancerStudyMetadata) throws Exception {
    ArrayList toReturn = new ArrayList<String>();
    ...
    for (CaseListMetadata caseListMetadata : config.getCaseListMetadata(Config.ALL)) {
      ...
      if (!caseListFile.exists()) {
        toReturn.add(caseListFilename);
      }
    }
    return toReturn;
  }

  @Override public void generateCaseLists(boolean overwrite, boolean strict, String stagingDirectory, CancerStudyMetadata cancerStudyMetadata) throws Exception {
    for (CaseListMetadata caseListMetadata : config.getCaseListMetadata(Config.ALL)) {
      if (LOG.isInfoEnabled()) {
        LOG.info("generateCaseLists(), processing cancer study: " + cancerStudyMetadata + ", case list: " + caseListMetadata.getCaseListFilename());
      }
      File caseListFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), "case_lists", caseListMetadata.getCaseListFilename());
      ...
      boolean unionCaseList = caseListMetadata.getStagingFilenames().contains(CaseListMetadata.CASE_LIST_UNION_DELIMITER);
      boolean intersectionCaseList = caseListMetadata.getStagingFilenames().contains(CaseListMetadata.CASE_LIST_INTERSECTION_DELIMITER);
      if (unionCaseList) {
  ...
  @Override public List<String> getCaseListFromStagingFile(boolean strict, CaseIDs caseIDs, CancerStudyMetadata cancerStudyMetadata, String stagingDirectory, String stagingFilename) throws Exception {
    if (LOG.isInfoEnabled()) {
    ...
    if (stagingFilename.equals(DatatypeMetadata.MUTATIONS_STAGING_FILENAME)) {
      File sequencedSamplesFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), DatatypeMetadata.SEQUENCED_SAMPLES_FILENAME);
      if (sequencedSamplesFile.exists()) {
        if (LOG.isInfoEnabled()) {
          LOG.info("getCaseListFromStagingFile(), sequenceSamplesFile exists, calling getCaseListFromSequencedSamplesFile()");
        }
        return getCaseListFromSequencedSamplesFile(sequencedSamplesFile);
      }
    }
    File stagingFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), stagingFilename);
      ...
      while (it.hasNext()) {
        ...
        if (processHeader) {
          ...
          if (mafCaseIDColumnIndex == -1) {
            if (LOG.isInfoEnabled()) {
              LOG.info("getCaseListFromStagingFile(), this is not a MAF header contains sample ids...");
            }
            ...
            if (LOG.isInfoEnabled()) {
              LOG.info("getCaseListFromStagingFile(), this is a MAF, samples ids in col: " + mafCaseIDColumnIndex);
            }
          }
          processHeader = false;
          continue;
  ...
  @Override public File createTmpFileWithContents(String filename, String fileContent) throws Exception {
    File tmpFile = org.apache.commons.io.FileUtils.getFile(org.apache.commons.io.FileUtils.getTempDirectoryPath(), filename);
    org.apache.commons.io.FileUtils.writeStringToFile(tmpFile, fileContent, false);
    return tmpFile;
  }

  @Override public File createFileWithContents(String filename, String fileContent) throws Exception {
  ...
  @Override public File createFileFromStream(String filename, InputStream is) throws Exception {
    File file = org.apache.commons.io.FileUtils.getFile(filename);
    org.apache.commons.io.FileUtils.copyInputStreamToFile(is, file);
    return file;
  }

  @Override public void downloadFile(String urlSource, String urlDestination) throws Exception {
    if (urlSource == null || urlSource.length() == 0 || urlDestination == null || urlDestination.length() == 0) {
    ...
    if (GzipUtils.isCompressedFilename(urlSource)) {
      File tempDestinationFile = org.apache.commons.io.FileUtils.getFile(org.apache.commons.io.FileUtils.getTempDirectory(), "" + System.currentTimeMillis() + "." + new File(source.getFile()).getName());
      if (LOG.isInfoEnabled()) {
      ...
      org.apache.commons.io.FileUtils.moveFile(org.apache.commons.io.FileUtils.getFile(GzipUtils.getUncompressedFilename(tempDestinationFile.getCanonicalPath())), destinationFile);
      tempDestinationFile.delete();
    } else {
      if (LOG.isInfoEnabled()) {
        LOG.info("downloadFile(), " + urlSource + ", this may take a while...");
      }
      org.apache.commons.io.FileUtils.copyURLToFile(source, org.apache.commons.io.FileUtils.getFile(destination.getFile()));
    }
  }

  @Override public LineIterator getFileContents(String urlFile) throws Exception {
    return org.apache.commons.io.FileUtils.lineIterator(new File(new URL(urlFile).getFile()));
  }

  @Override public void writeCancerStudyMetadataFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, int numCases) throws Exception {
    File metaFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), cancerStudyMetadata.getCancerStudyMetadataFilename());
    if (LOG.isInfoEnabled()) {
    ...
    String name = (cancerStudyMetadata.getName().length() > 0) ? cancerStudyMetadata.getName() : cancerStudyMetadata.getTumorTypeMetadata().getName();
    name = name.replaceAll(CancerStudyMetadata.TUMOR_TYPE_NAME_TAG, cancerStudyMetadata.getTumorTypeMetadata().getName());
    ...
    description = description.replaceAll(CancerStudyMetadata.TUMOR_TYPE_TAG, cancerStudyMetadata.getTumorTypeMetadata().getType().toUpperCase());
    description = description.replaceAll(CancerStudyMetadata.TUMOR_TYPE_NAME_TAG, cancerStudyMetadata.getTumorTypeMetadata().getName());
  ...
  public void writeMetadataFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, DatatypeMetadata datatypeMetadata, int numCases) throws Exception {
    ...
    profileDescription = profileDescription.replaceAll(DatatypeMetadata.NUM_CASES_TAG, Integer.toString(numCases));
  ...
  @Override public void writeMetadataFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, DatatypeMetadata datatypeMetadata, DataMatrix dataMatrix) throws Exception {
    File metaFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), datatypeMetadata.getMetaFilename());
    if (LOG.isInfoEnabled()) {
      LOG.info("writeMetadataFile(), meta file: " + metaFile);
    }
    PrintWriter writer = new PrintWriter(org.apache.commons.io.FileUtils.openOutputStream(metaFile, false));
  ...
  @Override public void writeCopyNumberSegmentMetadataFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, DatatypeMetadata datatypeMetadata, DataMatrix dataMatrix) throws Exception {
    ...
    File metaFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), metaFilename);
    if (LOG.isInfoEnabled()) {
  ...
  @Override public void writeStagingFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, DatatypeMetadata datatypeMetadata, DataMatrix dataMatrix) throws Exception {
  ...
  @Override public void writeMutationStagingFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, DatatypeMetadata datatypeMetadata, DataMatrix dataMatrix) throws Exception {
    File oncotatorInputFile = org.apache.commons.io.FileUtils.getFile(org.apache.commons.io.FileUtils.getTempDirectory(), "" + System.currentTimeMillis() + ".oncotatorInputFile");
    ...
    stagingFilename = stagingFilename.replaceAll(DatatypeMetadata.CANCER_STUDY_TAG, cancerStudyMetadata.toString());
    File stagingFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), stagingFilename);
    oncotateMAF(org.mskcc.cbio.importer.FileUtils.FILE_URL_PREFIX + oncotatorInputFile.getCanonicalPath(), org.mskcc.cbio.importer.FileUtils.FILE_URL_PREFIX + stagingFile.getCanonicalPath());
    if (oncotatorInputFile.exists()) {
      org.apache.commons.io.FileUtils.forceDelete(oncotatorInputFile);
    }
  }

  @Override public boolean writeZScoresStagingFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, DatatypeMetadata datatypeMetadata, DatatypeMetadata[] dependencies) throws Exception {
    ...
    File cnaFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), dependencies[0].getStagingFilename());
    if (!cnaFile.exists()) {
      if (LOG.isInfoEnabled()) {
    ...
    File expressionFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), dependencies[1].getStagingFilename());
    ...
    File zScoresFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), datatypeMetadata.getStagingFilename());
    String[] args = { cnaFile.getCanonicalPath(), expressionFile.getCanonicalPath(), zScoresFile.getCanonicalPath(), NormalizeExpressionLevels.TCGA_NORMAL_SUFFIX };
    if (LOG.isInfoEnabled()) {
    ...
    try {
      ...
      if (LOG.isInfoEnabled()) {
        LOG.info("writingZScoresStagingFlie(), exception thrown by NormalizeExpressionLevels: " + e.getMessage() + ", aborting...");
      }
  ...
  @Override public File getOverrideFile(PortalMetadata portalMetadata, CancerStudyMetadata cancerStudyMetadata, String filename) throws Exception {
    File overrideFile = org.apache.commons.io.FileUtils.getFile(portalMetadata.getOverrideDirectory(), cancerStudyMetadata.getStudyPath(), filename);
    return (overrideFile.exists()) ? overrideFile : null;
  }

  @Override public void applyOverride(String overrideDirectory, String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, String overrideFilename, String stagingFilename) throws Exception {
    File overrideFile = org.apache.commons.io.FileUtils.getFile(overrideDirectory, cancerStudyMetadata.getStudyPath(), overrideFilename);
    if (overrideFile.exists()) {
      File stagingFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), stagingFilename);
      if (LOG.isInfoEnabled()) {
        LOG.info("applyOverride(), override file exists for " + stagingFile.getCanonicalPath() + ": " + overrideFile.getCanonicalPath());
      }
  ...
  @Override public void writeCaseListFile(String stagingDirectory, CancerStudyMetadata cancerStudyMetadata, CaseListMetadata caseListMetadata, String[] caseList) throws Exception {
    File caseListFile = org.apache.commons.io.FileUtils.getFile(stagingDirectory, cancerStudyMetadata.getStudyPath(), "case_lists", caseListMetadata.getCaseListFilename());
    if (LOG.isInfoEnabled()) {
  ...
  /**
	 ...
	 * @throws Exception
	 */
  @Override public void oncotateAllMAFs(DataSourcesMetadata dataSourcesMetadata) throws Exception {
    ...
    for (File maf : listFiles(new File(dataSourcesMetadata.getDownloadDirectory()), extensions, true)) {
      ...
      oncotateMAF(org.mskcc.cbio.importer.FileUtils.FILE_URL_PREFIX + oncotatorInputFile.getCanonicalPath(), org.mskcc.cbio.importer.FileUtils.FILE_URL_PREFIX + maf.getCanonicalPath());
      org.apache.commons.io.FileUtils.forceDelete(oncotatorInputFile);
    }
  }

  @Override public void oncotateMAF(String inputMAFURL, String outputMAFURL) throws Exception {
    if (inputMAFURL == null || inputMAFURL.length() == 0 || outputMAFURL == null || outputMAFURL.length() == 0) {
    ...
    File oncotatorOutputFile = org.apache.commons.io.FileUtils.getFile(org.apache.commons.io.FileUtils.getTempDirectory(), "" + System.currentTimeMillis() + ".oncotatorOutputFile");
    if (LOG.isInfoEnabled()) {
  ...
  /**
	 ...
	 * @throws Exception
	 */
  @Override public void copySegFiles(PortalMetadata portalMetadata, DatatypeMetadata datatypeMetadata, String remoteUserName) throws Exception {
    if (LOG.isInfoEnabled()) {
    ...
    for (CancerStudyMetadata cancerStudyMetadata : config.getCancerStudyMetadata(portalMetadata.getName())) {
      ...
      String sourceFilename = (rootDirectory + File.separator + cancerStudyMetadata.getStudyPath() + File.separator + datatypeMetadata.getStagingFilename());
      ...
      String[] command = new String[] { "scp", sourceFilename, remoteUserName + "@" + segFileLocation.getHost() + ":" + segFileLocation.getFile() + destinationFilename };
      if (LOG.isInfoEnabled()) {
        ...
        while ((entry = tis.getNextTarEntry()) != null) {
          ...
          if (dataFilename.contains(DatatypeMetadata.CLINICAL_FOLLOWUP_VERSION)) {
            ...
            if (clinicalPatientFollowupMatcher.find()) {
              dataFilename = dataFilename.replace(DatatypeMetadata.CLINICAL_FOLLOWUP_VERSION, clinicalPatientFollowupMatcher.group(1));
            }
          }
    ...
    while (it.hasNext()) {
      if (++count == 0) {
        ...
        if (methylationCorrelation == null) {
          ...
          if (probeIdMap.containsKey(thisRow.getFirst())) {
            rowData.add(thisRow);
          }
        }
      }
  ...
  private static Map initProbMap(DataMatrix methylationCorrelation) {
    ...
    for (String probeId : methylationCorrelation.getColumnData(MethylationConverterImpl.CORRELATE_METH_PROBE_COLUMN_HEADER_NAME).get(0)) {
