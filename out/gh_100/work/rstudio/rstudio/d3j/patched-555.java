...
class AceThemes
{
   @Inject
   public
   AceThemes(ThemeServerOperations themeServerOperations, 
             final Provider<UIPrefs> prefs, EventBus events) {
     ...
     prefs.get().theme().bind(new CommandWithArg<AceTheme>()
                              {
                                 public
                                 void execute(AceTheme theme) {
                                   applyTheme(theme);
                                 }
                              });
   }
   private
   void applyTheme(Document document, final AceTheme theme) {
     ...
     if (theme.isDark())
       document.getBody().addClassName("editor_dark"); else
   ...
   private
   void applyTheme(final AceTheme theme) {
     applyTheme(Document.get(), theme);
   }
   public
   void applyTheme(Document document) {
     applyTheme(document, prefs_.get().theme().getValue());
   }
   public
   void getThemes(Consumer<HashMap<String,AceTheme>> themeConsumer) {
     themeServerOperations_.getThemes(new ServerRequestCallback<JsArray<AceTheme>>()
                                      {
                                         @Override
                                         public
                                         void onResponseReceived(JsArray<AceTheme> jsonThemeArray) {
                                           HashMap<String,AceTheme> themes =
                                             new HashMap<>();
                                           int len = jsonThemeArray.length();
                                           for (int i = 0; i < len; ++i) {
                                             AceTheme theme =
                                               jsonThemeArray.get(i);
                                             themes.put(theme.getName(),
                                               theme);
                                           }
                                         ...
                                         void onError(ServerError error) {
                                           HashMap<String,AceTheme> themes =
                                             new HashMap<>();
                                           AceTheme defaultTheme =
                                             AceTheme.createDefault();
                                           themes.put(defaultTheme.getName(),
                                             defaultTheme);
                                           themeConsumer.accept(themes);
                                         }
                                      });
