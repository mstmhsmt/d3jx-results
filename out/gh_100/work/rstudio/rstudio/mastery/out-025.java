...
public class VisualModeChunk {
    ...
    final private Element element_;

    final private DivElement outputHost_;

    final private DivElement host_;

    final private DivElement chunkHost_;

    final private DivElement gutterHost_;

    final private DivElement editorHost_;

    final private PanmirrorUIChunkEditor chunk_;

    final private AceEditor editor_;

    final private Element editorContainer_;

    final private DocDisplay parent_;

    final private List<Command> destroyHandlers_;

    final private ArrayList<HandlerRegistration> releaseOnDismiss_;

    final private VisualModeEditorSync sync_;

    final private EditingTargetCodeExecution codeExecution_;

    final private VisualModeCollapseToggle collapse_;

    @SuppressWarnings("unused")
    final private LintManager lintManager_;

    final private DivElement summary_;

    final private Map<Integer, VisualModeChunkRowState> rowState_;

    final private TextEditingTarget target_;

    final private int markdownIndex_;

    final private static ViewsSourceConstants constants_ = GWT.create(ViewsSourceConstants.class);
