...
public class AnnotatedTypeFactory implements AnnotationProvider {
    ...
    final private Set<Class<? extends Annotation>> supportedQuals;

    private Map<Element, AnnotatedTypeMirror> indexTypes;

    private Map<String, Set<AnnotationMirror>> indexDeclAnnos;

    final private Map<Element, Set<AnnotationMirror>> cacheDeclAnnos;

    final private Set<AnnotationMirror> inheritedAnnotations = AnnotationUtils.createAnnotationSet();

    protected final BaseTypeChecker checker;

    final private Map<String, AnnotationMirror> aliases = new HashMap<String, AnnotationMirror>();

    final private Map<String, Pair<AnnotationMirror, Set<String>>> declAliases = new HashMap<>();

    private static int uidCounter = 0;

    final public int uid;

    final private AnnotationMirror fromByteCode;
    ...
    protected void postInit() {
        ...
        addAliasedDeclAnnotation(org.jmlspecs.annotation.Pure.class, org.checkerframework.dataflow.qual.Pure.class, AnnotationUtils.fromClass(elements, org.checkerframework.dataflow.qual.Pure.class));
        addInheritedAnnotation(AnnotationUtils.fromClass(elements, org.checkerframework.dataflow.qual.Pure.class));
        addInheritedAnnotation(AnnotationUtils.fromClass(elements, org.checkerframework.dataflow.qual.SideEffectFree.class));
        addInheritedAnnotation(AnnotationUtils.fromClass(elements, org.checkerframework.dataflow.qual.Deterministic.class));
        addInheritedAnnotation(AnnotationUtils.fromClass(elements, org.checkerframework.dataflow.qual.TerminatesExecution.class));
        addInheritedAnnotation(AnnotationUtils.fromClass(elements, org.checkerframework.dataflow.qual.LockingFree.class));
    ...
    final public QualifierHierarchy getQualifierHierarchy() {
        return qualHierarchy;
    }

    protected TypeHierarchy createTypeHierarchy() {
        return new DefaultTypeHierarchy(checker, getQualifierHierarchy(), checker.hasOption("ignoreRawTypeArguments"), checker.hasOption("invariantArrays"));
    }

    final public TypeHierarchy getTypeHierarchy() {
    ...
    final public Set<Class<? extends Annotation>> getSupportedTypeQualifiers() {
    ...
    final private static int CACHE_SIZE = 300;

    final private Map<Tree, AnnotatedTypeMirror> treeCache = createLRUCache(CACHE_SIZE);

    protected final Map<Tree, AnnotatedTypeMirror> fromTreeCache = createLRUCache(CACHE_SIZE);

    final private Map<Element, AnnotatedTypeMirror> elementCache = createLRUCache(CACHE_SIZE);

    final private Map<Element, Tree> elementToTreeCache = createLRUCache(CACHE_SIZE);

    final private TreePathCacher treePathCache = new TreePathCacher();
    ...
    public AnnotatedTypeMirror fromElement(Element elt) {
        ...
        if (decl == null && indexTypes != null && indexTypes.containsKey(elt)) {
            ...
            ErrorReporter.errorAbort("AnnotatedTypeFactory.fromElement: cannot be here! decl: " + decl.getKind() + " elt: " + elt, null);
            type = null;
        }
    ...
    private void addFromByteCode(Element elt) {
        ...
        if (elt.getKind() == ElementKind.CONSTRUCTOR || elt.getKind() == ElementKind.METHOD || elt.getKind() == ElementKind.FIELD) {
            if (ElementUtils.isElementFromByteCode(elt)) {
                Set<AnnotationMirror> annos = indexDeclAnnos.get(ElementUtils.getVerboseName(elt));
                if (annos == null) {
                    annos = AnnotationUtils.createAnnotationSet();
                    indexDeclAnnos.put(ElementUtils.getVerboseName(elt), annos);
                }
                if (!annos.contains(AnnotationUtils.fromClass(elements, FromStubFile.class))) {
    ...
    public AnnotatedTypeMirror fromTypeTree(Tree tree) {
        ...
        if (result.getKind() == TypeKind.DECLARED) {
            ...
            if (dt.wasRaw()) {
                ...
                if (ctx != null) {
                    if (ctx.second.getKind() == TypeKind.DECLARED && types.isSameType(types.erasure(ctx.second.actualType), types.erasure(dt.actualType))) {
    ...
    public void postTypeVarSubstitution(AnnotatedTypeVariable varDecl, AnnotatedTypeVariable varUse, AnnotatedTypeMirror value) {
        if (!varUse.annotations.isEmpty() && !AnnotationUtils.areSame(varUse.annotations, varDecl.annotations)) {
    ...
    protected static class InheritedFromClassAnnotator extends AnnotatedTypeScanner<Void, AnnotatedTypeFactory> {

        final static public InheritedFromClassAnnotator INSTANCE = new InheritedFromClassAnnotator();
        ...
        final private Map<TypeParameterElement, AnnotatedTypeVariable> visited = new HashMap<TypeParameterElement, AnnotatedTypeVariable>();
    ...
    final public boolean isMostEnclosingThisDeref(ExpressionTree tree) {
    ...
    final public boolean isAnyEnclosingThisDeref(ExpressionTree tree) {
    ...
    private boolean isSubtype(TypeElement a1, TypeElement a2) {
        return (a1.equals(a2) || types.isSubtype(types.erasure(a1.asType()), types.erasure(a2.asType())));
    }

    final public AnnotatedTypeMirror getReceiverType(ExpressionTree expression) {
    ...
    final public AnnotatedDeclaredType getAnnotatedType(ClassTree tree) {
        return (AnnotatedDeclaredType) getAnnotatedType((Tree) tree);
    }

    final public AnnotatedDeclaredType getAnnotatedType(NewClassTree tree) {
        return (AnnotatedDeclaredType) getAnnotatedType((Tree) tree);
    }

    final public AnnotatedArrayType getAnnotatedType(NewArrayTree tree) {
        return (AnnotatedArrayType) getAnnotatedType((Tree) tree);
    }

    final public AnnotatedExecutableType getAnnotatedType(MethodTree tree) {
        return (AnnotatedExecutableType) getAnnotatedType((Tree) tree);
    }

    final public AnnotatedDeclaredType getAnnotatedType(TypeElement elt) {
        return (AnnotatedDeclaredType) getAnnotatedType((Element) elt);
    }

    final public AnnotatedExecutableType getAnnotatedType(ExecutableElement elt) {
        return (AnnotatedExecutableType) getAnnotatedType((Element) elt);
    }

    final public AnnotatedDeclaredType fromElement(TypeElement elt) {
        return (AnnotatedDeclaredType) fromElement((Element) elt);
    }

    final public AnnotatedExecutableType fromElement(ExecutableElement elt) {
    ...
    final public AnnotatedTypeMirror toAnnotatedType(TypeMirror t, boolean declaration) {
    ...
    protected final Tree declarationFromElement(Element elt) {
        ...
        switch(elt.getKind()) {
            ...
            case CONSTRUCTOR:
                ...
                break;
            default:
                fromElt = com.sun.tools.javac.tree.TreeInfo.declarationFor((com.sun.tools.javac.code.Symbol) elt, (com.sun.tools.javac.tree.JCTree) root);
                break;
        }
        if (shouldCache)
    ...
    protected final boolean isWithinConstructor(Tree tree) {
        if (visitorState.getClassType() != null)
            return visitorState.getMethodTree() != null && TreeUtils.isConstructor(visitorState.getMethodTree());
    ...
    final private Element getMostInnerClassOrMethod(Tree tree) {
    ...
    final private Map<Tree, Element> pathHack = new HashMap<>();

    final public void setPathHack(Tree node, Element enclosing) {
        pathHack.put(node, enclosing);
    }

    final public TreePath getPath(Tree node) {
    ...
    final public Element getEnclosingMethod(Tree node) {
        return pathHack.get(node);
    }

    final static boolean validAnnotatedType(AnnotatedTypeMirror type) {
        if (type == null)
            return false;
    ...
    final private static boolean validType(TypeMirror type) {
        if (type == null)
            return false;
        switch(type.getKind()) {
            case ERROR:
    ...
    static public <K, V> Map<K, V> createLRUCache(final int size) {
        return new LinkedHashMap<K, V>() {

            final private static long serialVersionUID = 5261489276168775084L;
    ...
    protected void buildIndexTypes() {
        ...
        Map<Element, AnnotatedTypeMirror> indexTypes = new HashMap<Element, AnnotatedTypeMirror>();
        Map<String, Set<AnnotationMirror>> indexDeclAnnos = new HashMap<String, Set<AnnotationMirror>>();
        ...
        for (String stubPath : stubArray) {
            ...
            if (stubs.size() == 0) {
                ...
                checker.message(Kind.NOTE, "Did not find stub file or files within directory: " + stubPath);
            }
    ...
    private AnnotationMirror getDeclAnnotation(Element elt, String annoName, boolean checkAliases) {
        ...
        if (checkAliases) {
            Pair<AnnotationMirror, Set<String>> aliases = declAliases.get(annoName);
            if (aliases != null) {
    ...
    private void inheritOverriddenDeclAnnos(Element elt, Set<AnnotationMirror> results) {
        Map<AnnotatedDeclaredType, ExecutableElement> overriddenMethods = null;
        if (elt instanceof ExecutableElement) {
            overriddenMethods = AnnotatedTypes.overriddenMethods(elements, this, (ExecutableElement) elt);
        }
        if (overriddenMethods != null) {
            for (Map.Entry<AnnotatedDeclaredType, ExecutableElement> pair : overriddenMethods.entrySet()) {
                AnnotatedDeclaredType overriddenType = pair.getKey();
                AnnotatedExecutableType overriddenMethod = AnnotatedTypes.asMemberOf(types, this, overriddenType, pair.getValue());
                ...
                for (AnnotationMirror annotation : superAnnos) {
                    ...
                    try {
                        ...
                        checker.message(Kind.WARNING, annotation.getAnnotationType().asElement(), "annotation.not.completed", ElementUtils.getVerboseName(elt), annotation);
                        continue;
                    }
                    if (AnnotationUtils.containsSameByClass(annotationsOnAnnotation, InheritedAnnotation.class) || AnnotationUtils.containsSameIgnoringValues(inheritedAnnotations, annotation)) {
    ...
    public List<Pair<AnnotationMirror, AnnotationMirror>> getDeclAnnotationWithMetaAnnotation(Element element, Class<? extends Annotation> metaAnnotation) {
        ...
        for (AnnotationMirror annotation : annotationMirrors) {
            ...
            try {
                ...
                checker.message(Kind.WARNING, annotation.getAnnotationType().asElement(), "annotation.not.completed", ElementUtils.getVerboseName(element), annotation);
                continue;
            }
    ...
    private Pair<AnnotatedDeclaredType, AnnotatedExecutableType> getFnInterfaceFromTree(Tree tree) {
        ...
        Element fnElement = javacTypes.findDescriptorSymbol(((Type) functionalInterfaceType.getUnderlyingType()).asElement());
        AnnotatedExecutableType methodExe = (AnnotatedExecutableType) AnnotatedTypes.asMemberOf(types, this, functionalInterfaceType, fnElement);
        return Pair.of(functionalInterfaceType, methodExe);
    }

    private AnnotatedDeclaredType getFunctionalInterfaceType(Tree lambdaTree, com.sun.tools.javac.code.Types javacTypes) {
        ...
        switch(parentTree.getKind()) {
            ...
            case RETURN:
                Tree enclosing = TreeUtils.enclosingOfKind(TreePath.getPath(this.root, parentTree), new HashSet<>(Arrays.asList(Tree.Kind.METHOD, Tree.Kind.LAMBDA_EXPRESSION)));
            ...
            case LAMBDA_EXPRESSION:
                ...
                return (AnnotatedDeclaredType) methodExe.getReturnType();
            default:
                ErrorReporter.errorAbort("Could not find functional interface from assignment context. " + "Unexpected tree type: " + parentTree.getKind() + " For lambda tree: " + lambdaTree);
                return null;
        }
    }
    ...
    private void assertFunctionalInterface(com.sun.tools.javac.code.Types javacTypes, Type type, Tree contextTree, Tree lambdaTree) {
        if (!javacTypes.isFunctionalInterface(type)) {
            ErrorReporter.errorAbort(String.format("Expected the type of %s tree in assignment context to be a functional interface. " + "Found type: %s for tree: %s in lambda tree: %s", contextTree.getKind(), type, contextTree, lambdaTree));
        }
    }
    ...
    final private AnnotationFormatter annotationFormatter;
