...
public
class JoinGameScreen extends CoreScreenLayer
{
   public static final ResourceUrn ASSET_URI =
     new ResourceUrn("engine:joinGameScreen");
   private static final Logger logger =
     LoggerFactory.getLogger(JoinGameScreen.class);
   ...
   private Map<ServerInfo,Future<ServerInfoMessage>> extInfo =
     new HashMap<>();
   ...
   void initialise() {
     ...
     ActivateEventListener activateOnline =
       e -> {
     ...
     bindCustomButtons();
     bindInfoLabels();
     WidgetUtil.trySubscribe(this, "close",
       button -> {
         config.save();
   ...
   void onOpened() {
     ...
     if (playerConfig.playerName.getDefaultValue().equals(playerConfig.playerName.get())) {
       getManager().pushScreen(EnterUsernamePopup.ASSET_URI,
         EnterUsernamePopup.class);
     }
     if (storageServiceWorker.hasConflictingIdentities()) {
       new IdentityConflictHelper(storageServiceWorker, getManager(),
         translationSystem).runSolver();
     }
   }
   @Override
   public
   void update(float delta) {
     ...
     if (!updateComplete) {
       ...
       listedServers.clear();
       listedServers.addAll(Collections2.filter(downloader.getServers(),
                              activeServersOnly));
     }
   }
   @Override
   ...
   private
   void join(final String address, final int port) {
     Callable<JoinStatus> operation =
       () -> {
       JoinStatus joinStatus = networkSystem.join(address, port);
       return joinStatus;
     };
     final WaitPopup<JoinStatus> popup =
       getManager().pushScreen(WaitPopup.ASSET_URI, WaitPopup.class);
     popup.setMessage(translationSystem.translate("${engine:menu#join-game-online}"),
       translationSystem.translate("${engine:menu#connecting-to}") + " '" + address + ":" + port + "' - " + 
     ...
     popup.onSuccess(result -> {
         if (result.getStatus() != JoinStatus.Status.FAILED) {
           ...
           MessagePopup screen =
             getManager().pushScreen(MessagePopup.ASSET_URI,
               MessagePopup.class);
   ...
   private
   void configureServerList(final UIList<ServerInfo> serverList) {
     serverList.subscribe((widget, item) -> join(item.getAddress(),
                                              item.getPort()));
     serverList.subscribeSelection((widget, item) -> {
         extInfo.remove(item);
         if (item != null) {
           extInfo.put(item,
             infoService.requestInfo(item.getAddress(), item.getPort()));
           refreshPing();
         }
       });
     serverList.setItemRenderer(new StringTextRenderer<ServerInfo>()
                                {
                                   @Override
                                   public
                                   String getString(ServerInfo value) {
   ...
   private
   void bindInfoLabels() {
     final ReadOnlyBinding<ServerInfo> infoBinding =
       new ReadOnlyBinding<ServerInfo>()
       {
          @Override
          public
          ServerInfo get() {
     ...
     if (name != null) {
       name.bindText(BindHelper.bindBoundBeanProperty("name", infoBinding,
                       ServerInfo.class, String.class));
     }
     UILabel owner = find("owner", UILabel.class);
     if (owner != null) {
       owner.bindText(BindHelper.bindBoundBeanProperty("owner", infoBinding,
                        ServerInfo.class, String.class));
     }
     UILabel address = find("address", UILabel.class);
     if (address != null) {
       address.bindText(BindHelper.bindBoundBeanProperty("address",
                          infoBinding, ServerInfo.class, String.class));
     }
     UILabel port = find("port", UILabel.class);
     if (port != null) {
       port.bindText(new IntToStringBinding(BindHelper.bindBoundBeanProperty("port",
                                              infoBinding, ServerInfo.class,
                                              int.class)));
     }
     ...
     onlinePlayers.bindText(new ReadOnlyBinding<String>()
                            {
                               ...
                               public
                               String get() {
                                 Future<ServerInfoMessage> info =
                                   extInfo.get(visibleList.getSelection());
                                 if (info != null) {
                                   if (info.isDone()) {
   ...
   private
   void bindCustomButtons() {
     UIList<?> customServerList = find("customServerList", UIList.class);
     ReadOnlyBinding<Boolean> localSelectedServerOnly =
     ...
     if (edit != null) {
       ...
       edit.subscribe(button -> {
           ...
           popup.onSuccess(item -> extInfo.put(item,
                                     infoService.requestInfo(item.getAddress(),
                                       item.getPort())));
         });
     }
       ...
       downloadLabel.bindText(new ReadOnlyBinding<String>()
                              {
                                 ...
                                 public
                                 String get() {
                                   return translationSystem.translate(
   ...
   private
   String getOnlinePlayersText(Future<ServerInfoMessage> info) {
     try {
       List<String> codedWorldInfo = new ArrayList<>();
       ServerInfoMessage serverInfoMessage = info.get();
       if (serverInfoMessage == null) {
       ...
       codedWorldInfo.add(String.format("%d",
                            serverInfoMessage.getOnlinePlayersAmount()));
       return Joiner.on('\n').join(codedWorldInfo);
     } catch (ExecutionException|InterruptedException e) {
       return FontColor.getColored(translationSystem.translate("${engine:menu#connection-failed}"),
                Color.RED);
   ...
   private
   String getModulesText(Future<ServerInfoMessage> info) {
     try {
       ServerInfoMessage serverInfoMessage = info.get();
       if (serverInfoMessage == null) {
         return FontColor.getColored(translationSystem.translate("${engine:menu#connection-failed}"),
       ...
       for (NameVersion entry : serverInfoMessage.getModuleList()) {
         boolean isInstalled =
           reg.getModule(entry.getName(), entry.getVersion()) != null;
     ...
     Thread getPing =
       new Thread(() -> {
           ...
           try {
             ...
             String text =
               translationSystem.translate("${engine:menu#connection-failed}");
             if (name.equals(visibleList.getSelection().getName()))
               GameThread.asynch(() -> ping.setText(FontColor.getColored(text,
                                                      Color.RED)));
           } 
         });
     getPing.start();
