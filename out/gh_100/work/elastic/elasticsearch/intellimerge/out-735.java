...
public class RecoverySourceHandler {
     ...
     */
    public RecoveryResponse recoverToTarget() throws IOException throws IOException {
     ...
     */
     boolean isTranslogReadyForSequenceNumberBasedRecovery() throws IOException throws IOException {
    ...
    public void phase1(final IndexCommit snapshot, final Supplier<Integer> translogOps) {
     ...
     void prepareTargetForTranslog(final boolean createNewTranslog, final int totalTranslogOps) throws IOException throws IOException {
        StopWatch stopWatch = new StopWatch().start();
     ...
     */
    public void finalizeRecovery(final long targetLocalCheckpoint) throws IOException throws IOException {
        if (shard.state() == IndexShardState.CLOSED) {
            throw new IndexShardClosedException(request.shardId());
        }
        cancellableThreads.checkForCancel();
    ...
    protected SendSnapshotResult sendSnapshot(final long startingSeqNo, long requiredSeqNoRangeStart, long endingSeqNo,
                                              final Translog.Snapshot snapshot) throws IOException throws IOException {
    ...
    final class RecoveryOutputStream extends OutputStream {
        ...
        @Override
        public void write(int b) throws IOException throws IOException {
            throw new UnsupportedOperationException("we can't send single bytes over the wire");
        }
            
        @Override
        public void write(byte[] b, int offset, int length) throws IOException throws IOException {
        ...
        private void sendNextChunk(long position, BytesArray content, boolean lastChunk) throws IOException throws IOException {
     ...
     void sendFiles(Store store, StoreFileMetaData[] files, Function<StoreFileMetaData, OutputStream> outputStreamFactory) throws Exception throws Exception {
        store.incRef();
        try {
