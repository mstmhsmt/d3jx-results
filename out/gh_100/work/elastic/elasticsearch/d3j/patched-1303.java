...
public
class MetaDataIndexTemplateService extends AbstractComponent
{
   ...
   public
   void removeTemplates(final RemoveRequest request, 
     ...
     clusterService.submitStateUpdateTask("remove-index-template [" + request.name + "]",
       new ClusterStateUpdateTask(Priority.URGENT)
       {
          ...
          @Override
          public
          void clusterStateProcessed(String source, ClusterState oldState, 
   ...
   public
   void putTemplate(final PutRequest request, final PutListener listener) {
            ...
            for (Alias alias : request.aliases) {
              AliasMetaData aliasMetaData =
                AliasMetaData.builder(alias.name()).filter(alias.filter()).indexRouting(
                  alias.indexRouting()).searchRouting(alias.searchRouting()).build();
   ...
   private
   static
   void validateAndAddTemplate(final PutRequest request, 
     ...
     try {
       Settings dummySettings =
         Settings.builder().put(IndexMetaData.SETTING_VERSION_CREATED,
           Version.CURRENT).put(request.settings).put(IndexMetaData.SETTING_NUMBER_OF_SHARDS,
           1).put(IndexMetaData.SETTING_NUMBER_OF_REPLICAS, 0).put(IndexMetaData.SETTING_INDEX_UUID,
       ...
       Map<String,Map<String,Object>> mappingsForValidation =
         new HashMap<>();
       for (Map.Entry<String,String> entry : request.mappings.entrySet()) {
         try {
           templateBuilder.putMapping(entry.getKey(), entry.getValue());
         } catch (Exception e) {
           throw new MapperParsingException("Failed to parse mapping [{}]: {}",
                   e, entry.getKey(), e.getMessage());
         } 
         mappingsForValidation.put(entry.getKey(),
           MapperService.parseMapping(entry.getValue()));
       }
       dummyIndexService.mapperService().merge(mappingsForValidation, false);
     } finally{
       if (createdIndex != null) {
         indicesService.removeIndex(createdIndex,
           " created for parsing template mapping");
       }
     } 
   }
   private
   void validate(PutRequest request) {
     ...
     List<String> indexSettingsValidation =
       metaDataCreateIndexService.getIndexSettingsValidationErrors(request.settings);
     ...
     for (Alias alias : request.aliases) {
       ...
       if (request.indexPatterns.contains(alias.name())) {
         throw new IllegalArgumentException("Alias [" + alias.name() + "] cannot be the same as any pattern in [" + 
                 String.join(", ", request.indexPatterns) + "]");
       }
     }
   }
   public
   ...
   class PutRequest
   {
      ...
      public
      PutRequest mappings(Map<String,String> mappings) {
      ...
      public
      PutRequest customs(Map<String,IndexMetaData.Custom> customs) {
        this.customs.putAll(customs);
        return this;
      }
      public
      PutRequest putMapping(String mappingType, String mappingSource) {
        mappings.put(mappingType, mappingSource);
        return this;
      }
      public
      PutRequest masterTimeout(TimeValue masterTimeout) {
   ...
   class PutResponse
   {
      ...
      public
      PutResponse(boolean acknowledged, IndexTemplateMetaData template) {
   ...
   class RemoveRequest
   {
      ...
      public
      RemoveRequest masterTimeout(TimeValue masterTimeout) {
        this.masterTimeout = masterTimeout;
        return this;
      }
   }
