            ...
            execute(task, request, new ActionListener<Response>() {
                ...
                public void onFailure(Exception e) {
                    try {
                        ...
                        logger.warn(() -> new <actionName>ParameterizedMessage("Failed to send response for {}"), inner);
                    }
                }
            });
        }
    ...
    private final class AsyncReplicaAction extends AbstractRunnable implements ActionListener<Releasable> {
        ...
        public void onFailure(Exception e) {
            if (e instanceof RetryOnReplicaException) {
                logger.trace(() -> new <transportReplicaAction>ParameterizedMessage("Retrying operation on replica, action [{}], request [{}]", request), e);
                request.onRetry();
        ...
        protected void responseWithFailure(Exception e) {
            try {
                ...
                logger.warn(() -> new <transportReplicaAction>ParameterizedMessage("failed to send error message back to client for action [{}]"), responseException);
            }
        }
