...
class AssemblyFileImporter
{
   ...
   public
   AssemblyFileImporter(String assemblyFilePath, boolean modified) {
     this.assemblyFilePath = assemblyFilePath;
     this.modified = modified;
   }
   public
   AssemblyFileImporter(SuperAdapter superAdapter) {
     this.superAdapter = superAdapter;
   }
   public
   AssemblyFileImporter() {
   ...
   void importAssembly() {
     ...
     new AssemblyScaffoldHandler(listOfScaffolds, listOfSuperscaffolds,
       listOfUnattempted);
   }
   private
   ...
   private
   void parseAssemblyFileNoBundling() throws IOException {
     try {
       for (String row : rawFileData) {
         if (row.startsWith(">")) {
           String[] splitRow = row.split(" ");
           Scaffold scaffold =
             new Scaffold(splitRow[0].substring(1),
               Integer.parseInt(splitRow[1]), Integer.parseInt(splitRow[2]));
   ...
   private
   void parseAssemblyFile() throws IOException {
     if (!needsBundling) {
     ...
     try {
       ...
       if (modified && start > 1) {
         List<Scaffold> originalScaffolds =
           AssemblyHeatmapHandler.getSuperAdapter().getAssemblyStateTracker().getInitialAssemblyScaffoldHandler().getListOfScaffolds();
         while (true) {
       ...
       for (i = 1; i < start; i++) {
         Scaffold scaffold =
           new Scaffold(name.get(i - 1).substring(1), i, size.get(i - 1));
         listOfScaffolds.add(scaffold);
       ...
       listOfScaffolds.add(new Scaffold("unattempted:::debris", start,
                             len - bundlelen));
       List<Integer> superscaffold = new ArrayList<>();
   ...
   private
   void setModifiedInitialState() {
     List<Scaffold> originalScaffolds =
       AssemblyHeatmapHandler.getSuperAdapter().getAssemblyStateTracker().getInitialAssemblyScaffoldHandler().getListOfScaffolds();
     long modifiedShift = 0;
     int originalScaffoldIterator = 0;
     Scaffold originalScaffold =
       originalScaffolds.get(originalScaffoldIterator);
   ...
   public
   AssemblyScaffoldHandler getAssemblyScaffoldHandler() {
