...
public class MainWindow extends JFrame {
        ...
        public void run() {
          try {
            ...
            if (control) {
              ...
              options = new MatrixType[] { MatrixType.OBSERVED, MatrixType.OE, MatrixType.PEARSON, MatrixType.EXPECTED, MatrixType.RATIO, MatrixType.CONTROL };
            } else {
              hic.reset();
  ...
  private void updateThumbnail() {
    if (hic.getMatrix() != null) {
      ...
      Image thumbnail = heatmapPanel.getThumbnailImage(zd0, zdControl, thumbnailPanel.getWidth(), thumbnailPanel.getHeight(), hic.getDisplayOption());
      if (thumbnail != null) {
  ...
  private void setInitialZoom() {
    if (hic.getXContext().getChromosome().getName().equals("All")) {
      ...
      List<HiCZoom> zooms = (currentUnit == HiC.Unit.BP ? hic.getDataset().getBpZooms() : hic.getDataset().getFragZooms());
      ...
      if (currentUnit == HiC.Unit.BP) {
        ...
        len = Math.max(hic.getDataset().getFragmentCounts().get(hic.getXContext().getChromosome().getName()), hic.getDataset().getFragmentCounts().get(hic.getYContext().getChromosome().getName()));
      }
  ...
  class FileDropTargetListener implements DropTargetListener {
    ...
    public void drop(DropTargetDropEvent event) {
      ...
      try {
        @SuppressWarnings(value = { "unchecked" }) java.util.List<File> files = (java.util.List<File>) transferable.getTransferData(DataFlavor.javaFileListFlavor);
  ...
  private void initComponents() {
    ...
    colorRangeSlider.addMouseListener(new MouseAdapter() {
      @Override @Override public void mouseEntered(MouseEvent mouseEvent) {
  ...
  private void colorRangeSliderUpdateToolTip() {
    if (hic.getDisplayOption() == MatrixType.OBSERVED) {
      colorRangeSlider.setToolTipText("<html>Range: " + (int) (colorRangeSlider.getMinimum() / colorRangeScaleFactor) + " " + (int) (colorRangeSlider.getMaximum() / colorRangeScaleFactor) + "<br>Showing: " + (int) (colorRangeSlider.getLowerValue() / colorRangeScaleFactor) + " " + (int) (colorRangeSlider.getUpperValue() / colorRangeScaleFactor) + "</html>");
    } else {
      if (hic.getDisplayOption() == MatrixType.OE) {
        double mymaximum = colorRangeSlider.getMaximum() / 8;
        colorRangeSlider.setToolTipText("Range: " + new DecimalFormat("##.##").format(1 / mymaximum) + " " + new DecimalFormat("##.##").format(mymaximum));
      }
    }
  }
  ...
  private class QCDialog extends JDialog {
    ...
    public QCDialog(Dataset dataset) {
      ...
      if (graphs != null) {
        ...
        final JFreeChart readTypeChart = ChartFactory.createXYLineChart("Types of reads vs distance", "Distance (log)", "Binned Reads (log)", readTypeCollection, PlotOrientation.VERTICAL, true, true, false);
        ...
        final JFreeChart reChart = ChartFactory.createXYLineChart("Distance from closest RE site", "Distance (bp)", "Fraction of Reads (log)", reCollection, PlotOrientation.VERTICAL, true, true, false);
        ...
        final JFreeChart intraChart = ChartFactory.createXYLineChart("Intra reads vs distance", "Distance (log)", "Cumulative Sum of Binned Reads (log)", intraCollection, PlotOrientation.VERTICAL, true, true, false);
        ...
        final JFreeChart mapqChart = ChartFactory.createXYLineChart("MapQ Threshold Count", "MapQ threshold", "Count", mapqCollection, PlotOrientation.VERTICAL, true, true, false);
      ...
      final ExpectedValueFunction df = hic.getDataset().getExpectedValues(hic.getZoom(), hic.getNormalizationType());
      if (df != null) {
        ...
        final JFreeChart readTypeChart = ChartFactory.createXYLineChart(title, "Distance between reads (log)", "Genome-wide expected (log)", collection, PlotOrientation.VERTICAL, false, true, false);
        final XYPlot readTypePlot = readTypeChart.getXYPlot();
        ...
        try {
          if (box.getSelectedItem().equals("Matrix")) {
            if (hic.getDisplayOption() == MatrixType.OBSERVED) {
              ...
              if (hic.getDisplayOption() == MatrixType.OE || hic.getDisplayOption() == MatrixType.PEARSON) {
                final ExpectedValueFunction df = hic.getDataset().getExpectedValues(hic.getZd().getZoom(), hic.getNormalizationType());
                if (df == null) {
                  JOptionPane.showMessageDialog(this, box.getSelectedItem() + " not available", "Error", JOptionPane.ERROR_MESSAGE);
                  return;
                }
                if (hic.getDisplayOption() == MatrixType.OE) {
                  hic.getZd().dumpOE(df, "oe", hic.getNormalizationType(), null, new PrintWriter(getSelectedFile()));
                } else {
                  hic.getZd().dumpOE(df, "pearson", hic.getNormalizationType(), null, new PrintWriter(getSelectedFile()));
                }
              }
            }
          } else {
            if (box.getSelectedItem().equals("Norm vector")) {
              if (hic.getNormalizationType() == NormalizationType.NONE) {
                JOptionPane.showMessageDialog(this, "Selected normalization is None, nothing to write", "Error", JOptionPane.ERROR_MESSAGE);
              } else {
                NormalizationVector nv = hic.getNormalizationVector(hic.getZd().getChr1Idx());
                HiCTools.dumpVector(new PrintWriter(getSelectedFile()), nv.getData(), false);
              }
            } else {
              if (box.getSelectedItem().toString().contains("Expected")) {
                final ExpectedValueFunction df = hic.getDataset().getExpectedValues(hic.getZd().getZoom(), hic.getNormalizationType());
                if (df == null) {
                  JOptionPane.showMessageDialog(this, box.getSelectedItem() + " not available", "Error", JOptionPane.ERROR_MESSAGE);
                  return;
                ...
                if (box.getSelectedItem().equals("Eigenvector")) {
    ...
    public SaveImageDialog() {
      ...
      if (actionDialog == JFileChooser.APPROVE_OPTION) {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(MainWindow.getInstance(), "Width and Height must be integers", "Error", JOptionPane.ERROR_MESSAGE);
        }
      }
    }
  ...
  private class LoadDialog extends JDialog implements TreeSelectionListener, ActionListener {
    ...
    public LoadDialog(Properties properties) {
      ...
      DefaultMutableTreeNode top = new DefaultMutableTreeNode(new ItemInfo("root", "root", ""));
    ...
    private boolean createNodes(DefaultMutableTreeNode top, Properties properties) {
      ...
      for (String key : keys) {
        ...
        if (parent == null) {
          JOptionPane.showMessageDialog(this, "Improperly formatted properties file; unable to find parent menu " + ((ItemInfo) hashMap.get(key).getUserObject()).parentKey + " for " + key, "Error", JOptionPane.ERROR_MESSAGE);
          return false;
        } else {
          parent.add(node);
        }
  ...
  private class ResolutionControl extends JPanel {
    ...
    private ResolutionControl() {
      ...
      resolutionSlider.addChangeListener(new ChangeListener() {
        public void stateChanged(ChangeEvent e) {
          if (hic == null || hic.getMatrix() == null || hic.getZd() == null) {
            return;
          }
    ...
    private void reset() {
      ...
      List<HiCZoom> binSizes = unit == HiC.Unit.BP ? hic.getDataset().getBpZooms() : hic.getDataset().getFragZooms();
      idxZoomMap.clear();
      ...
      if (e.getID() == KeyEvent.KEY_PRESSED && e.getKeyCode() == KeyEvent.VK_F1) {
        if (hic.getControlZd() != null) {
          ...
          if (displayOption == MainWindow.MatrixType.CONTROL) {
            ...
            if (displayOption == MainWindow.MatrixType.OBSERVED) {
  ...
  public class JSplitButton extends AbstractButton implements Accessible {
    ...
    private Icon createDefaultPopupIcon() {
      Image image = new BufferedImage(DEFAULT_POPUP_ICON_LENGTH, DEFAULT_POPUP_ICON_LENGTH, BufferedImage.TYPE_INT_ARGB);
      Graphics g = image.getGraphics();
      g.setColor(new Color(255, 255, 255, 0));
      g.fillRect(0, 0, DEFAULT_POPUP_ICON_LENGTH, DEFAULT_POPUP_ICON_LENGTH);
      g.setColor(Color.BLACK);
      Polygon traingle = new Polygon(new int[] { 0, DEFAULT_POPUP_ICON_LENGTH, DEFAULT_POPUP_ICON_LENGTH / 2, 0 }, new int[] { 0, 0, DEFAULT_POPUP_ICON_LENGTH, 0 }, 4);
    ...
    public void setAlwaysShowPopup(boolean alwaysShowPopup) {
      ...
      firePropertyChange(ALWAYS_SHOWS_POPUP_CHANGED_PROPERTY, oldValue, alwaysShowPopup);
    ...
    public void setDisabledPopupIcon(Icon icon) {
      ...
      firePropertyChange(DISABLED_POPUP_ICON_CHANGED_PROPERTY, oldValue, icon);
      if (!oldValue.equals(icon)) {
    ...
    public void setDisabledSelectedPopupIcon(Icon icon) {
      ...
      firePropertyChange(DISABLED_SELECTED_POPUP_ICON_CHANGED_PROPERTY, oldValue, icon);
      if (!oldValue.equals(icon)) {
    ...
    public void setPressedPopupIcon(Icon icon) {
      ...
      firePropertyChange(PRESSED_POPUP_ICON_CHANGED_PROPERTY, oldValue, icon);
      if (!oldValue.equals(icon)) {
    ...
    public void setRolloverPopupIcon(Icon icon) {
      ...
      firePropertyChange(ROLLOVER_POPUP_ICON_CHANGED_PROPERTY, oldValue, icon);
      if (!oldValue.equals(icon)) {
    ...
    public void setRolloverSelectedPopupIcon(Icon icon) {
      ...
      firePropertyChange(ROLLOVER_SELECTED_POPUP_ICON_CHANGED_PROPERTY, oldValue, icon);
      if (!oldValue.equals(icon)) {
    ...
    public void setSelectedPopupIcon(Icon icon) {
      ...
      firePropertyChange(SELECTED_POPUP_ICON_CHANGED_PROPERTY, oldValue, icon);
      if (!oldValue.equals(icon)) {
    ...
    @Override public void addItemListener(ItemListener listener) {
      mainButton.addItemListener(listener);
      super.addItemListener(listener);
    }

    @Override public void removeItemListener(ItemListener listener) {
    ...
    @Override public void setComponentPopupMenu(JPopupMenu popupMenu) {
      if (popupAction != null) {
    ...
    @Override public void setMnemonic(int mnemonic) {
    ...
    @Override public void setDisplayedMnemonicIndex(int index) throws IllegalArgumentException {
      int oldValue = getDisplayedMnemonicIndex();
      mainButton.setDisplayedMnemonicIndex(index);
      firePropertyChange("displayedMnemonicIndex", oldValue, index);
    }

    @Override public void setHideActionText(boolean hideActionText) {
    ...
    private Point getPopupLocationRelativeTo(Component comp) {
      ...
      if (comp == popupButton) {
        ...
        Insets mainButtonInsets = mainButton.getInsets();
        int width = mainButton.getWidth() + mainButtonInsets.left + mainButtonInsets.right + splitGap;
        if (comp == this) {
          width += insets.left;
        }
