...
public class ChangeSet implements Conditional, LiquibaseSerializable {
  ...
  @Override public void load(ParsedNode node, ResourceAccessor resourceAccessor) throws ParsedNodeException {
  ...
  protected void handleChildNode(ParsedNode child, ResourceAccessor resourceAccessor) throws ParsedNodeException {
    if (child.getName().equals("rollback")) {
      ...
      if (child.getName().equals("validCheckSum") || child.getName().equals("validCheckSums")) {
        ...
        if (child.getName().equals("modifySql")) {
          ...
          if (child.getName().equals("preConditions")) {
            ...
            if (child.getName().equals("changes")) {
  ...
  @Override public ParsedNode serialize() {
    ...
    try {
      ...
      try {
        ...
        if (preconditions.getOnFail().equals(PreconditionContainer.FailOption.HALT)) {
          ...
          if (preconditions.getOnFail().equals(PreconditionContainer.FailOption.CONTINUE)) {
            skipChange = true;
            ...
            if (preconditions.getOnFail().equals(PreconditionContainer.FailOption.MARK_RAN)) {
              execType = ExecType.MARK_RAN;
              skipChange = true;
              log.info("Marking ChangeSet: " + toString() + " ran despite precondition failure due to onFail=\'MARK_RAN\': " + message);
            } else {
              if (preconditions.getOnFail().equals(PreconditionContainer.FailOption.WARN)) {
                execType = null;
              } else {
        ...
        if (preconditions.getOnError().equals(PreconditionContainer.ErrorOption.HALT)) {
          ...
          if (preconditions.getOnError().equals(PreconditionContainer.ErrorOption.CONTINUE)) {
            skipChange = true;
            execType = ExecType.SKIPPED;
          } else {
            if (preconditions.getOnError().equals(PreconditionContainer.ErrorOption.MARK_RAN)) {
              execType = ExecType.MARK_RAN;
              skipChange = true;
              log.info("Marking ChangeSet: " + toString() + " ran despite precondition error: " + message);
            } else {
              if (preconditions.getOnError().equals(PreconditionContainer.ErrorOption.WARN)) {
                execType = null;
              } else {
  ...
  @Override public PreconditionContainer getPreconditions() {
    return preconditions;
  }

  @Override public void setPreconditions(PreconditionContainer preconditionContainer) {
  ...
  @Override public String getSerializedObjectName() {
    return "changeSet";
  }

  @Override public Set<String> getSerializableFields() {
    return new HashSet<String>(Arrays.asList("id", "author", "runAlways", "runOnChange", "failOnError", "context", "dbms", "comment", "changes", "rollback", "objectQuotingStrategy"));
  }

  @Override public Object getSerializableFieldValue(String field) {
    if (field.equals("id")) {
      return this.getId();
    }
  ...
  @Override public SerializationType getSerializableFieldType(String field) {
  ...
  @Override public String getSerializableFieldNamespace(String field) {
