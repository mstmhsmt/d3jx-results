...
public class Main {
  ...
  private static XMLResourceBundle commandLineHelpBundle = ((XMLResourceBundle) getBundle("liquibase/i18n/liquibase-commandline-helptext", new XmlResourceBundleControl()));
      ...
      @Override public Integer run() throws Exception {
        ...
        try {
          if (!GlobalConfiguration.SHOULD_RUN.getCurrentValue()) {
            Scope.getCurrentScope().getUI().sendErrorMessage((String.format(coreBundle.getString("did.not.run.because.param.was.set.to.false"), GlobalConfiguration.SHOULD_RUN.getKey())));
            return 0;
          }
          if ((args.length == 0) || ((args.length == 1) && ("--" + OPTIONS.HELP).equals(args[0]))) {
            ...
            if (("--" + OPTIONS.VERSION).equals(args[0])) {
              main.command = "";
          ...
          Scope.child(innerScopeObjects, () -> {
            ...
            if (COMMANDS.UPDATE.equals(main.command)) {
              ...
              if (main.command.startsWith(COMMANDS.ROLLBACK)) {
  ...
  private static boolean setupNeeded(Main main) throws CommandLineParsingException {
    ...
    return !main.command.toLowerCase().startsWith(COMMANDS.ROLLBACK_ONE_CHANGE_SET.toLowerCase()) && !main.command.toLowerCase().startsWith(COMMANDS.ROLLBACK_ONE_UPDATE.toLowerCase()) && (!main.command.toLowerCase().startsWith(COMMANDS.DIFF.toLowerCase()) || !main.isFormattedDiff());
  }
  ...
  /**
     ...
     */
  @SuppressWarnings(value = { "squid:S109" }) private static String[] splitArg(String arg) throws CommandLineParsingException {
    String[] splitArg = arg.split("=", 2);
    if (splitArg.length < 2) {
  ...
  private static boolean isHubEnabled(String command) {
    return COMMANDS.CHANGELOG_SYNC_SQL.equalsIgnoreCase(command) || COMMANDS.UPDATE_COUNT.equalsIgnoreCase(command) || COMMANDS.UPDATE_TO_TAG.equalsIgnoreCase(command) || COMMANDS.UPDATE.equalsIgnoreCase(command) || COMMANDS.ROLLBACK.equalsIgnoreCase(command) || COMMANDS.ROLLBACK_TO_DATE.equalsIgnoreCase(command) || COMMANDS.ROLLBACK_COUNT.equalsIgnoreCase(command) || COMMANDS.ROLLBACK_ONE_CHANGE_SET.equalsIgnoreCase(command) || COMMANDS.ROLLBACK_ONE_UPDATE.equalsIgnoreCase(command) || COMMANDS.DROP_ALL.equalsIgnoreCase(command);
  }
  ...
  private static boolean isNoArgCommand(String arg) {
    return COMMANDS.MIGRATE.equals(arg) || COMMANDS.MIGRATE_SQL.equalsIgnoreCase(arg) || COMMANDS.UPDATE.equalsIgnoreCase(arg) || COMMANDS.UPDATE_SQL.equalsIgnoreCase(arg) || COMMANDS.FUTURE_ROLLBACK_SQL.equalsIgnoreCase(arg) || COMMANDS.UPDATE_TESTING_ROLLBACK.equalsIgnoreCase(arg) || COMMANDS.LIST_LOCKS.equalsIgnoreCase(arg) || COMMANDS.RELEASE_LOCKS.equalsIgnoreCase(arg) || COMMANDS.VALIDATE.equalsIgnoreCase(arg) || COMMANDS.HELP.equalsIgnoreCase(arg) || COMMANDS.CLEAR_CHECKSUMS.equalsIgnoreCase(arg) || COMMANDS.CHANGELOG_SYNC.equalsIgnoreCase(arg) || COMMANDS.CHANGELOG_SYNC_SQL.equalsIgnoreCase(arg) || COMMANDS.MARK_NEXT_CHANGESET_RAN.equalsIgnoreCase(arg) || COMMANDS.MARK_NEXT_CHANGESET_RAN_SQL.equalsIgnoreCase(arg);
  }
  ...
  private static File extract(JarFile jar, JarEntry entry) throws IOException {
    ...
    try (BufferedInputStream inStream = new BufferedInputStream(jar.getInputStream(entry)); BufferedOutputStream outStream = new BufferedOutputStream(new FileOutputStream(tempFile))) {
      int status;
  ...
  private void checkForUnexpectedCommandParameter(List<String> messages) {
    if (COMMANDS.UPDATE_COUNT.equalsIgnoreCase(command) || COMMANDS.UPDATE_COUNT_SQL.equalsIgnoreCase(command) || COMMANDS.UPDATE_TO_TAG.equalsIgnoreCase(command) || COMMANDS.UPDATE_TO_TAG_SQL.equalsIgnoreCase(command) || COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(command) || COMMANDS.DB_DOC.equalsIgnoreCase(command) || COMMANDS.TAG.equalsIgnoreCase(command) || COMMANDS.TAG_EXISTS.equalsIgnoreCase(command)) {
      ...
      if (COMMANDS.STATUS.equalsIgnoreCase(command) || COMMANDS.UNEXPECTED_CHANGESETS.equalsIgnoreCase(command)) {
        if ((!commandParams.isEmpty()) && !commandParams.iterator().next().equalsIgnoreCase("--" + OPTIONS.VERBOSE)) {
          messages.add(coreBundle.getString(ERRORMSG_UNEXPECTED_PARAMETERS) + commandParams);
        }
      } else {
        if (COMMANDS.DIFF.equalsIgnoreCase(command) || COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(command)) {
          if ((!commandParams.isEmpty())) {
            for (String cmdParm : commandParams) {
              if (!cmdParm.startsWith("--" + OPTIONS.REFERENCE_USERNAME) && !cmdParm.startsWith("--" + OPTIONS.REFERENCE_PASSWORD) && !cmdParm.startsWith("--" + OPTIONS.REFERENCE_DRIVER) && !cmdParm.startsWith("--" + OPTIONS.REFERENCE_DEFAULT_CATALOG_NAME) && !cmdParm.startsWith("--" + OPTIONS.REFERENCE_DEFAULT_SCHEMA_NAME) && !cmdParm.startsWith("--" + OPTIONS.INCLUDE_SCHEMA) && !cmdParm.startsWith("--" + OPTIONS.INCLUDE_CATALOG) && !cmdParm.startsWith("--" + OPTIONS.INCLUDE_TABLESPACE) && !cmdParm.startsWith("--" + OPTIONS.SCHEMAS) && !cmdParm.startsWith("--" + OPTIONS.OUTPUT_SCHEMAS_AS) && !cmdParm.startsWith("--" + OPTIONS.REFERENCE_SCHEMAS) && !cmdParm.startsWith("--" + OPTIONS.REFERENCE_URL) && !cmdParm.startsWith("--" + OPTIONS.EXCLUDE_OBJECTS) && !cmdParm.startsWith("--" + OPTIONS.INCLUDE_OBJECTS) && !cmdParm.startsWith("--" + OPTIONS.DIFF_TYPES) && !cmdParm.startsWith("--" + OPTIONS.FORMAT) && !cmdParm.startsWith("--" + OPTIONS.HELP) && !cmdParm.startsWith("--" + OPTIONS.SNAPSHOT_FORMAT)) {
          ...
          if ((COMMANDS.SNAPSHOT.equalsIgnoreCase(command) || COMMANDS.GENERATE_CHANGELOG.equalsIgnoreCase(command)) && (!commandParams.isEmpty())) {
            for (String cmdParm : commandParams) {
              if (!cmdParm.startsWith("--" + OPTIONS.INCLUDE_SCHEMA) && !cmdParm.startsWith("--" + OPTIONS.INCLUDE_CATALOG) && !cmdParm.startsWith("--" + OPTIONS.INCLUDE_TABLESPACE) && !cmdParm.startsWith("--" + OPTIONS.SCHEMAS) && !cmdParm.startsWith("--" + OPTIONS.SNAPSHOT_FORMAT) && !cmdParm.startsWith("--" + OPTIONS.DATA_OUTPUT_DIRECTORY) && !cmdParm.startsWith("--" + OPTIONS.OUTPUT_SCHEMAS_AS)) {
                messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
              }
            }
          } else {
            if (COMMANDS.ROLLBACK_ONE_CHANGE_SET.equalsIgnoreCase(command)) {
              for (String cmdParm : commandParams) {
                if (!cmdParm.startsWith("--" + OPTIONS.CHANGE_SET_ID) && !cmdParm.startsWith("--" + OPTIONS.HELP) && !cmdParm.startsWith("--" + OPTIONS.FORCE) && !cmdParm.startsWith("--" + OPTIONS.CHANGE_SET_PATH) && !cmdParm.startsWith("--" + OPTIONS.CHANGE_SET_AUTHOR) && !cmdParm.startsWith("--" + OPTIONS.ROLLBACK_SCRIPT)) {
                  messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                }
              }
            } else {
              if (COMMANDS.ROLLBACK_ONE_CHANGE_SET_SQL.equalsIgnoreCase(command)) {
                for (String cmdParm : commandParams) {
                  if (!cmdParm.startsWith("--" + OPTIONS.CHANGE_SET_ID) && !cmdParm.startsWith("--" + OPTIONS.HELP) && !cmdParm.startsWith("--" + OPTIONS.FORCE) && !cmdParm.startsWith("--" + OPTIONS.CHANGE_SET_PATH) && !cmdParm.startsWith("--" + OPTIONS.CHANGE_SET_AUTHOR) && !cmdParm.startsWith("--" + OPTIONS.ROLLBACK_SCRIPT)) {
                    messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                  }
                ...
                if (COMMANDS.ROLLBACK_ONE_UPDATE.equalsIgnoreCase(command)) {
                  for (String cmdParm : commandParams) {
                    if (!cmdParm.startsWith("--" + OPTIONS.DEPLOYMENT_ID) && !cmdParm.startsWith("--" + OPTIONS.HELP) && !cmdParm.startsWith("--" + OPTIONS.FORCE)) {
                      messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                    }
                  }
                } else {
                  if (COMMANDS.ROLLBACK_ONE_UPDATE_SQL.equalsIgnoreCase(command)) {
                    for (String cmdParm : commandParams) {
                      if (!cmdParm.startsWith("--" + OPTIONS.DEPLOYMENT_ID) && !cmdParm.startsWith("--" + OPTIONS.HELP) && !cmdParm.startsWith("--" + OPTIONS.FORCE)) {
                        messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                      }
  ...
  private void checkForMalformedCommandParameters(final List<String> messages) {
    ...
    if (COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(command)) {
      ...
      if (COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(command) && (diffTypes != null) && diffTypes.toLowerCase().contains("data")) {
        messages.add(String.format(coreBundle.getString("including.data.diffchangelog.has.no.effect"), OPTIONS.DIFF_TYPES, COMMANDS.GENERATE_CHANGELOG));
      }
    }
  }
    ...
    for (String arg : args) {
      if (isCommand(arg)) {
        ...
        if (this.command.equalsIgnoreCase(COMMANDS.MIGRATE)) {
          ...
          if (this.command.equalsIgnoreCase(COMMANDS.MIGRATE_SQL)) {
  ...
  private void parseOptionArgument(String arg, boolean okIfNotAField) throws CommandLineParsingException {
    ...
    if (arg.toLowerCase().startsWith("--" + OPTIONS.VERBOSE) || arg.toLowerCase().startsWith("--" + OPTIONS.HELP)) {
      return;
    }
    ...
    if (PROMPT_FOR_VALUE.equalsIgnoreCase(StringUtil.trimToEmpty(value))) {
      ...
      if (c == null) {
        throw new CommandLineParsingException(String.format(MessageFormat.format(coreBundle.getString("cannot.prompt.for.the.value.no.console"), attributeName)));
      }
  ...
  @SuppressWarnings(value = { "HardCodedStringLiteral" }) protected void applyDefaults() {
    ...
    if (this.classpath != null) {
      ...
      for (String classpathEntry : classpathSoFar) {
        ...
        if (classpathEntry.endsWith(FILE_SUFFIXES.WAR_FILE_SUFFIX)) {
          ...
          if (classpathEntry.endsWith(FILE_SUFFIXES.FILE_SUFFIX_EAR)) {
  ...
  protected void doMigration() throws Exception {
    ...
    CompositeResourceAccessor fileOpener = new CompositeResourceAccessor(new FileSystemResourceAccessor(Paths.get(".").toAbsolutePath().toFile()), new CommandLineResourceAccessor(classLoader));
    ...
    try {
      ...
      if (COMMANDS.DIFF.equalsIgnoreCase(command)) {
        ...
        if (isFormattedDiff()) {
          ...
          DiffCommand diffCommand = CommandLineUtils.createDiffCommand(createReferenceDatabaseFromCommandParams(commandParams, fileOpener), database, StringUtil.trimToNull(diffTypes), finalSchemaComparisons, objectChangeFilter, new PrintStream(getOutputStream()));
        ...
        if (COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(command)) {
          ...
          if (COMMANDS.GENERATE_CHANGELOG.equalsIgnoreCase(command)) {
            ...
            if (COMMANDS.SNAPSHOT.equalsIgnoreCase(command)) {
              SnapshotCommand snapshotCommand = (SnapshotCommand) Scope.getCurrentScope().getSingleton(CommandFactory.class).getCommand(COMMANDS.SNAPSHOT);
              ...
              if (COMMANDS.EXECUTE_SQL.equalsIgnoreCase(command)) {
                ...
                if (COMMANDS.SNAPSHOT_REFERENCE.equalsIgnoreCase(command)) {
                  SnapshotCommand snapshotCommand = (SnapshotCommand) Scope.getCurrentScope().getSingleton(CommandFactory.class).getCommand(COMMANDS.SNAPSHOT);
      ...
      if (COMMANDS.LIST_LOCKS.equalsIgnoreCase(command)) {
        ...
        if (COMMANDS.RELEASE_LOCKS.equalsIgnoreCase(command)) {
          ...
          if (COMMANDS.TAG.equalsIgnoreCase(command)) {
            ...
            if (COMMANDS.TAG_EXISTS.equalsIgnoreCase(command)) {
              ...
              if (COMMANDS.ROLLBACK_ONE_CHANGE_SET.equals(command)) {
                ...
                if (COMMANDS.ROLLBACK_ONE_CHANGE_SET_SQL.equals(command)) {
                  Writer outputWriter = getOutputWriter();
                  Map<String, Object> argsMap = new HashMap<>();
                  loadChangeSetInfoToMap(argsMap);
                  ...
                  if (COMMANDS.ROLLBACK_ONE_UPDATE.equals(command)) {
                    Map<String, Object> argsMap = new HashMap<>();
                    ...
                    if (COMMANDS.ROLLBACK_ONE_UPDATE_SQL.equals(command)) {
                      Writer outputWriter = getOutputWriter();
                      Map<String, Object> argsMap = new HashMap<>();
                      argsMap.put("deploymentId", getCommandParam(OPTIONS.DEPLOYMENT_ID, null));
                      argsMap.put("outputWriter", outputWriter);
                      ...
                      if (COMMANDS.REGISTER_CHANGELOG.equalsIgnoreCase(command)) {
                        Map<String, Object> argsMap = new HashMap<>();
                        RegisterChangeLogCommand liquibaseCommand = (RegisterChangeLogCommand) createLiquibaseCommand(database, liquibase, COMMANDS.REGISTER_CHANGELOG, argsMap);
                        ...
                        if (COMMANDS.SYNC_HUB.equalsIgnoreCase(command)) {
                          executeSyncHub(database, liquibase);
                          return;
                        } else {
                          if (COMMANDS.DROP_ALL.equals(command)) {
                            ...
                            if (COMMANDS.STATUS.equalsIgnoreCase(command)) {
                              ...
                              if (COMMANDS.UNEXPECTED_CHANGESETS.equalsIgnoreCase(command)) {
                                boolean runVerbose = false;
                                if (commandParams.contains("--" + OPTIONS.VERBOSE)) {
                                  runVerbose = true;
                                }
                                ...
                                if (COMMANDS.VALIDATE.equalsIgnoreCase(command)) {
                                  try {
                                    liquibase.validate();
                                  ...
                                  if (COMMANDS.CLEAR_CHECKSUMS.equalsIgnoreCase(command)) {
                                    ...
                                    if (COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(command)) {
                                      ...
                                      if (COMMANDS.DB_DOC.equalsIgnoreCase(command)) {
                                        if (commandParams.isEmpty()) {
      ...
      try {
        if (COMMANDS.UPDATE.equalsIgnoreCase(command)) {
          ...
          if (COMMANDS.CHANGELOG_SYNC.equalsIgnoreCase(command)) {
            ...
            if (COMMANDS.CHANGELOG_SYNC_SQL.equalsIgnoreCase(command)) {
              liquibase.changeLogSync(new Contexts(contexts), new LabelExpression(labels), getOutputWriter());
            } else {
              if (COMMANDS.MARK_NEXT_CHANGESET_RAN.equalsIgnoreCase(command)) {
                liquibase.markNextChangeSetRan(new Contexts(contexts), new LabelExpression(labels));
              } else {
                if (COMMANDS.MARK_NEXT_CHANGESET_RAN_SQL.equalsIgnoreCase(command)) {
                  ...
                  if (COMMANDS.UPDATE_COUNT.equalsIgnoreCase(command)) {
                    ...
                    if (COMMANDS.UPDATE_COUNT_SQL.equalsIgnoreCase(command)) {
                      ...
                      if (COMMANDS.UPDATE_TO_TAG.equalsIgnoreCase(command)) {
                        ...
                        if (COMMANDS.UPDATE_TO_TAG_SQL.equalsIgnoreCase(command)) {
                          if ((commandParams == null) || commandParams.isEmpty()) {
                          ...
                          if (COMMANDS.UPDATE_SQL.equalsIgnoreCase(command)) {
                            liquibase.update(new Contexts(contexts), new LabelExpression(labels), getOutputWriter());
                          } else {
                            if (COMMANDS.ROLLBACK.equalsIgnoreCase(command)) {
                              ...
                              if (COMMANDS.ROLLBACK_TO_DATE.equalsIgnoreCase(command)) {
                                if (getCommandArgument() == null) {
                                ...
                                if (COMMANDS.ROLLBACK_COUNT.equalsIgnoreCase(command)) {
                                  ...
                                  if (COMMANDS.ROLLBACK_SQL.equalsIgnoreCase(command)) {
                                    if (getCommandArgument() == null) {
                                    ...
                                    if (COMMANDS.ROLLBACK_TO_DATE_SQL.equalsIgnoreCase(command)) {
                                      if (getCommandArgument() == null) {
                                      ...
                                      if (COMMANDS.ROLLBACK_COUNT_SQL.equalsIgnoreCase(command)) {
                                        if (getCommandArgument() == null) {
                                        ...
                                        if (COMMANDS.FUTURE_ROLLBACK_SQL.equalsIgnoreCase(command)) {
                                          liquibase.futureRollbackSQL(new Contexts(contexts), new LabelExpression(labels), getOutputWriter());
                                        } else {
                                          if (COMMANDS.FUTURE_ROLLBACK_COUNT_SQL.equalsIgnoreCase(command)) {
                                            if (getCommandArgument() == null) {
                                            ...
                                            if (COMMANDS.FUTURE_ROLLBACK_FROM_TAG_SQL.equalsIgnoreCase(command)) {
                                              if (getCommandArgument() == null) {
                                              ...
                                              if (COMMANDS.UPDATE_TESTING_ROLLBACK.equalsIgnoreCase(command)) {
                                                liquibase.updateTestingRollback(new Contexts(contexts), new LabelExpression(labels));
                                              } else {
                                                if (COMMANDS.HISTORY.equalsIgnoreCase(command)) {
  ...
  private boolean isLicenseableCommand(String formatValue) {
    return COMMANDS.ROLLBACK_ONE_CHANGE_SET.equalsIgnoreCase(command) || COMMANDS.ROLLBACK_ONE_CHANGE_SET_SQL.equalsIgnoreCase(command) || COMMANDS.ROLLBACK_ONE_UPDATE.equalsIgnoreCase(command) || COMMANDS.ROLLBACK_ONE_UPDATE_SQL.equalsIgnoreCase(command) || (COMMANDS.DIFF.equalsIgnoreCase(command) && formatValue != null && !formatValue.toLowerCase().equals("txt"));
  }
  ...
  private LiquibaseCommand createLiquibaseCommand(Database database, Liquibase liquibase, String commandName, Map<String, Object> argsMap) throws LiquibaseException {
  ...
  private Database createReferenceDatabaseFromCommandParams(Set<String> commandParams, ResourceAccessor resourceAccessor) throws CommandLineParsingException, DatabaseException {
    ...
    for (String param : commandParams) {
      ...
      if (OPTIONS.REFERENCE_DRIVER.equalsIgnoreCase(attributeName)) {
        ...
        if (OPTIONS.REFERENCE_URL.equalsIgnoreCase(attributeName)) {
          refUrl = value;
        } else {
          if (OPTIONS.REFERENCE_USERNAME.equalsIgnoreCase(attributeName)) {
            refUsername = value;
          } else {
            if (OPTIONS.REFERENCE_PASSWORD.equalsIgnoreCase(attributeName)) {
              refPassword = value;
            } else {
              if (OPTIONS.REFERENCE_DEFAULT_CATALOG_NAME.equalsIgnoreCase(attributeName)) {
                defCatalogName = value;
              } else {
                if (OPTIONS.REFERENCE_DEFAULT_SCHEMA_NAME.equalsIgnoreCase(attributeName)) {
                  defSchemaName = value;
                } else {
                  if (OPTIONS.DATA_OUTPUT_DIRECTORY.equalsIgnoreCase(attributeName)) {
  ...
  @SuppressWarnings(value = { "HardCodedStringLiteral" }) private enum FILE_SUFFIXES {
  ...
  @SuppressWarnings(value = { "HardCodedStringLiteral" }) private enum COMMANDS {
  ...
  @SuppressWarnings(value = { "HardCodedStringLiteral" }) private enum OPTIONS {
    ...
    @SuppressWarnings(value = { "squid:S2068" }) private static final String REFERENCE_PASSWORD = "referencePassword";
