            ...
            public Integer run() throws Exception {
                ...
                try {
                    if (!GlobalConfiguration.SHOULD_RUN.getCurrentValue()) {
                        Scope.getCurrentScope().getUI().sendErrorMessage(String.format(Main.coreBundle.getString("did.not.run.because.param.was.set.to.false"), GlobalConfiguration.SHOULD_RUN.getKey()));
                        return 0;
                    }
                    if ((args.length == 0) || ((args.length == 1) && ("--" + OPTIONS.HELP).equals(args[0]))) {
                        ...
                        Scope.getCurrentScope().getUI().sendMessage(String.format(Main.coreBundle.getString("version.number"), LiquibaseUtil.getBuildVersion()));
                    ...
                    Logger liquibaseLogger = java.util.logging.Logger.getLogger("liquibase");
...
//                        hubConfiguration.setLiquibaseHubUrl(main.liquibaseHubUrl);
                    ...
                    try {
                        if (e.getCause() instanceof ValidationFailedException) {
                            ...
                            Scope.getCurrentScope().getUI().sendErrorMessage(String.format(Main.coreBundle.getString("unexpected.error"), message), e);
                        } else {
                            ...
                            Scope.getCurrentScope().getLog(getClass()).severe(String.format(Main.coreBundle.getString("unexpected.error"), message), e);
                        }
                    } catch (IllegalFormatException e1) {
                        e1.printStackTrace();
                    }
                    throw new <e>LiquibaseException(String.format(Main.coreBundle.getString("unexpected.error"), message));
                }
...
//            LiquibaseConfiguration.getInstance().getConfiguration(GlobalConfiguration.class).setShouldSnapshotData(b);
            ...
            CompareControl[] finalSchemaComparisons = computedSchemas.finalSchemaComparisons;
