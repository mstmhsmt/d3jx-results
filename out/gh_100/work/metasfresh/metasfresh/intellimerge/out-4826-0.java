...
public enum WFDurationUnit implements ReferenceListAwareEnum
{
 ...
 Year(X_AD_Workflow.DURATIONUNIT_Year, Duration.ofDays(365), ChronoUnit.YEARS), Month(X_AD_Workflow.DURATIONUNIT_Month, Duration.ofDays(30), ChronoUnit.MONTHS), Day(X_AD_Workflow.DURATIONUNIT_Day, Duration.ofDays(1), ChronoUnit.DAYS), Hour(X_AD_Workflow.DURATIONUNIT_Hour, Duration.ofHours(1), ChronoUnit.HOURS), Minute(X_AD_Workflow.DURATIONUNIT_Minute, Duration.ofMinutes(1), ChronoUnit.MINUTES), Second(X_AD_Workflow.DURATIONUNIT_Second, Duration.ofSeconds(1), ChronoUnit.SECONDS); private static final ReferenceListAwareEnums.ValuesIndex<WFDurationUnit> typesByCode = ReferenceListAwareEnums.index(values()); private static final ImmutableMap<TemporalUnit, WFDurationUnit> typesByTemporalUnit = Maps.uniqueIndex(Arrays.asList(values()), WFDurationUnit::getTemporalUnit);
  @Getter
 @NonNull
