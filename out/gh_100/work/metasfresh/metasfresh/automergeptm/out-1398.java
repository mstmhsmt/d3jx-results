...
public class FlatrateBLTest extends ContractsTestBase {
  ...
  @Test public void beforeCompleteDataEntry_test() {
    ...
    ContractDocumentLocationAdapterFactory.billLocationAdapter(currentTerm).setFrom(bpartnerLocationId);
    save(currentTerm);
  ...
  private void setupTaxAndActivity(final I_M_Product product, final I_C_Flatrate_DataEntry dataEntry, final I_C_Flatrate_Term currentTerm) {
    ...
    Mockito.when(productActivityProvider.getActivityForAcct(clientId, orgId, ProductId.ofRepoId(product.getM_Product_ID()))).thenReturn(activityId);
    Mockito.when(taxBL.getTaxNotNull(any(I_C_Flatrate_Term.class), any(TaxCategoryId.class), anyInt(), any(Timestamp.class), any(OrgId.class), any(WarehouseId.class), any(BPartnerLocationAndCaptureId.class), any(SOTrx.class), any(VatCodeId.class))).thenReturn(TaxId.ofRepoId(3));
  }

  @Test public void testUpdateNoticeDateAndEndDate_endsWithCalendarYear_CorrectData() {
  ...
  @Test public void testUpdateNoticeDateAndEndDate_endsWithCalendarYear_IncorrectTermDuration() {
    prepareForUpdateEndDate();
    transition.setTermDurationUnit(X_C_Flatrate_Transition.TERMDURATIONUNIT_MonatE);
    transition.setTermDuration(25);
    save(transition);
    assertThatThrownBy(() -> Services.get(IFlatrateBL.class).updateNoticeDateAndEndDate(term)).isInstanceOf(AdempiereException.class);
  }

  @Test public void testUpdateNoticeDateAndEndDate_endsWithCalendarYear_YearNotInCalendar() {
    prepareForUpdateEndDate();
    transition.setTermDurationUnit(X_C_Flatrate_Transition.TERMDURATIONUNIT_MonatE);
    transition.setTermDuration(48);
    save(transition);
    assertThatThrownBy(() -> Services.get(IFlatrateBL.class).updateNoticeDateAndEndDate(term)).isInstanceOf(AdempiereException.class);
  }

  @Test public void testUpdateNoticeDateAndEndDate_noEndsWithCalendarYear() {
    prepareForUpdateEndDate();
  ...
  @Test public void testhasOverlappingTerms() {
    final I_C_Calendar calendar = newInstance(I_C_Calendar.class);
    save(calendar);
    final I_C_Year year1 = newInstance(I_C_Year.class);
    year1.setC_Calendar_ID(calendar.getC_Calendar_ID());
