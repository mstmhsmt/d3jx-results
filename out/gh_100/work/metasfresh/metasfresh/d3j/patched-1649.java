...
public
final
class MADBoilerPlate extends X_AD_BoilerPlate
{
   private static final long serialVersionUID = 5825759144157759944L;
   private static final Logger log =
     LogManager.getLogger(MADBoilerPlate.class);
   public static final String FunctionSeparator = "/";
   public static final String NamePattern = "[a-zA-Z0-9-_+]+";
   public static final String NamePatternWithFunctions =
     "[a-zA-Z0-9-_+" + FunctionSeparator + "]+";
   public static final String TagBegin = "@";
   public static final String TagEnd = "@";
   public static final Pattern NameTagPattern =
     Pattern.compile(TagBegin + "([ ]*" + NamePatternWithFunctions + "[ ]*)" + TagEnd,
   ...
   private static final CCache<Integer,MADBoilerPlate> s_cache =
     new CCache<>(Table_Name, 20);
   public
   static
   MADBoilerPlate get(final Properties ctx, final int AD_BoilerPlate_ID) {
   ...
   public
   static
   void sendEMail(final IEMailEditor editor) {
     sendEMail(editor, true);
   }
   public
   static
   void sendEMail(final IEMailEditor editor, final boolean withRequest) {
     final BoilerPlateContext context =
       createEditorContextFromObject(editor.getBaseObject());
   ...
   private
   static
   void createRequest(final EMail email, final int parent_table_id, 
                      final int parent_record_id, 
                      final BoilerPlateContext context) {
     if (email == null) {
       return;
     }
     ...
     final MRequestTypeService rtService =
       new MRequestTypeService(Env.getCtx());
     final Integer SalesRep_ID = context.getSalesRep_ID();
     final MRequest requestRecord =
       new MRequest(Env.getCtx(),
         SalesRep_ID == null ? 0 : SalesRep_ID.intValue(),
         rtService.getDefault(I_R_RequestType.COLUMNNAME_IsDefaultForEMail),
         email.getSubject(), false, null);
     String message = getPlainText(email.getMessageHTML());
     if (Check.isEmpty(message, true)) {
       message = email.getMessageCRLF();
     }
     requestRecord.setResult(message);
     updateRequestDetails(requestRecord, parent_table_id, parent_record_id,
       context);
     requestRecord.saveEx();
     try {
       final List<AttachmentEntryCreateRequest> attchmentRequests =
         email.getAttachments().stream().map(EMailAttachment::createDataSource).map(
           AttachmentEntryCreateRequest::fromDataSource).collect(ImmutableList.toImmutableList());
       if (attchmentRequests.isEmpty()) {
         return;
       }
       final AttachmentEntryService attachmentEntryService =
         Adempiere.getBean(AttachmentEntryService.class);
       for (final AttachmentEntryCreateRequest attchmentRequest : attchmentRequests) {
         attachmentEntryService.createNewAttachment(requestRecord,
           attchmentRequest);
       }
     } catch (final Exception ex) {
       log.warn("Failed attaching email attachments to request: {}",
         requestRecord, ex);
     } 
   }
   public
   static
   void createRequest(final File pdf, final int parent_table_id, 
                      final int parent_record_id, 
                      final BoilerPlateContext context) {
     final MRequestTypeService rtService =
       new MRequestTypeService(Env.getCtx());
     ...
     final MRequest requestRecord =
       new MRequest(Env.getCtx(),
         ...
         Services.get(IMsgBL.class).translate(Env.getCtx(),
           "de.metas.letter.RequestLetterSubject"),
         false, ITrx.TRXNAME_ThreadInherited);
     updateRequestDetails(requestRecord, parent_table_id, parent_record_id,
       context);
   ...
   private
   static
   void updateRequestDetails(final I_R_Request rq, final int parent_table_id, 
                             final int parent_record_id, 
                             final BoilerPlateContext context) {
     ...
     if (parent_table_id == tableDAO.retrieveTableId(I_C_BPartner.Table_Name)) {
       ...
       final MOrderLine oLine =
         new MOrderLine(Env.getCtx(), parent_record_id, null);
       if (oLine != null) {
   ...
   public
   MADBoilerPlate(final Properties ctx, final int AD_BoilerPlate_ID, 
   ...
   public
   MADBoilerPlate(final Properties ctx, final ResultSet rs, 
                  final String trxName) {
     super(ctx,
     rs,
     trxName);
   ...
   public
   static
   Query getByNameQuery(final Properties ctx, final String name, 
                        final String trxName) {
     final String whereClause = COLUMNNAME_Name + "=? AND AD_Client_ID=?";
     return new Query(ctx, Table_Name, whereClause, trxName).setParameters(new Object[]{
              name, Env.getAD_Client_ID(ctx) });
   }
   public
   static
   int getIdByName(final Properties ctx, final String name, 
                   final String trxName) {
     return getByNameQuery(ctx, name, trxName).firstIdOnly();
   }
   public
   static
   MADBoilerPlate getByName(final Properties ctx, final String name, 
                            final String trxName) {
     return getByNameQuery(ctx, name, trxName).firstOnly();
   }
   public
   static
   KeyNamePair[] getDependsOn(final Properties ctx, 
                              final int AD_BoilerPlate_ID, 
                              final String trxName) {
     final String sql =
       "SELECT " + COLUMNNAME_AD_BoilerPlate_ID + "," + COLUMNNAME_Name + " FROM " + Table_Name + " WHERE EXISTS (SELECT 1 FROM " + I_AD_BoilerPlate_Ref.Table_Name + " r" + " WHERE r." + I_AD_BoilerPlate_Ref.COLUMNNAME_Ref_BoilerPlate_ID + "=" + Table_Name + "." + COLUMNNAME_AD_BoilerPlate_ID + " AND r." + I_AD_BoilerPlate_Ref.COLUMNNAME_AD_BoilerPlate_ID + "=?)";
     return DB.getKeyNamePairs(trxName, sql, false, AD_BoilerPlate_ID);
   }
   public
   KeyNamePair toKeyNamePair() {
     return new KeyNamePair(getAD_BoilerPlate_ID(), getName());
   }
   public
   static
   KeyNamePair[] getAllSnippetsKeyNamePair() {
   ...
   public
   static
   Lookup getAllLookup(final int windowNo) {
     final Lookup lookup;
     try {
       final Properties ctx = Env.getCtx();
       lookup =
       MLookupFactory.get(ctx, windowNo, 0, DisplayType.TableDir, null,
         I_AD_BoilerPlate.COLUMNNAME_AD_BoilerPlate_ID, 0, false,
         IValidationRule.AD_Val_Rule_ID_Null);
   ...
   public
   static
   SortedMap<String,String> getAllSnippetsMap() {
   ...
   public
   static
   List<MADBoilerPlate> getAll(final Properties ctx) {
     return Services.get(IQueryBL.class).createQueryBuilder(I_AD_BoilerPlate.class,
              ctx, ITrx.TRXNAME_None).addOnlyContextClientOrSystem().orderBy().addColumn(I_AD_BoilerPlate.COLUMNNAME_Name).addColumn(I_AD_BoilerPlate.COLUMNNAME_AD_Client_ID,
              Direction.Descending, Nulls.Last).endOrderBy().create().list(MADBoilerPlate.class);
   }
   public
   static
   Stream<MADBoilerPlate> streamAllReadable(@NonNull final IUserRolePermissions permissions) {
     return getAll(Env.getCtx()).stream().filter(template -> isReadable(template,
                                                               permissions));
   }
   private
   static
   boolean isReadable(@NonNull final MADBoilerPlate template, 
   ...
   private
   void checkCycles(int AD_BoilerPlate_ID, Collection<KeyNamePair> trace) {
     if (trace == null) {
       trace = new ArrayList<>();
     }
     if (AD_BoilerPlate_ID <= 0) {
     ...
     for (final KeyNamePair dependOn : MADBoilerPlate.getDependsOn(getCtx(),
                                         AD_BoilerPlate_ID, get_TrxName())) {
   ...
   public
   String getTextSnippet(String AD_Language) {
     if (AD_Language == null) {
   ...
   public
   static
   String parseText(final Properties ctx, final String text, 
                    final boolean isEmbeded, 
                    final BoilerPlateContext context, final String trxName) {
     if (text == null) {
       return null;
     }
     ...
     if (isEmbeded) {
       textFixed =
       textFixed.replace("<html>", "").replace("</html>", "").replace("<head>",
   ...
   private
   static
   String applyTagFunctions(String text, final String[] functions) {
     if (text == null) {
       text = "";
     }
   ...
   private
   static
   String applyTagFunction(final String text, final String function) {
   ...
   public
   String getTextSnippetParsed(final BoilerPlateContext context) {
     return getTextSnippetParsed(false, context);
   }
   public
   String getTextSnippetParsed(final boolean isEmbeded, 
                               final BoilerPlateContext context) {
   ...
   public
   static
   String createTag(final String name) {
   ...
   @Override
   protected
   boolean beforeSave(final boolean newRecord) {
   ...
   @Override
   protected
   boolean afterSave(final boolean newRecord, final boolean success) {
     if (!success) {
       return false;
     }
     rebuildReferences();
   ...
   public
   Collection<String> parseNeededReferences() {
   ...
   protected
   static
   String getTagString(final Matcher m) {
     String refName;
     if (m.groupCount() >= 1) {
       refName = m.group(1);
     } else {
   ...
   protected
   static
   String getTagName(final Matcher m) {
     final String[] parts = getTagString(m).split(FunctionSeparator);
     return parts != null && parts.length > 0 ? parts[0] : "";
   }
   protected
   static
   String[] getTagNameAndFunctions(final Matcher m) {
     final String[] parts = getTagString(m).split(FunctionSeparator);
     return parts;
   }
   public
   void rebuildReferences() {
     DB.executeUpdateEx("DELETE FROM " + I_AD_BoilerPlate_Ref.Table_Name + " WHERE " + I_AD_BoilerPlate_Ref.COLUMNNAME_AD_BoilerPlate_ID + "=?",
       new Object[]{ getAD_BoilerPlate_ID() }, get_TrxName());
   ...
   public
   static
   BoilerPlateContext createEditorContextFromObject(final Object sourceDocumentObj) {
     final SourceDocument sourceDocument =
       SourceDocument.toSourceDocumentOrNull(sourceDocumentObj);
     return createEditorContext(sourceDocument);
   }
   public
   static
   BoilerPlateContext createEditorContext(final SourceDocument sourceDocument) {
     final Properties ctx = Env.getCtx();
     final BoilerPlateContext.Builder attributesBuilder =
       BoilerPlateContext.builder();
     ...
     final I_AD_User salesRep =
       Services.get(IUserDAO.class).retrieveUserOrNull(Env.getCtx(),
     ...
     final int AD_User_ID =
       sourceDocument != null ? sourceDocument.getFieldValueAsInt("AD_User_ID",
                                  -1) : -1;
     String email = null;
     if (AD_User_ID > 0) {
       user =
       Services.get(IUserDAO.class).retrieveUserOrNull(ctx, AD_User_ID);
     ...
     if (C_BPartner_ID <= 0) {
       C_BPartner_ID =
       sourceDocument != null ? sourceDocument.getFieldValueAsInt("C_BPartner_ID",
                                  -1) : -1;
     }
     if (C_BPartner_ID > 0) {
       attributesBuilder.setC_BPartner_ID(C_BPartner_ID);
       final I_C_BPartner bp =
         Services.get(IBPartnerDAO.class).getById(C_BPartner_ID);
       if (email == null) {
         final I_AD_User contact =
           getDefaultContactOrFirstWithValidEMail(bp);
         if (contact != null) {
     ...
     if (C_BPartner_Location_ID <= 0) {
       ...
       sourceDocument != null ? sourceDocument.getFieldValueAsInt("C_BPartner_Location_ID",
                                  -1) : -1;
     }
     if (C_BPartner_Location_ID > 0) {
     ...
     int adOrgId =
       sourceDocument != null ? sourceDocument.getFieldValueAsInt("AD_Org_ID",
                                  -1) : -1;
     if (adOrgId < 0) {
   ...
   private
   static
   I_AD_User getDefaultContactOrFirstWithValidEMail(final I_C_BPartner bpartner) {
   ...
   public
   static
   String getAD_Language(final Properties ctx, 
                         final BoilerPlateContext context) {
     if (context != null) {
   ...
   public
   static
   void createSpoolRecord(final Properties ctx, final int AD_Client_ID, 
                          final PInstanceId pinstanceId, final String text, 
                          final String trxName) {
     final String sql =
       "INSERT INTO " + I_T_BoilerPlate_Spool.Table_Name + "(" + " " + I_T_BoilerPlate_Spool.COLUMNNAME_AD_Client_ID + "," + I_T_BoilerPlate_Spool.COLUMNNAME_AD_Org_ID + "," + I_T_BoilerPlate_Spool.COLUMNNAME_AD_PInstance_ID + "," + I_T_BoilerPlate_Spool.COLUMNNAME_SeqNo + "," + I_T_BoilerPlate_Spool.COLUMNNAME_MsgText + ") VALUES (?,?,?,?,?)";
     DB.executeUpdateEx(sql, new Object[]{ AD_Client_ID, 0, pinstanceId, 10,
       text }, trxName);
   }
   @ToString
   public
   ...
   class BoilerPlateContext
   {
      ...
      public static final BoilerPlateContext EMPTY =
        new BoilerPlateContext(ImmutableMap.of());
      ...
      private static final String VAR_C_BPartner_Location_ID =
        "C_BPartner_Location_ID";
      ...
      private
      BoilerPlateContext(@NonNull final Map<String,Object> attributes) {
      ...
      public
      Object get(final String attributeName) {
        return attributes.get(attributeName);
      }
      @Nullable
      public
      Integer getSalesRep_ID() {
        return (Integer)get(VAR_SalesRep_ID);
      }
      public
      I_AD_User getSalesRepUser() {
      ...
      @Nullable
      public
      Integer getC_BPartner_ID() {
        return (Integer)get(VAR_C_BPartner_ID);
      }
      @Nullable
      public
      Integer getC_BPartner_ID(final Integer defaultValue) {
        final Integer bpartnerId = getC_BPartner_ID();
        return bpartnerId != null ? bpartnerId : defaultValue;
      }
      @Nullable
      public
      Integer getC_BPartner_Location_ID() {
        return (Integer)get(VAR_C_BPartner_Location_ID);
      }
      @Nullable
      public
      Integer getC_BPartner_Location_ID(final Integer defaultValue) {
      ...
      @Nullable
      public
      Integer getAD_User_ID(final Integer defaultValue) {
        final Integer adUserId = getAD_User_ID();
        return adUserId != null ? adUserId : defaultValue;
      }
      @Nullable
      public
      Integer getAD_Org_ID(final Integer defaultValue) {
      ...
      public
      SourceDocument getSourceDocumentOrNull() {
      ...
      class Builder
      {
         private final Map<String,Object> attributes;
         private
         Builder(final Map<String,Object> attributes) {
           this.attributes =
           attributes != null && !attributes.isEmpty() ? new HashMap<>(attributes) : 
           new HashMap<>();
         }
         public
         BoilerPlateContext build() {
         ...
         private
         void setAttribute(final String attributeName, final Object value) {
           if (value == null) {
         ...
         public
         Builder setWindowNo(final int windowNo) {
           setAttribute(VAR_WindowNo, windowNo);
           return this;
         }
         public
         Builder setSourceDocument(final SourceDocument sourceDocument) {
           setAttribute(VAR_SourceDocument, sourceDocument);
           return this;
         }
         public
         Builder setAD_Language(final String adLanguage) {
           setAttribute(VAR_AD_Language, adLanguage);
           return this;
         }
         public
         Builder setSourceDocumentFromObject(final Object sourceDocumentObj) {
           final SourceDocument sourceDocument =
             SourceDocument.toSourceDocumentOrNull(sourceDocumentObj);
         ...
         public
         Builder setSalesRep(final I_AD_User salesRepUser) {
           setAttribute(VAR_SalesRep, salesRepUser);
           setAttribute(VAR_SalesRep_ID,
             salesRepUser != null ? salesRepUser.getAD_User_ID() : null);
           return this;
         }
         public
         Builder setC_BPartner_ID(final int bpartnerId) {
           setAttribute(VAR_C_BPartner_ID, bpartnerId);
           return this;
         }
         public
         Builder setC_BPartner_Location_ID(final int bpartnerLocationId) {
           setAttribute(VAR_C_BPartner_Location_ID, bpartnerLocationId);
           return this;
         }
         public
         Builder setUser(final I_AD_User user) {
           setAttribute(VAR_AD_User, user);
           setAttribute(VAR_AD_User_ID,
             user != null ? user.getAD_User_ID() : null);
           return this;
         }
         public
         ...
         public
         Builder setAD_Org_ID(final int adOrgId) {
           setAttribute(VAR_AD_Org_ID, adOrgId);
           return this;
         }
         public
         Builder setCustomAttribute(final String attributeName, 
                                    final Object value) {
   ...
   interface SourceDocument
   {
      ...
      default
      int getFieldValueAsInt(final String fieldName, final int defaultValue);
      static
      SourceDocument toSourceDocumentOrNull(final Object obj);
   }
   @AllArgsConstructor
   private
   static
   final
   class POSourceDocument implements SourceDocument
   {
      @NonNull private final PO po;
      @Override
      public
      boolean hasFieldValue(final String fieldName) {
   ...
   static
   final
   class GridTabSourceDocument implements SourceDocument
   {
