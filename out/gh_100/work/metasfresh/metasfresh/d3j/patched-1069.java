...
final
class PreliminaryDocumentNoBuilder implements IPreliminaryDocumentNoBuilder
{
   private final transient IDocumentSequenceDAO documentSequenceDAO =
     Services.get(IDocumentSequenceDAO.class);
   ...
   IDocumentNoInfo buildOrNull0() {
     ...
     final String docSubType =
       CoalesceUtil.coalesce(newDocType.getDocSubType(), DOCSUBTYPE_NONE);
     ...
     if (isDocNoControlled) {
       final DocTypeSequenceList newDocTypeSequenceList =
         documentSequenceDAO.retrieveDocTypeSequenceList(newDocType);
       final DocSequenceId newDocSequenceId =
         newDocTypeSequenceList.getDocNoSequenceId(getClientId(), getOrgId(),
           getCountryId());
       final boolean isNewDocumentNo =
         isNewDocumentNo() || !DocSequenceId.equals(newDocSequenceId,
                                 getOldSequenceId());
       if (isNewDocumentNo) {
   ...
   private
   String retrieveCurrentDocumentNo(final DocSequenceId docSequenceId) {
     final ClientId adClientId = getClientId();
     if (MSequence.isAdempiereSys(adClientId.getRepoId())) {
       final String documentNo =
         documentSequenceDAO.retrieveDocumentNoSys(docSequenceId.getRepoId());
       return IPreliminaryDocumentNoBuilder.withPreliminaryMarkers(documentNo);
     } else {
       final DocumentSequenceInfo newDocumentSeqInfo =
         documentSequenceDAO.retriveDocumentSequenceInfo(docSequenceId);
       final boolean isStartNewYear =
         newDocumentSeqInfo != null && newDocumentSeqInfo.isStartNewYear();
       if (isStartNewYear) {
         final String dateColumnName = newDocumentSeqInfo.getDateColumn();
         final Date date = getDocumentDate(dateColumnName);
         final boolean isStartNewMonth =
           newDocumentSeqInfo.isStartNewMonth();
         final String documentNo;
         if (isStartNewMonth) {
           documentNo =
           documentSequenceDAO.retrieveDocumentNoByYearAndMonth(docSequenceId.getRepoId(),
             date);
         } else {
           documentNo =
           documentSequenceDAO.retrieveDocumentNoByYear(docSequenceId.getRepoId(),
             date);
         }
         return IPreliminaryDocumentNoBuilder.withPreliminaryMarkers(documentNo);
       } else {
         final String documentNo =
           documentSequenceDAO.retrieveDocumentNo(docSequenceId.getRepoId());
         return IPreliminaryDocumentNoBuilder.withPreliminaryMarkers(documentNo);
       }
     }
   }
   ...
   private
   DocSequenceId getOldSequenceId() {
   ...
   private
   DocSequenceId retrieveOldSequenceId() {
     ...
     final DocTypeSequenceList oldDocTypeSequenceList =
       documentSequenceDAO.retrieveDocTypeSequenceList(oldDocType);
     return oldDocTypeSequenceList.getDocNoSequenceId(getClientId(),
              getOrgId(), getCountryId());
   }
   private
   Properties getCtx() {
     return Env.getCtx();
   ...
   ClientId extractClientId() {
     ...
     if (documentModel != null) {
       final Integer adClientId =
         InterfaceWrapperHelper.getValueOrNull(documentModel, "AD_Client_ID");
       if (adClientId != null) {
   ...
   OrgId extractOrgId() {
     final Object documentModel = getDocumentModel();
     if (documentModel != null) {
       final Integer adOrgId =
         InterfaceWrapperHelper.getValueOrNull(documentModel, "AD_Org_ID");
       if (adOrgId != null) {
   ...
   @Override
   public
   IPreliminaryDocumentNoBuilder setNewDocType(@Nullable final I_C_DocType newDocType) {
     assertNotBuilt();
     _newDocType = newDocType;
     return this;
   }
   private
   I_C_DocType getNewDocType() {
     return _newDocType;
   }
   @Override
   public
   IPreliminaryDocumentNoBuilder setOldDocType_ID(final int oldDocType_ID) {
     assertNotBuilt();
     _oldDocType_ID = oldDocType_ID > 0 ? oldDocType_ID : -1;
     return this;
   }
   private
   I_C_DocType getOldDocType() {
     if (_oldDocType == null) {
       final int oldDocTypeId = _oldDocType_ID;
       if (oldDocTypeId > 0) {
         _oldDocType =
         InterfaceWrapperHelper.create(getCtx(), oldDocTypeId,
           I_C_DocType.class, ITrx.TRXNAME_None);
       }
     }
     return _oldDocType;
   }
   @Override
   public
   IPreliminaryDocumentNoBuilder setOldDocumentNo(final String oldDocumentNo) {
     assertNotBuilt();
     _oldDocumentNo = oldDocumentNo;
     return this;
   }
   private
   String getOldDocumentNo() {
     return _oldDocumentNo;
   }
   private
   boolean isNewDocumentNo() {
   ...
   @Override
   public
   IPreliminaryDocumentNoBuilder setDocumentModel(final Object documentModel) {
   ...
   private
   java.util.Date getDocumentDate(final String dateColumnName) {
     final Object documentModel = getDocumentModel();
     final Optional<java.util.Date> date =
       InterfaceWrapperHelper.getValue(documentModel, dateColumnName);
     return date.orElse(null);
   }
   private
   CountryId getCountryId() {
     ICountryIdProvider.ProviderResult billToProviderResult =
       ICountryIdProvider.ProviderResult.EMPTY;
     for (final ICountryIdProvider countryIdProvider : countryIdProviders) {
       billToProviderResult =
       countryIdProvider.computeValueInfo(getDocumentModel());
