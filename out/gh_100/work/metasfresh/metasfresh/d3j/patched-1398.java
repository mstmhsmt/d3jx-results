...
public
class FlatrateBLTest extends ContractsTestBase
{
   ...
   void init() {
     ...
     SpringContextHolder.registerJUnitBean(IBPartnerBL.class,
   ...
   private
   Timestamp day(final int year, final int month, final int day) {
     return TimeUtil.getDay(year, month, day);
   }
   @Test
   public
   void beforeCompleteDataEntry_test() {
     final IFlatrateBL flatrateBL = Services.get(IFlatrateBL.class);
     final I_C_Flatrate_Transition flatrateTransition =
       newInstance(I_C_Flatrate_Transition.class);
     ...
     final I_M_PricingSystem pricingSystem =
       newInstance(I_M_PricingSystem.class);
     save(pricingSystem);
     final I_C_Flatrate_Conditions flatrateConditions =
       newInstance(I_C_Flatrate_Conditions.class);
     ...
     final I_C_BPartner_Location bpLocation =
       newInstance(I_C_BPartner_Location.class);
     ...
     final I_C_Flatrate_Term currentTerm =
       newInstance(I_C_Flatrate_Term.class);
     ...
     final BPartnerLocationAndCaptureId bpartnerLocationId =
       BPartnerLocationAndCaptureId.ofRepoIdOrNull(bpartner.getC_BPartner_ID(),
         bpLocation.getC_BPartner_Location_ID(), location.getC_Location_ID());
     ...
     final I_C_Flatrate_DataEntry dataEntry =
       newInstance(I_C_Flatrate_DataEntry.class);
   ...
   private
   void setupTaxAndActivity(final I_M_Product product, 
                            final I_C_Flatrate_DataEntry dataEntry, 
                            final I_C_Flatrate_Term currentTerm) {
     Services.registerService(IProductActivityProvider.class,
       productActivityProvider);
     Services.registerService(ITaxBL.class, taxBL);
     Mockito.when(productActivityProvider.getActivityForAcct(clientId, orgId,
                    ProductId.ofRepoId(product.getM_Product_ID()))).thenReturn(activityId);
     Mockito.when(taxBL.getTaxNotNull(any(I_C_Flatrate_Term.class),
                    any(TaxCategoryId.class), anyInt(), any(Timestamp.class),
                    any(OrgId.class), any(WarehouseId.class),
                    any(BPartnerLocationAndCaptureId.class),
                    any(SOTrx.class), any(VatCodeId.class))).thenReturn(
       TaxId.ofRepoId(3));
   }
   @Test
   public
   void testUpdateNoticeDateAndEndDate_endsWithCalendarYear_CorrectData() {
   ...
   @Test
   public
   void testUpdateNoticeDateAndEndDate_endsWithCalendarYear_IncorrectTermDuration() {
     prepareForUpdateEndDate();
     transition.setTermDurationUnit(X_C_Flatrate_Transition.TERMDURATIONUNIT_MonatE);
   ...
   @Test
   public
   void testUpdateNoticeDateAndEndDate_endsWithCalendarYear_YearNotInCalendar() {
     prepareForUpdateEndDate();
     transition.setTermDurationUnit(X_C_Flatrate_Transition.TERMDURATIONUNIT_MonatE);
   ...
   @Test
   public
   void testUpdateNoticeDateAndEndDate_noEndsWithCalendarYear() {
     prepareForUpdateEndDate();
   ...
   void testhasOverlappingTerms() {
     ...
     save(conditions1);
     final I_C_Flatrate_Matching matching1 =
       newInstance(I_C_Flatrate_Matching.class);
     ...
     save(conditions2);
     final I_C_Flatrate_Matching matching2 =
       newInstance(I_C_Flatrate_Matching.class);
     ...
     save(partner);
     final I_C_Flatrate_Data flatrateData =
       newInstance(I_C_Flatrate_Data.class);
     ...
     boolean hasOverlappingTerms =
       Services.get(IFlatrateBL.class).hasOverlappingTerms(term);
     assertThat(hasOverlappingTerms).isTrue();
     hasOverlappingTerms =
     Services.get(IFlatrateBL.class).hasOverlappingTerms(overlappingTerm);
     assertThat(hasOverlappingTerms).isTrue();
   }
}