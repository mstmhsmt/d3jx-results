...
class CacheInvalidateRequest
{
   ...
   public
   static
   CacheInvalidateRequest all() {
     return ALL;
   }
   public
   static
   CacheInvalidateRequest allRecordsForTable(@NonNull final String rootTableName) {
     ...
     final String debugFrom =
       DEBUG ? Trace.toOneLineStackTraceString() : null;
     return new CacheInvalidateRequest(rootTableName, rootRecordId,
              childTableName, childRecordId, debugFrom);
   }
   public
   static
   CacheInvalidateRequest rootRecord(@NonNull final String rootTableName, 
   ...
   static
   CacheInvalidateRequest rootRecord(@NonNull final String rootTableName, 
                                     @NonNull final RepoIdAware rootId) {
     return rootRecord(rootTableName, rootId.getRepoId());
   }
   public
   static
   CacheInvalidateRequest allChildRecords(@NonNull final String rootTableName,
                                          @NonNull final RepoIdAware rootId, 
                                          @NonNull final String childTableName) {
     return allChildRecords(rootTableName, rootId.getRepoId(),
              childTableName);
   }
   public
   static
   ...
   public
   static
   CacheInvalidateRequest fromTableNameAndRecordId(final String tableName, 
                                                   final int recordId) {
     if (tableName == null) {
       return all();
     } else if (recordId < 0) {
   ...
   private static final CacheInvalidateRequest ALL =
     new CacheInvalidateRequest(null, RECORD_ID_ALL, null, RECORD_ID_ALL,
       "ALL");
   @JsonProperty("rootTableName") private final String rootTableName;
   @JsonProperty("rootRecordId") private final int rootRecordId;
   @JsonProperty("childTableName") private final String childTableName;
   @JsonProperty("childRecordId") private final int childRecordId;
   @JsonProperty("debugFrom") final String debugFrom;
   @JsonCreator
   private
   CacheInvalidateRequest(@JsonProperty("rootTableName") final String rootTableName,
   ...
   @Nullable
   public
   TableRecordReference getRootRecordOrNull() {
   ...
   @Nullable
   public
   TableRecordReference getChildRecordOrNull() {
   ...
   public
   TableRecordReference getRecordEffective() {
     if (childTableName != null && childRecordId >= 0) {
       return TableRecordReference.of(childTableName, childRecordId);
   ...
   class Builder
   {
      ...
      public
      Builder rootRecord(@NonNull final String tableName, final int recordId) {
      ...
      public
      Builder childRecord(@NonNull final String tableName, final int recordId) {
        Check.assume(recordId >= 0, "recordId >= 0");
