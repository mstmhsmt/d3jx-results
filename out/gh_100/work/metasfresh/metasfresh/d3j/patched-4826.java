...
public enum WFDurationUnit implements ReferenceListAwareEnum {
  ...
  Second(X_AD_Workflow.DURATIONUNIT_Second, Duration.ofSeconds(1),
         ChronoUnit.SECONDS);
  private static final ReferenceListAwareEnums
      .ValuesIndex<WFDurationUnit> typesByCode =
      ReferenceListAwareEnums.index(values());
  private static final ImmutableMap<TemporalUnit, WFDurationUnit>
      typesByTemporalUnit = Maps.uniqueIndex(Arrays.asList(values()),
                                             WFDurationUnit::getTemporalUnit);
  ...
  public static WFDurationUnit
  ofTemporalUnit(@NonNull final TemporalUnit temporalUnit) {
    final WFDurationUnit type = typesByTemporalUnit.get(temporalUnit);
    if (type == null) {
      throw new AdempiereException(
          "No " + WFDurationUnit.class.getSimpleName() +
          " found for temporal unit: " + temporalUnit);
    }
    return type;
  }
