...
final
class BPartnerCompositeSaver
{
   public
   void save(@NonNull final BPartnerComposite bpartnerComposite) {
     final ImmutableList<ITranslatableString> validateResult =
       bpartnerComposite.validate();
     if (!validateResult.isEmpty()) {
       final String errors =
         validateResult.stream().map(trl -> trl.translate(Env.getADLanguageOrBaseLanguage())).collect(
           Collectors.joining("\n"));
       throw new AdempiereException("Can't save an invalid bpartnerComposite").appendParametersToMessage().setParameter("errors",
               errors).setParameter("bpartnerComposite", bpartnerComposite);
     }
     final BPartner bpartner = bpartnerComposite.getBpartner();
     try (final MDCCloseable bpartnerRecordMDC =
            TableRecordMDC.putTableRecordReference(I_C_BPartner.Table_Name,
              bpartner.getId())) {
     ...
     try (final MDCCloseable bpartnerRecordMDC =
            TableRecordMDC.putTableRecordReference(I_C_BPartner.Table_Name,
              bpartner.getId())) {
       saveBPartnerLocations(bpartner.getId(),
         bpartnerComposite.getLocations(), bpartnerComposite.getOrgId());
       saveBPartnerContacts(bpartner.getId(),
         bpartnerComposite.getContacts(), bpartnerComposite.getOrgId());
       saveBPartnerBankAccounts(bpartner.getId(),
         bpartnerComposite.getBankAccounts(), bpartnerComposite.getOrgId());
     } 
   }
   private
   void saveBPartner(@NonNull final BPartner bpartner, 
                     @Nullable final OrgId orgId) {
     final I_C_BPartner bpartnerRecord =
       loadOrNew(bpartner.getId(), I_C_BPartner.class);
   ...
   private
   void saveBPartnerLocations(@NonNull final BPartnerId bpartnerId, 
                              @NonNull final List<BPartnerLocation> bpartnerLocations,
                              @Nullable final OrgId orgId) {
     final ArrayList<BPartnerLocationId> savedBPartnerLocationIds =
       new ArrayList<>();
     ...
     final ICompositeQueryUpdater<I_C_BPartner_Location> columnUpdater =
       queryBL.createCompositeQueryUpdater(I_C_BPartner_Location.class).addSetColumnValue(I_C_BPartner_Location.COLUMNNAME_IsActive,
         false);
     queryBL.createQueryBuilder(I_C_BPartner_Location.class).addOnlyActiveRecordsFilter().addOnlyContextClient().addEqualsFilter(I_C_BPartner_Location.COLUMNNAME_C_BPartner_ID,
       bpartnerId).addNotInArrayFilter(I_C_BPartner_Location.COLUMN_C_BPartner_Location_ID,
       savedBPartnerLocationIds).create().update(columnUpdater);
   }
   private
   void saveBPartnerLocation(@NonNull final BPartnerId bpartnerId, 
                             @NonNull final BPartnerLocation bpartnerLocation,
                             @Nullable final OrgId orgId) {
     try (final MDCCloseable bpartnerLocationRecordMDC =
            TableRecordMDC.putTableRecordReference(I_C_BPartner_Location.Table_Name,
              bpartnerLocation.getId())) {
       final I_C_BPartner_Location bpartnerLocationRecord =
         loadOrNew(bpartnerLocation.getId(), I_C_BPartner_Location.class);
       ...
       final BPartnerLocationType locationType =
         bpartnerLocation.getLocationType();
       ...
       anyLocationChange =
       anyLocationChange || bpartnerLocation.isAddress1Changed();
       locationRecord.setAddress1(bpartnerLocation.getAddress1());
       anyLocationChange =
       anyLocationChange || bpartnerLocation.isAddress2Changed();
       locationRecord.setAddress2(bpartnerLocation.getAddress2());
       anyLocationChange =
       anyLocationChange || bpartnerLocation.isAddress3Changed();
       locationRecord.setAddress3(bpartnerLocation.getAddress3());
       anyLocationChange =
       anyLocationChange || bpartnerLocation.isAddress4Changed();
       locationRecord.setAddress4(bpartnerLocation.getAddress4());
       anyLocationChange =
       anyLocationChange || bpartnerLocation.isCountryCodeChanged();
       if (!isEmpty(bpartnerLocation.getCountryCode(), true)) {
         final ICountryDAO countryDAO = Services.get(ICountryDAO.class);
         final CountryId countryId =
           countryDAO.getCountryIdByCountryCode(bpartnerLocation.getCountryCode());
         locationRecord.setC_Country_ID(CountryId.toRepoId(countryId));
       }
       boolean postalDataSetFromPostalRecord = false;
       anyLocationChange =
       anyLocationChange || bpartnerLocation.isPostalChanged();
       if (!isEmpty(bpartnerLocation.getPostal(), true)) {
         final IQueryBuilder<I_C_Postal> postalQueryBuilder =
           Services.get(IQueryBL.class).createQueryBuilder(I_C_Postal.class).addOnlyActiveRecordsFilter().addOnlyContextClient().filter(
             PostalQueryFilter.of(bpartnerLocation.getPostal().trim()));
         if (!isEmpty(bpartnerLocation.getDistrict(), true)) {
           postalQueryBuilder.addEqualsFilter(I_C_Postal.COLUMN_District,
             bpartnerLocation.getDistrict());
         } else {}
         postalQueryBuilder.orderBy().addColumn(I_C_Postal.COLUMNNAME_District,
           Direction.Ascending, Nulls.First);
         final List<I_C_Postal> postalRecords =
           postalQueryBuilder.create().list();
         ...
         if (postalRecords.isEmpty()) {
           ...
           postalRecord =
           postalRecords.stream().filter(r -> (r.getC_Country_ID() == 
             locationRecord.getC_Country_ID())).findFirst().orElse(null);
         } else {
           postalRecord = null;
         }
       ...
       anyLocationChange =
       anyLocationChange || bpartnerLocation.isPoBoxChanged();
       ...
       final BPartnerLocationId bpartnerLocationId =
         BPartnerLocationId.ofRepoId(bpartnerLocationRecord.getC_BPartner_ID(),
   ...
   private
   void saveBPartnerContacts(@NonNull final BPartnerId bpartnerId, 
                             @NonNull final List<BPartnerContact> contacts, 
                             @Nullable final OrgId orgId) {
     final ArrayList<BPartnerContactId> savedBPartnerContactIds =
       new ArrayList<>();
     ...
     final IQueryBL queryBL = Services.get(IQueryBL.class);
     final ICompositeQueryUpdater<I_AD_User> columnUpdater =
       queryBL.createCompositeQueryUpdater(I_AD_User.class).addSetColumnValue(I_AD_User.COLUMNNAME_IsActive,
         false);
     queryBL.createQueryBuilder(I_AD_User.class).addOnlyActiveRecordsFilter().addEqualsFilter(I_AD_User.COLUMN_C_BPartner_ID,
       bpartnerId).addNotInArrayFilter(I_AD_User.COLUMN_AD_User_ID,
       savedBPartnerContactIds).create().update(columnUpdater);
   }
   private
   void saveBPartnerContact(@NonNull final BPartnerId bpartnerId, 
                            @NonNull final BPartnerContact bpartnerContact, 
                            @Nullable final OrgId orgId) {
     try (final MDCCloseable bpartnerContactRecordMDC =
            TableRecordMDC.putTableRecordReference(I_AD_User.Table_Name,
              bpartnerContact.getId())) {
       final I_AD_User bpartnerContactRecord =
         loadOrNew(bpartnerContact.getId(), I_AD_User.class);
       ...
       final BPartnerContactType contactType =
         bpartnerContact.getContactType();
       if (contactType != null) {
       ...
       bpartnerContactRecord.setC_Greeting_ID(GreetingId.toRepoIdOr(bpartnerContact.getGreetingId(),
                                                0));
       assertCanCreateOrUpdate(bpartnerContactRecord);
       saveRecord(bpartnerContactRecord);
       final BPartnerContactId bpartnerContactId =
         BPartnerContactId.ofRepoId(bpartnerId,
           bpartnerContactRecord.getAD_User_ID());
   ...
   private
   void saveBPartnerBankAccounts(@NonNull final BPartnerId bpartnerId, 
                                 @NonNull final List<BPartnerBankAccount> bankAccounts,
                                 @Nullable final OrgId orgId) {
     final ArrayList<BPartnerBankAccountId> savedBPBankAccountIds =
       new ArrayList<>();
     ...
     final IBPBankAccountDAO bpBankAccountsDAO =
       Services.get(IBPBankAccountDAO.class);
     bpBankAccountsDAO.deactivateByBPartnerExcept(bpartnerId,
       savedBPBankAccountIds);
   }
   private
   void saveBPartnerBankAccount(@NonNull final BPartnerId bpartnerId, 
                                @NonNull final BPartnerBankAccount bankAccount,
                                @Nullable final OrgId orgId) {
     try (final MDCCloseable bankAccountRecordMDC =
            TableRecordMDC.putTableRecordReference(I_C_BP_BankAccount.Table_Name,
              bankAccount.getId())) {
       final I_C_BP_BankAccount record =
         loadOrNew(bankAccount.getId(), I_C_BP_BankAccount.class);
       ...
       saveRecord(record);
       final BPartnerBankAccountId id =
         BPartnerBankAccountId.ofRepoId(bpartnerId,
           record.getC_BP_BankAccount_ID());
   ...
   private
   void assertCanCreateOrUpdate(@NonNull final Object record) {
