 ...
 */
@ToString(exclude = { "huShipperTransportationBL", "huShipmentScheduleDAO", "invoiceCandDAO", "invoiceCandBL", "trxManager", "inOutCandidateBL", "shipmentDAO", "shipmentService" }) public class HUShippingFacade {
  ...
  private final Supplier<ShipperGatewayFacade> shipperGatewayFacadeSupplier = () -> SpringContextHolder.instance.getBean(ShipperGatewayFacade.class);
  ...
  private List<ShipmentScheduleWithHU> _candidates;

  private InOutGenerateResult shipmentsGenerateResult;

  private final ArrayList<I_M_Package> mPackagesCreated = new ArrayList<>();

  @Builder private HUShippingFacade(@NonNull @Singular(value = "hu") final List<I_M_HU> hus, final int addToShipperTransportationId, final boolean completeShipments, @Nullable final BillAssociatedInvoiceCandidates invoiceMode, final boolean createShipperDeliveryOrders, final boolean failIfNoShipmentCandidatesFound) {
  ...
  @VisibleForTesting public List<ShipmentScheduleWithHU> getCandidates() {
    if (_candidates == null) {
  ...
  @VisibleForTesting public List<I_M_InOut> getGeneratedShipments() {
  ...
  private void addHUsToShipperTransportationIfNeeded() {
    if (addToShipperTransportationId > 0) {
      final List<I_M_Package> result = trxManager.callInNewTrx(() -> huShipperTransportationBL.addHUsToShipperTransportation(ShipperTransportationId.ofRepoId(addToShipperTransportationId), hus));
  ...
  private void generateShipmentsFromCandidates() {
    ...
    if (candidates.isEmpty()) {
      new AdempiereException("No shipment candidates found").appendParametersToMessage().setParameter("context", this).throwOrLogWarning(failIfNoShipmentCandidatesFound, logger);
      return;
    }
    final ImmutableSet<ShipmentScheduleId> candidatesIds = candidates.stream().map(ShipmentScheduleWithHU::getShipmentScheduleId).collect(ImmutableSet.toImmutableSet());
    final GenerateShipmentsForSchedulesRequest request = GenerateShipmentsForSchedulesRequest.builder().scheduleIds(candidatesIds).quantityTypeToUse(M_ShipmentSchedule_QuantityTypeToUse.TYPE_PICKED_QTY).isShipDateToday(true).isCompleteShipment(completeShipments).build();
  ...
  private void generateInvoicesIfNeeded() {
    ...
    final Set<InvoiceCandidateId> invoiceCandidateIds = invoiceCandDAO.retrieveInvoiceCandidatesQueryForInOuts(shipments).listIds(InvoiceCandidateId::ofRepoId);
    if (invoiceCandidateIds.isEmpty()) {
      throw new AdempiereException("@NotFound@ @C_Invoice_Candidate_ID@").setParameter("shipments", shipments);
    }
    ...
    final IInvoiceCandidateEnqueueResult enqueueResult = invoiceCandBL.enqueueForInvoicing().setFailOnChanges(false).setInvoicingParams(invoicingParams).enqueueInvoiceCandidateIds(invoiceCandidateIds);
  ...
  private void generateShipperDeliveryOrdersIfNeeded() {
    ...
    mPackagesCreated.stream().collect(GuavaCollectors.toImmutableListMultimap(HUShippingFacade::extractShipperId)).asMap().forEach(this::generateShipperDeliveryOrderIfNeeded);
  }

  @NonNull private static ShipperId extractShipperId(@NonNull final I_M_Package mPackage) {
    return ShipperId.ofRepoId(mPackage.getM_Shipper_ID());
  }

  private void generateShipperDeliveryOrderIfNeeded(final ShipperId shipperId, @NonNull final Collection<I_M_Package> mPackages) {
    ...
    final DeliveryOrderCreateRequest request = DeliveryOrderCreateRequest.builder().pickupDate(getPickupDate(shipperTransportation)).timeFrom(TimeUtil.asLocalTime(shipperTransportation.getPickupTimeFrom())).timeTo(TimeUtil.asLocalTime(shipperTransportation.getPickupTimeTo())).packageIds(mPackageIds).shipperTransportationId(ShipperTransportationId.ofRepoId(addToShipperTransportationId)).shipperGatewayId(shipperGatewayId).build();
