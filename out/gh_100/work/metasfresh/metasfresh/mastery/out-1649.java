...
final public class MADBoilerPlate extends X_AD_BoilerPlate {
    ...
    static final public String FunctionSeparator = "/";

    static final public String NamePattern = "[a-zA-Z0-9-_+]+";

    static final public String NamePatternWithFunctions = "[a-zA-Z0-9-_+" + FunctionSeparator + "]+";

    static final public String TagBegin = "@";

    static final public String TagEnd = "@";

    static final public Pattern NameTagPattern = Pattern.compile(TagBegin + "([ ]*" + NamePatternWithFunctions + "[ ]*)" + TagEnd, Pattern.CASE_INSENSITIVE);

    static final public String FUNCTION_stripWhitespaces = "stripWhitespaces";

    static final public String FUNCTION_trim = "trim";

    static final public String FUNCTION_urlEncode = "urlEncode";

    static final public String FUNCTION_upperCase = "upperCase";

    static final public String FUNCTION_lowerCase = "lowerCase";

    private static final CCache<Integer, MADBoilerPlate> s_cache = new CCache<>(Table_Name, 20);

    static public MADBoilerPlate get(final Properties ctx, final int AD_BoilerPlate_ID) {
    ...
    static public void sendEMail(final IEMailEditor editor) {
        sendEMail(editor, true);
    }

    static public void sendEMail(final IEMailEditor editor, final boolean withRequest) {
    ...
    private static void createRequest(final EMail email, final int parent_table_id, final int parent_record_id, final BoilerPlateContext context) {
        if (email == null) {
            return;
        }
        ...
        final MRequest requestRecord = new MRequest(Env.getCtx(), SalesRep_ID == null ? 0 : SalesRep_ID.intValue(), rtService.getDefault(I_R_RequestType.COLUMNNAME_IsDefaultForEMail), email.getSubject(), false, null);
        ...
        try {
            final List<AttachmentEntryCreateRequest> attchmentRequests = email.getAttachments().stream().map(EMailAttachment::createDataSource).map(AttachmentEntryCreateRequest::fromDataSource).collect(ImmutableList.toImmutableList());
    ...
    static public void createRequest(final File pdf, final int parent_table_id, final int parent_record_id, final BoilerPlateContext context) {
        final MRequestTypeService rtService = new MRequestTypeService(Env.getCtx());
        final Integer SalesRep_ID = context.getSalesRep_ID();
        final MRequest requestRecord = new MRequest(Env.getCtx(), SalesRep_ID == null ? 0 : SalesRep_ID.intValue(), rtService.getDefault(I_R_RequestType.COLUMNNAME_IsDefaultForLetter), Services.get(IMsgBL.class).translate(Env.getCtx(), "de.metas.letter.RequestLetterSubject"), false, ITrx.TRXNAME_ThreadInherited);
        updateRequestDetails(requestRecord, parent_table_id, parent_record_id, context);
    ...
    private static void updateRequestDetails(final I_R_Request rq, final int parent_table_id, final int parent_record_id, final BoilerPlateContext context) {
    ...
    static public Query getByNameQuery(final Properties ctx, final String name, final String trxName) {
        final String whereClause = COLUMNNAME_Name + "=? AND AD_Client_ID=?";
        return new Query(ctx, Table_Name, whereClause, trxName).setParameters(new Object[] { name, Env.getAD_Client_ID(ctx) });
    }

    static public int getIdByName(final Properties ctx, final String name, final String trxName) {
        return getByNameQuery(ctx, name, trxName).firstIdOnly();
    }

    static public MADBoilerPlate getByName(final Properties ctx, final String name, final String trxName) {
        return getByNameQuery(ctx, name, trxName).firstOnly();
    }

    static public KeyNamePair[] getDependsOn(final Properties ctx, final int AD_BoilerPlate_ID, final String trxName) {
        final String sql = "SELECT " + COLUMNNAME_AD_BoilerPlate_ID + "," + COLUMNNAME_Name + " FROM " + Table_Name + " WHERE EXISTS (SELECT 1 FROM " + I_AD_BoilerPlate_Ref.Table_Name + " r" + " WHERE r." + I_AD_BoilerPlate_Ref.COLUMNNAME_Ref_BoilerPlate_ID + "=" + Table_Name + "." + COLUMNNAME_AD_BoilerPlate_ID + " AND r." + I_AD_BoilerPlate_Ref.COLUMNNAME_AD_BoilerPlate_ID + "=?)";
    ...
    static public KeyNamePair[] getAllSnippetsKeyNamePair() {
    ...
    static public Lookup getAllLookup(final int windowNo) {
        final Lookup lookup;
        try {
            final Properties ctx = Env.getCtx();
            lookup = MLookupFactory.get(ctx, windowNo, 0, DisplayType.TableDir, null, I_AD_BoilerPlate.COLUMNNAME_AD_BoilerPlate_ID, 0, false, IValidationRule.AD_Val_Rule_ID_Null);
    ...
    static public SortedMap<String, String> getAllSnippetsMap() {
    ...
    static public List<MADBoilerPlate> getAll(final Properties ctx) {
        return Services.get(IQueryBL.class).createQueryBuilder(I_AD_BoilerPlate.class, ctx, ITrx.TRXNAME_None).addOnlyContextClientOrSystem().orderBy().addColumn(I_AD_BoilerPlate.COLUMNNAME_Name).addColumn(I_AD_BoilerPlate.COLUMNNAME_AD_Client_ID, Direction.Descending, Nulls.Last).endOrderBy().create().list(MADBoilerPlate.class);
    }
    ...
    static public String getPlainText(String html) {
    ...
    static public String parseText(final Properties ctx, final String text, final boolean isEmbeded, final BoilerPlateContext context, final String trxName) {
        if (text == null) {
            return null;
        }
        String textFixed = text;
        if (isEmbeded) {
            textFixed = textFixed.replace("<html>", "").replace("</html>", "").replace("<head>", "").replace("</head>", "").replace("<body>", "").replace("</body>", "").trim();
        }
    ...
    static public String createTag(final String name) {
    ...
    static protected String getTagString(final Matcher m) {
        String refName;
        if (m.groupCount() >= 1) {
            refName = m.group(1);
        } else {
    ...
    static protected String getTagName(final Matcher m) {
        final String[] parts = getTagString(m).split(FunctionSeparator);
        return parts != null && parts.length > 0 ? parts[0] : "";
    }

    static protected String[] getTagNameAndFunctions(final Matcher m) {
        final String[] parts = getTagString(m).split(FunctionSeparator);
        return parts;
    }

    public void rebuildReferences() {
        DB.executeUpdateEx("DELETE FROM " + I_AD_BoilerPlate_Ref.Table_Name + " WHERE " + I_AD_BoilerPlate_Ref.COLUMNNAME_AD_BoilerPlate_ID + "=?", new Object[] { getAD_BoilerPlate_ID() }, get_TrxName());
    ...
    static public BoilerPlateContext createEditorContextFromObject(final Object sourceDocumentObj) {
    ...
    static public BoilerPlateContext createEditorContext(final SourceDocument sourceDocument) {
        final Properties ctx = Env.getCtx();
    ...
    static public String getAD_Language(final Properties ctx, final BoilerPlateContext context) {
        if (context != null) {
    ...
    static public void createSpoolRecord(final Properties ctx, final int AD_Client_ID, final PInstanceId pinstanceId, final String text, final String trxName) {
        final String sql = "INSERT INTO " + I_T_BoilerPlate_Spool.Table_Name + "(" + " " + I_T_BoilerPlate_Spool.COLUMNNAME_AD_Client_ID + "," + I_T_BoilerPlate_Spool.COLUMNNAME_AD_Org_ID + "," + I_T_BoilerPlate_Spool.COLUMNNAME_AD_PInstance_ID + "," + I_T_BoilerPlate_Spool.COLUMNNAME_SeqNo + "," + I_T_BoilerPlate_Spool.COLUMNNAME_MsgText + ") VALUES (?,?,?,?,?)";
        DB.executeUpdateEx(sql, new Object[] { AD_Client_ID, 0, pinstanceId, 10, text }, trxName);
    }

    @ToString
    static final public class BoilerPlateContext {

        static public Builder builder() {
            return new Builder(ImmutableMap.of());
        }

        static final public BoilerPlateContext EMPTY = new BoilerPlateContext(ImmutableMap.of());
    ...
    static public Stream<MADBoilerPlate> streamAllReadable(@NonNull final IUserRolePermissions permissions) {
        return getAll(Env.getCtx()).stream().filter(template -> isReadable(template, permissions));
    }
}
