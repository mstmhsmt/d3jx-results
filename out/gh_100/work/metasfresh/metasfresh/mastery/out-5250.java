...
final public class ProductsProposalRowsLoader {
    ...
    @Builder
    private ProductsProposalRowsLoader(@NonNull final BPartnerProductStatsService bpartnerProductStatsService, @Nullable final CampaignPriceProvider campaignPriceProvider, @NonNull @Singular final ImmutableSet<PriceListVersionId> priceListVersionIds, @Nullable final Order order, @NonNull final BPartnerId bpartnerId, @NonNull final SOTrx soTrx, @Nullable final Set<ProductId> productIdsToExclude) {
    ...
    public ProductsProposalRowsData load() {
        ...
        if (order != null) {
            ...
            headerProperties.group(ViewHeaderPropertiesGroup.builder().entry(ViewHeaderProperty.builder().caption(msgBL.translatable("C_BPartner_ID")).value(order.getBpartnerName()).build()).build());
        }
    ...
    private List<ProductsProposalRow> loadRows() {
        return priceListVersionIds.stream().flatMap(this::loadAndStreamRowsForPriceListVersionId).sorted(Comparator.comparing(ProductsProposalRow::getSeqNo).thenComparing(ProductsProposalRow::getProductName)).filter(p -> !Services.get(IProductDAO.class).getById(p.getProductId()).isDiscontinued()).collect(ImmutableList.toImmutableList());
    }
    ...
    private ProductsProposalRow toProductsProposalRowOrNull(@NonNull final I_M_ProductPrice record) {
        ...
        return ProductsProposalRow.builder().id(nextRowIdSequence.nextDocumentId()).product(product).packingMaterialId(packingMaterialId).packingDescription(packingDescription).asiDescription(extractProductASIDescription(record)).price(extractProductProposalPrice(record)).qty(null).lastShipmentDays(null).seqNo(record.getSeqNo()).productPriceId(ProductPriceId.ofRepoId(record.getM_ProductPrice_ID())).build().withExistingOrderLine(order);
    }
    ...
    private ProductsProposalRow updateRowFromStats(@NonNull final ProductsProposalRow row, @Nullable final BPartnerProductStats stats) {
