...
final class OrderLineQuickInputDescriptorFactory implements IQuickInputDescriptorFactory {
	...
	final IBPartnerBL bpartnersService, @NonNull
	final AvailableToPromiseAdapter availableToPromiseAdapter, @NonNull
	final AvailableForSaleAdapter availableForSaleAdapter, @NonNull
	final AvailableForSalesConfigRepo availableForSalesConfigRepo, @NonNull
	final LookupDescriptorProviders lookupDescriptorProviders, @NonNull
	final PackingItemProductFieldHelper packingItemProductFieldHelper, @NonNull
	...
	private DocumentFieldDescriptor.Builder createVatCodeField(@NonNull
	final Optional<SOTrx> soTrx) {
		final SqlLookupDescriptorProviderBuilder descriptorProviderBuilder = // ctxTableName
		lookupDescriptorProviders.sql().setCtxTableName(null).setCtxColumnName(IOrderLineQuickInput.COLUMNNAME_C_VAT_Code_ID).setDisplayType(DisplayType.TableDir).setPageLength(QuickInputConstants.BIG_ENOUGH_PAGE_LENGTH);
		if (soTrx.orElse(SOTrx.PURCHASE).isSales()) {
		...
		return // 
		DocumentFieldDescriptor.builder(IOrderLineQuickInput.COLUMNNAME_C_VAT_Code_ID).setCaption(msgBL.translatable(IOrderLineQuickInput.COLUMNNAME_C_VAT_Code_ID)).setWidgetType(DocumentFieldWidgetType.Lookup).setLookupDescriptorProvider(descriptorProviderBuilder.build()).setValueClass(IntegerLookupValue.class).setReadonlyLogic(ConstantLogicExpression.FALSE).setAlwaysUpdateable(true).setMandatoryLogic(ConstantLogicExpression.TRUE).setDisplayLogic(ConstantLogicExpression.TRUE).addCharacteristic(Characteristic.PublicField).usePreviousValueAsDefaultValue(IntegerLookupValue.class, OrderLineQuickInputDescriptorFactory.class.getSimpleName());
	}
