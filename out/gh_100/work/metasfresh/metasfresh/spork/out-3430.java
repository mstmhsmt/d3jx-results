...
public class HUShippingFacade {
	...
	@Builder
	private HUShippingFacade(@NonNull
	@Singular("hu")
	final List<I_M_HU> hus, final int addToShipperTransportationId, final boolean completeShipments, @Nullable
	final BillAssociatedInvoiceCandidates invoiceMode, final boolean createShipperDeliveryOrders, final boolean failIfNoShipmentCandidatesFound) {
	...
	private void generateShipmentsFromCandidates() {
		...
		final ImmutableSet<ShipmentScheduleId> candidatesIds = candidates.stream().map(ShipmentScheduleWithHU::getShipmentScheduleId).collect(ImmutableSet.toImmutableSet(ImmutableSet));
	...
	private void generateInvoicesIfNeeded() {
		...
		final IInvoiceCandidateEnqueueResult enqueueResult = // 
		invoiceCandBL.enqueueForInvoicing().setFailOnChanges(false).setInvoicingParams(invoicingParams).enqueueInvoiceCandidateIds(invoiceCandidateIds);
