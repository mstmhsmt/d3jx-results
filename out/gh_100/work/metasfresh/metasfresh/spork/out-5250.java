...
public final class ProductsProposalRowsLoader {
	...
	@Builder
	private ProductsProposalRowsLoader(@NonNull
	final BPartnerProductStatsService bpartnerProductStatsService, @Nullable
	final CampaignPriceProvider campaignPriceProvider, // 
	@NonNull
	@Singular
	final ImmutableSet<PriceListVersionId> priceListVersionIds, @Nullable
	final Order order, @NonNull
	final BPartnerId bpartnerId, @NonNull
	final SOTrx soTrx, @Nullable
	...
	public ProductsProposalRowsData load() {
		...
		// 
		// 
		ProductsProposalRowsData.builder().nextRowIdSequence(nextRowIdSequence).campaignPriceProvider(campaignPriceProvider).singlePriceListVersionId(singlePriceListVersionId).basePriceListVersionId(basePriceListVersionId).order(order).bpartnerId(bpartnerId).soTrx(soTrx).headerProperties(headerProperties.build()).rows(rows).build();
	}

	private List<ProductsProposalRow> loadRows() {
		return priceListVersionIds.stream().flatMap(this::loadAndStreamRowsForPriceListVersionId).sorted(Comparator.comparing(ProductsProposalRow::getSeqNo).thenComparing(ProductsProposalRow::getProductName)).filter(p -> !Services.get(IProductDAO.class).getById(p.getProductId()).isDiscontinued()).collect(ImmutableList.toImmutableList(ImmutableList));
	}
